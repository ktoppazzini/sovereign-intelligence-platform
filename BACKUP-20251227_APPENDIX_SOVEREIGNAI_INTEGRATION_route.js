...
function generateFallbackChartSummary(chartSpec, lang = 'English') {
  try {
    ...
    // Generate in target language using translation function
    const overall = `${icon} ${t(trend.charAt(0).toUpperCase() + trend.slice(1))} ${t('trend')}: ${firstVal} → ${lastVal} (${changePercent > 0 ? '+' : ''}${changePercent}%)`;
    const keyInsights = `${t('Peak')}: ${dataMax} | ${t('Average')}: ${dataAvg}`;
...
`;
    ...
    const getDefaultTable = (sec) => {
      const defaults = {
        exec: {
          title: isNonEnglish ? `[${lang}] Executive Summary Key Metrics` : 'Executive Summary Key Metrics',
          headers: ['Metric', 'Current', 'Target', 'Impact'],
          rows: [
            ['ROI', '0%', '300%+', 'High'],
        ...
        current: {
          title: isNonEnglish ? `[${lang}] Current State Analysis` : 'Current State Analysis',
...
export async function POST(req) {
  ...
  */
  ...
  console.log(TAG, '🔍 [ROUTE-TO-TEMPLATE] Passing translatedLabels:', {
    lang: canon.lang,
...
CRITICAL: ALL text must be in ${targetLang}. Include 4-6 appendices with types: timeline, checklist, framework, resource_plan, decision_matrix, roadmap.`;
    ...
    if (!aiResponse?.success) {
      console.warn(TAG, '[SOVEREIGN-AI-APPENDIX] SovereignAI call failed:', aiResponse?.error);
