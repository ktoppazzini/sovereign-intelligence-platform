...
Return ONLY this JSON format (no other text): { "translations": ["1 - Translated Name", "2 - Translated Name", ...] }`;
    ...
    translationPromises.push(
      openai.chat.completions.create({
        ...
        const sanitized = sanitizeJSONForParsing(rawContent);
        try {
          return { type: 'categories', data: JSON.parse(sanitized) };
        } catch (e) {
          console.error(TAG, '[JSON-PARSE] Categories failed:', e.message);
