...
export async function POST(req) {
  ...
  try {
    ...
    const {
      html = '',
      rid = '', // New: support 'rid' from side form
      recordId = rid || '', // Fallback to recordId for backward compatibility
      preparedFor = '',
      preparedForName = '', // New: from side form
      preparedBy = '',
      preparedByName = '', // New: from side form
      preparedByEmail = '', // New: email lookup result
      preparedForEmail = '', // New: email lookup result
      orgName = '',
      lang = 'English',
      approverEmail = '', // Primary approver
      requesterEmail = preparedByEmail || '', // Person who prepared the report
      fromEmail = '', // New: from side form "From Email"
      subject = 'Reform Report Approval Required',
      fileName = 'Reform_Report.pdf',
      notes = '', // New: submission notes
      changeReqId = '', // New: if resubmitting after "changes requested"
      changeNotes = '', // New: original change request notes
    } = body || {};

    const finalRecordId = recordId || rid;
    const finalPreparedBy = preparedByName || preparedBy;
    const finalPreparedFor = preparedForName || preparedFor;
    const finalRequesterEmail = requesterEmail || fromEmail || preparedByEmail || '';

    console.log('[submit] Starting submission process:', {
      recordId: finalRecordId,
      lang,
      approverEmail,
      requesterEmail: finalRequesterEmail,
      htmlLength: html.length,
      isChangesResubmit: !!changeReqId,
    });
    ...
    const approveToken = generateToken(finalRecordId, 'approve', expiry, secret);
    const rejectToken = generateToken(finalRecordId, 'reject', expiry, secret);
    const changeToken = generateToken(finalRecordId, 'change', expiry, secret);

    const approveUrl = `${baseUrl}/api/reform/decision?rid=${finalRecordId}&action=approve&token=${approveToken}&exp=${expiry}&lang=${encodeURIComponent(lang)}&req=${encodeURIComponent(finalRequesterEmail)}`;
    const rejectUrl = `${baseUrl}/api/reform/decision?rid=${finalRecordId}&action=reject&token=${rejectToken}&exp=${expiry}&lang=${encodeURIComponent(lang)}&req=${encodeURIComponent(finalRequesterEmail)}`;
    const changeUrl = `${baseUrl}/api/reform/decision?rid=${finalRecordId}&action=change&token=${changeToken}&exp=${expiry}&lang=${encodeURIComponent(lang)}&req=${encodeURIComponent(finalRequesterEmail)}`;

    // 1) Translate email content
    const reportContext = changeReqId
      ? `A reform report has been resubmitted following your request for changes.\n\nOriginal Change Request: ${changeNotes}\n\nPrepared For: ${finalPreparedFor}\nPrepared By: ${finalPreparedBy}\n\nPlease review the revised report and select one of the options below:`
    ...
    const [tSubject, tApprove, tReject, tChange, tMessage] = await Promise.all([
      ...
      translate(lang, reportContext, false),
    ]);
...
`;
    ...
    try {
      ...
      if (atApiKey && atBaseId && finalRecordId) {
        const timestamp = new Date().toISOString();
        const atUrl = `https://api.airtable.com/v0/${atBaseId}/${encodeURIComponent(atTableRequests)}/${finalRecordId}`;
