// Report Graphs with full translation support
import { translateContent } from './dynamicTranslation';
...
(function () {
  ...
  function applyType(card) {
    try {
      ...
      var sel = card.querySelector('select.chart-type-select') || 
                ...
                card.querySelector('label.chart-type select') || // Added label.chart-type select
                card.querySelector('.chart-ui select') ||
                card.querySelector('.chart-type select') ||
                card.querySelector('select[class*="chart"]') ||
                card.querySelector('header select') || // Added header select
  ...
  function ensureChangeListenerAttached() {
    ...
    // [KT:FIX-DROPDOWN-SWITCHING] Enhanced event listener with proper bubbling and delegation
      ...
      function (e) {
        try {
          ...
          var isChartSelect = (
            ...
            (target.className && typeof target.className === 'string' && target.className.toLowerCase().includes('chart')) ||
            target.closest('.chart-type') ||
            target.closest('.chart-ui') ||
            target.closest('.chart-card') ||
            target.closest('label.chart-type') // Added label.chart-type selector
          );
          ...
          // Don't stop propagation - allow event to bubble
          ...
          if (card) {
            ...
            setTimeout(function() { applyType(card); }, 10);
          } else {
...
function svgAxes({ w, h, p, pTop = 60, pBottom = 70, xTitle, yTitle, maxY, ticks = 5, labels = [], skipNiceMax = false }) {
  ...
  // [KT:FIX-PADDING-v6] Use separate top/bottom padding for proper plot area
  const topMargin = pTop || 60;
  const bottomMargin = pBottom || 70;
  const baselineY = h - bottomMargin;
  const plotHeight = baselineY - topMargin; // Actual available vertical space for plot

  // grid + Y tick values
  // [KT:FIX-Y-AXIS-SPACING] Improved grid to use actual plot height and better tick spacing
  const grid = [];
  for (let i = 0; i <= ticks; i++) {
    // Y position: from top (topMargin) to baseline (baselineY), using actual plot height
    const y = topMargin + (i * plotHeight) / ticks;
  ...
  // X tick labels - horizontal with word wrapping for long labels
  const slot = (w - 2 * p) / Math.max(labels.length, 1);
  // [KT:FIX-XAXIS-HORIZONTAL-WRAP] Use horizontal labels with wrapping instead of rotation
  const wrapXLabel = (text, maxCharsPerLine = 12) => {
    if (!text) return [''];
  ...
  const xTicks = labels.map((t, i) => {
    ...
    // [KT:FIX-OVERCROWDING] More generous character limit - use at least 12 chars even with narrow slots
    const wrappedLines = wrapXLabel(labelText, Math.max(12, Math.floor(slot / 6))); // Increased from /8 to /6 for more space
    const lineHeight = 15; // Increased from 14 to 15 for better readability
    const startY = baselineY + 20; // Increased from 18 to 20 for more spacing
    
    // [KT:FIX-XAXIS-HORIZONTAL] Horizontal multi-line labels instead of rotated
    return wrappedLines.map((line, idx) => 
      `<text x="${x}" y="${startY + idx * lineHeight}" text-anchor="middle" fill="#FFFFFF" font-size="11" font-weight="600" class="x-axis-label">${esc(line)}</text>`
    ).join('');
  }).join('');

  // [KT:FIX-AXES-v6] axes path - use topMargin for top of Y-axis
  const axes = `<polyline points="${p},${topMargin} ${p},${baselineY} ${w - p},${baselineY}" fill="none" stroke="rgba(255,255,255,.65)"/>`;

  // Axis titles - Y-axis with TEXT WRAPPING for long translations, X-axis centered below
  // [KT:FIX-YAXIS-WRAP-v7] Improved wrapping for very long Y-axis titles - shorter lines
  const wrapYTitle = (text, maxCharsPerLine = 14) => {
    if (!text) return [];
    const fullText = String(text);
    if (fullText.length <= maxCharsPerLine) return [fullText];
    ...
    for (const word of words) {
      ...
      if (testLine.length <= maxCharsPerLine) {
        ...
        if (currentLine) lines.push(currentLine);
        // If single word exceeds limit, force break it
        currentLine = word.length > maxCharsPerLine ? word.substring(0, maxCharsPerLine - 1) + '-' : word;
      }
    }
    if (currentLine) lines.push(currentLine);
    
    // Limit to 4 lines max to prevent excessive height
    return lines.slice(0, 4);
  };
  
  // Generate wrapped Y-axis title SVG with multiple lines
  // [KT:FIX-YAXIS-TOP-v7] Position Y-axis label at TOP with better spacing for wrapped long titles
  const yTitleLines = wrapYTitle(String(yTitle || ''), 14);
  let yTitleText = '';
  if (yTitleLines.length > 0) {
    const lineHeight = 14; // Tighter space between wrapped lines (was 16)
    ...
    const startY = topMargin + 15; // Reduced from 20 to 15 for tighter spacing
    
    // Create multiline Y-axis title with smaller font for long titles
    const fontSize = yTitleLines.length > 2 ? '11' : '12'; // Smaller font if wrapped to 3+ lines
    yTitleText = yTitleLines.map((line, idx) => 
      `<text x="${p + 8}" y="${startY + idx * lineHeight}" text-anchor="start" fill="#FFFFFF" font-size="${fontSize}" font-weight="600" opacity="0.9">${esc(line)}</text>`
    ).join('\n    ');
  }
  
  // X-title positioning: account for wrapped x-axis labels
  // [KT:FIX-XAXIS-HORIZONTAL-WRAP] Calculate space needed for wrapped labels (up to 3 lines * 14px + buffer)
  const maxXLabelLines = Math.max(...labels.map(l => wrapXLabel(String(l || ''), Math.max(8, Math.floor(slot / 8))).length));
  const xLabelSpace = maxXLabelLines * 14 + 12;
  const xTitleY = baselineY + xLabelSpace + 8;
...
/* ===========================================================
   ...
   ===========================================================*/
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '', lang = 'English', ui, aiSummary } = {}) 
  ...
  // [KT:FIX-PADDING-OVERFLOW-v6] Separate left padding from vertical padding to prevent chart collapse
  // Left padding (p) is for Y-axis labels, but plot height must remain positive
  ...
  // Left padding can be up to 180px max (was 250, which broke the chart)
  const p = Math.min(Math.max(110 + extraPadding, 110), 180);
  // Top/bottom padding is fixed at 60px to ensure adequate plot height
  const pTop = 60;
  const pBottom = 70;
  
  // [KT:FIX-ARRAY-OF-ARRAYS] Handle both flat arrays and arrays of arrays (multiple series)
  let dataArray = series;
  if (Array.isArray(series) && series.length > 0 && Array.isArray(series[0])) {
    // If series is array of arrays (multiple datasets), use first dataset for line chart
    dataArray = series[0];
  }
  
  const data = (Array.isArray(dataArray) ? dataArray : []).map(Number).filter(x => Number.isFinite(x) && x >= 0);
  ...
  // [KT:FIX-PLOT-HEIGHT-v6] Calculate actual plot area height using separate top/bottom padding
  const plotHeight = h - pTop - pBottom;  // e.g. 450 - 60 - 70 = 320px
  const baseY = h - pBottom;              // e.g. 450 - 70 = 380
  ...
  // [KT:FIX-CHART-POINTS-v6] Use plotHeight and baseY for correct Y positioning
  const ptsArr = data.map((v, i) => {
    const x = p + i * slot;
    // Y = baseY - (value/max * plotHeight) : 0 at baseY, max at pTop
    const y = baseY - (v / M) * plotHeight;
    // Clamp Y to ensure points stay within plot area
    const clampedY = Math.max(pTop, Math.min(y, baseY));
    return { x, y: clampedY };
  });
  ...
  // [KT:AI-SUMMARY-v1.0] Use AI-generated summary if provided, otherwise fall back to template
  console.log('[KT:AI-SUMMARY-DEBUG] aiSummary received in lineChartHTML:', aiSummary ? JSON.stringify(aiSummary).slice(0, 100) : 'null/undefined');
  const summary = chartSummaryRow({ trend, firstVal, lastVal, changePercent, dataMax, dataAvg, yTitle, aiSummary });
  ...
  function chartSummaryRow({ trend, firstVal, lastVal, changePercent, dataMax, dataAvg, yTitle, aiSummary }) {
    // [KT:AI-SUMMARY] If AI-generated summary is provided, use it directly
    if (aiSummary && aiSummary.overall && aiSummary.keyInsights) {
      console.log('[KT:AI-SUMMARY] Using AI-generated summary for chart');
      return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">${t('Overall')}:</span> <span style="color:#fff;">${aiSummary.overall}</span></div>
        <div><span style="font-weight:bold;">${t('Key Insights')}:</span> <span style="color:#fff;">${aiSummary.keyInsights}</span></div>
      </div>
    `;
    }
    ...
    <div class="canvas-wrap">
      ...
      <svg class="plot line" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        <defs>
          ...
          <clipPath id="plotClip">
            <rect x="${p - 10}" y="${pTop - 10}" width="${w - p + 20}" height="${plotHeight + 20}"/>
          </clipPath>
        </defs>
        ${svgAxes({ w, h, p, pTop, pBottom, xTitle: xTitleT, yTitle: yTitleT, maxY: M, labels: lbl, skipNiceMax: true })}
      ...
      <svg class="plot bar" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ...
        ${data.map((v, i) => {
          ...
          const barH = (v / M) * plotHeight;
          const barY = baseY - barH;
          return `<rect x="${x.toFixed(1)}" y="${barY.toFixed(1)}" width="${bw.toFixed(1)}" height="${barH.toFixed(1)}" fill="#10B981"/>`;
        }).join('')}
      </svg>
...
export function barChartHTML(args) { 
  ...
  */
  ...
  // [KT:AI-SUMMARY] Pass through aiSummary to lineChartHTML
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]], lang = 'English' }) {
  ...
  // [KT:HEATMAP-YAXIS-CUTOFF-FIX-v3] Calculate padding first, then adjust width to maintain chart area
  ...
  const charWidth = (hasParentheses || hasCurrency) ? 14 : 13;
  const extraPadding = Math.max(0, Math.ceil((yMaxLength - 6) * charWidth));
  const leftPadding = Math.min(Math.max(100 + extraPadding, 100), 200); // Left padding for Y-axis labels
  const rightPadding = 40; // Smaller right padding
  const p = 40; // Top/bottom padding (for calculations that use p)
  
  // [KT:HEATMAP-WIDTH-FIX] Increase total width when labels are longer to maintain chart area size
  const baseWidth = 760;
  const w = baseWidth + (leftPadding - 100); // Add extra width for longer labels
  const h = 320;
  ...
  console.log('[KT:HEATMAP-PADDING-DIAGNOSTIC]', {
    ...
    total_width: w,
    formula: `baseWidth(${baseWidth}) + (leftPadding(${leftPadding}) - 100) = ${w}px`
  });
  ...
  // [KT:HEATMAP-CELL-SIZE-FIX] Calculate chart area after accounting for left and right padding
  const chartWidth = w - leftPadding - rightPadding;
  const chartHeight = h - 2 * p;
  const cw = chartWidth / c.length, ch = chartHeight / r.length;
  ...
  const cells = r.flatMap((_, ri) => c.map((_, ci) => {
    ...
    const x = leftPadding + ci * cw, y = p + ri * ch;
    return `<rect x="${x}" y="${y}" width="${cw}" height="${ch}" fill="hsl(${hue},60%,45%)"/>`;
  })).join('');

  const grid = `
    ${Array.from({ length: r.length + 1 }, (_, i) => {
      const y = p + i * ch; return `<line x1="${leftPadding}" y1="${y}" x2="${w - rightPadding}" y2="${y}" stroke="rgba(255,255,255,.35)"/>`;
    }).join('')}
    ${Array.from({ length: c.length + 1 }, (_, i) => {
      const x = leftPadding + i * cw; return `<line x1="${x}" y1="${p}" x2="${x}" y2="${h - p}" stroke="rgba(255,255,255,.35)"/>`;
    }).join('')}
  `;

  const xlab = c.map((label, i) => `<text x="${leftPadding + (i + .5) * cw}" y="${h - 6}" text-anchor="middle" fill="#FFFFFF" font-size="14" font-weight="700">${esc(t(label))}</text>`).join('');
  const ylab = r.map((label, i) => `<text x="${leftPadding - 8}" y="${p + (i + .5) * ch + 6}" text-anchor="end" fill="#FFFFFF" font-size="14" font-weight="700">${esc(t(label))}</text>`).join('');
...
.canvas-wrap{ position:relative; overflow:hidden; border-radius:8px; max-width:100%; }
/* [KT:SVG-OVERFLOW-FIX] Constrain SVGs to fit container width on smaller screens */
.canvas-wrap svg {
  max-width: 100%;
  height: auto;
}
