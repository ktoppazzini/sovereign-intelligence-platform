...
function sanitizeJSONForParsing(raw) {
  ...
  // Step 1: Remove embedded newlines that break JSON
  jsonStr = jsonStr.replace(/[\r\n]+/g, ' ');
  
  // Step 2: Comprehensive UTF-8 corruption replacement
...
async function buildPrioritizationMatrixSpec(canon = {}, sections = {}, reportHTML = '', translatedLabels = {}) {
  try {
    ...
    const prompt = `
You are a transformation consultant. Analyze this implementation plan and identify 5-6 KEY INITIATIVES.
For each initiative, estimate its Business Impact (0-100) and Implementation Effort (0-100).

Return STRICT JSON:
{
  "title": "Initiative Prioritization",
  "initiatives": [
    { "name": "Initiative name (e.g., 'Digital Transformation', 'Cost Optimization')", "impact": number, "effort": number, "value": number },
    ... (5-6 items)
  ]
}

Rules:
- Extract actual initiatives mentioned in the report (NOT generic defaults)
...
- Use professional initiative names from the report context
- Return in ENGLISH; we will translate
- NO commentary; JSON only.

Context (HTML from nextSteps/implementation section):
...
`;
    ...
    if (!parsed || !parsed.initiatives || !Array.isArray(parsed.initiatives)) {
      ...
      return {
        title: 'Initiative Prioritization',
        initiatives: [
          { name: 'Digital Transformation', impact: 95, effort: 90, value: 4.2 },
          { name: 'Process Automation', impact: 85, effort: 30, value: 2.5 },
          { name: 'Quick Wins', impact: 45, effort: 15, value: 0.8 },
          { name: 'Cloud Migration', impact: 70, effort: 80, value: 1.8 },
          { name: 'Team Training', impact: 50, effort: 20, value: 0.5 },
          { name: 'Vendor Consolidation', impact: 60, effort: 40, value: 1.2 }
        ]
      };
    }

    // Translate initiative names if language is not English
    if (lang && lang !== 'English' && translatedLabels) {
      for (let init of parsed.initiatives) {
        if (init.name && translatedLabels[init.name]) {
          init.nameTranslated = translatedLabels[init.name];
        }
      }
    }

    console.log(TAG, '[MATRIX-PREDICTIVE] Spec generated', {
      initiativeCount: parsed.initiatives?.length || 0,
      hasTranslations: lang !== 'English'
    });
...
`;
    ...
    // Translate component names if language is not English
    if (lang && lang !== 'English' && translatedLabels) {
      for (let comp of parsed.components) {
        if (comp.name && translatedLabels[comp.name]) {
          comp.nameTranslated = translatedLabels[comp.name];
        }
      }
      if (parsed.currentState?.label && translatedLabels[parsed.currentState.label]) {
        parsed.currentState.labelTranslated = translatedLabels[parsed.currentState.label];
      }
      if (parsed.targetState?.label && translatedLabels[parsed.targetState.label]) {
        parsed.targetState.labelTranslated = translatedLabels[parsed.targetState.label];
      }
    }

    console.log(TAG, '[WATERFALL-PREDICTIVE] Spec generated', {
...
`;
    ...
    // Translate risk names if language is not English
    if (lang && lang !== 'English' && translatedLabels) {
      for (let risk of parsed.risks) {
        if (risk.name && translatedLabels[risk.name]) {
          risk.nameTranslated = translatedLabels[risk.name];
        }
        if (risk.description && translatedLabels[risk.description]) {
          risk.descriptionTranslated = translatedLabels[risk.description];
        }
      }
    }

    console.log(TAG, '[RISK-HEATMAP-PREDICTIVE] Spec generated', {
...
async function buildChartSummaryTranslations(targetLang) {
  console.log(TAG, '🔍 [TRANSLATION-DEBUG] buildChartSummaryTranslations called with:', targetLang, 'type:', typeof targetLang);
  if (!targetLang || /^english$/i.test(targetLang)) {
    console.log(TAG, '⚠️ [TRANSLATION-DEBUG] Early return - English detected or no lang');
    return {}; // No translation needed for English
  }
  console.log(TAG, '✅ [TRANSLATION-DEBUG] Proceeding with translation for:', targetLang);
  ...
  try {
    const phraseCount = Object.keys(phrasesObject).length;
    console.log(TAG, '🔧 [TRANSLATION-DEBUG] Starting translation for:', targetLang, 'with', phraseCount, 'phrases');
    const openai = await getOpenAI();
    console.log(TAG, '✅ [TRANSLATION-DEBUG] OpenAI client obtained');
    
    // Send as JSON object - this is what GPT-5-nano expects
    const prompt = `Translate all values in this JSON object from English to ${targetLang}.
Keep all keys exactly as they are (in English). Only translate the empty string values.

Input JSON:
${JSON.stringify(phrasesObject, null, 2)}

Return a JSON object with the SAME keys but with ${targetLang} translations as values.
Example output format:
{
  "Overall": "[${targetLang} translation]",
  "Key Insights": "[${targetLang} translation]",
  ...
}

Translate ALL ${phraseCount} phrases.`;
    
    console.log(TAG, '📝 [TRANSLATION-DEBUG] Prompt length:', prompt.length, 'chars');
    console.log(TAG, '📤 [TRANSLATION-DEBUG] Sending JSON object with', phraseCount, 'keys to translate');
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: `You are a professional translator. You receive a JSON object with English keys and empty values. Return the same JSON with the SAME English keys but populate the values with ${targetLang} translations. Preserve all keys exactly.` },
        { role: 'user', content: prompt }
      ],
      max_completion_tokens: 15000
    });
    
    console.log(TAG, '✅ [TRANSLATION-DEBUG] API response received');
    console.log(TAG, '📊 [TRANSLATION-DEBUG] Response status:', res.choices?.[0]?.finish_reason);
    console.log(TAG, '🔍 [TRANSLATION-DEBUG] Full response object:', JSON.stringify(res, null, 2).substring(0, 1000));
    
    const rawContent = res.choices?.[0]?.message?.content || '{}';
    console.log(TAG, '🔍 [TRANSLATION-DEBUG] Content length:', rawContent.length, 'chars');
    console.log(TAG, '📄 [TRANSLATION-DEBUG] FULL Raw content:', rawContent);
    
    const translations = JSON.parse(rawContent);
    console.log(TAG, '✅ [TRANSLATION-DEBUG] JSON parsed successfully');
    console.log(TAG, '📋 [TRANSLATION-DEBUG] Translation keys:', Object.keys(translations).slice(0, 10));
    console.log('\n' + '📈'.repeat(40));
    console.log('📈 [CHART-TRANSLATION-v3.0] CHART UI PHRASES TRANSLATED');
    console.log(TAG, 'chartSummary.translations.built', { 
      lang: targetLang, 
      count: Object.keys(translations).length,
      '🔤 Key Phrases': {
        Overall: translations['Overall'],
        'Key Insights': translations['Key Insights'],
        increasing: translations['increasing'],
        Years: translations['Years'],
        Months: translations['Months'],
        Discovery: translations['Discovery'],
        Waterfall: translations['Waterfall']
      }
    });
    console.log('📈'.repeat(40) + '\n');
    
    // [KT:EMERGENCY-FIX] If GPT-5-nano returned empty, try without JSON mode
    if (Object.keys(translations).length === 0) {
      console.log(TAG, '⚠️ [TRANSLATION-EMERGENCY] GPT-5-nano returned empty JSON, retrying without json_object mode...');
      
      const firstPhrases = Object.keys(phrasesObject).slice(0, 20);
      const retryPrompt = `Translate these business chart phrases from English to ${targetLang}. Return a valid JSON object with English keys and ${targetLang} values.

English phrases to translate:
${firstPhrases.map((p, i) => `${i + 1}. "${p}"`).join('\n')}

Return format: {"phrase1": "translation1", "phrase2": "translation2", ...}`;
      
      const retryRes = await openai.chat.completions.create({
        model: TRANSLATION_MODEL,
        temperature: 1,
        messages: [
          { role: 'system', content: `You are a professional translator. Return ONLY a JSON object.` },
      ...
      console.log(TAG, '🔧 [TRANSLATION-EMERGENCY] Retry response length:', retryContent.length);
      console.log(TAG, '📄 [TRANSLATION-EMERGENCY] Retry content:', retryContent);
      
      try {
        const retryTranslations = JSON.parse(retryContent);
        console.log(TAG, '✅ [TRANSLATION-EMERGENCY] Retry succeeded with', Object.keys(retryTranslations).length, 'phrases');
        return retryTranslations;
      } catch (e) {
        console.log(TAG, '❌ [TRANSLATION-EMERGENCY] Retry also failed:', e.message);
      }
    }
    
    return translations;
  } catch (e) {
    console.error(TAG, '❌ [TRANSLATION-ERROR] Full error:', e);
    console.error(TAG, '❌ [TRANSLATION-ERROR] Error message:', String(e?.message || e));
    console.error(TAG, '❌ [TRANSLATION-ERROR] Error stack:', e?.stack);
...
// Try to extract array first (for KPI selection and similar batch responses)
const firstBracket = str.indexOf('[');
const lastBracket = str.lastIndexOf(']');
if (firstBracket >= 0 && lastBracket > firstBracket) {
  return str.slice(firstBracket, lastBracket + 1);
}
// Fallback to extracting object
...
function sectionPrompt(section, canon, minWords, L) {
  ...
  const languageInstruction = canon.lang && !/^english$/i.test(canon.lang) 
    ? `\n\nIMPORTANT: Generate ALL content in ${canon.lang}. Use professional business terminology appropriate for ${canon.lang}. Do NOT use English phrases like "So what" - use natural ${canon.lang} expressions.`
    : '';
...
<div class="insight-callout">
  <div class="insight-callout-header">${getPromptLabel('KEY INSIGHT', 'KEY INSIGHT')}</div>
  <div class="insight-callout-title">[Bold statement of finding]</div>
  <div class="insight-callout-impact">$[X]M ${getPromptLabel('Potential value', 'Potential value')}</div>
...
function expandPrompt(section, canon, remainingWords) {
  const languageInstruction = canon.lang && !/^english$/i.test(canon.lang) 
    ? `\n\nIMPORTANT: Generate ALL content in ${canon.lang}. Use professional business terminology appropriate for ${canon.lang}. Do NOT use English phrases - use natural ${canon.lang} expressions throughout.`
    : '';
...
/* ========================================================================== */
function buildAppendicesHTML(canon, sections, L) {
  // [KT:SURGICAL] Original placeholder commented to preserve history:
  // $appendix
  // [KT:TRANSLATION-FIX] Pass L to translate appendix labels

  // Translation helper
  const getLabel = (key, defaultEn) => {
    if (L && L[key]) return L[key];
    if (L && L[key.toLowerCase()]) return L[key.toLowerCase()];
    return defaultEn;
  };
  ...
  try {
    ...
    const appendixHTML = `<section id="appendices">
         <h3>${getLabel('Appendices', 'Appendices')}</h3>
         <h4>${getLabel('Generation Diagnostics', 'Generation Diagnostics')}</h4>
         <table class="report-table">
           <thead>
             <tr><th>${getLabel('Section', 'Section')}</th><th>${getLabel('Words', 'Words')}</th><th>${getLabel('Charts', 'Charts')}</th><th>${getLabel('Heatmaps', 'Heatmaps')}</th><th>${getLabel('Benchmarks', 'Benchmarks')}</th><th>${getLabel('Tables', 'Tables')}</th></tr>
           </thead>
           <tbody>${rows}</tbody>
         </table>
       </section>`;
    ...
    return `<section id="appendices"><h3>${getLabel('Appendices', 'Appendices')}</h3><p>${getLabel('Appendix generation failed gracefully.', 'Appendix generation failed gracefully.')}</p></section>`;
  }
}
...
async function translateChartLabels(obj, lang) {
  ...
  try {
    ...
    if (!textsToTranslate.title && !textsToTranslate.xTitle && !textsToTranslate.yTitle && textsToTranslate.labels.length === 0) {
      return obj;
    }

    const prompt = `Translate the following chart elements from English to ${lang}. Keep the same structure and JSON format.

Chart elements to translate:
${JSON.stringify(textsToTranslate, null, 2)}

Return a JSON object with the same keys but with ${lang} translations as values.
Only translate the text values, do NOT change keys or structure.
Example output:
{
  "title": "[${lang} translation of title]",
  "xTitle": "[${lang} translation of xTitle]",
  "yTitle": "[${lang} translation of yTitle]",
  "labels": ["[${lang} translation]", "[${lang} translation]", ...]
}`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a translator. Translate all text to ${lang}. Return ONLY valid JSON, no markdown, no code blocks, no explanations. Do NOT use response_format, just return raw JSON.`
        },
        {
          role: 'user',
          content: prompt
...
async function translateLabels(targetLang) {
  ...
  try {
    ...
    const criticalLabels = {
      ...
      trendOutperform: '+70pp vs baseline',
...
export async function POST(req) {
  ...
  if (canon.lang && !/^english$/i.test(canon.lang)) {
    ...
    console.log(TAG, '✅ [TRANSLATION-INIT] Graph translators enabled with', Object.keys(summaryTranslations).length, 'phrases');
    console.log(TAG, '🔍 [TRANSLATION-INIT] Sample:', {
      'Overall': summaryTranslations['Overall'],
      'Key Insights': summaryTranslations['Key Insights'],
      'increasing': summaryTranslations['increasing']
    });
  }
  ...
  // === PRE-BUILD CHART TRANSLATIONS BEFORE SECTION PROCESSING ===
  // KPI charts are generated INSIDE Promise.all, so we need translations loaded first
  console.log(TAG, '🔧 [PRE-TRANSLATE] Building chart translations BEFORE Promise.all...');
  let translatedLabels = DEFAULT_LABELS;
  
  if (canon.lang && !/^english$/i.test(canon.lang)) {
    try {
      // Build chart summary translations early
      const chartTranslations = await buildChartSummaryTranslations(canon.lang);
      __chartSummaryTranslationCache[canon.lang] = chartTranslations;
      
      // Merge into translatedLabels so KPI section can use them
      translatedLabels = Object.assign({}, DEFAULT_LABELS, chartTranslations);
      console.log(TAG, '✅ [PRE-TRANSLATE] Chart translations loaded:', {
        lang: canon.lang,
        chartTransCount: Object.keys(chartTranslations).length,
        merged: Object.keys(translatedLabels).length
      });
    } catch (e) {
      console.warn(TAG, '⚠️  [PRE-TRANSLATE] Failed to pre-build translations:', String(e?.message || e));
  ...
  const sectionResults = await Promise.all(order.map(async (key) => {
    ...
    if (key === 'kpis') {
      try {
        console.log(TAG, '🔧 [KPI-PREDICTIVE] Starting AI-selected KPI chart generation...');
        ...
        if (canon.lang && !/^english$/i.test(canon.lang)) {
          ...
          setGraphTranslator(translatorFn);
          setWorldClassVisualsTranslator(translatorFn);
          console.log(TAG, '✅ [KPI-PREDICTIVE] Graph translators enabled for:', canon.lang);
        }
        
        // Verify chart functions are available
        if (!barChartHTML || !lineChartHTML) {
          console.error(TAG, '❌ [KPI-PREDICTIVE] ERROR: Chart functions not imported!');
          throw new Error('Chart functions not available');
        }
        console.log(TAG, '✅ [KPI-PREDICTIVE] Chart functions imported successfully');
        
        // [KT:SURGICAL:KPI-SELECTION] Step 1: AI selects 4 relevant KPIs from the report
        console.log(TAG, '🔧 [KPI-PREDICTIVE] Step 1: Analyzing report to select relevant KPIs...');
        const selectedKPIs = await selectRelevantKPIs(canon, sections, html);
        console.log(TAG, 'kpi.selection.completed', {
          count: selectedKPIs?.length || 0,
          kpis: selectedKPIs?.map(k => k.name) || []
        });
        
        if (!selectedKPIs || selectedKPIs.length < 4) {
          console.error(TAG, '❌ [KPI-PREDICTIVE] KPI selection returned < 4 KPIs');
          throw new Error('KPI selection failed');
        }
        
        // Wrap all KPI charts in dashboard-wide section
        let kpiCharts = '<section class="kpi-dash dashboard-wide">\n';
        console.log(TAG, '✅ [KPI-PREDICTIVE] Opened dashboard-wide section');
        
        const kpiResults = { generated: 0, translated: 0, failed: 0, charts: [] };
        
        // [KT:SURGICAL:KPI-DATA-EXTRACTION] Step 2: For each selected KPI, extract data via predictive model
        for (let i = 0; i < selectedKPIs.length; i++) {
          const kpiName = selectedKPIs[i].name;
          const kpiDesc = selectedKPIs[i].description;
          
          console.log(TAG, `🔧 [KPI-PREDICTIVE] KPI ${i + 1}: Extracting data for "${kpiName}"...`);
          
          try {
            const kpiSpec = await buildPredictiveModelSpec(canon, sections, html, translatedLabels);
            console.log(TAG, `kpi.${i + 1}.${kpiName.toLowerCase().replace(/ /g, '_')}.extracted`, {
              hasLabels: !!kpiSpec?.labels,
              labelsLen: kpiSpec?.labels?.length || 0,
              hasValues: !!kpiSpec?.values,
              valuesLen: kpiSpec?.values?.length || 0,
              xTitle: kpiSpec?.xTitle,
              yTitle: kpiSpec?.yTitle
            });
            
            if (kpiSpec && kpiSpec.labels && kpiSpec.values) {
              ...
              let kpiChartSpec = {
                title: kpiName,
                labels: kpiSpec.labels,
                series: [kpiSpec.values],
                xTitle: kpiSpec.xTitle || 'Time Period',
                yTitle: kpiSpec.yTitle || 'Value'
              };
              
              if (canon.lang && !/^english$/i.test(canon.lang)) {
                try {
                  kpiChartSpec = await translateChartLabels(kpiChartSpec, canon.lang);
                  console.log(TAG, `kpi.${i + 1}.translated`, {
                    title: kpiChartSpec.title,
                    xTitle: kpiChartSpec.xTitle,
                    yTitle: kpiChartSpec.yTitle
                  });
                  kpiResults.translated++;
                } catch (e) {
                  console.log(TAG, `kpi.${i + 1}.translate.error`, String(e?.message || e));
                }
              }
              
              const kpiChart = chartFn({ ...kpiChartSpec, lang: canon.lang });
              kpiCharts += kpiChart + '\n';
              kpiResults.generated++;
              kpiResults.charts.push({
                kpi: kpiName,
                type: chartType,
                htmlSize: kpiChart.length,
                title: kpiChartSpec.title
              });
              console.log(TAG, `✅ [KPI-PREDICTIVE] KPI ${i + 1} (${kpiName}) generated:`, kpiChart.length, 'chars');
            } else {
              console.log(TAG, `⚠️ [KPI-PREDICTIVE] KPI ${i + 1} (${kpiName}) returned no data, skipping`);
              kpiResults.failed++;
            }
          } catch (e) {
            console.error(TAG, `❌ [KPI-PREDICTIVE] KPI ${i + 1} (${kpiName}) generation error:`, String(e?.message || e));
            kpiResults.failed++;
          }
        }
        
        kpiCharts += '</section>';
        console.log(TAG, '✅ [KPI-PREDICTIVE] Closed dashboard-wide section');
        
        console.log(TAG, '📊 [KPI-PREDICTIVE] FINAL SUMMARY:', {
          totalSelected: selectedKPIs?.length || 0,
          selectedKPIs: selectedKPIs?.map(k => k.name) || [],
          totalGenerated: kpiResults.generated,
          totalTranslated: kpiResults.translated,
          totalFailed: kpiResults.failed,
          totalHtmlSize: kpiCharts.length,
          hasDashboardWideClass: kpiCharts.includes('dashboard-wide'),
          lang: canon.lang,
          chartsDetail: kpiResults.charts
        });
        
        // Prepend KPI charts to section HTML
        const beforePrepend = html.length;
        html = kpiCharts + '\n' + html;
        console.log(TAG, '✅ [KPI-PREDICTIVE] Charts prepended to section HTML');
        console.log(TAG, '📈 [KPI-PREDICTIVE] HTML size: before =', beforePrepend, ', after =', html.length, ', added =', html.length - beforePrepend);
        
      } catch (e3) {
        console.error(TAG, '❌ [KPI-PREDICTIVE] FATAL ERROR:', String(e3?.message || e3));
        console.error(TAG, '❌ [KPI-PREDICTIVE] Stack trace:', e3?.stack);
      }
    }
  ...
  for (const { key, html } of sectionResults) {
    ...
    // [KT:TABLE-TRANSLATION-FIX] Translate ALL sections for non-English languages
    // Each section contains tables, text, and charts that were generated in English by GPT
    if (canon.lang && !/^english$/i.test(canon.lang) && html) {
      try {
        sections[key] = await translateHTML(html, canon.lang);
        console.log(TAG, `✅ Translated ${key} section to`, canon.lang);
      } catch (e) {
        console.log(TAG, `⚠️ ${key} translation error:`, String(e?.message || e));
      }
    }
  }
  ...
  let appendicesHTML = buildAppendicesHTML(canon, sections, canon.lang);
  console.log(TAG, 'appendix.build.done', { bytes: String(appendicesHTML || '').length });

  // [KT:APPENDIX-TRANSLATION-FIX] Translate appendices HTML for non-English languages
  if (appendicesHTML && canon.lang && !/^english$/i.test(canon.lang)) {
    try {
      appendicesHTML = await translateHTML(appendicesHTML, canon.lang);
      console.log(TAG, '✅ Translated appendices to', canon.lang);
    } catch (e) {
      console.log(TAG, '⚠️ Appendices translation error:', String(e?.message || e));
    }
  }
  ...
  // === MULTILINGUAL: Translate UI labels via GPT-5-nano ===
  // [KT:REASSIGN] Re-assign translatedLabels with GPT-translated version (was initialized early at line ~3403)
  translatedLabels =
    (await translateLabels(canon.lang)) || DEFAULT_LABELS;

console.log(TAG, 'labels.translated', {
  lang: canon.lang,
  executiveSummary: translatedLabels?.executiveSummary,
  tableOfContents: translatedLabels?.tableOfContents,
  financialAnalysis: translatedLabels?.financialAnalysis,
  projectedROI: translatedLabels?.projectedROI,
  ...
  if (canon.lang && canon.lang.toLowerCase() !== 'english') {
    console.log(TAG, '🔧 [WORLDCLASS-TRANSLATOR] Setting up translator for world-class visuals:', canon.lang);
    
    // Ensure translation cache is available
  ...
  console.log(TAG, '🔍 [EXEC-INFOGRAPHIC-DEBUG] Passing to executiveSummaryInfographicHTML:', {
    lang: canon.lang,
    translatedLabelsKeys: translatedLabels ? Object.keys(translatedLabels).slice(0, 15) : 'undefined',
    hasExecutiveSummaryAtAGlance: translatedLabels ? translatedLabels['Executive Summary at a Glance'] : 'N/A',
    totalTranslatedLabels: translatedLabels ? Object.keys(translatedLabels).length : 0
  });
  ...
  // 4. Risk Heatmap - extract from RAID if available
    ...
    // [KT:SURGICAL:PREDICTIVE:ATTACH]
    try {
      let predictiveModel = await buildPredictiveModelSpec(canon, sections, '', DEFAULT_LABELS);
  ...
  try {
    ...
    // [KT:RISK-HEATMAP-PREDICTIVE] AI-driven risk selection with full translation
    const riskSpec = await buildRiskHeatmapSpec(canon, sections, sections.risks || '', translatedLabels);
    
    if (riskSpec && riskSpec.risks && riskSpec.risks.length > 0) {
      ...
      console.log(TAG, 'risk.selection.completed', {
        selectedRisks: risks.map(r => r.name),
        count: risks.length
      });
      console.log(TAG, 'risk.translated', {
        hasTranslations: Object.keys(translatedLabels).length > 0,
        translated: risks.filter(r => r.name !== r.name.split(' ')[0]).length
      });
    }
  } catch (e) {
  ...
  // [KT:SURGICAL:PREDICTIVE:ATTACH]
  try {
    const predictiveModel = await buildPredictiveModelSpec(canon, sections, '', DEFAULT_LABELS);
    sections.predictiveModel = predictiveModel;
    console.log(TAG, 'predictiveModel.generated', {
      labels: predictiveModel?.labels?.length || 0,
      values: predictiveModel?.values?.length || 0,
  ...
  // 5. Waterfall Chart - AI-driven value bridge generation with translations
  let waterfallChart = '';
  try {
    const waterfallSpec = await buildWaterfallSpec(canon, sections, sections.financials || '', translatedLabels);
    
    if (waterfallSpec && waterfallSpec.components) {
...
async function buildAppendicesFromAI(reportHTML, canon, sections) {
  ...
  const languageInstruction = canon.lang && !/^english$/i.test(canon.lang)
    ? `\n\nIMPORTANT: Generate ALL content in ${canon.lang}. Use professional business terminology appropriate for ${canon.lang}. All titles, headers, labels, and data values must be in ${canon.lang}.`
    : '';
  ...
  try {
    ...
    const openai = await getOpenAI();
    const prompt = `You are a business consultant specializing in implementation strategy.
...
TASK: Based on these specific recommendations and insights, generate a set of detailed implementation appendices that will guide the client in executing these recommendations.

AI MUST DECIDE:
- Which appendices are actually needed for this project? (typically 3-6)
- What should each appendix contain to provide actionable implementation guidance?
- What are the key tables, checklists, or frameworks needed?
...
RETURN VALID JSON ONLY with structure:
{
  "appendices": [
    {
      "id": "A",
      "title": "Appendix A — [Title reflecting recommendation]",
      "description": "Brief purpose of this appendix",
      "type": "checklist|framework|timeline|resource_plan|decision_matrix|other",
      "columns": [
        { "name": "Column 1", "width": "30%", "description": "What this column contains" },
        { "name": "Column 2", "width": "40%", "description": "What this column contains" },
        { "name": "Column 3", "width": "30%", "description": "What this column contains" }
      ],
      "rows": [
        { "col1": "Row 1 Value 1", "col2": "Row 1 Value 2", "col3": "Row 1 Value 3" },
        { "col1": "Row 2 Value 1", "col2": "Row 2 Value 2", "col3": "Row 2 Value 3" }
      ]
    },
    {
      "id": "B",
  ...
  "summary": "Brief overview of appendices and their collective purpose"
}

CONSTRAINTS:
- All text must be in ${canon.lang || 'English'}
- Appendix IDs should be: A, B, C, D, E, F (using letters, not numbers)
- At least 3 appendices, maximum 6
- Each appendix must have 2-5 columns
- Each appendix must have 4-10 data rows
- Ensure appendices provide ACTIONABLE GUIDANCE for the recommendations
- Do NOT include generic content; everything must relate to the recommendations provided${languageInstruction}`;
    ...
    if (!appendixSpec || !Array.isArray(appendixSpec.appendices)) {
      console.warn(TAG, 'invalid appendix spec structure, using fallback');
      return {
        appendices: [],
        summary: 'Appendix generation failed - using fallback'
      };
    }
    ...
    return {
      appendices: [],
      summary: 'Appendix generation encountered an error'
    };
  }
}
...
function renderAppendicesHTML(appendixSpec, L = {}) {
  ...
  try {
    const appendicesHTML = appendixSpec.appendices.map((app, idx) => {
      if (!app.title || !Array.isArray(app.columns) || !Array.isArray(app.rows)) {
        console.warn(TAG, `appendix ${idx} has invalid structure, skipping`);
        return '';
      }
      ...
      // Build table header row with column widths
      const headerCells = app.columns.map(col => {
        const width = col.width || `${Math.round(100 / app.columns.length)}%`;
        return `<th style="width:${width}; padding:12px; text-align:left; background:#333333; color:#FFFFFF; font-weight:700; border-bottom:2px solid #10B981;">${sanitize(col.name || '')}</th>`;
      }).join('');

      // Build data rows - map each row object to cells based on column names
      const dataRows = app.rows.map((rowData, rIdx) => {
        const cells = app.columns.map((col, cIdx) => {
          ...
          const cellClass = rIdx % 2 === 0 ? 'style="background:#ffffff; color:#333333;"' : 'style="background:#f9f9f9; color:#333333;"';
          return `<td ${cellClass} style="padding:12px; border-bottom:1px solid #e0e0e0; font-size:0.95rem;">${sanitize(value)}</td>`;
        }).join('');
        return `<tr>${cells}</tr>`;
      }).join('');

      // Build complete appendix section with McKinsey styling
      const appendixHTML = `
        <section id="${appendixID}" style="margin: 3rem 0; padding: 2rem; background:#ffffff; border-radius:8px; page-break-inside:avoid;">
          <h2 style="margin:0 0 0.5rem 0; color:#000000; font-size:1.8rem; font-weight:700; font-family:'Merriweather', Georgia, serif;">${sanitize(app.title)}</h2>
          ${app.description ? `<p style="margin:0 0 1.5rem 0; color:#666666; font-size:0.95rem; line-height:1.6;">${sanitize(app.description)}</p>` : ''}
          
          <table class="report-table" style="width:100%; border-collapse:collapse; background:#ffffff; border:1px solid #e0e0e0; border-radius:4px; overflow:hidden;">
            <thead>
              <tr>${headerCells}</tr>
            </thead>
            <tbody>
    ...
    const appendicesSection = `
      <section id="appendices" style="margin:3rem 0 0 0;">
        <h1 style="margin:3rem 0 1.5rem 0; color:#000000; font-size:2.5rem; font-weight:700; font-family:'Merriweather', Georgia, serif; border-bottom:3px solid #10B981; padding-bottom:1rem;">${getLabel('appendices', 'Appendices')}</h1>
        ${appendixSpec.summary ? `<p style="margin:1rem 0 2rem 0; color:#666666; font-size:1rem; line-height:1.8;">${sanitize(appendixSpec.summary)}</p>` : ''}
        ${appendicesHTML}
      </section>
    `;
