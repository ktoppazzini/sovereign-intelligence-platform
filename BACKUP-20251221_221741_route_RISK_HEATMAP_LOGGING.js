...
`;
    ...
    let raw = (res?.choices?.[0]?.message?.content || '').trim();
    console.log(TAG, '[RISK-HEATMAP] GPT raw response length:', raw.length, 'preview:', raw.slice(0, 200));
    ...
    try {
      parsed = JSON.parse(jsonStr);
      console.log(TAG, '[RISK-HEATMAP] Parsed successfully, risks count:', parsed?.risks?.length || 0);
    } catch (parseErr) {
      console.error("[SR:REFORM] buildRiskHeatmapSpec.parse.error", parseErr, 'jsonStr preview:', jsonStr.slice(0, 300));
      parsed = null;
    }

    if (!parsed || !parsed.risks || !Array.isArray(parsed.risks) || parsed.risks.length === 0) {
      console.log(TAG, '⚠️ [RISK-HEATMAP-PREDICTIVE] AI risk generation failed or empty, using defaults');
      parsed = {
        risks: [
...
// [KT:FIX-v6] REMOVED cache - always do fresh translation call for each report
...
async function translateLabels(targetLang) {
  ...
  // [KT:FIX-v6] REMOVED cache check - always make fresh GPT call
  // This ensures all phrases get translated correctly each time
  ...
  try {
    ...
    // [KT:FIX-v6] REMOVED cache storage - fresh translation each time
    // __translateLabelsCache[targetLang] = labels;
    console.log(TAG, 'labels.translate.complete.fresh', { lang: targetLang, keyCount: Object.keys(labels).length });
