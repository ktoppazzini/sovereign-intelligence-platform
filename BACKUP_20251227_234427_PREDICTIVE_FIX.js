...
// [KT:SOVEREIGN-AI-INTEGRATION-v2.0] Import SovereignAI for self-learning on all API calls
import { sovereignAICall } from '@/lib/ai/sovereignAI.js';
...
// ============================================================================
// [KT:AI-CHART-SUMMARY] AI-Generated Chart Summaries
// Analyzes chart data and generates intelligent, contextual summaries in target language
// ============================================================================
async function generateAIChartSummary(chartSpec, lang = 'English', context = '') {
  try {
    const openai = await getOpenAI();
    const { title, labels, series, xTitle, yTitle } = chartSpec;
    
    // Flatten series data
    const data = Array.isArray(series[0]) ? series[0] : series;
    ...
    const prompt = `Analyze this chart data and write 2 brief insights in ${lang}:

Chart: "${title}"
X-Axis: ${xTitle}, Y-Axis: ${yTitle}
Data points: ${labels.map((l, i) => `${l}: ${data[i]}`).join(', ')}

Statistics:
- Trend: ${trend} (${changePercent}% change)
- Range: ${dataMin} to ${dataMax}
- Average: ${Math.round(dataAvg)}
...
{
  "overall": "One sentence describing the overall trend and its business meaning (with emoji 📈/📉/➡️)",
  "keyInsights": "One sentence with 2 specific insights about peak performance and notable patterns"
}

RULES:
- Write ENTIRELY in ${lang} (no English words)
- Be specific with numbers and percentages
- Focus on business impact, not just data description
...
- Return ONLY valid JSON, no markdown`;
    ...
    const res = await openai.chat.completions.create({
      ...
      temperature: 1,
      max_completion_tokens: 16000,  // [KT:FIX-TOKEN-LIMIT] Increased to 8000 for GPT-5-nano reasoning tokens
      messages: [
        { role: 'system', content: `You are a business analyst. Write chart insights in ${lang}. Return only JSON.` },
    ...
    if (jsonMatch) {
      const parsed = JSON.parse(sanitizeJSONForParsing(jsonMatch[0]));
      console.log('[KT:AI-CHART-SUMMARY] Generated:', { title, lang, overall: parsed.overall?.slice(0, 50) });
      return parsed;
    }
    
    console.warn('[KT:AI-CHART-SUMMARY] Failed to parse, using fallback');
    return null;
  } catch (e) {
    console.warn('[KT:AI-CHART-SUMMARY] Error:', e.message);
    return null;
  }
}

// Cache for AI-generated chart summaries to avoid duplicate calls
const __aiChartSummaryCache = new Map();

// [KT:FALLBACK] Simple fallback summary generator when AI fails
function generateFallbackChartSummary(chartSpec, lang = 'English') {
  try {
    const { title, labels, series, yTitle } = chartSpec;
    const data = Array.isArray(series[0]) ? series[0] : series;
    const firstVal = data[0];
    const lastVal = data[data.length - 1];
    const change = lastVal - firstVal;
    const changePercent = firstVal !== 0 ? ((change / firstVal) * 100).toFixed(1) : 0;
    const dataMax = Math.max(...data);
    const dataAvg = Math.round(data.reduce((a, b) => a + b, 0) / data.length);
    const trend = change > 0 ? 'increasing' : change < 0 ? 'decreasing' : 'stable';
    const icon = trend === 'increasing' ? '📈' : trend === 'decreasing' ? '📉' : '➡️';
    
    // Generate in target language using translatedLabels parameter
    const trendLabel = translatedLabels?.[trend.charAt(0).toUpperCase() + trend.slice(1)] || trend;
    const trendWord = translatedLabels?.trend || 'trend';
    const peakLabel = translatedLabels?.Peak || 'Peak';
    const avgLabel = translatedLabels?.Average || 'Average';
    
    const overall = `${icon} ${trendLabel} ${trendWord}: ${firstVal} → ${lastVal} (${changePercent > 0 ? '+' : ''}${changePercent}%)`;
    const keyInsights = `${peakLabel}: ${dataMax} | ${avgLabel}: ${dataAvg}`;
    
    console.log('[KT:FALLBACK-SUMMARY] Generated fallback for:', title);
    return { overall, keyInsights };
  } catch (e) {
    console.warn('[KT:FALLBACK-SUMMARY] Error:', e.message);
    return null;
  }
}

// [KT:RETRY] Simplified retry prompt for when first attempt fails
async function generateAIChartSummarySimple(chartSpec, lang = 'English') {
  try {
    const openai = await getOpenAI();
    const { title, labels, series } = chartSpec;
    const data = Array.isArray(series[0]) ? series[0] : series;
    const firstVal = data[0];
    const lastVal = data[data.length - 1];
    const dataMax = Math.max(...data);
    
    // Much simpler prompt for retry
    const prompt = `Chart "${title}" shows values from ${firstVal} to ${lastVal}, max ${dataMax}.
Write 2 short insights in ${lang}:
1. overall: Describe the trend (use 📈/📉/➡️)
2. keyInsights: Key finding

Return JSON: {"overall":"...","keyInsights":"..."}`;

    const res = await openai.chat.completions.create({
      model: process.env.OPENAI_MODEL || 'gpt-5-nano-2025-08-07',
      temperature: 1,
      max_completion_tokens: 16000,  // [KT:FIX-TOKEN-LIMIT] Increased to 6000 for GPT-5-nano reasoning tokens
    ...
    if (jsonMatch) {
      const parsed = JSON.parse(sanitizeJSONForParsing(jsonMatch[0]));
      if (parsed.overall && parsed.keyInsights) {
        console.log('[KT:AI-CHART-SUMMARY-RETRY] Success:', title);
        return parsed;
      }
    }
    return null;
  } catch (e) {
    console.warn('[KT:AI-CHART-SUMMARY-RETRY] Error:', e.message);
    return null;
  }
}

async function getOrGenerateChartSummary(chartSpec, lang, context = '') {
  console.log('[KT:AI-CHART-SUMMARY] Starting getOrGenerateChartSummary for:', chartSpec.title, 'lang:', lang);
  const cacheKey = `${chartSpec.title}_${lang}`;
  if (__aiChartSummaryCache.has(cacheKey)) {
    console.log('[KT:AI-CHART-SUMMARY] Returning cached summary for:', chartSpec.title);
    return __aiChartSummaryCache.get(cacheKey);
  }
  
  // Attempt 1: Full AI generation
  let summary = await generateAIChartSummary(chartSpec, lang, context);
  console.log('[KT:AI-CHART-SUMMARY] Attempt 1 result:', summary ? 'SUCCESS' : 'FAILED');
  
  // Attempt 2: Simplified AI prompt (try harder)
  if (!summary) {
    console.log('[KT:AI-CHART-SUMMARY] First attempt failed, trying simpler prompt...');
    summary = await generateAIChartSummarySimple(chartSpec, lang);
    console.log('[KT:AI-CHART-SUMMARY] Attempt 2 result:', summary ? 'SUCCESS' : 'FAILED');
  }
  
  // Attempt 3: Programmatic fallback (always works)
  if (!summary) {
    console.log('[KT:AI-CHART-SUMMARY] AI failed, using programmatic fallback...');
    summary = generateFallbackChartSummary(chartSpec, lang);
    console.log('[KT:AI-CHART-SUMMARY] Attempt 3 (fallback) result:', summary ? 'SUCCESS' : 'FAILED');
  }
  
  if (summary) {
    __aiChartSummaryCache.set(cacheKey, summary);
    console.log('[KT:AI-CHART-SUMMARY] Final summary cached:', JSON.stringify(summary).slice(0, 100));
  }
  return summary;
}

// ============================================================================
// [KT:PREDICTIVE-AI-SUMMARIES] Inject AI summaries into all predictive chart placeholders
// This function scans section HTML for <figure data-chart="..."> placeholders and
// generates AI summaries for each, injecting them back into the spec JSON
// ============================================================================
async function injectAISummariesIntoChartPlaceholders(sectionsObj, lang = 'English') {
  const TAG = '[KT:PREDICTIVE-AI-SUMMARIES]';
  console.log(TAG, 'start', { sectionCount: Object.keys(sectionsObj).length, lang });
  
  let totalCharts = 0;
  let successCount = 0;
  
  for (const [key, html] of Object.entries(sectionsObj)) {
    if (!html || typeof html !== 'string') continue;
    
    // Find all <figure data-chart="..."> placeholders
    const chartRegex = /<figure[^>]*data-chart=["']([^"']+)["'][^>]*>/gi;
    let match;
    ...
    while ((match = chartRegex.exec(html)) !== null) {
      ...
      const encodedSpec = match[1];
      
      try {
        // Decode the spec JSON
        const decoded = encodedSpec
          .replace(/&quot;/g, '"')
          .replace(/&amp;/g, '&')
          .replace(/&lt;/g, '<')
          .replace(/&gt;/g, '>');
        ...
        // Skip if already has aiSummary
        if (spec.aiSummary) {
          console.log(TAG, 'skip.hasAISummary', key, spec.title);
          successCount++;
          continue;
        }
        
        // Generate AI summary for this chart
        const chartSpec = {
          title: spec.title || 'Chart',
          labels: spec.labels || [],
          series: spec.series || spec.values || [],
          xTitle: spec.xTitle || '',
          yTitle: spec.yTitle || ''
        };
        
        console.log(TAG, 'generating', key, chartSpec.title);
        
        const aiSummary = await getOrGenerateChartSummary(
          chartSpec,
          lang,
          `Predictive chart for ${key} section: ${spec.title || 'metric projection'}`
        );
        
        if (aiSummary) {
          // Inject aiSummary into spec
          spec.aiSummary = aiSummary;
          
          // Re-encode the spec
          const newEncoded = JSON.stringify(spec)
            .replace(/&/g, '&amp;')
            .replace(/"/g, '&quot;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;');
          
          // Replace old data-chart value with new one
          const newTag = fullMatch.replace(
            /data-chart=["'][^"']+["']/,
            `data-chart="${newEncoded}"`
          );
          
          replacements.push({ old: fullMatch, new: newTag });
          successCount++;
          console.log(TAG, 'injected', key, {
            title: spec.title,
            overall: aiSummary.overall?.substring(0, 50) + '...',
            insights: aiSummary.insights?.length || 0
          });
        } else {
          console.log(TAG, 'noSummary', key, spec.title);
        }
      } catch (e) {
        console.log(TAG, 'error', key, String(e?.message || e));
      }
    }
    
    // Apply all replacements
    for (const r of replacements) {
      modifiedHtml = modifiedHtml.replace(r.old, r.new);
    }
    
    sectionsObj[key] = modifiedHtml;
  }
  
  console.log(TAG, 'done', { totalCharts, successCount });
  return sectionsObj;
}
...
async function buildPredictiveModelSpec(canon = {}, sections = {}, reportHTML = '', L = {}) {
  try {
    ...
    const langNote = isNonEnglish
      ? `CRITICAL: ALL human-readable strings ("labels", "xTitle", "yTitle") MUST be written in ${lang}. Do NOT use English.`
      : 'Use clear business English for all human-readable strings.';
    const systemLangNote = isNonEnglish ? ` Write ALL text output in ${lang}.` : '';
...
`;
    ...
    try {
      const res = await openai.chat.completions.create({
        ...
        temperature: 1,
        messages: [
          { role: 'system', content: `Return STRICT JSON only. No prose, no markdown.${systemLangNote}` },
          { role: 'user', content: prompt }
        ]
      });
...
async function buildKPIDashboardSpec(canon = {}, sections = {}, reportHTML = '', translatedLabels = {}) {
  try {
    ...
    const isNonEnglish = !/^english$/i.test(lang);
    const langNote = isNonEnglish
      ? `CRITICAL: ALL human-readable strings MUST be written in ${lang}. Do NOT use English.`
      : 'Use clear business English for all human-readable strings.';
    const systemLangNote = isNonEnglish ? ` Write ALL text output in ${lang}.` : '';
...
`;
    ...
    if (!parsed || !Array.isArray(parsed.kpis) || parsed.kpis.length < 4) {
      ...
      // [KT:TRANSLATION-FIX] Generate translated fallback KPIs
      const fallbackKPIs = isNonEnglish ? await generateFallbackKPIsTranslated(lang) : null;
      parsed = fallbackKPIs || {
        kpis: [
          {
            name: 'Cost Savings',
...
// [KT:FIX-2025-12-22] Added retry logic, removed hardcoded fallbacks
// ============================================================================
async function buildPrioritizationMatrixSpec(canon = {}, sections = {}, reportHTML = '', translatedLabels = {}) {
  try {
    ...
    const reportContext = String(reportHTML || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').slice(0, 5000);

    // [KT:RETRY] Try up to 3 times with different prompts
    let parsed = null;
...
- Use the EXACT wording from the report when possible
...
${reportContext}
` : attempt === 2 ? `
Extract 5 specific recommendations from this report. Return JSON only:
{"initiatives": [{"name": "...", "impact": 0-100, "effort": 0-100, "value": 1.0}]}

Look for: action items, projects, improvements, upgrades, implementations mentioned.

Report:
${reportContext.slice(0, 4000)}
` : `
List initiatives from this text as JSON array:
{"initiatives": [{"name": "project name", "impact": 70, "effort": 50, "value": 1.5}]}

Text: ${reportContext.slice(0, 3000)}
`;
      ...
      try {
        const res = await openai.chat.completions.create({
          model: process.env.OPENAI_MODEL || 'gpt-5-nano-2025-08-07',
          temperature: 1,
          max_completion_tokens: 16000,  // [KT:FIX] Changed from max_tokens for GPT-5-nano
          messages: [
            { role: 'system', content: 'Return STRICT JSON only. No prose, no markdown. Return English names.' },
            { role: 'user', content: prompt }
          ]
        });
        ...
        if (!raw || raw.length < 10) {
          console.warn(TAG, `[MATRIX-EXTRACTION] Attempt ${attempt}: Empty response`);
          continue;
        }
        ...
        if (candidate?.initiatives && Array.isArray(candidate.initiatives) && candidate.initiatives.length >= 3) {
          parsed = candidate;
          console.log(TAG, `[MATRIX-EXTRACTION] Success on attempt ${attempt}:`, { count: parsed.initiatives.length });
          break;
        } else {
          console.warn(TAG, `[MATRIX-EXTRACTION] Attempt ${attempt}: Invalid structure`, { hasInitiatives: !!candidate?.initiatives });
        }
      } catch (attemptErr) {
        console.warn(TAG, `[MATRIX-EXTRACTION] Attempt ${attempt} error:`, String(attemptErr?.message || attemptErr));
      }
      
      if (attempt < 3) await new Promise(r => setTimeout(r, 300));
    }

    if (!parsed || !parsed.initiatives || !Array.isArray(parsed.initiatives) || parsed.initiatives.length < 3) {
      console.error(TAG, '❌ [MATRIX-PREDICTIVE] All extraction attempts failed - report may lack initiative content');
      console.error(TAG, '❌ [MATRIX-PREDICTIVE] Report context preview:', reportContext.slice(0, 500));
      // Return empty - do NOT use hardcoded generic initiatives
      return { initiatives: [], extractionFailed: true };
    }
    ...
    if (isNonEnglish && parsed.initiatives?.length > 0) {
      try {
        ...
        const translationPrompt = `Translate to ${lang}: ${initiativeNames.join(', ')}

Return JSON: {"translations": ["translation1", "translation2", ...]}`;

        let transRaw = '';
        // [KT:RETRY] GPT sometimes returns empty - retry up to 3 times with longer delay
        for (let attempt = 1; attempt <= 3; attempt++) {
          const translationRes = await openai.chat.completions.create({
            model: process.env.OPENAI_MODEL || 'gpt-5-nano-2025-08-07',
            temperature: 1,
            messages: [
              { role: 'system', content: `Translator. Return JSON only. Translate the initiative names.` },
              { role: 'user', content: translationPrompt }
            ],
            max_completion_tokens: 16000 // [KT:GPT5-NANO-FIX] Increased for reasoning tokens
          });
          transRaw = (translationRes?.choices?.[0]?.message?.content || '').trim();
          if (transRaw && transRaw.length > 5) break;
          console.warn(TAG, '[MATRIX-TRANSLATION] Empty response, attempt', attempt);
          if (attempt < 3) await new Promise(r => setTimeout(r, 1000)); // [KT:FIX] Longer delay
        }
        
        if (!transRaw || transRaw.length < 5) {
          console.warn(TAG, '[MATRIX-TRANSLATION] GPT returned empty after retries');
...
`;
    ...
    // [KT:FIX-2025-12-22] Improved prompt for better translation success
    if (isNonEnglish) {
      ...
      while (translationAttempts < maxAttempts) {
        translationAttempts++;
        try {
          // [KT:NANO-FIX] Clear explicit prompt with example
          ...
          const translationPrompt = `Translate these ${allTerms.length} business terms from English to ${lang}:

${allTerms.map((t, i) => `${i + 1}. "${t}"`).join('\n')}

Return JSON with translations in the SAME ORDER:
{"translations": ["translated term 1", "translated term 2", ...]}

Example for Spanish: {"translations": ["Estado Actual", "Ahorros Año 1", ...]}`;

          const translationRes = await openai.chat.completions.create({
            model: process.env.OPENAI_MODEL || 'gpt-5-nano-2025-08-07',
            temperature: 1,
            messages: [
              { role: 'system', content: `You are a translator. Translate business/financial terms to ${lang}. Return JSON only.` },
              { role: 'user', content: translationPrompt }
            ],
            max_completion_tokens: 16000  // [KT:GPT5-NANO-FIX] Increased for reasoning tokens
          });
...
Return STRICT JSON:
{
  ...
  "title": "SHORT title (max 6 words) for the visualization",
  "xTitle": "X-axis label",
...
async function buildAllSectionGraphSpecs(canon = {}, sections = {}) {
  try {
    ...
    const isNonEnglish = !/^english$/i.test(lang);
    const langNote = isNonEnglish
      ? `CRITICAL LANGUAGE REQUIREMENT: ALL text output (titles, labels, axis names, insights) MUST be written in ${lang}. Do NOT use English.`
      : '';
    const systemLangNote = isNonEnglish
      ? ` Write ALL text in ${lang} language only.`
      : '';
...
Return STRICT JSON with this structure:
{
  "exec": {
    "type": "bar|line",
    "title": "Specific title about executive summary key metrics",
    "xTitle": "X-axis label",
  ...
  "kpis": {
    "type": "bar|line",
    "title": "Specific KPI performance visualization",
    "xTitle": "X-axis label",
    "yTitle": "Y-axis label (%, score, etc.)",
    "labels": ["Label1", "Label2", ...],
    "data": [number, number, ...],
    "insight": "One sentence key insight"
  },
  ...
  "risk": {
    "type": "bar|line",
    "title": "Risk assessment or mitigation progress",
    "xTitle": "X-axis label",
    "yTitle": "Y-axis label",
    "labels": ["Label1", "Label2", ...],
    "data": [number, number, ...],
...
`;
    ...
    const res = await openai.chat.completions.create({
      ...
      temperature: 1,
      messages: [
        { role: 'system', content: `Return STRICT JSON only. No prose, no markdown. Create company-specific visualizations.${systemLangNote}` },
        { role: 'user', content: prompt }
      ]
    });
    ...
    // [KT:ALL-SECTIONS] Include ALL sections for mandatory graph generation
    const sections_list = ['exec', 'current', 'financials', 'kpis', 'timeline', 'ops', 'roi', 'risk', 'nextSteps'];
...
// ============================================================================
// [KT:AI-SECTION-SELECTION] AI-Driven Section Selection
// Uses SovereignAI to dynamically determine which sections are most relevant
// for this specific report based on organization context and strategic goals
// ============================================================================
async function selectReportSections(canon = {}) {
  console.log(TAG, '[AI-SECTION-SELECTION] Starting AI-driven section selection...');
  
  // Default sections - always include executive summary first
  const ALL_SECTIONS = ['exec', 'current', 'financials', 'kpis', 'timeline', 'ops', 'risk', 'roi', 'nextSteps'];
  const MANDATORY_SECTIONS = ['exec', 'nextSteps']; // Always include these
  ...
  try {
    ...
    const prompt = `
You are a senior strategy consultant deciding which sections to include in a transformation report.
Analyze the organization context and determine the OPTIMAL section order and which sections to PRIORITIZE.

ORGANIZATION CONTEXT:
- Name: ${canon.orgName || 'Organization'}
- Industry: ${canon.industry || 'Government'}
- Country: ${canon.country || 'Global'}
- Tier: ${canon.tier || 'National'}
- Size: ${canon.companySize || '5,001-10,000 employees'}
- Time Frame: ${canon.timeFrame || '2 years'}
- Strategic Goal: ${canon.strategicGoal || 'Strategic transformation'}
- Cost Savings Goal: ${canon.costSavingsGoal || 'Not specified'}
- Desired Outcome: ${canon.desiredOutcome || 'Improved efficiency'}
- Secondary Objectives: ${canon.secondaryObjectives || 'None specified'}

AVAILABLE SECTIONS:
1. exec - Executive Summary (MANDATORY - always first)
2. current - Current State Analysis
3. financials - Financial Impact & Investment Analysis
4. kpis - Key Performance Indicators Dashboard
5. timeline - Implementation Timeline & Phases
6. ops - Operational Excellence & Process Optimization
7. risk - Risk Assessment & Mitigation
8. roi - Return on Investment Analysis
9. nextSteps - Action Plan & Next Steps (MANDATORY - always last)

Based on the organization's context and goals, return the optimal section order.
Consider:
- For cost-focused organizations: Prioritize financials, roi, ops
- For timeline-sensitive: Prioritize timeline, current, ops
- For risk-averse organizations: Prioritize risk, current, financials
- For performance-focused: Prioritize kpis, ops, roi
- For government/public sector: Include all sections for comprehensive coverage

Return STRICT JSON only:
{
  "sections": ["exec", "section2", "section3", ..., "nextSteps"],
  "rationale": "Brief explanation of why this order is optimal for this organization"
}

RULES:
- MUST start with "exec" and end with "nextSteps"
- Include minimum 7 sections, maximum 9
- Order should reflect organization's priorities
...
`;
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: 'You are a strategic consultant. Return STRICT JSON only. No markdown, no prose.' },
        { role: 'user', content: prompt }
      ]
    });
    ...
    try {
      ...
      console.warn(TAG, '[AI-SECTION-SELECTION] Parse error, using default order');
      return { sections: ALL_SECTIONS, rationale: 'Default order - parse error' };
    }

    if (parsed?.sections && Array.isArray(parsed.sections)) {
      // Validate sections
      let selectedSections = parsed.sections.filter(s => ALL_SECTIONS.includes(s));
      
      // Ensure mandatory sections are present
      if (!selectedSections.includes('exec')) {
        selectedSections = ['exec', ...selectedSections];
      }
      if (!selectedSections.includes('nextSteps')) {
        selectedSections = [...selectedSections.filter(s => s !== 'nextSteps'), 'nextSteps'];
      }
      
      console.log(TAG, '[AI-SECTION-SELECTION] AI selected sections:', {
      ...
      return {
        sections: selectedSections,
        rationale: parsed.rationale || 'AI-optimized section order'
      };
    }
    
    console.warn(TAG, '[AI-SECTION-SELECTION] Invalid response, using default order');
    return { sections: ALL_SECTIONS, rationale: 'Default order - invalid response' };
    
  } catch (e) {
    console.error(TAG, '[AI-SECTION-SELECTION] Error:', String(e?.message || e));
    return { sections: ALL_SECTIONS, rationale: 'Default order - error' };
  }
}

// ============================================================================
// [KT:AI-SECTION-TABLES] AI-Driven Section Tables
// Generates company/industry-specific tables for ALL sections in ONE call
// Tables include meaningful data relevant to each section, fully translated
// ============================================================================
async function buildAllSectionTableSpecs(canon = {}, sections = {}) {
  try {
    const openai = await getOpenAI();
    const lang = String(canon?.lang || 'English');
    const isNonEnglish = !/^english$/i.test(lang);
    const langNote = isNonEnglish
      ? `CRITICAL: ALL table headers and cell content MUST be written in ${lang}. No English text.`
      : '';

    // Build comprehensive context from all sections
    const sectionContext = {
      current: (sections.current || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      financials: (sections.financials || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      timeline: (sections.timeline || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      ops: (sections.ops || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      roi: (sections.roi || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      risk: (sections.risk || '').replace(/<[^>]*>/g, ' ').slice(0, 600),
      nextSteps: (sections.nextSteps || '').replace(/<[^>]*>/g, ' ').slice(0, 600)
    };

    const prompt = `
You are a senior strategy consultant creating executive data tables for a transformation report.
Generate ONE impactful, company-specific table for EACH of the following sections.
...
OPERATIONS: ${sectionContext.ops}
ROI: ${sectionContext.roi}
RISK: ${sectionContext.risk}
NEXT STEPS: ${sectionContext.nextSteps}

Return STRICT JSON with this structure (one table per section):
{
  "exec": {
    "title": "Executive Summary Key Metrics Table Title",
    "headers": ["Metric", "Current", "Target", "Impact"],
    "rows": [row arrays],
    "insight": "One sentence key executive insight"
  },
  "current": {
    "title": "Current State Assessment Table Title",
    ...
    "insight": "One sentence explaining the table's key finding"
  },
  "financials": {
    "title": "Financial Impact Summary Table Title",
    "headers": ["Initiative", "Investment", "Annual Savings", "Payback Period"],
    "rows": [row arrays],
    "insight": "Key financial insight"
  },
  "kpis": {
    "title": "KPI Performance Dashboard Table Title",
    "headers": ["KPI", "Baseline", "Target", "Gap", "Status"],
    "rows": [row arrays],
    "insight": "Key KPI performance insight"
  },
  "timeline": {
    "title": "Implementation Timeline Table Title",
    "headers": ["Phase", "Duration", "Key Deliverables", "Status"],
    "rows": [row arrays],
    "insight": "Key timeline insight"
  },
  "ops": {
    "title": "Operational Metrics Table Title",
    "headers": ["Process", "Current", "Target", "Gap"],
    "rows": [row arrays],
    "insight": "Key operational insight"
  },
  "roi": {
    "title": "ROI Analysis Table Title",
    "headers": ["Category", "Year 1", "Year 2", "Year 3", "Total"],
    "rows": [row arrays],
    "insight": "Key ROI insight"
  },
  "risk": {
    "title": "Risk Assessment Table Title",
    "headers": ["Risk", "Impact", "Likelihood", "Mitigation"],
    "rows": [row arrays],
    "insight": "Key risk insight"
  },
  "nextSteps": {
    "title": "Action Plan Summary Table Title",
    "headers": ["Action", "Owner", "Timeline", "Priority"],
    "rows": [row arrays],
    "insight": "Key action plan insight"
  }
}

CRITICAL RULES:
- Each table must be SPECIFIC to ${canon.orgName || 'this organization'} and ${canon.industry || 'their industry'}
- Extract REAL data from section content where possible
- If data unavailable, create realistic industry-specific placeholder values
- Tables should have 3-6 rows and 3-5 columns
- Include currency symbols where appropriate (${canon.currencySymbol || '$'})
- ${langNote}
- NO commentary; JSON only.
`;
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: `Return STRICT JSON only. No prose, no markdown. Create company-specific tables.${isNonEnglish ? ` Write ALL content in ${lang}.` : ''}` },
        { role: 'user', content: prompt }
      ]
    });
    ...
    try {
      ...
      console.error("[SR:REFORM] buildAllSectionTableSpecs.parse.error", parseErr);
      parsed = null;
    }

    // Generate default table for any missing section
    const getDefaultTable = (sec) => {
      const defaults = {
        exec: {
          title: translatedLabels?.['Executive Summary Key Metrics'] || (isNonEnglish ? `[${lang}]` : 'Executive Summary Key Metrics'),
          headers: ['Metric', 'Current', 'Target', 'Impact'],
          rows: [
            ['ROI', '0%', '300%+', 'High'],
            ['Cost Savings', 'Baseline', `${canon.currencySymbol || '$'}${canon.costSavingsGoal || '1M'}`, 'High'],
            ['Timeline', '-', canon.timeFrame || '24 months', 'Medium'],
            ['Payback Period', '-', '18 months', 'High']
          ],
          insight: 'Transformation delivers significant value with manageable risk.'
        },
        current: {
          title: translatedLabels?.['Current State Analysis'] || (isNonEnglish ? `[${lang}]` : 'Current State Analysis'),
          headers: ['Capability', 'Current Level', 'Target Level', 'Gap'],
          rows: [
            ['Process Efficiency', 'Baseline', 'Optimized', '35%'],
            ['Digital Maturity', 'Emerging', 'Advanced', '40%'],
            ['Data Analytics', 'Basic', 'Predictive', '50%']
          ],
          insight: 'Current capabilities show significant optimization opportunity.'
        },
        financials: {
          title: isNonEnglish ? `[${lang}] Financial Impact Summary` : 'Financial Impact Summary',
          headers: ['Initiative', 'Investment', 'Annual Savings', 'Payback'],
          rows: [
            ['Process Optimization', `${canon.currencySymbol || '$'}500K`, `${canon.currencySymbol || '$'}200K`, '2.5 years'],
            ['Technology Upgrade', `${canon.currencySymbol || '$'}1.2M`, `${canon.currencySymbol || '$'}600K`, '2 years'],
            ['Training Program', `${canon.currencySymbol || '$'}150K`, `${canon.currencySymbol || '$'}100K`, '1.5 years']
          ],
          insight: 'Total investment shows positive ROI within transformation timeline.'
        },
        kpis: {
          title: isNonEnglish ? `[${lang}] KPI Performance Dashboard` : 'KPI Performance Dashboard',
          headers: ['KPI', 'Baseline', 'Target', 'Gap', 'Status'],
          rows: [
            ['Customer Satisfaction', '75%', '90%', '15%', 'On Track'],
            ['Operational Efficiency', '65%', '85%', '20%', 'Needs Attention'],
            ['Revenue Growth', '5%', '15%', '10%', 'On Track'],
            ['Cost Reduction', '0%', '25%', '25%', 'In Progress']
          ],
          insight: 'Key performance indicators show clear improvement trajectory.'
        },
        timeline: {
          title: isNonEnglish ? `[${lang}] Implementation Phases` : 'Implementation Phases',
          headers: ['Phase', 'Duration', 'Key Deliverables', 'Status'],
          rows: [
            ['Discovery', '4 weeks', 'Assessment Report', 'In Progress'],
            ['Design', '8 weeks', 'Solution Blueprint', 'Planned'],
            ['Pilot', '12 weeks', 'Proof of Concept', 'Planned'],
            ['Scale', '16 weeks', 'Full Deployment', 'Planned']
          ],
          insight: 'Phased approach minimizes risk while ensuring rapid value delivery.'
        },
        ops: {
          title: isNonEnglish ? `[${lang}] Operational Metrics` : 'Operational Metrics',
          headers: ['Process', 'Current', 'Target', 'Improvement'],
          rows: [
            ['Cycle Time', '45 days', '30 days', '33%'],
            ['Error Rate', '5%', '1%', '80%'],
            ['Throughput', '100 units/day', '150 units/day', '50%']
          ],
          insight: 'Key operational metrics show clear improvement targets.'
        },
        roi: {
          title: isNonEnglish ? `[${lang}] ROI Projection` : 'ROI Projection',
          headers: ['Category', 'Year 1', 'Year 2', 'Year 3', 'Total'],
          rows: [
            ['Revenue Uplift', `${canon.currencySymbol || '$'}500K`, `${canon.currencySymbol || '$'}1.2M`, `${canon.currencySymbol || '$'}2M`, `${canon.currencySymbol || '$'}3.7M`],
            ['Cost Savings', `${canon.currencySymbol || '$'}200K`, `${canon.currencySymbol || '$'}500K`, `${canon.currencySymbol || '$'}800K`, `${canon.currencySymbol || '$'}1.5M`],
            ['Net Benefit', `${canon.currencySymbol || '$'}300K`, `${canon.currencySymbol || '$'}900K`, `${canon.currencySymbol || '$'}1.6M`, `${canon.currencySymbol || '$'}2.8M`]
          ],
          insight: 'Cumulative ROI exceeds 300% over transformation horizon.'
        },
        risk: {
          title: isNonEnglish ? `[${lang}] Risk Assessment Summary` : 'Risk Assessment Summary',
          headers: ['Risk', 'Impact', 'Likelihood', 'Mitigation'],
          rows: [
            ['Change Resistance', 'High', 'Medium', 'Change management program'],
            ['Technology Integration', 'Medium', 'Medium', 'Phased rollout'],
            ['Resource Constraints', 'Medium', 'Low', 'External partnerships']
          ],
          insight: 'All identified risks have defined mitigation strategies.'
        },
        nextSteps: {
          title: isNonEnglish ? `[${lang}] Immediate Action Plan` : 'Immediate Action Plan',
          headers: ['Action', 'Owner', 'Timeline', 'Priority'],
          rows: [
            ['Stakeholder Alignment', 'Executive Sponsor', 'Week 1-2', 'High'],
            ['Team Formation', 'Project Lead', 'Week 2-3', 'High'],
            ['Baseline Assessment', 'Analytics Team', 'Week 3-4', 'Medium'],
            ['Quick Wins Identification', 'Core Team', 'Week 4-6', 'Medium']
          ],
          insight: 'Early actions focus on building momentum and stakeholder buy-in.'
        }
      };
      return defaults[sec] || defaults.current;
    };

    // Fill in any missing sections with defaults
    // [KT:ALL-SECTIONS] Include ALL sections including exec and kpis
    const sections_list = ['exec', 'current', 'financials', 'kpis', 'timeline', 'ops', 'roi', 'risk', 'nextSteps'];
    for (const sec of sections_list) {
      if (!parsed || !parsed[sec] || !parsed[sec].headers || !parsed[sec].rows) {
        if (!parsed) parsed = {};
        parsed[sec] = getDefaultTable(sec);
      }
    }

    console.log(TAG, '[SECTION-TABLES] All section tables generated', {
      sections: Object.keys(parsed || {}),
      lang: lang,
      currentRows: parsed?.current?.rows?.length,
      financialsRows: parsed?.financials?.rows?.length
    });

    return parsed;
  } catch (e) {
    console.error("[SR:REFORM] buildAllSectionTableSpecs.error", String(e?.message || e));
    return null;
  }
}

// ============================================================================
// [KT:RENDER-SECTION-TABLE] Render AI-generated table spec to HTML
// ============================================================================
function renderSectionTableHTML(tableSpec, sectionName = '') {
  if (!tableSpec || !tableSpec.headers || !tableSpec.rows) {
    return '';
  }
  
  // [KT:AI-TABLES] Local HTML escape helper
  const escHTML = (s) => String(s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

  const title = tableSpec.title || `${sectionName} Summary`;
  const headers = tableSpec.headers || [];
  const rows = tableSpec.rows || [];
  const insight = tableSpec.insight || '';

  const headerCells = headers.map(h => `<th>${escHTML(h)}</th>`).join('');
  const rowsHTML = rows.map(row => {
    const cells = (Array.isArray(row) ? row : []).map(cell => `<td>${escHTML(cell)}</td>`).join('');
    return `<tr>${cells}</tr>`;
  }).join('');

  return `
    <div class="ai-section-table" data-section="${escHTML(sectionName)}" style="margin: 24px 0;">
      <h4 style="color:#FFFFFF; font-size:16px; margin-bottom:12px; font-weight:600;">${escHTML(title)}</h4>
      <table class="report-table" style="width:100%; border-collapse:collapse;">
        <thead>
          <tr style="background:rgba(59,130,246,0.2);">${headerCells}</tr>
        </thead>
        <tbody>${rowsHTML}</tbody>
      </table>
      ${insight ? `<p style="color:#94A3B8; font-size:13px; margin-top:8px; font-style:italic;">💡 ${escHTML(insight)}</p>` : ''}
    </div>
  `;
}
...
// [KT:FIX-2025-12-22] Added retry logic, removed hardcoded fallbacks
// [KT:FIX-2025-12-22b] Made extraction language-aware - content may be in target language
async function buildRiskHeatmapSpec(canon = {}, sections = {}, reportHTML = '', translatedLabels = {}) {
  try {
    const openai = await getOpenAI();
    const lang = String(canon?.lang || 'English');
    const isNonEnglish = lang && !/^english$/i.test(lang);
    const reportContext = String(reportHTML || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').slice(0, 6000);

    // [KT:RETRY] Try up to 3 times with increasingly specific prompts
    let parsed = null;
    for (let attempt = 1; attempt <= 3; attempt++) {
      // [KT:FIX] Language-aware prompt - content may be in target language
      const langNote = isNonEnglish 
        ? `IMPORTANT: The report content is written in ${lang}. Read and understand it in ${lang}, but return risk names in ENGLISH for internal processing.`
        : '';
...
{
  "risks": [
    { "name": "Risk name in English", "description": "Brief description in English", "severity": 1-5, "likelihood": 1-5 },
    ... (5-8 risks)
  ]
}

Rules:
- Extract actual risks mentioned in the report - DO NOT invent generic risks
- Severity: 1=negligible, 2=minor, 3=moderate, 4=major, 5=critical
- Likelihood: 1=rare, 2=unlikely, 3=possible, 4=likely, 5=almost certain
- Return risk names in ENGLISH; we will translate later
- NO commentary; JSON only.

Report content:
${reportContext}
` : attempt === 2 ? `
Extract 5-8 business risks from this report. ${isNonEnglish ? `The report is in ${lang} - read it and extract risks, but return names in ENGLISH.` : ''}
Return JSON only:
{"risks": [{"name": "English name", "description": "English description", "severity": 1-5, "likelihood": 1-5}]}

Look for business challenges, operational concerns, financial risks, market threats, or implementation barriers.

Report:
${reportContext.slice(0, 4000)}
` : `
${isNonEnglish ? `This text is in ${lang}. Extract risks and return in ENGLISH.` : ''}
List 5 business risks from this text as JSON array:
{"risks": [{"name": "risk name", "severity": 3, "likelihood": 3, "description": "brief"}]}
...
`;
      ...
      try {
        const res = await openai.chat.completions.create({
          ...
          max_completion_tokens: 16000,  // [KT:FIX] Changed from max_tokens for GPT-5-nano
          messages: [
            { role: 'system', content: `Return STRICT JSON only. No prose, no markdown.${isNonEnglish ? ` Read ${lang} content but return English values.` : ''}` },
            { role: 'user', content: prompt }
          ]
        });
        ...
        let raw = (res?.choices?.[0]?.message?.content || '').trim();
        if (!raw || raw.length < 10) {
          console.warn(TAG, `[RISK-EXTRACTION] Attempt ${attempt}: Empty response`);
          continue;
        }
        
        // [KT:FIX-2025-12-28] Enhanced JSON extraction with detailed error logging
        console.log(TAG, `[RISK-EXTRACTION] Attempt ${attempt}: Raw response length: ${raw.length}`);
        
        let jsonStr = extractJsonObject(raw) || raw;
        jsonStr = sanitizeJSONForParsing(jsonStr);
        
        // [KT:FIX] Try to parse, with detailed error position logging
        let candidate;
        try {
          ...
          console.warn(TAG, `[RISK-EXTRACTION] Attempt ${attempt}: JSON parse error at position ${errorPos}`, {
            errorMsg: parseErr.message.substring(0, 100),
            jsonLen: jsonStr.length,
            charAtError: errorPos > 0 ? jsonStr.substring(errorPos - 20, errorPos + 20) : 'N/A'
          });
          throw parseErr; // Re-throw to be caught by outer catch
        }
        
        if (candidate?.risks && Array.isArray(candidate.risks) && candidate.risks.length >= 3) {
          parsed = candidate;
          console.log(TAG, `[RISK-EXTRACTION] Success on attempt ${attempt}:`, { count: parsed.risks.length });
          break;
        } else {
          console.warn(TAG, `[RISK-EXTRACTION] Attempt ${attempt}: Invalid structure`, { hasRisks: !!candidate?.risks });
        }
      } catch (attemptErr) {
        console.warn(TAG, `[RISK-EXTRACTION] Attempt ${attempt} error:`, String(attemptErr?.message || attemptErr));
      }
      
      // Brief delay before retry - [KT:FIX] Increased from 300ms to 800ms
      if (attempt < 3) await new Promise(r => setTimeout(r, 800));
    }

    if (!parsed || !parsed.risks || !Array.isArray(parsed.risks) || parsed.risks.length < 3) {
      console.error(TAG, '❌ [RISK-HEATMAP] All extraction attempts failed - report may lack risk content');
      console.error(TAG, '❌ [RISK-HEATMAP] Report context preview:', reportContext.slice(0, 500));
      // Return empty - do NOT use hardcoded generic risks
      return { risks: [], extractionFailed: true };
    }
...
Return JSON: {"names": ["name1", "name2", ...]}`;
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          const translationRes = await openai.chat.completions.create({
            ...
            temperature: 1,
            messages: [
              { role: 'system', content: `Translator. Return JSON only. Translate these risk names.` },
              { role: 'user', content: translationPrompt }
            ],
            max_completion_tokens: 16000 // [KT:GPT5-NANO-FIX] Increased for reasoning tokens
          });
    ...
    // [KT:FIX-2025-12-22] No hardcoded fallback - return empty to signal extraction failed
    console.error(TAG, '❌ [RISK-HEATMAP-FALLBACK] Critical error - no risks extracted');
    return { risks: [], extractionFailed: true };
  }
}
...
// [KT:SOVEREIGN-AI] Language context for self-learning
let currentLang = 'English';
function setReformLang(lang) { currentLang = lang || 'English'; }

// [KT:SOVEREIGN-AI] OpenAI compatibility shim for self-learning integration
const OpenAI = class SovereignOpenAIShim {
  constructor() {
    console.log('[SR:REFORM-SHIM] SovereignOpenAIShim instance created');
  }
  get chat() {
    return {
      completions: {
        create: async (params) => {
          ...
          const isJsonMode = params.response_format?.type === 'json_object' || 
                             ...
                             userPrompt.toLowerCase().includes('return only json') ||
                             userPrompt.toLowerCase().includes('strict json');
          
          // [KT:SOVEREIGN-AI-CALL-TYPE-DETECTION] Comprehensive call type detection for self-learning
          // This helps SovereignAI track performance by call type and optimize prompts
          let callType = 'reform-report-generation';
          const systemLower = systemPrompt.toLowerCase();
          const userLower = userPrompt.toLowerCase();
          
          // Translation calls
          if (systemLower.includes('translat') || systemLower.includes('translate')) {
            callType = 'reform-translation';
          }
          // Chart summaries
          else if (userLower.includes('chart') && (userLower.includes('insight') || userLower.includes('summary'))) {
            callType = 'reform-chart-summary';
          }
          // Matrices
          else if (userLower.includes('matrix') || userLower.includes('raci') || userLower.includes('raid')) {
            callType = 'reform-matrix-generation';
          }
          // Risk assessment
          else if (userLower.includes('risk') || systemLower.includes('risk')) {
            callType = 'reform-risk-assessment';
          }
          // Waterfall charts
          else if (userLower.includes('waterfall') || userLower.includes('component')) {
            callType = 'reform-waterfall-chart';
          }
          // Timeline
          else if (userLower.includes('timeline') || userLower.includes('phase') || userLower.includes('duration')) {
            callType = 'reform-timeline';
          }
          // Predictive
          else if (userLower.includes('predictive') || userLower.includes('forecast') || userLower.includes('project')) {
            callType = 'reform-predictive';
          }
          // Title generation
          else if (userLower.includes('report title') || userLower.includes('report name')) {
            callType = 'reform-title-generation';
          }
          // Section content
          else if (userLower.includes('section') || userLower.includes('executive summary') || userLower.includes('current state')) {
            callType = 'reform-section-content';
          }
          
          console.log('[SR:REFORM-SHIM] Routing through sovereignAICall', {
          ...
          // [KT:SOVEREIGN-AI-INTEGRATION] Route all calls through sovereignAICall for self-learning
          const result = await sovereignAICall({
            ...
            vertical: 'reform',
            lang: currentLang,
            expectedFormat: isJsonMode ? 'json' : 'text',
            temperature: params.temperature || 1, // [KT:GPT5-NANO-FIX] temperature=1 is mandatory
            maxTokens: params.max_tokens || params.max_completion_tokens || 16000,
            responseFormat: params.response_format,
            // Enhancement always enabled for self-learning capabilities
          });
          
          // [KT:SOVEREIGN-AI] sovereignAICall returns { ok, response, raw, metrics, learnings }
          const content = result?.response || result?.raw || '';
          
          // [KT:SOVEREIGN-AI] Log self-learning metrics and insights
          console.log('[SR:REFORM-SHIM] sovereignAICall result with learning', {
            ok: result?.ok,
            responseLen: result?.response?.length || 0,
            rawLen: result?.raw?.length || 0,
            contentLen: content.length,
            callType,
            successRate: result?.metrics?.successRate ? Math.round(result.metrics.successRate * 100) + '%' : 'N/A',
            learnings: result?.learnings ? {
              insights: result.learnings.recentInsights?.length || 0,
              skillLevel: result.learnings.skillLevel
            } : 'none',
...
// [KT:PREDICTIVE-INTELLIGENCE] Import predictive intelligence for self-learning predictions
import { generatePredictions, getConfidenceExplanation } from '@/lib/predictiveIntelligence.js';
...
async function buildChartSummaryTranslations(targetLang) {
  ...
  const phrasesObject = {
    ...
    // [KT:BENCHMARK-FALLBACK-I18N] Fallback benchmark titles - must be translated
    'Market Share Progress vs Industry Leaders': 'Market Share Progress vs Industry Leaders',
    'Cost Savings Realization vs Target': 'Cost Savings Realization vs Target',
    'KPI Performance vs Industry': 'KPI Performance vs Industry',
    'Savings Achievement Rate': 'Savings Achievement Rate',
    'vs target': 'vs target',
    'vs Target': 'vs Target',
    ...
    'riskHeatMap': 'Risk Heat Map',  // [KT:FIX] camelCase key for reportTemplate.js
...
// [KT:SOVEREIGN-AI] OpenAI shim is defined at the top of this file (SovereignOpenAIShim)
// The getOpenAI() function below returns the shim instance for self-learning capabilities

let __openaiInstance;

/**
 * Return a shared SovereignAI-backed OpenAI client instance.
 * Uses the SovereignOpenAIShim class defined at the top of this file
 * which routes all calls through SovereignAI for self-learning.
 */
async function getOpenAI() {
  ...
  // [KT:SOVEREIGN-AI] Use the SovereignOpenAIShim class (defined at top of file)
  // This routes all OpenAI calls through SovereignAI for self-learning capabilities
  console.log('[SR:REFORM] Initializing SovereignAI-backed OpenAI shim');
  __openaiInstance = new OpenAI();
  return __openaiInstance;
}

// ============================================================================
// [KT:SOVEREIGN-AI-HELPERS] Helper functions for all AI operations
// These ensure consistent language context and SovereignAI engagement
// ============================================================================

/**
 * [KT:SOVEREIGN-AI] Make an AI call with language context and call type tracking
 * This is the ONLY way to call AI - ensures all calls go through SovereignAI
 * @param {Object} options - { prompt, systemPrompt, callType, expectedFormat, temperature, maxTokens }
 * @returns {Promise<Object>} - { content, ok, metrics, learnings }
 */
async function makeAICall(options = {}) {
  const {
    prompt,
    systemPrompt,
    callType = 'reform-general',
    expectedFormat = 'text',
    temperature = 1,
    maxTokens = 16000,
  } = options;

  console.log('[KT:SOVEREIGN-AI-HELPERS] makeAICall', {
    callType,
    lang: currentLang,
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      ...(expectedFormat === 'json' ? { response_format: { type: 'json_object' } } : {})
    });

    const content = res?.choices?.[0]?.message?.content || '';
    
    return {
      ok: !!content && content.length > 10,
      content,
      raw: content,
      metrics: {
    ...
    console.error('[KT:SOVEREIGN-AI-HELPERS] makeAICall error:', String(e?.message || e));
    return {
      ok: false,
      content: null,
      raw: null,
...
/**
 * [KT:SOVEREIGN-AI] Generate text content in target language
 * Ensures the prompt includes language requirement
 * @param {string} basePrompt - The main prompt
 * @param {Object} options - { callType, context }
 */
async function generateInTargetLanguage(basePrompt, options = {}) {
  const { callType = 'reform-section-content', context = '' } = options;

  // Inject language requirement into prompt
  const languageRequirement = (currentLang && !/^english$/i.test(currentLang))
    ? `\n\nCRITICAL: Write ALL content in ${currentLang}. Do NOT use English words or phrases.`
    : '';

  const systemPrompt = `You are a senior consultant creating transformation reports.
Write clear, professional, business-focused content.${languageRequirement}
Return clean HTML or JSON as requested.`;

  const fullPrompt = `${basePrompt}${languageRequirement}`;
...
/**
 * [KT:SOVEREIGN-AI] Generate JSON in target language
 * @param {string} prompt - The main prompt
 * @param {Object} options - { callType, returnStructure }
 */
async function generateJSONInTargetLanguage(prompt, options = {}) {
  const { callType = 'reform-json-generation', returnStructure } = options;

  const languageNote = (currentLang && !/^english$/i.test(currentLang))
    ? `All string values must be in ${currentLang}. JSON keys stay in English.`
    : 'Use English for all text values.';

  const systemPrompt = `You are a data analyst. Return STRICT JSON only. ${languageNote}`;
...
const TAG = '[SR:REFORM]';
// [KT:SURGICAL:JSON-EXTRACTOR-ROBUST-v3]
...
// [KT:FIX-2025-12-28] Added comprehensive repair for malformed JSON at any position
function extractJsonObject(raw) {
  try {
    let str = String(raw || '').trim();
    
    // [KT:PRE-PROCESS] Remove markdown code fences and control characters
    str = str.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '');
    str = str.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
    
    // [KT:FIX] Handle embedded newlines in JSON strings (common GPT issue)
    // This is critical for fixing "position XXXX" errors
    ...
    if (firstBrace >= 0 && lastBrace > firstBrace) {
      let extracted = str.slice(firstBrace, lastBrace + 1);
      
      // [KT:FIX] Fix trailing commas before closing braces/brackets
      extracted = extracted.replace(/,(\s*[}\]])/g, '$1');
      
      // Validate brace balance before returning
      const openBraces = countChar(extracted, '{');
      const closeBraces = countChar(extracted, '}');
      
      if (openBraces === closeBraces) {
        return extracted;
      }
      
      // [KT:FIX] If unbalanced, try to fix by adding missing braces
      if (openBraces > closeBraces) {
    ...
    if (firstBracket >= 0 && lastBracket > firstBracket) {
      let extracted = str.slice(firstBracket, lastBracket + 1);
      
      // [KT:FIX] Fix trailing commas
      extracted = extracted.replace(/,(\s*[}\]])/g, '$1');
      
      // Validate bracket balance before returning
      const openBrackets = countChar(extracted, '[');
      const closeBrackets = countChar(extracted, ']');
      
      if (openBrackets === closeBrackets) {
        return extracted;
      }
      
      // [KT:FIX] If unbalanced, try to fix
      if (openBrackets > closeBrackets) {
    ...
    if (firstBrace >= 0) {
      // Incomplete object - try to salvage by adding closing braces
      let partial = str.slice(firstBrace);
      
      // [KT:FIX] Truncate any dangling strings (common when response cut off mid-string)
      if (partial.match(/"[^"]*$/)) {
        partial = partial.replace(/"[^"]*$/, '"');
      }
      
      // [KT:FIX] Fix trailing commas
      partial = partial.replace(/,(\s*)$/, '$1');
      
      const openB = countChar(partial, '{');
      const closeB = countChar(partial, '}');
      const openA = countChar(partial, '[');
      const closeA = countChar(partial, ']');
      
      if (openB > closeB) {
        partial += '}'.repeat(openB - closeB);
      }
      if (openA > closeA) {
        partial += ']'.repeat(openA - closeA);
      }
      
      console.log(TAG, '[JSON-REPAIR] Emergency recovery attempted:', { 
        originalLen: str.length, 
        repairedLen: partial.length,
        braceBalance: openB - closeB,
        bracketBalance: openA - closeA
      });
...
// [KT:SOVEREIGN-AI] Use shared SovereignAI-backed client instead of direct OpenAI
// const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY }); // REPLACED - see getOpenAI()
let openai = null; // Will be initialized via getOpenAI() which uses SovereignAI shim
...
async function generateKPIBenchmarks(canon, sectionHtml) {
  ...
  // [KT:BENCHMARK-I18N-FIX] Helper function to get translated fallback titles
  const lang = String(canon?.lang || 'English');
  const getTranslatedLabel = (key) => {
    const cache = __chartSummaryTranslationCache[lang];
   ...
   try {
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Return pure JSON only with property "benchmarks". No prose, no markdown. Generate diverse, context-specific benchmark values.${langNote}`
        },
        {
          role: 'user',
...
`.trim()
    ...
    benchmarks = benchmarks.map((bm, idx) => {
      ...
      if (current === 0 && benchmark === 0) {
        ...
        // [KT:BENCHMARK-I18N-FIX] Use translated fallback titles
        return idx === 0 
          ? { title: getTranslatedLabel("Market Share Progress vs Industry Leaders"), current: 75 + Math.floor(Math.random() * 10), benchmark: 92, xTitle: getTranslatedLabel("Metric"), yTitle: getTranslatedLabel("Percent") }
          : { title: getTranslatedLabel("Cost Savings Realization vs Target"), current: 58 + Math.floor(Math.random() * 12), benchmark: 85, xTitle: getTranslatedLabel("Progress"), yTitle: getTranslatedLabel("Percent") };
      }
    ...
    // [KT:BENCHMARK-I18N-FIX] Fallback to safe defaults with translated titles
    const random1 = 70 + Math.floor(Math.random() * 15); // 70-84
    const random2 = 50 + Math.floor(Math.random() * 15); // 50-64 (always lower)
    return [
      {
        title: getTranslatedLabel("KPI Performance vs Industry"),
        current: random1,
        benchmark: 95,
        xTitle: getTranslatedLabel("Metric"),
        yTitle: getTranslatedLabel("Percent")
      },
      {
        title: getTranslatedLabel("Savings Achievement Rate"),
        current: random2,
        benchmark: 88,
        xTitle: getTranslatedLabel("Progress"),
        yTitle: getTranslatedLabel("Percent")
      }
    ];
  }
...
async function __kt_generateReportTitle(canon = {}, sections = {}) {
  try {
    ...
    const resp = await openai.chat.completions.create({
      ...
      temperature: 1,
      max_completion_tokens: 16000,  // [KT:FIX-TOKEN-LIMIT] Increased from 32 for GPT-5-nano reasoning tokens
      messages: [
        {
          role: 'system',
          content:
    ...
    if (!title) {
      ...
      try {
        const fallbackResp = await openai.chat.completions.create({
          model: MODEL,
          temperature: 1,
          max_completion_tokens: 16000,  // [KT:FIX-TOKEN-LIMIT] Increased from 40 for GPT-5-nano reasoning tokens
          messages: [
...
The HTML document contains a business transformation report with charts, tables, and executive content. Translate all headings, paragraphs, table content, chart titles, and button labels while preserving the document structure.`
      ...
      max_completion_tokens: 100000  // [KT:FIX-TOKEN-LIMIT] Increased to 100000 for GPT-5-nano reasoning tokens
    });
...
Return JSON: {"translations": ["t1", "t2", ...]}`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_completion_tokens: 16000, // [KT:FIX] Increased from 1500 to avoid token limit errors
    });
...
async function translateLabels(targetLang) {
  ...
  try {
    ...
    const criticalLabels = {
      ...
      implementationTimeline: DEFAULT_LABELS.implementationTimeline || 'Implementation Timeline',
      ...
      predictiveIntelligence: DEFAULT_LABELS.predictiveIntelligence || 'Predictive Intelligence',
      appendices: DEFAULT_LABELS.appendices,
      
      // Executive Summary Infographic (Items 3-4)
      'Executive Summary at a Glance': DEFAULT_LABELS.executiveSummaryAtAGlance || 'Executive Summary at a Glance',
      ...
      'Industry benchmark': DEFAULT_LABELS.industryBenchmark || 'Industry benchmark',
      'faster than industry average': DEFAULT_LABELS.fasterThanAverage || 'faster than industry average',
      'Best-in-class timing': DEFAULT_LABELS.bestInClassTiming || 'Best-in-class timing',
      'in annual value': DEFAULT_LABELS.inAnnualValue || 'in annual value',
      industryBenchmark: DEFAULT_LABELS.industryBenchmark || 'Industry benchmark',
      fasterThanAverage: DEFAULT_LABELS.fasterThanAverage || 'faster than industry average',
      bestInClassTiming: DEFAULT_LABELS.bestInClassTiming || 'Best-in-class timing',
      inAnnualValue: DEFAULT_LABELS.inAnnualValue || 'in annual value',
      topQuartile: DEFAULT_LABELS.topQuartile || 'top quartile',
      acceleratedDelivery: DEFAULT_LABELS.acceleratedDelivery || 'Accelerated delivery',
      acrossValue: DEFAULT_LABELS.acrossValue || 'across value chain',
      trendIndustryBenchmark: DEFAULT_LABELS.industryBenchmark || 'Industry benchmark',
      trendAnnualValue: DEFAULT_LABELS.inAnnualValue || 'in annual value',
      trendFasterThanIndustry: DEFAULT_LABELS.fasterThanAverage || 'faster than industry average',
      trendBestInClass: DEFAULT_LABELS.bestInClassTiming || 'Best-in-class timing',
      trendAccelerated: DEFAULT_LABELS.acceleratedDelivery || 'Accelerated delivery',
      ...
      acrossValueChain: DEFAULT_LABELS.acrossValue || 'across value chain',
      
      // Implementation phases for infographic
      Discovery: DEFAULT_LABELS.discovery || 'Discovery',
      Design: DEFAULT_LABELS.design || 'Design',
      Pilot: DEFAULT_LABELS.pilotPhase || 'Pilot',
      Scale: DEFAULT_LABELS.scalePhase || 'Scale',
      ...
      Overall: DEFAULT_LABELS.overall || 'Overall',
      overall: DEFAULT_LABELS.overall || 'Overall',
      'Key Insights': DEFAULT_LABELS.keyInsights || 'Key Insights',
      keyInsights: DEFAULT_LABELS.keyInsights || 'Key Insights',
      'KEY INSIGHT': DEFAULT_LABELS.keyInsight || 'KEY INSIGHT',
      'Recommendation': DEFAULT_LABELS.recommendationLabel || 'Recommendation',
      Type: DEFAULT_LABELS.type || 'Type',
      type: DEFAULT_LABELS.type || 'Type',
      Waterfall: 'Waterfall',
      Bar: DEFAULT_LABELS.bar || 'Bar',
      bar: DEFAULT_LABELS.bar || 'Bar',
      Line: DEFAULT_LABELS.line || 'Line',
      line: DEFAULT_LABELS.line || 'Line',
      Pie: DEFAULT_LABELS.pie || 'Pie',
      pie: DEFAULT_LABELS.pie || 'Pie',
      Doughnut: DEFAULT_LABELS.doughnut || 'Doughnut',
      doughnut: DEFAULT_LABELS.doughnut || 'Doughnut',
      
      // [KT:FALLBACK-SUMMARY] Chart summary fallback labels
      Increasing: DEFAULT_LABELS.increasing || 'Increasing',
      Decreasing: DEFAULT_LABELS.decreasing || 'Decreasing',
      Stable: DEFAULT_LABELS.stable || 'Stable',
      increasing: DEFAULT_LABELS.increasing || 'increasing',
      decreasing: DEFAULT_LABELS.decreasing || 'decreasing',
      stable: DEFAULT_LABELS.stable || 'stable',
      trend: DEFAULT_LABELS.trend || 'trend',
      Peak: DEFAULT_LABELS.peak || 'Peak',
      Average: DEFAULT_LABELS.average || 'Average',
      
      // [KT:CHART-SUMMARY-TEMPLATE] Template phrases for chart summary fallback
      'Values show': DEFAULT_LABELS.valuesShow || 'Values show',
      'trend from': DEFAULT_LABELS.trendFrom || 'trend from',
      'to': DEFAULT_LABELS.to || 'to',
      'achieved': DEFAULT_LABELS.achieved || 'achieved',
      'Average performance': DEFAULT_LABELS.averagePerformance || 'Average performance',
      'Peak value of': DEFAULT_LABELS.peakValueOf || 'Peak value of',
      // [KT:FIX-v3] Prioritization matrix specific labels
      initiativesDistributed: DEFAULT_LABELS.initiativesDistributed || 'Initiatives are distributed across quadrants, highlighting both quick wins and major projects.',
      highestImpact: DEFAULT_LABELS.highestImpact || 'Highest impact: Digital transformation. Most efficient: Quick wins.',
      ...
      Value: DEFAULT_LABELS.value,
      Index: DEFAULT_LABELS.index || 'Index',
    ...
    try {
      // [KT:FIX-2025-12-28] Use robust JSON extraction for translation responses
      let jsonStr = extractJsonObject(raw) || raw;
      jsonStr = sanitizeJSONForParsing(jsonStr);
      translatedCritical = JSON.parse(jsonStr);
    } catch (err) {
...
// ============================================================================
// [KT:PREDICTIVE-INTELLIGENCE] Helper function to map country to region
// ============================================================================
function getRegionFromCountry(country) {
  if (!country) return 'north_america';
  const c = country.toLowerCase();
  if (['usa', 'united states', 'canada', 'mexico'].some(x => c.includes(x))) return 'north_america';
  if (['uk', 'united kingdom', 'germany', 'france', 'italy', 'spain', 'netherlands', 'switzerland', 'sweden', 'ireland'].some(x => c.includes(x))) return 'europe';
  if (['china', 'japan', 'korea', 'singapore', 'india', 'australia', 'taiwan'].some(x => c.includes(x))) return 'asia_pacific';
  if (['brazil', 'argentina', 'chile', 'colombia', 'peru'].some(x => c.includes(x))) return 'latin_america';
  if (['saudi', 'uae', 'qatar', 'israel', 'turkey', 'egypt', 'south africa', 'nigeria', 'kenya'].some(x => c.includes(x))) return 'middle_east_africa';
  return 'north_america';
}

// ============================================================================
// [KT:PREDICTIVE-INTELLIGENCE-v1.0] AI-Powered Predictive Intelligence Section
// Self-learning predictions with Success Probability, Timeline, ROI, Risks, Insights
// ============================================================================
async function generatePredictiveSection(canon, lang) {
  const openai = await getOpenAI();
  
  // Generate predictions using our predictive engine
  const predictionResult = generatePredictions({
    industry: 'government',
    region: getRegionFromCountry(canon.country),
    transformationType: canon.transformationType || 'operational_efficiency',
    budget: canon.budget || 50000000,
    workforceSize: canon.workforce || 10000,
    currentMaturity: canon.maturityLevel || 2,
    targetMaturity: canon.targetMaturity || 4,
    existingFactors: {
      hasDigitalFoundation: canon.hasDigitalFoundation || false,
      hasChangeManagement: canon.hasChangeManagement || true,
      executiveSponsorship: canon.executiveSponsorship || 'high',
      regulatoryComplexity: 'medium',
    },
  });
  
  // Extract predictions from result object with fallback
  const predictions = predictionResult?.predictions || {
    successProbability: 60,
    timeline: { expected: 18, minimum: 12, maximum: 24 },
    roi: { expected: 150, minimum: 100, maximum: 200 },
    risks: [],
  ...
  const labels = {
    ...
    subtitle: 'Self-Improving Analysis That Learns From Every Transformation',
    ...
    learningIndicator: 'This analysis improves with each report generated',
    govDisclaimer: 'Predictions account for government procurement cycles, regulatory requirements, and stakeholder complexity',
    months: 'months',
    high: 'High',
    medium: 'Medium',
    low: 'Low',
  ...
  let t = labels;
  if (lang && !/^english$/i.test(lang)) {
    const langInstruction = `Translate the following JSON object values to ${lang}. Keep keys unchanged. Return valid JSON only:\n${JSON.stringify(labels)}`;
    try {
      const translated = await openai.chat.completions.create({
        model: MODEL,
        temperature: 1, // [KT:GPT5-NANO-FIX] gpt-5-nano only supports temperature: 1
        max_completion_tokens: 16000, // [KT:GPT5-NANO-FIX] Increased from 1000
        messages: [{ role: 'user', content: langInstruction }],
      });
      const rawResponse = translated.choices?.[0]?.message?.content || '';
      t = JSON.parse(rawResponse.replace(/```json\n?|```/g, '').trim());
    } catch (e) {
      console.warn(TAG, '[PREDICT] Translation failed, using English');
    }
  }
  
  // Color helpers
  const getSuccessColor = (prob) => prob >= 75 ? '#10b981' : prob >= 50 ? '#f59e0b' : '#ef4444';
  const getRiskColor = (prob) => prob >= 60 ? '#ef4444' : prob >= 30 ? '#f59e0b' : '#10b981';
  const getRiskBg = (prob) => prob >= 60 ? 'rgba(239,68,68,0.1)' : prob >= 30 ? 'rgba(245,158,11,0.1)' : 'rgba(16,185,129,0.1)';
  
  return `
    <div class="section predictive-intelligence" style="background: linear-gradient(135deg, #1e3a5f 0%, #0f2744 100%); border: 2px solid #3b82f6; margin-top: 2rem; border-radius: 16px; overflow: hidden;">
      <div class="section-header" style="border-bottom: 1px solid rgba(59,130,246,0.3); padding: 1.5rem; background: rgba(59,130,246,0.1);">
        <div style="display: flex; align-items: center; gap: 1rem;">
          <div class="section-icon" style="font-size: 2rem;">🧠</div>
          <div>
            <div class="section-title" style="font-size: 1.5rem; font-weight: 700; color: #60a5fa;">${t.title}</div>
            <p style="color: #93c5fd; font-style: italic; margin: 0.5rem 0 0 0; font-size: 0.9rem;">
              ${t.subtitle}
            </p>
          </div>
        </div>
      ...
      <div class="section-content" style="padding: 1.5rem;">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 2rem;">
          <div style="background: rgba(59,130,246,0.15); padding: 1.25rem; border-radius: 12px; text-align: center; border: 1px solid rgba(59,130,246,0.3);">
            <div style="font-size: 2.25rem; font-weight: 700; color: ${getSuccessColor(predictions.successProbability)};">
              ${predictions.successProbability}%
            </div>
            <div style="color: #93c5fd; font-size: 0.85rem; margin-top: 0.25rem;">${t.successProbability}</div>
            <div style="margin-top: 0.75rem; height: 6px; background: rgba(255,255,255,0.1); border-radius: 3px; overflow: hidden;">
              <div style="width: ${predictions.successProbability}%; height: 100%; background: ${getSuccessColor(predictions.successProbability)}; transition: width 1s;"></div>
            </div>
          </div>
          
          <div style="background: rgba(59,130,246,0.15); padding: 1.25rem; border-radius: 12px; text-align: center; border: 1px solid rgba(59,130,246,0.3);">
            <div style="font-size: 2.25rem; font-weight: 700; color: #3b82f6;">
              ${predictions.timeline.expected}
            </div>
            <div style="color: #93c5fd; font-size: 0.85rem; margin-top: 0.25rem;">${t.estimatedTimeline}</div>
            <div style="color: #60a5fa; font-size: 0.75rem; margin-top: 0.5rem;">
              ${predictions.timeline.minimum} - ${predictions.timeline.maximum} ${t.months}
            </div>
          </div>
          
          <div style="background: rgba(59,130,246,0.15); padding: 1.25rem; border-radius: 12px; text-align: center; border: 1px solid rgba(59,130,246,0.3);">
            <div style="font-size: 2.25rem; font-weight: 700; color: #10b981;">
              ${predictions.roi.expected}%
            </div>
            <div style="color: #93c5fd; font-size: 0.85rem; margin-top: 0.25rem;">${t.projectedROI}</div>
            <div style="color: #34d399; font-size: 0.75rem; margin-top: 0.5rem;">
              ${predictions.roi.minimum}% - ${predictions.roi.maximum}%
            </div>
          </div>
          
          <div style="background: rgba(59,130,246,0.15); padding: 1.25rem; border-radius: 12px; text-align: center; border: 1px solid rgba(59,130,246,0.3);">
            <div style="font-size: 2.25rem; font-weight: 700; color: #a78bfa;">
              ${predictions.confidence?.overall || 70}%
            </div>
            <div style="color: #93c5fd; font-size: 0.85rem; margin-top: 0.25rem;">${t.confidenceLevel}</div>
            <div style="color: #c4b5fd; font-size: 0.75rem; margin-top: 0.5rem;">
              ${getConfidenceExplanation(predictions.confidence?.overall || 70)?.level || 'Moderate'}
            </div>
          </div>
        </div>
        ...
        <div style="margin-bottom: 2rem;">
          <h4 style="color: #60a5fa; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; font-size: 1.1rem;">
            ⚠️ ${t.riskAssessment}
          </h4>
          <table style="width: 100%; border-collapse: collapse; border-radius: 8px; overflow: hidden;">
            <thead>
              <tr style="background: rgba(59,130,246,0.25);">
                <th style="padding: 0.875rem; text-align: left; color: #93c5fd; border-bottom: 1px solid rgba(59,130,246,0.3); font-weight: 600;">${t.riskFactor}</th>
                <th style="padding: 0.875rem; text-align: center; color: #93c5fd; border-bottom: 1px solid rgba(59,130,246,0.3); font-weight: 600;">${t.probability}</th>
                <th style="padding: 0.875rem; text-align: center; color: #93c5fd; border-bottom: 1px solid rgba(59,130,246,0.3); font-weight: 600;">${t.impact}</th>
                <th style="padding: 0.875rem; text-align: left; color: #93c5fd; border-bottom: 1px solid rgba(59,130,246,0.3); font-weight: 600;">${t.mitigation}</th>
              </tr>
            </thead>
            <tbody>
              ${predictions.risks.slice(0, 5).map(risk => `
                <tr style="background: ${getRiskBg(risk.probability)};">
                  <td style="padding: 0.875rem; color: #e2e8f0; border-bottom: 1px solid rgba(59,130,246,0.2);">${risk.factor}</td>
                  <td style="padding: 0.875rem; text-align: center; border-bottom: 1px solid rgba(59,130,246,0.2);">
                    <span style="color: ${getRiskColor(risk.probability)}; font-weight: 600;">${risk.probability}%</span>
                  </td>
                  <td style="padding: 0.875rem; text-align: center; border-bottom: 1px solid rgba(59,130,246,0.2);">
                    <span style="padding: 0.25rem 0.75rem; border-radius: 50px; font-size: 0.75rem; font-weight: 600;
                      background: ${risk.impact === 'High' ? 'rgba(239,68,68,0.2)' : risk.impact === 'Medium' ? 'rgba(245,158,11,0.2)' : 'rgba(16,185,129,0.2)'};
                      color: ${risk.impact === 'High' ? '#f87171' : risk.impact === 'Medium' ? '#fbbf24' : '#34d399'};">
                      ${risk.impact === 'High' ? t.high : risk.impact === 'Medium' ? t.medium : t.low}
                    </span>
                  </td>
                  <td style="padding: 0.875rem; color: #94a3b8; font-size: 0.9rem; border-bottom: 1px solid rgba(59,130,246,0.2);">${risk.mitigation}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
        ...
        <!-- AI Insights -->
        <div style="background: rgba(59,130,246,0.1); border-radius: 12px; padding: 1.5rem; border: 1px solid rgba(59,130,246,0.3);">
          <h4 style="color: #60a5fa; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; font-size: 1.1rem;">
            💡 ${t.aiInsights}
          </h4>
          <ul style="list-style: none; padding: 0; margin: 0;">
            ${(predictions.insights || []).map(insight => `
              <li style="padding: 0.5rem 0; color: #e2e8f0; display: flex; align-items: flex-start; gap: 0.75rem;">
                <span style="color: #3b82f6; font-size: 1.1rem;">${insight?.icon || '✦'}</span>
        ...
        <!-- Learning Indicator & Government Disclaimer -->
        <div style="margin-top: 1.5rem; padding: 1rem; background: rgba(124,58,237,0.1); border-radius: 8px; border: 1px solid rgba(124,58,237,0.3); display: flex; align-items: center; gap: 1rem;">
          <div style="width: 12px; height: 12px; background: #a78bfa; border-radius: 50%; animation: pulse 2s infinite;"></div>
          <div>
            <span style="color: #c4b5fd; font-size: 0.85rem;">${t.learningIndicator}</span>
            <div style="color: #94a3b8; font-size: 0.75rem; margin-top: 0.25rem;">${t.govDisclaimer}</div>
          </div>
        </div>
...
async function genSectionFirstPass(section, canon, floor) {
  ...
  // [KT:LANGUAGE-ENFORCEMENT-FIX] Translate any remaining English h3 titles to target language
  if (isNonEnglish) {
    try {
      const h3Titles = [];
      ...
      while ((match = h3Regex.exec(html)) !== null) {
        h3Titles.push(match[1].replace(/<[^>]+>/g, '').trim());
      }
      
      // Check if any titles look like English (if they contain common English words)
      const englishIndicators = /^(Overall|Strategic|Implementation|Financial|Key|Risk|Executive|Digital|Process|Operational|Timeline|Roadmap|Analysis|Assessment|Metrics|Performance|Forecast|Dashboard)/i;
      const hasEnglishTitles = h3Titles.some(title => englishIndicators.test(title));
      
      if (hasEnglishTitles && h3Titles.length > 0) {
        console.log(TAG, `[LANGUAGE-ENFORCEMENT] Found potentially English h3 titles in ${section}, translating...`, h3Titles.slice(0, 3));
        
        // Translate h3 titles using SovereignAI
        const titlesToTranslate = h3Titles.join(' | ');
        const transRes = await sovereignAICall({
          prompt: `Translate these section titles to ${canon.lang}:\n${titlesToTranslate}`,
          systemPrompt: `You are a translator. Translate each title to ${canon.lang}. Return ONLY the translated titles in the same order, separated by " | ". No explanations.`,
          callType: 'section-title-translation',
          vertical: 'reform',
          lang: canon.lang,
          temperature: 1,
          maxTokens: 500
        ...
        if (transRes?.success && transRes.content) {
          const translatedTitles = transRes.content.split('|').map(t => t.trim());
          // Replace h3 titles with translated versions
          let titleIndex = 0;
          html = html.replace(/<h3[^>]*>(.*?)<\/h3>/gi, () => {
            const translated = translatedTitles[titleIndex] || h3Titles[titleIndex];
            titleIndex++;
            return `<h3>${translated}</h3>`;
          });
          console.log(TAG, `[LANGUAGE-ENFORCEMENT] Translated ${titleIndex} h3 titles to ${canon.lang}`);
        }
      }
    } catch (e) {
      console.log(TAG, `[LANGUAGE-ENFORCEMENT] Error translating h3 titles:`, String(e?.message || e));
      // Continue with untranslated HTML if translation fails
    }
  }
...
export async function POST(req) {
  ...
  // [KT:SOVEREIGN-AI] Initialize the SovereignAI-backed OpenAI shim
  openai = await getOpenAI();
  console.log(TAG, '🤖 [SOVEREIGN-AI] OpenAI shim initialized with self-learning capabilities');
  ...
  const canon = {
    ...
    // [KT:INDUSTRY-FIX] Explicitly set industry for government reform vertical
    industry: 'Government',
    vertical: 'reform',
    // Additional fields from form
    targetSavings: input.targetSavings || input.costSavingsGoal || '',
  ...
  // [KT:SOVEREIGN-AI] Set language context for self-learning AI
  setReformLang(canon.lang);
  ...
  console.log(TAG, '🎯 [AI-TITLE] Starting title generation...', { lang: canon.lang, orgName: canon.orgName });
  try {
    const titleResult = await __kt_generateReportTitle(canon, {});
    if (titleResult && titleResult.trim()) {
      canon.aiTitle = titleResult;
      console.log(TAG, '✅ [AI-TITLE] SUCCESS - Generated title:', { title: canon.aiTitle, length: canon.aiTitle.length, lang: canon.lang });
    } else {
      throw new Error('Empty title returned from AI');
    }
  } catch (e) {
    console.error(TAG, '⚠️ [AI-TITLE] FAILED - Using fallback:', String(e?.message || e));
    canon.aiTitle = `${canon.orgName} Strategic Transformation & Value Creation Plan`;
    console.log(TAG, '⚠️ [AI-TITLE] Fallback title set:', { title: canon.aiTitle });
  }
  
  // [KT:DIAGNOSTIC] Verify aiTitle is set before proceeding
  if (!canon.aiTitle) {
    console.error(TAG, '🚨 [CRITICAL] canon.aiTitle is UNDEFINED after generation!');
    canon.aiTitle = `${canon.orgName} Strategic Transformation & Value Creation Plan`;
  }
  ...
  if (canon.lang && !/^english$/i.test(canon.lang)) {
    ...
    console.log(TAG, '✅ [TRANSLATION-INIT] Graph translators enabled with', Object.keys(labelsFromTranslate).length, 'phrases');
    console.log(TAG, '🔍 [TRANSLATION-INIT] Sample:', {
      'Overall': labelsFromTranslate['Overall'],
      'Key Insights': labelsFromTranslate['Key Insights'],
      'increasing': labelsFromTranslate['increasing']
    });
  }

  // [KT:AI-SECTION-SELECTION] Use SovereignAI to determine optimal section order
  // This replaces the hardcoded section order with AI-driven selection
  console.log(TAG, '🤖 [AI-SECTION-SELECTION] Determining optimal section order...');
  const sectionSelection = await selectReportSections(canon);
  const order = sectionSelection.sections;
  console.log(TAG, '✅ [AI-SECTION-SELECTION] Selected sections:', {
    order: order.join(' → '),
    rationale: sectionSelection.rationale,
    count: order.length
  });
  ...
  const sectionResults = await Promise.all(order.map(async (key) => {
    ...
    if (key === 'exec') {
      try {
        ...
        // [KT:AI-SUMMARY] Generate AI summary for savings chart in target language
        let aiSummary = null;
        try {
          aiSummary = await getOrGenerateChartSummary(savingsSpec, canon.lang || 'English', 'Cost savings over time for executive summary');
        } catch (e) {
          console.log(TAG, 'exec.savings.aiSummary.error', String(e?.message || e));
        }
        
        const execSavingsChart = barChartHTML({ ...savingsSpec, lang: canon.lang, aiSummary });
      ...
      try {
        ...
        if (impactfulSpec && impactfulSpec.type && impactfulSpec.labels && impactfulSpec.datasets) {
          ...
          // [KT:AI-SUMMARY] Generate AI summary for impactful chart in target language
          let aiSummary = null;
          try {
            aiSummary = await getOrGenerateChartSummary(chartSpec, canon.lang || 'English', 'Most impactful metric for executive summary - key business outcome');
          } catch (e) {
            console.log(TAG, 'exec.impactful.aiSummary.error', String(e?.message || e));
          }
          chartSpec.aiSummary = aiSummary;
    ...
    if (key === 'kpis') {
      try {
        // [KT:FIX-2025-12-22] REMOVED duplicate setGraphTranslator call that was overwriting 
        // the better translator (with case-matching fallbacks) set at TRANSLATION-INIT.
        // The translator is already set at line ~4630 with proper case-matching logic.
        // This duplicate was causing "Overall" and "Key Insights" to remain untranslated
        // because the simpler lookup here doesn't have titleCase/camelCase fallbacks.
        console.log(TAG, '📊 [KPI-SECTION] Using existing graph translator from TRANSLATION-INIT');
        ...
        for (let i = 0; i < selectedKPIs.length; i++) {
          ...
          try {
            ...
            if (kpiData && kpiData.labels && kpiData.values) {
              ...
              // [KT:AI-SUMMARY] Generate AI summary for KPI chart in target language
              let aiSummary = null;
              try {
                aiSummary = await getOrGenerateChartSummary(kpiChartSpec, canon.lang || 'English', `KPI tracking: ${kpiName}`);
                // [KT:FIX-NULL-SAFETY] Ensure aiSummary has required properties
                if (aiSummary && (!aiSummary.overall || !aiSummary.keyInsights)) {
                  console.warn(TAG, 'kpi.aiSummary.incomplete', { has: !!aiSummary, overall: !!aiSummary?.overall, insights: !!aiSummary?.keyInsights });
                  aiSummary = null;
                }
              } catch (e) {
                console.log(TAG, 'kpi.aiSummary.error', String(e?.message || e));
              }
              
              const kpiChart = chartFn({ ...kpiChartSpec, lang: canon.lang, aiSummary });
              kpiCharts += kpiChart + '\n';
              kpiGenerated++;
            }
          } catch (e) {
  ...
  // ============================================================================
  // [KT:PREDICTIVE-INTELLIGENCE] Generate AI-Powered Predictive Intelligence Section
  // ============================================================================
  try {
    console.log(TAG, '🧠 [PREDICTIVE] Generating AI-powered predictive intelligence section...');
    sections.predictiveIntelligence = await generatePredictiveSection(canon, canon.lang || 'English');
    console.log(TAG, '✅ [PREDICTIVE] Predictive intelligence section generated');
  } catch (e) {
    console.error(TAG, '❌ [PREDICTIVE] Error generating predictive section:', String(e?.message || e));
    sections.predictiveIntelligence = ''; // Fallback to empty to not break report
  }
  ...
  try {
    ...
    if (sectionGraphSpecs) {
      // [KT:DYNAMIC-SECTIONS] Use AI-selected sections from 'order' variable
      // Only inject graphs for sections that AI actually selected and generated
      console.log(TAG, '[SECTION-GRAPHS] Injecting graphs for AI-selected sections:', order.join(', '));
      ...
      for (const secKey of order) {
        ...
        try {
          ...
          // [KT:AI-SUMMARY] Generate AI summary for section chart in target language
          let aiSummary = null;
          try {
            aiSummary = await getOrGenerateChartSummary(chartSpec, canon.lang || 'English', `Section analysis: ${secKey}`);
          } catch (e) {
            console.log(TAG, `[SECTION-GRAPHS] aiSummary.error for ${secKey}:`, String(e?.message || e));
          }
          chartSpec.aiSummary = aiSummary;
          
          // [KT:LANGUAGE-PROPAGATION-FIX] Ensure language parameter is passed to all chart rendering functions
          // This ensures chart titles, axes, and summaries are in the target language
          const chartWithLang = {
            ...chartSpec,
            lang: canon.lang || 'English' // CRITICAL: Pass language to chart rendering
          };
          
          if (spec.type === 'line') {
            sectionChart = lineChartHTML(chartWithLang);
          } else if (spec.type === 'pie' || spec.type === 'doughnut') {
            // Use bar as fallback for pie/doughnut
            sectionChart = barChartHTML(chartWithLang);
          } else {
            sectionChart = barChartHTML(chartWithLang);
          }
          ...
          // [KT:DYNAMIC-SECTIONS] Only inject if section exists (was AI-selected)
          if (sectionChart && sections[secKey]) {
            ...
            console.log(TAG, `⚠️ [SECTION-GRAPHS] Skipped ${secKey} - section not in AI selection`);
          }
        } catch (e) {
  ...
  // ============================================================================
  // [KT:AI-SECTION-TABLES] Generate and inject AI-driven tables for each section
  // These are company/industry-specific tables with data relevant to each section
  // Tables are fully translated if non-English language is selected
  // ============================================================================
  try {
    console.log(TAG, '📊 [SECTION-TABLES] Generating AI-driven section tables...');
    
    const sectionTableSpecs = await buildAllSectionTableSpecs(canon, sections);
    
    if (sectionTableSpecs) {
      // [KT:DYNAMIC-SECTIONS] Use AI-selected sections from 'order' variable
      // Only inject tables for sections that AI actually selected and generated
      console.log(TAG, '[SECTION-TABLES] Injecting tables for AI-selected sections:', order.join(', '));
      
      for (const secKey of order) {
        const tableSpec = sectionTableSpecs[secKey];
        if (!tableSpec || !tableSpec.headers || !tableSpec.rows) {
          console.log(TAG, `⚠️ [SECTION-TABLES] Skipped ${secKey} - no valid table spec`);
          continue;
        }
        
        try {
          // Render the table HTML
          const tableHTML = renderSectionTableHTML(tableSpec, secKey);
          
          // [KT:DYNAMIC-SECTIONS] Only inject if section exists (was AI-selected)
          if (tableHTML && sections[secKey]) {
            // Append the table AFTER the section content (before any closing tags)
            sections[secKey] = sections[secKey] + '\n' + tableHTML;
            
            console.log(TAG, `✅ [SECTION-TABLES] Injected table for ${secKey}:`, {
              title: tableSpec.title,
              rows: tableSpec.rows?.length || 0,
              cols: tableSpec.headers?.length || 0
            });
          } else if (!sections[secKey]) {
            console.log(TAG, `⚠️ [SECTION-TABLES] Skipped ${secKey} - section not in AI selection`);
          }
        } catch (e) {
          console.error(TAG, `❌ [SECTION-TABLES] Error injecting table for ${secKey}:`, String(e?.message || e));
        }
      }
      
      console.log(TAG, '✅ [SECTION-TABLES] All section tables injected successfully');
    } else {
      console.log(TAG, '⚠️ [SECTION-TABLES] No specs returned - skipping section table injection');
    }
  } catch (e) {
    console.error(TAG, '❌ [SECTION-TABLES] Fatal error:', String(e?.message || e));
  }

  // ============================================================================
  // [KT:PREDICTIVE-AI-SUMMARIES] Inject AI summaries into all predictive chart placeholders
  // This runs after all chart placeholders are added but before final rendering
  // Ensures all charts (including predictive ones) have translated AI summaries
  // ============================================================================
  try {
    console.log(TAG, '🎯 [PREDICTIVE-AI-SUMMARIES] Starting AI summary injection for predictive charts...');
    await injectAISummariesIntoChartPlaceholders(sections, canon.lang || 'English');
    console.log(TAG, '✅ [PREDICTIVE-AI-SUMMARIES] AI summaries injected into predictive charts');
  } catch (e) {
    console.error(TAG, '❌ [PREDICTIVE-AI-SUMMARIES] Error:', String(e?.message || e));
  }
  ...
  // [KT:FIX-2025-12-22] Fixed section key: was 'risksAndMitigations' but actual key is 'risk'
  // Also increased slice from 3000 to 8000 to capture more risk content from the report
  const reportContentForRisks = [
    sections.risk || '',          // [KT:FIX] Correct section key - this is where AI writes risks
    sections.business || '',
    sections.implementation || '',
    sections.timeline || ''
  ].filter(Boolean).join('\n\n').slice(0, 8000);
  
  // [KT:FIX-2025-12-22] Use actual section keys that exist
  // Valid sections: exec, current, financials, kpis, timeline, ops, risk, roi, nextSteps
  // Note: appendix is generated AFTER this point, so we can't use it here
  const reportContentForPriority = [
    sections.nextSteps || '',     // Contains recommendations and action items
    sections.roi || '',           // Contains ROI projections and initiatives
    sections.exec || '',          // Executive summary with key initiatives
    sections.timeline || '',      // Implementation timeline
    sections.ops || ''            // Operational recommendations
  ].filter(Boolean).join('\n\n').slice(0, 6000); // Increased to 6000 for more context
  ...
  if (priorityResult.success && priorityResult.data) {
    ...
    if (prioritySpec.initiatives && prioritySpec.initiatives.length > 0) {
      ...
      // [KT:FIX-2025-12-22] No hardcoded fallback - skip matrix if no initiatives extracted
      console.error(TAG, '❌ [PRIORITY-MATRIX] No initiatives extracted from report - skipping matrix');
      prioritizationMatrix = '<!-- Priority matrix skipped: no initiatives extracted from report -->';
    }
  } else {
    // [KT:FIX-2025-12-22] No hardcoded fallback - skip matrix if extraction failed
    console.error(TAG, '❌ [PRIORITY-MATRIX] Extraction failed - skipping matrix', { success: priorityResult.success, hasData: !!priorityResult.data });
    prioritizationMatrix = '<!-- Priority matrix skipped: extraction failed -->';
  }
  ...
  if (waterfallResult.success && waterfallResult.data) {
    ...
    if (waterfallSpec.components) {
      ...
      // [KT:AI-SUMMARY] Generate AI summary for waterfall chart in target language
      let aiSummary = null;
      try {
        // Create a spec-like object for the AI summary function
        const waterfallSummarySpec = {
          title: 'Value Bridge Analysis',
          labels: waterfallComponents.map(c => c.name),
          series: waterfallComponents.map(c => c.value),
          xTitle: 'Component',
          yTitle: currency
        };
        aiSummary = await getOrGenerateChartSummary(waterfallSummarySpec, canon.lang || 'English', 'Value bridge showing financial transformation from current to target state');
      } catch (e) {
        console.log(TAG, 'waterfall.aiSummary.error', String(e?.message || e));
      }
      
      waterfallChart = waterfallChartHTML(waterfallComponents, currency, aiSummary);
  ...
  */
  ...
  // [KT:SURGICAL:I18N-ROUTE] New translated builder — same inputs, translated labels
  // [KT:TRANSLATION-DIAGNOSTIC] Log what we're passing to the template
  console.log(TAG, '🔍 [ROUTE-TO-TEMPLATE] Passing to buildReformReportHTMLWithTranslation:', {
    lang: canon.lang,
    orgName: canon.orgName,
    aiTitle: canon.aiTitle,
    aiTitleLength: canon.aiTitle?.length || 0,
    keysCount: Object.keys(translatedLabels || {}).length,
    isDefaultLabels: translatedLabels === DEFAULT_LABELS,
    sectionOrderLength: order?.length || 0,
    sectionsCount: Object.keys(sections || {}).length,
    sample: {
      preparedFor: translatedLabels?.preparedFor,
      preparedBy: translatedLabels?.preparedBy,
      executiveSummary: translatedLabels?.executiveSummary,
      currentState: translatedLabels?.currentState,
      transformationPlan: translatedLabels?.transformationPlan,
      tableOfContents: translatedLabels?.tableOfContents
    }
  });
  
  // [KT:CRITICAL-CHECK] Verify canon.aiTitle exists
  if (!canon.aiTitle || canon.aiTitle.trim() === '') {
    console.error(TAG, '🚨 [CRITICAL] canon.aiTitle is MISSING before template rendering!');
    canon.aiTitle = `${canon.orgName} Strategic Transformation & Value Creation Plan`;
  }
  ...
  let html = await buildReformReportHTMLWithTranslation({
    ...
    sectionOrder: order,               // [KT:DYNAMIC-SECTIONS] Pass AI-determined section order
    worldClassVisuals: {
      execInfographic,
      visualTimeline,
      radarChart,
...
async function patchStandardChartLabelsForLanguage(html, lang) {
  ...
  // [KT:CHART-CSS-INJECTION] Inject GRAPH_CSS and TYPE_SWITCH_SCRIPT for chart type switching
  // This ensures all charts show only one plot type at a time based on the dropdown selection
  const chartCssAndScripts = `
    <style id="graph-css">
      ${GRAPH_CSS}
    </style>
    <script id="type-switch-script">
      ${TYPE_SWITCH_SCRIPT}
    </script>
  `;
  
  // Inject at the start of the HTML if not already present
  if (!finalizedHtml.includes('id="graph-css"')) {
    finalizedHtml = chartCssAndScripts + finalizedHtml;
    console.log(TAG, 'chartCssAndScripts.injected', { success: true });
  }
...
async function buildAppendicesFromAI(reportHTML, canon, sections) {
  ...
  try {
    ...
    // ========== STEP 2: Call SovereignAI with recommendation context ==========
    console.log(TAG, 'step2: calling SovereignAI to generate implementation appendices');

    // [KT:SOVEREIGN-AI-APPENDIX] Embed SovereignAI for self-learning on appendix generation
    // This ensures the AI learns what kinds of appendices work best for implementation
    const appendixWord = isEnglish ? 'Appendix' : `the word for "Appendix" in ${targetLang}`;
    
    const systemPrompt = `You are an expert implementation consultant specializing in transformation strategy. Your role is to design detailed, actionable implementation appendices that guide project execution.

You analyze recommendations and determine exactly what guidance, checklists, frameworks, timelines, and resource plans are needed for successful implementation.

All appendices you generate:
- Are specific to the organization's context
- Follow a structured format with clear columns and rows
- Include actionable recommendations
- Are written in the target language${languageInstruction}`;

    const prompt = `Generate 4-6 detailed implementation appendices based on these strategic recommendations:
...
Return ONLY valid JSON in this structure:
{
  "appendices": [
    {
      "id": "A",
      "title": "Appendix A — [Specific Title in ${targetLang}]",
      ...
      "rows": [
        { "[col1]": "[value in ${targetLang}]", "[col2]": "[value]", "[col3]": "[value]", "[col4]": "[value]" },
        { "[col1]": "[value in ${targetLang}]", "[col2]": "[value]", "[col3]": "[value]", "[col4]": "[value]" }
      ]
    }
  ],
...
CRITICAL: ALL text must be in ${targetLang}. Include 4-6 appendices with types: timeline, checklist, framework, resource_plan, decision_matrix, roadmap.`;

    // [KT:SOVEREIGN-AI-INTEGRATION] Use SovereignAI for appendix generation with self-learning
    const aiResponse = await sovereignAICall({
      prompt,
      systemPrompt,
      callType: 'appendix-generation',
      vertical: 'reform',
      lang: canon.lang,
      expectedFormat: 'json',
      temperature: 1,
    ...
    if (!aiResponse?.success) {
      console.warn(TAG, '[SOVEREIGN-AI-APPENDIX] SovereignAI call failed:', String(aiResponse?.error || aiResponse?.raw || 'Unknown error'));
      // Fall through to error handling below
    }
    
    // Get response from SovereignAI wrapper
    let raw = aiResponse?.content || '';
    
    // Fallback to direct OpenAI if SovereignAI doesn't have content
    if (!raw || raw.length < 100) {
      console.log(TAG, 'SovereignAI appendix response empty, falling back to direct OpenAI call');
      const openai = await getOpenAI();
      const res = await openai.chat.completions.create({
        model: MODEL,
        temperature: 1,
    ...
    const res = { choices: [{ message: { content: raw } }] };

    // ========== STEP 3: Parse and validate response ==========
    let parsedRaw = (res?.choices?.[0]?.message?.content || '').trim();
    console.log(TAG, 'step3: parsing response', { rawLen: parsedRaw.length });

    // Clean up markdown code blocks if present
    parsedRaw = parsedRaw.replace(/^```json\s*/i, '').replace(/^```\s*/, '').replace(/```\s*$/, '').trim();

    // Apply UTF-8 and newline sanitization (same as buildPredictiveModelSpec)
    let jsonStr = extractJsonObject(parsedRaw) || parsedRaw;
    try {
      jsonStr = jsonStr.replace(/^[^\[{]*/, '').trim();
      if (!jsonStr.match(/^[\[{]/)) {
        jsonStr = parsedRaw;
    ...
    try {
      ...
      console.error(TAG, 'json.parse.error', parseErr, {
        rawPreview: parsedRaw.substring(0, 200),
        jsonStrLength: jsonStr.length
      });
      appendixSpec = null;
    }
...
export async function __kt_buildAppendices_FromImplementationKit(reportHTML, canon, sections){
  ...
  try {
    ...
    console.log(TAG, '🔍 starting appendix generation', {
      ...
      sectionKeys: sections ? Object.keys(sections).slice(0, 5) : [],
      reportHTMLSize: reportHTML ? reportHTML.length : 0
    });
    ...
    if (!appendixSpec || !Array.isArray(appendixSpec.appendices) || appendixSpec.appendices.length === 0) {
      console.warn(TAG, '⚠️ AI appendix generation returned empty, no appendices will be generated');
      console.warn(TAG, 'appendixSpec details:', { 
        exists: !!appendixSpec, 
        isArray: Array.isArray(appendixSpec?.appendices),
        length: appendixSpec?.appendices?.length || 0
      });
    ...
    console.log(TAG, '✅ AI generated appendices:', {
      count: appendixSpec.appendices.length,
      ids: appendixSpec.appendices.map(a => a.id).join(', '),
      types: appendixSpec.appendices.map(a => a.type).join(', ')
    });
    ...
    console.log(TAG, '✅ Appendices rendered to HTML:', {
      htmlSize: appendicesHTML ? appendicesHTML.length : 0,
      success: !!appendicesHTML
    });
    ...
    console.error(TAG, '❌ critical appendix generation error:', {
