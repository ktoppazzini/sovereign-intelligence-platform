...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]], lang = 'English' }) {
  ...
  // [KT:HEATMAP-YAXIS-CUTOFF-FIX-v2] Increased padding to prevent Y-axis label cutoff in all languages
  let basePadding = 140; // Increased from 100 to accommodate longer translated labels
  ...
  const charWidth = (hasParentheses || hasCurrency) ? 14 : 13; // Increased char width for better fit
  const extraPadding = Math.max(0, Math.ceil((yMaxLength - 6) * charWidth)); // Trigger earlier (at 6 chars instead of 8)
  const p = Math.min(Math.max(basePadding + extraPadding, 140), 250); // Min 140, max 250
