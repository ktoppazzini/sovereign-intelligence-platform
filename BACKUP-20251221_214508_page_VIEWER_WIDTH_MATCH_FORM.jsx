...
// [KT:SURGICAL] Duplicate import commented out to avoid "already been declared" error
// import ReformReportForm from './ReformReportForm.client';
// import { useAutoPageTranslation } from "@/lib/useAutoPageTranslation";
...
export default function ReformReportPage() {
  // [KT:SURGICAL:AUTO-TRANSLATION] One-line dynamic translation for the main content
  // useAutoPageTranslation("#reform-report-root");

  // Prefer URL ?lang=..., fall back to English, run through normalizeLang
  ...
  useEffect(() => {
    console.log("[AUDIT] UI language set to", lang, "(urlLangRaw =", urlLangRaw, ")");
  }, [lang, urlLangRaw]);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("si-reform-generated", onGenerated);
    }
    return () => {
      if (typeof window !== "undefined") {
        window.removeEventListener("si-reform-generated", onGenerated);
      }
    };
  }, []);
        ...
        >
          ...
          {genDone && generatedHtml && (
            <div
              ...
              style={{
                width: "100%",
                maxWidth: 700,  /* [KT:FIX-v6] Match form width instead of 1200 */
                margin: "0 auto",
                background: "#000",
                color: "#e8eefb",
                borderRadius: 12,
