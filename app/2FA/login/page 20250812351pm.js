 ...
 * - "Forgot password?" link is OUTSIDE the login card, white/bold/larger.
 * - Clicking it POSTS to /api/requestPasswordReset (no email client), translated by lang.
 * - On "Sign In" -> shows green "code sent" (no redirect). Verification visible immediately.
 * - On successful verify -> router.push(/?lang=...).
 * - GPT i18n + console debug. Added keys `forgotEmailRequired`, `forgotSent`, `forgotError`.
 */
...
// const FORGOT_ROUTE = '/2FA/forgot'; // not used (we send directly instead)
...
export default function TranslationLogin() {
  ...
  // i18n (keeps your endpoint usage; now includes forgot keys)
  useEffect(() => {
...
{"title":"Secure Login","emailLabel":"Email:","passwordLabel":"Password:","signIn":"Sign In","codeLabel":"Verification Code:","verifyBtn":"Verify","codeSent":"✅ Code successfully sent. Please check your phone.","verifiedOK":"✅ Verified.","errorGeneric":"Unexpected error occurred","errorSending":"Error sending 2FA","errorVerifying":"Error verifying code","forgotPassword":"Forgot password?","forgotEmailRequired":"Please enter your email first.","forgotSent":"📧 Check your email for steps to reset your password.","forgotError":"Could not send reset email. Please try again."}`;
  ...
  // Sign In (send code) -> stay on page
  const handleSendCode = async (e) => {
    ...
    try {
      ...
      setIsError(false);
      setTranslatedMessage(translations.codeSent);
    } catch (err) {
  ...
  // Forgot password -> directly send email (translated) via API
  const handleForgot = async (e) => {
    ...
    try {
      ...
      if (!res.ok || data?.ok !== true) {
        ...
        setIsError(false);
        setTranslatedMessage(translations.forgotSent);
      }
    } catch (err) {
      setIsError(true);
      setTranslatedMessage(translations.forgotError);
    ...
    <div className={styles.container}>
      <div style={wrap}>
        ...
        <div style={rightCol}>
          ...
          {/* Forgot password link (direct send), OUTSIDE the login card */}
          <div
            style={{
              marginTop: 10,
              fontWeight: 800,
