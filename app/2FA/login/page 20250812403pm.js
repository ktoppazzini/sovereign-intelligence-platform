...
/**
 * Page: /2FA/login
 * TRACK_ID: LOGIN-2FA-v1.6 (one-page 2FA + Forgot Password link)
 * - Layout & styling preserved. We DO NOT touch your CSS modules.
 * - Added a translated "Forgot password?" link OUTSIDE the login card (keeps your visual layout).
 * - Still: On "Sign In" -> shows green "code sent" (no redirect). Verification visible immediately.
 * - On successful verify -> router.push(/?lang=...).
 * - GPT i18n + console debug. Added key `forgotPassword`.
 *
 * Debugging notes (to cut future back-and-forth):
 * - The link label is fetched via the same /api/gptTranslation call as the rest of the labels
 *   to avoid a second request and keep cost down.
 * - If GPT fails/quota exceeded, we fall back to English safely.
 * - Link URL preserves ?lang so the downstream page can also translate.
 */
...
const FORGOT_ROUTE = '/2FA/forgot'; // placeholder route (not used now; mailto instead)
    ...
    <div className={styles.container}>
      <div style={wrap}>
        ...
        <div style={rightCol}>
          ...
          {/* NEW: Forgot password link (translated), OUTSIDE the login card */}
          <div
            style={{
              marginTop: 10,
              fontWeight: 800,
              color: '#ffffff', // WHITE per your request
              fontSize: '1.2rem', // larger
            }}
            dir={isRTL ? 'rtl' : 'ltr'}
          >
            <a
              href={`mailto:${encodeURIComponent(email || '')}?subject=${encodeURIComponent('Steps to reset your password')}&body=${encodeURIComponent('Please click the link below to reset your password:\n' + ((typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000') + '/2FA/reset'))}`}
