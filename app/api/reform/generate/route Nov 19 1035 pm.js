...
if (key === 'nextSteps') {
  ...
  let nextStepsHTML = sections.nextSteps || sections['nextSteps'] || '';
  if (nextStepsHTML) {
    // Ensure nextSteps section uses same wrapper as other report sections
    nextStepsHTML = nextStepsHTML.replace(/<section id=["']nextSteps["']/, '<section id="nextSteps" class="report-section"');
    html += '\n' + nextStepsHTML;
  }
  // === Legacy note cleanup ===
  try {
