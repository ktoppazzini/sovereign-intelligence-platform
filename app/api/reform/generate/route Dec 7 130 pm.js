...
async function translateChartLabels(obj, lang) {
  ...
  // Build comprehensive translation payload including all text fields
  const safe = {
    title: obj?.title || '',
    xTitle: obj?.xTitle || '',
    yTitle: obj?.yTitle || '',
    // Include axis labels array if present (e.g., ["M1", "M2"] or ["Jan", "Feb"])
    labels: Array.isArray(obj?.labels) ? obj.labels : [],
    // Include legend labels from datasets if present
    datasetLabels: Array.isArray(obj?.datasets) 
      ? obj.datasets.map(ds => ds?.label || '') 
      : [],
    // Include series labels if using simple series array structure
    seriesLabels: Array.isArray(obj?.series) && obj.series.length > 0 && typeof obj.series[0] === 'object'
      ? obj.series.map(s => s?.label || '')
      : [],
  };
  ...
  try {
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content:
            `You translate ALL chart text into ${lang}: titles, axis labels, legends, data labels. ` +
            `Respond ONLY with strict JSON matching this structure: ` +
            `{"title":"...","xTitle":"...","yTitle":"...","labels":["...","..."],"datasetLabels":["..."],"seriesLabels":["..."]}. ` +
            `Preserve array lengths exactly. If an array is empty, return empty array [].`,
        },
        {
          role: 'user',
      ...
      max_completion_tokens: 512, // Increased for larger label arrays
    });
    ...
    if (!out || out.length < 2) {
      console.log(TAG, 'chartLabels.translate.emptyResponse', { lang });
      return obj;
    }
    ...
    if (parsed.xTitle) result.xTitle = parsed.xTitle;
    if (parsed.yTitle) result.yTitle = parsed.yTitle;
    
    // Apply translated labels array if present
    if (Array.isArray(parsed.labels) && parsed.labels.length > 0) {
      result.labels = parsed.labels;
    }
    
    // Apply translated dataset labels if present
    if (Array.isArray(parsed.datasetLabels) && parsed.datasetLabels.length > 0 && Array.isArray(result.datasets)) {
      result.datasets = result.datasets.map((ds, i) => ({
        ...ds,
        label: parsed.datasetLabels[i] || ds.label,
      }));
    }
    
    // Apply translated series labels if present
    if (Array.isArray(parsed.seriesLabels) && parsed.seriesLabels.length > 0 && Array.isArray(result.series)) {
      result.series = result.series.map((s, i) => 
        typeof s === 'object' 
          ? { ...s, label: parsed.seriesLabels[i] || s.label }
          : s
      );
    }
...
console.log(TAG, 'labels.translated', {
  lang: canon.lang,
  sample: translatedLabels?.projectedROI,
});
