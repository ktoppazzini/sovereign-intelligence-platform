...
function synthTitleIfMissing(section, frag) {
  return String(frag).replace(
    ...
    (m, pre, jsonRaw, post) => {
      ...
      // Detect which quote the attribute started with (pre ends with ' or ")
      const quoteChar = pre.slice(-1); // ' or "
      // Always escape JSON for attribute safety
      const escaped = toAttrJSON(obj);

      // --- ORIGINAL (commented: could inject raw JSON under double-quoted attr) ---
      // return pre + JSON.stringify(obj) + post;

      // --- SURGICAL: keep original pre (with its quote), inject escaped JSON ---
      // Note: escaped uses &quot; so it is safe inside either ' or " attributes.
