...
8. Professional business tone
  ...
  try {
    let out = injectTableTitles(html); // Add placeholder titles to all tables
    
    // Match ALL placeholders + tables (not just "generic" ones)
    ...
    while ((m = tblRe.exec(out)) !== null) {
      ...
      // ALWAYS generate AI title for EVERY table - no heuristic skip
      // This ensures consistency and quality across all tables
      let newTitle = oldTitle;
      try {
        const sectionHTML = out;
        const tbl = (block.match(/<table[\s\S]*<\/table>/i) || [])[0] || '';

        // Generate AI title - handles all cases: headers, no headers, headerless, etc
        const aiTitle = await __kt_aiTableTitle(sectionHTML, tbl, canon);
        if (aiTitle) {
          newTitle = aiTitle;
          processed++;
        } else {
          // AI returned null (appendix, error, invalid) - keep placeholder
          newTitle = '(Data Table)'; // Better fallback than placeholder text
        }
      } catch (e) {
      ...
      if (aiTitle) {
        console.log('[SR:REFORM] table.title.ai', { was: oldTitle.slice(0, 40), now: newTitle.slice(0, 40) });
      }
    }
    parts.push(out.slice(last));
    console.log('[KT:SURGICAL:TABLE-TITLE-AI-FIX] complete', { processed, total: processed });
    return parts.join('');
  } catch(e) {
