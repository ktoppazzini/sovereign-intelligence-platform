  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: 'system', 
          content: `You are an implementation consultant. Generate a detailed week-by-week implementation checklist. Return ONLY valid JSON with no markdown.${languageInstruction}` 
        },
        { 
          role: 'user', 
          content: `Create a detailed implementation checklist for ${canon.orgName || 'the client'} to achieve ${canon.costSavingsGoal ? '$' + (canon.costSavingsGoal/1000000).toFixed(1) + 'M' : 'target savings'} over ${canon.timeFrame || '24 months'}.

Industry: ${canon.industry || 'N/A'}
Company Size: ${canon.companySize || 'N/A'}

Generate 3 phases with 6-8 specific, actionable tasks per phase:
- Phase 1: Foundation (Weeks 1-4) - Setup, team, baselines
- Phase 2: Quick Wins (Weeks 5-12) - Pilot programs, process improvements
- Phase 3: Scale & Sustain (Weeks 13-24) - Rollout, training, governance
...
Return JSON:
{
  "phases": [
    ...
    const parsed = JSON.parse(raw);
    console.log(TAG, 'implementationChecklist.generated', { phases: parsed?.phases?.length || 0 });
    return parsed;
  } catch (err) {
    console.error(TAG, 'implementationChecklist.error', String(err?.message || err));
    return { phases: [] };
  }
}
