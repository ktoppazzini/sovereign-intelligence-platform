...
// --- ADD just above oneShotBestViz (tiny helper; no removals) ---
function isGraphLikeJSON(section, obj) {
  if (!obj || obj.feasible !== true) return false;
  // never allow tables via predictive path
  if (obj.kind && /table/i.test(obj.kind)) return false;
  // only allow heatmap in risk; otherwise force chart
  if (obj.kind === 'heatmap' && section !== 'risk') return false;
  return true;
}

// --- REPLACE the whole oneShotBestViz with this version (commenting any removals) ---
async function oneShotBestViz(section, canon, sectionHtml, existingSigSet) {
  // Call #1: keyword pool (unchanged)
  ...
  // Mine numbers from the section so the model can reuse/derive
  ...
  // --- OVERRIDE: predictive path must never return tables ---
  // NOTE: We keep the section-writer table guidance elsewhere; this path supersedes it.
  const prompt = `
...
- Title must be specific (3–8 words), not generic ("Chart", "Trend", "Value").
...
`.trim();
  ...
  try {
    ...
    // Pick first feasible, graph-only candidate
    const cand = Array.isArray(obj.candidates)
      ? obj.candidates.find((c) => isGraphLikeJSON(section, c))
      : null;
    ...
    // de-dupe against existing visuals
    ...
    // Render to our canonical fragment (figure/div only)
    const frag = renderVizFromJSON(section, cand);

    // Final safety: never allow table injection from this path
...
/*async function oneShotBestViz(section, canon, sectionHtml, existingSigSet) {
  ...
  const xTitle = isYears ? 'Years' : 'Months';
  const yTitle =
    (section==='financials' || section==='exec') ? yCur :
    (section==='kpis' || section==='roi' || section==='current') ? 'Percent' :
    'Index';
  const allowHeat = (section === 'risk');
...
`.trim();
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: 'Return VALID JSON only (no markdown) following the requested schema.' },
        { role: 'user',   content: prompt }
      ],
    });

    let obj = {};
    try { obj = JSON.parse(res.choices?.[0]?.message?.content || '{}'); }
    catch (e) { console.log(TAG, 'json.parse.error', section, String(e?.message||e)); return ''; }

    const cand = Array.isArray(obj.candidates) ? obj.candidates.find(c => c && c.feasible === true) : null;
    if (!cand) { console.log(TAG, 'json.best.miss', section); return ''; }

    // de-dupe against existing title/kind
    const sig = `${(cand.kind||'chart')}|${String(cand.title||'').trim().toLowerCase()}`;
    if (existingSigSet.has(sig)) {
      console.log(TAG, 'json.best.duplicate', section, { sig });
      return '';
    }
    ...
    const frag = renderVizFromJSON(section, cand);
    if (!frag) { console.log(TAG, 'json.render.fail', section); return ''; }
