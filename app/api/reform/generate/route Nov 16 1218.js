...
// [KT:SURGICAL:DISABLED-DUPLICATE-POST] Duplicate POST handler commented out to resolve ESM/export error.
// export async function POST(req) {
//     const input = await req.json();
//
//     // [KT:SURGICAL:PROFILE] Start timing for profiling
// (removed invalid ``````json block causing syntax error)
// let __openai;
...
`;
    ...
    } // <-- Add this closing brace to fix the try/catch block
    return parsed;
    } catch (e) {
      // Emergency fallback
    ...
    // Top-level fallback in case of unexpected errors
    const labels = ['T1','T2','T3','T4','T5','T6'];
    const values = [10,14,17,20,23,26];
    return { labels, values, xTitle: 'Time', yTitle: 'Value ($)' };
  }
...
/* ===============================
   ...
   =============================== */
let NextResponse, OpenAI, buildReformReportHTML, DEFAULT_LABELS, planDiagramHTML;
let makeCharter, makeRaci, makeRaid, makeBenefits, makePlan100, makePilot, makeAssumptions;
let lineChartHTML, barChartHTML, heatmapHTML, benchmarkHTML, GRAPH_CSS;
let visualTimelineHTML, radarChartHTML, riskHeatmapHTML, waterfallChartHTML, executiveSummaryInfographicHTML;

// Dynamic import helper to load all dependencies at runtime
async function __kt_load_dependencies() {
  if (!NextResponse) {
    const mod = await import('next/server');
    NextResponse = mod.NextResponse;
  }
  if (!OpenAI) {
    const mod = await import('openai');
    OpenAI = mod.OpenAI;
  }
  if (!buildReformReportHTML) {
    const mod = await import('@/lib/reportTemplate.js');
    buildReformReportHTML = mod.default;
    DEFAULT_LABELS = mod.DEFAULT_LABELS;
  }
  if (!planDiagramHTML) {
    const mod = await import('@/lib/implPlan.js');
    planDiagramHTML = mod.planDiagramHTML;
  }
  if (!makeCharter) {
    const mod = await import('@/lib/implKitSchemas.js');
    makeCharter = mod.makeCharter;
    makeRaci = mod.makeRaci;
    makeRaid = mod.makeRaid;
    makeBenefits = mod.makeBenefits;
    makePlan100 = mod.makePlan100;
    makePilot = mod.makePilot;
    makeAssumptions = mod.makeAssumptions;
  }
  if (!lineChartHTML) {
    const mod = await import('@/lib/reportGraphs.js');
    lineChartHTML = mod.lineChartHTML;
    barChartHTML = mod.barChartHTML;
    heatmapHTML = mod.heatmapHTML;
    benchmarkHTML = mod.benchmarkHTML;
    GRAPH_CSS = mod.GRAPH_CSS;
  }
  if (!visualTimelineHTML) {
    const mod = await import('@/lib/worldClassVisuals.js');
    visualTimelineHTML = mod.visualTimelineHTML;
    radarChartHTML = mod.radarChartHTML;
    riskHeatmapHTML = mod.riskHeatmapHTML;
    waterfallChartHTML = mod.waterfallChartHTML;
    executiveSummaryInfographicHTML = mod.executiveSummaryInfographicHTML;
  }
}
  ...
  try {
    ...
    try {
      ...
      console.log(TAG, 'width.guard.injected');
              ...
              console.log(TAG, 'section.viz.debug.error', key, String(e?.message || e));
            }
  ...
  if (heat) {
    ...
    try {
      ...
      if (!json) {
        ...
        // [KT:SURGICAL:HEATMAP-DEBUG] Log parsed heatmap meta for diagnostics
        console.log('[KT:SURGICAL:HEATMAP-DEBUG]', {
          title: json?.title,
          cols: Array.isArray(json?.cols) ? json.cols.length : 0,
          rows: Array.isArray(json?.rows) ? json.rows.length : 0,
          data: Array.isArray(json?.data) ? json.data.length : 0,
          sampleCols: Array.isArray(json?.cols) ? json.cols.slice(0, 3) : [],
          sampleRows: Array.isArray(json?.rows) ? json.rows.slice(0, 3) : [],
          sampleData: Array.isArray(json?.data) ? json.data.slice(0, 3) : []
        });
        console.log(TAG, 'predictive.parse.heatmap', {
          title: json?.title,
          cols: Array.isArray(json?.cols) ? json.cols.length : 0,
          rows: Array.isArray(json?.rows) ? json.rows.length : 0,
          data: Array.isArray(json?.data) ? json.data.length : 0
...
- Situation-Complication-Resolution: State the current reality, the problem/opportunity, then the recommendation
  ...
  try {
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: 'system', 
          content: `You are a professional business translator. Translate the following UI labels into ${targetLang}.
CRITICAL RULES:
- Return ONLY a raw JSON object with the same keys
...
- Maintain formal tone suitable for executive consulting reports
...
The HTML document contains a business transformation report with charts, tables, and executive content. Translate all headings, paragraphs, table content, chart titles, and button labels while preserving the document structure.`
      ...
      max_tokens: 16000 // Large token limit for full document translation
    });
...
async function topUpSection(section, canon, currentHTML, floor) {
  ...
  while (words < floor && guard < 8) {
    ...
    const res = await openai.chat.completions.create({
      ...
      temperature: 1,
      messages: [
        { // Ensure this opening brace has a corresponding closing brace
          role: 'system',
...
async function POST(req) {
    ...
    // ...existing POST logic...
    // [KT:SURGICAL:PROFILE] End timing and log duration
    const profileEnd = Date.now();
    console.log('[SR:REFORM][PROFILE] Total report generation time:', (profileEnd - profileStart), 'ms');

    // [KT:SURGICAL:FIX] Declare and initialize sections object before use
    let sections = {};

    // ... rest of your code, including sectionPromises, etc.

    // [KT:SURGICAL:CACHE] Store result in cache for future identical requests
    // (Place this at the point where you return the report)
    // Example: after generating the final report and before returning response
    // reportCache.set(cacheKey, { ok: true, report: finalReportHtml, ...otherFields });
  const sectionPromises = order.map(async (key) => {
    const t0 = Date.now();
    ...
    // Generate dynamic KPI benchmarks + standard charts inside the map, only for 'kpis'
    if (key === 'kpis') {
      try {
  ...
  async function generateSectionsAndPhases() {
  ...
  const { sections: generatedSections, phases } = await generateSectionsAndPhases();
  ...
  // [KT:SURGICAL:APPENDIX-GRAPH-DEDUP+LOG] Inject a single predictive graph between appendix D and E, styled and logged
  let predictiveGraphHTML = '';
  let predictiveGraphLog = '';
  try {
    // Use the predictive model spec generated earlier (if present)
    const pm = sections.predictiveModel;
    if (pm && Array.isArray(pm.labels) && Array.isArray(pm.values) && pm.labels.length && pm.values.length) {
      // Dynamically import chart renderers for style consistency
      const { barChartHTML } = await import('@/lib/reportGraphs.js');
      const chartArgs = {
        title: pm.title || 'Predictive Outcome (Projected)',
        labels: pm.labels,
        series: pm.values,
        xTitle: pm.xTitle || 'Time',
        yTitle: pm.yTitle || 'Value ($)'
      };
      predictiveGraphHTML = barChartHTML(chartArgs);
      predictiveGraphLog = '[KT:APPENDIX-GRAPH] Predictive graph rendered successfully.';
    } else {
      predictiveGraphHTML = `<figure class="chart-card appendix-chart-fallback" style="padding:32px;text-align:center;color:#aaa;font-style:italic;border:2px dashed #ccc;border-radius:12px;min-height:180px;">
        <div>No predictive graph data available for this appendix section.</div>
      </figure>`;
      predictiveGraphLog = '[KT:APPENDIX-GRAPH] Predictive graph NOT rendered: missing or invalid data.';
    }
  } catch (e) {
    predictiveGraphHTML = `<figure class="chart-card appendix-chart-fallback" style="padding:32px;text-align:center;color:#aaa;font-style:italic;border:2px dashed #ccc;border-radius:12px;min-height:180px;">
      <div>Error rendering predictive graph: ${String(e?.message || e)}</div>
    </figure>`;
    predictiveGraphLog = '[KT:APPENDIX-GRAPH] Predictive graph ERROR: ' + String(e?.message || e);
  }
  // Log to VS terminal (stdout)
  if (typeof process !== 'undefined' && process.stdout) {
    process.stdout.write(predictiveGraphLog + '\n');
  } else {
    console.log(predictiveGraphLog);
  }

  // Insert predictiveGraphHTML after Appendix D (Benefits), before E (100-Day Plan)
  appendicesHTML = appendicesHTML.replace(
    /(<section[^>]*>\s*<h3[^>]*>Appendix D[\s\S]*?<\/section>)/i,
    (m) => m + '\n' + predictiveGraphHTML + '\n'
  );
  // Remove any other predictive graph between D and E (deduplication)
  let foundGraph = false;
  appendicesHTML = appendicesHTML.replace(/(<figure[^>]*class=["']chart-card appendix-chart["'][^>]*>[\s\S]*?<\/figure>)/gi, (m) => {
    if (!foundGraph) { foundGraph = true; return m; }
    return '';
  });
  // Reset for next request
  foundGraph = undefined;
...
console.log(TAG, 'worldClassVisuals.prepare.done', {
  ...
  // [KT:SURGICAL:DEDUP_FINAL+BLANK-APPENDIX-FILTER] Remove repeated and blank appendix sections (A–J) in the final HTML.
  try {
    // Remove duplicate Appendix A–J blocks (by comment marker and section)
    html = html.replace(/<!-- Appendix [A-J]:[\s\S]*?-->\s*<section[\s\S]*?<\/section>/gi, (match, offset, str) => {
      const appendixMatch = match.match(/<!-- Appendix ([A-J]):/i);
      if (!appendixMatch) return match;
      const appendixLetter = appendixMatch[1];
      if (!globalThis.__kt_appendix_seen) globalThis.__kt_appendix_seen = {};
      if (globalThis.__kt_appendix_seen[appendixLetter]) {
        return '';
      }
      globalThis.__kt_appendix_seen[appendixLetter] = true;
      return match;
    });
    // Remove any static HTML appendix blocks that might have been appended twice (by id or class)
    html = html.replace(/(<section[^>]+id=["']appendices["'][\s\S]*?<\/section>)([\s\S]*?)\1/gi, '$1');
    // Remove blank/empty appendix sections (no table rows, no chart, no content)
    html = html.replace(/(<section[^>]*class=["']appendix[^"']*["'][^>]*>)([\s\S]*?)(<\/section>)/gi, (m, open, body, close) => {
      // If no <tr>, no <figure>, no <canvas>, no <svg>, and no non-whitespace text, remove
      if (!/(<tr[\s\S]*?<\/tr>|<figure|<canvas|<svg)/i.test(body) && !body.replace(/<[^>]+>/g, '').trim()) {
        console.log('[KT:SURGICAL:APPENDIX-FILTER] Removed blank appendix section.');
        return '';
      }
      return m;
    });
    // Reset for next request
    globalThis.__kt_appendix_seen = undefined;
    html = __kt_dedupeVisuals(html);
    console.log('[SR:REFORM] visuals.dedupe.injected + appendix dedupe + blank appendix filter');
  } catch (e) {
    console.log('[SR:REFORM] visuals.dedupe.invoke.error', String(e?.message || e));
  }
  ...
  // Return translated HTML (or original if English/error)
  return NextResponse.json({ ok: true, html: translatedHtml, wordCount: sectionsWords, htmlWordCount: htmlWords });
}

// - Profiled bottlenecks: (1) Redundant/serial AI calls, (2) Synchronous section generation, (3) No caching, (4) Inefficient regex/HTML processing
// - Solutions: (A) Parallelize all independent AI calls (sections, appendices, visualizations), (B) Add in-memory LRU cache for AI completions, (C) Batch AI requests where possible, (D) Use fast string/HTML processing, (E) Avoid duplicate work

// --- BEGIN OPTIMIZATION PATCH ---

// 1. LRU Cache for expensive AI completions (per-process, 1000 entries, 1hr expiry)
const __kt_aiCache = (() => {
  const cache = new Map();
  const max = 1000, ttl = 1000 * 60 * 60;
  return {
    get(key) {
      const v = cache.get(key);
      if (!v) return undefined;
      if (Date.now() - v.t > ttl) { cache.delete(key); return undefined; }
      // Move to end (LRU)
...
// 2. AI call wrapper with caching (keyed by prompt+model)
async function cachedAICompletion({ model, messages, temperature = 1, max_tokens }) {
  const key = JSON.stringify({ model, messages, temperature, max_tokens });
  const cached = __kt_aiCache.get(key);
  if (cached) return cached;
  const res = await openai.chat.completions.create({ model, messages, temperature, max_tokens });
  __kt_aiCache.set(key, res);
  return res;
}

// 3. Parallel section generation (exec, current, financials, kpis, timeline, ops, risk, roi)
const sectionKeys = ['exec', 'current', 'financials', 'kpis', 'timeline', 'ops', 'risk', 'roi'];
const sectionFloors = sectionKeys.map(k => SECTION_FLOORS[k] || 1200);

// 4. Generate all sections in parallel (first pass + top-up)
const sectionPromises = sectionKeys.map((key, idx) => (async () => {
  let html = '';
  // Use cached AI completions for sectionPrompt and expandPrompt
  const floor = sectionFloors[idx];
  const languageInstruction = canon.lang && !/^english$/i.test(canon.lang) 
    ? `\n\nIMPORTANT: Write all content in ${canon.lang}. Use professional business terminology appropriate for ${canon.lang}.`
    : '';
  // First pass
  {
    const prompt = sectionPrompt(key, canon, floor);
    const sys = `You are a senior consultant. Return clean HTML fragments only.${languageInstruction}`;
    const res = await cachedAICompletion({ model: MODEL, messages: [
      { role: 'system', content: sys },
      { role: 'user', content: prompt }
    ]});
    html = res.choices?.[0]?.message?.content?.trim() || '';
    html = sanitizeNonPredictiveVisuals(key, html);
  }
  // Top-up (if needed)
  let words = wc(html), guard = 0;
  while (words < floor && guard < 8) {
    const remaining = Math.max(200, floor - words);
    const prompt = expandPrompt(key, canon, remaining);
    const sys = `Extend the section with NEW, non-duplicative content. Return HTML only.${languageInstruction}`;
    const res = await cachedAICompletion({ model: MODEL, messages: [
      { role: 'system', content: sys },
      { role: 'user', content: prompt }
    ]});
    const add = res.choices?.[0]?.message?.content?.trim() || '';
    html += '\n' + sanitizeNonPredictiveVisuals(key, add);
    words = wc(html);
    guard++;
  }
  return html;
...
// 5. Await all sections in parallel
async function generateSectionsAndPredictives() {
  const sectionResults = await Promise.all(sectionPromises);
  sectionKeys.forEach((k, i) => { sections[k] = sectionResults[i]; });

  // 6. Predictive fragment generation (parallel for all eligible sections)
  const predictivePromises = sectionKeys.map(async (key) => {
    if (key === 'exec' || key === 'kpis') return '';
    const { sigs: existingSigs } = collectExistingVizMeta(sections[key]);
    return choosePredictiveFragment(key, canon, sections[key], existingSigs);
  });
  const predictiveResults = await Promise.all(predictivePromises);
  sectionKeys.forEach((k, i) => {
    if (predictiveResults[i]) sections[k] += '\n' + predictiveResults[i];
  });

  // 7. KPI Benchmarks (parallel with section generation)
  let kpiBenchmarks = [];
  // Ensure 'sections' is defined before using it
  if (typeof generateKPIBenchmarks === 'function' && typeof sections === 'object' && sections.kpis) {
    kpiBenchmarks = await generateKPIBenchmarks(canon, sections.kpis);
  }
  return { sections, kpiBenchmarks };
}
const { sections: generatedSections, kpiBenchmarks } = await generateSectionsAndPredictives();
sections = generatedSections;

// 8. Implementation Kit JSON (parallel)
const implKitPromise = genImplKitJSON(canon, sections);

// 9. World-class visuals (parallel)
...
const [implKit, phases] = await Promise.all([implKitPromise, phasesPromise]);

// 10. Translate labels (parallel)
const translatedLabelsPromise = translateLabels(canon.lang);

// 11. World-class visuals (parallel)
const execInfographicPromise = Promise.resolve(
...
const visualTimelinePromise = Promise.resolve(
  visualTimelineHTML((phases || []).map((p, i) => ({
    name: p.title || `Phase ${i + 1}`,
    startWeek: i * 6,
    endWeek: (i + 1) * 6,
    color: ['#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EF4444'][i % 5],
...
// 12. Await all parallel visual/label promises
const [translatedLabels, execInfographic, visualTimeline] = await Promise.all([
  translatedLabelsPromise, execInfographicPromise, visualTimelinePromise
]);

// 13. Build report HTML (single pass)
let html = buildReformReportHTML({ 
  ...
  worldClassVisuals: {
    execInfographic,
    visualTimeline
    // ...other visuals can be added here
  }
});

// 14. Deduplicate visuals (single pass, fast regex)
html = __kt_dedupeVisuals(html);

// --- END OPTIMIZATION PATCH ---
// === end injection point ===

  // [KT:SURGICAL:DEDUP_FINAL] Remove repeated visuals by title/kind (final pass)
  // [KT:SURGICAL:APPENDIX-DEDUP] Ensure each appendix section (A–J) appears only once in the final HTML.
  try {
    // Remove duplicate Appendix A–J blocks (by comment marker and section)
    html = html.replace(/<!-- Appendix [A-J]:[\s\S]*?-->\s*<section[\s\S]*?<\/section>/gi, (match, offset, str) => {
      // Only keep the first occurrence of each appendix marker
      const appendixMatch = match.match(/<!-- Appendix ([A-J]):/i);
      if (!appendixMatch) return match;
      const appendixLetter = appendixMatch[1];
      // If this appendix letter has already been seen, remove this block
      if (!globalThis.__kt_appendix_seen) globalThis.__kt_appendix_seen = {};
      if (globalThis.__kt_appendix_seen[appendixLetter]) {
        return '';
      }
    ...
    globalThis.__kt_appendix_seen = undefined;
    html = __kt_dedupeVisuals(html);
    console.log('[SR:REFORM] visuals.dedupe.injected + appendix dedupe');
  } catch (e) {
    console.log('[SR:REFORM] visuals.dedupe.invoke.error', String(e?.message || e));
  }
...
Problem Statement: ${sections.problem || 'N/A'}
Analysis Summary: ${sections.analysis || 'N/A'}
Solution Overview: ${sections.solution || 'N/A'}
Timeline: ${sections.timeline || 'N/A'}
    `.trim();
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role:'user',   content: 
`Build an Implementation Kit JSON from the report content below.
...
IMPORTANT: Return ONLY the JSON object. No markdown code blocks, no backticks, no explanatory text.` }
    ...
    try{
      ...
      // Ensure raid is only called once
      const raidValue = makeRaid();
      ...
      // This comment is for internal reference only and should NOT be rendered or injected into any HTML output.
      return {
...
============================================================================ */
// [KT:SURGICAL][2025-10-30] — Dynamic appendices I/J/K generator
// Purpose: Generate and inject self-implementation appendices (I: Implementation Checklist, J: Decision Framework, K: Resource Requirements)
// Each appendix is generated via AI using the canon and sections context, and rendered as a <section> with a unique id and heading.
// This function is called after the main report and appendices are built, and appends the new sections to the HTML.

export async function __kt_append_beyond_mckinsey_appendices(html, canon, sections) {
  try {
   let out = String(html || '');
   // Generate all three appendices in parallel
   const [checklist, framework, resources] = await Promise.all([
    genImplementationChecklist(canon, sections),
    genDecisionFramework(canon, sections),
    genResourceRequirements(canon, sections)
   ]);
   ...
   let appendixI = '';
   if (checklist && Array.isArray(checklist.phases) && checklist.phases.length) {
    appendixI = [
      '<section id="appendix-i" class="appendix">',
      '<h3>Appendix I: Implementation Checklist</h3>',
      '<table class="report-table">',
      '<thead><tr><th>Phase</th><th>Key Activities</th><th>Owner</th><th>Success Criteria</th></tr></thead>',
      '<tbody>',
      checklist.phases.map(phase =>
       `<tr>
        <td>${phase.phase || ''}</td>
        <td>${Array.isArray(phase.activities) ? phase.activities.join('<br>') : ''}</td>
        <td>${phase.owner || ''}</td>
   ...
   let appendixJ = '';
   if (framework && Array.isArray(framework.scenarios) && framework.scenarios.length) {
    appendixJ = [
      '<section id="appendix-j" class="appendix">',
      '<h3>Appendix J: Decision Framework for Roadblocks</h3>',
      '<table class="report-table">',
      '<thead><tr><th>Scenario</th><th>Symptoms</th><th>Root Cause</th><th>Recommended Action</th></tr></thead>',
      '<tbody>',
      framework.scenarios.map(s =>
       `<tr>
        <td>${s.scenario || ''}</td>
        <td>${s.symptom || ''}</td>
        <td>${s.rootCause || ''}</td>
   ...
   let appendixK = '';
   if (resources && (Array.isArray(resources.team) || Array.isArray(resources.budget) || Array.isArray(resources.tools))) {
    appendixK = [
      '<section id="appendix-k" class="appendix">',
      '<h3>Appendix K: Resource Requirements</h3>',
      // Team table
      (Array.isArray(resources.team) && resources.team.length ? [
       '<h4>Core Team</h4>',
       '<table class="report-table">',
       '<thead><tr><th>Role</th><th>FTE %</th><th>Duration</th><th>Responsibilities</th></tr></thead>',
       '<tbody>',
       resources.team.map(t =>
        `<tr>
          <td>${t.role || ''}</td>
          <td>${t.fte || ''}</td>
          <td>${t.duration || ''}</td>
      ...
      // Budget table
      (Array.isArray(resources.budget) && resources.budget.length ? [
       '<h4>Budget Breakdown</h4>',
       '<table class="report-table">',
       '<thead><tr><th>Category</th><th>Amount</th><th>Description</th></tr></thead>',
       '<tbody>',
       resources.budget.map(b =>
        `<tr>
          <td>${b.category || ''}</td>
          <td>${b.amount || ''}</td>
          <td>${b.description || ''}</td>
      ...
      (Array.isArray(resources.tools) && resources.tools.length ? [
       '<h4>Essential Tools & Software</h4>',
       '<table class="report-table">',
   ...
   // Append all appendices to the HTML
   out += '\n' + appendixI + '\n' + appendixJ + '\n' + appendixK;
   return out;
  } catch (e) {
   console.warn('[KT:SURGICAL][BEYOND-MCKINSEY-APPENDICES] Error:', String(e?.message || e));
   return html;
  }
}
...
export async function __kt_buildAppendices_FromImplementationKit(reportHTML, canon, sections){
  try{
    // [KT:SURGICAL:IMPLKIT-BRIDGE] Use real Implementation Kit JSON (implKit) instead of stub or sections.__implementationKitJSON
    // Legacy (stub fallback):
    // const json = (sections && sections.__implementationKitJSON) || { ... };
    const json = (typeof implKit !== 'undefined' && implKit) ? implKit : ((sections && sections.__implementationKitJSON) || {
      "charters": [], "raci":{"items":[]}, "raid":{"items":[]},
      "benefits":{"lines":[]}, "plan100":{"weeks":[]},
    ...
    const section = [
      ...
      '  <h2>Appendices</h2>',
      '  <h3>Implementation Kit</h3>',
      '  <div class="card">',
      '    <h4>Embedded JSON</h4>',
      '    <pre class="json-block">', JSON.stringify(json, null, 2).replace(/[<]/g,'&lt;'), '</pre>',
      '  </div>',
      '</section>'
    ].join('\n');
    return section + "\n" + embed;
  }catch(e){ console.warn('[KT] appendix build error', e); return '<section id="appendices"><h2>Appendices</h2><p>Appendix failed gracefully.</p></section>'; }
}
