...
function patchStandardChartLabelsForLanguage(html, lang) {
  if (!lang) return html;

  const normalized = String(lang).trim().toLowerCase();
  // If report is English, keep original labels.
  if (normalized === 'en' || normalized === 'english') return html;

  // Extendable: add more languages as needed.
  const replacementsByLang = {
    fr: [
      // Executive & KPI chart titles
      ['Projected Savings Over Time', 'Économies projetées dans le temps'],
      ['Savings Realization', 'Réalisation des économies'],
      ['Implementation Expenditures', 'Dépenses de mise en œuvre'],
      ['Cumulative Payback', 'Remboursement cumulatif'],
      ['ROI Trend (AI)', 'Tendance du ROI (IA)'],
      ['Ops Throughput Trend', 'Tendance du débit opérationnel'],
      ['Delivery Penetration vs Market', 'Pénétration de la livraison vs marché'],
      ['Market Share Progress vs Industry Leader', 'Progression de la part de marché vs leader du secteur'],
      ['Cost Savings Realization vs CAD Target', 'Réalisation des économies vs objectif CAD'],

      // Axis labels (generic, non–company-specific)
      ['Years', 'Années'],
      ['Year', 'Année'],
      ['Percent', 'Pourcentage'],
      ['Savings', 'Économies'],
      ...
      ['ROI Index', 'Indice de ROI'],
      // Keep "Index" generic but translate when used as a y-axis label.
      ['>Index<', '>Indice<'], // safer pattern for y-axis caption
    ],
  };

  const rules = replacementsByLang[normalized];
  if (!rules) return html;

  let patched = html;

  for (const [from, to] of rules) {
    // Use split/join so we fix occurrences inside data-chart JSON safely.
...
if (!__isEnglishReportLangSoft(reportLang)) {
  ...
  if (canon.lang && !__isEnglishReportLangSoft(canon.lang)) {
    translatedHtml = await patchExecutiveSummaryAndRiskLabels(translatedHtml, canon.lang);
  }
