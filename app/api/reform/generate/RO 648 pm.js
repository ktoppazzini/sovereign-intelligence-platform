...
async function polishMcKinsey(html, canon) {
  ...
  // Capture spec attributes BEFORE polish (byte-for-byte)
  const captureSpecs = (s, attr) => {
    const out = [];
    // Fixed: escape backslash so \1 is preserved as a regex backreference
...
- The result must be **better than McKinsey** in readability, visual hierarchy, and professional finish —
  WITHOUT changing document structure.
...
`.trim();
  ...
  try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'Return JSON with keys "applied" (true) and "html". Keep structure only, no content changes.',
            },
            {
              role: 'user',
              content:
    ...
    // --- Freeze check & guards (length ignored per product requirement) ---
    ...
    const lengthOK = true; // per requirement: audit must NOT gate on word count
...
export async function POST(req) {
  ...
  // McKinsey-style polish (structure & length guards; set to "better than McKinsey"; ignore word-count gate)
...
/* ========================================================================== */
/* [KT PATCH] ensureStandardVisuals — FIXED ATTR QUOTING                       */
/*  - Old HOTFIX wrapped the JSON in &quot;...&quot;, producing a value like  */
/*    \"{...}\" which the renderer couldn't JSON.parse.                     */
/*  - New version uses toAttrJSON(obj) to write safe attribute JSON.          */
/* ========================================================================== */
function ensureStandardVisuals(html, canon) {
  var out = String(html || '');
  ...
  if (!hasKpi) {
    ...
    /* ORIGINAL (commented, kept for history — incorrect extra &quot; wrappers)
    var dash = ''
      + '<section id="dashboard">'
      + '  <div class="chart-card">'
      + '    <figure data-chart="&quot;{'
    ...
    */
    ...
    // FIXED: Build specs as objects and serialize via toAttrJSON (no double-encoding)
    var spec1 = {
      type: 'line',
      title: 'Savings Over Time',
      xTitle: 'Months',
