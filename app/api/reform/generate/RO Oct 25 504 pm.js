 ...
 * Merged & corrected: OOM-safe regex, getOpenAI shim, predictive renderer defaults.
 * Surgical: only server-side generation logic. No front-end or layout edits.
 */
...
// Safe accessor for OpenAI client (quietly returns null if not available)
...
// Floors (section strategy from Oct-22)
...
// Inject a deterministic note for standard charts if missing
...
// Comment out legacy paragraphs following visuals (keeps them safe)
...
async function injectTableTitles(html, canon){
  try{
    const s = String(html||'');
    // OOM-safe class matcher (surgical fix)
    ...
    while((m=re.exec(s))!==null){
      ...
      // Bold white text to match requested style
...
function mergeSynth(section, canon, c){
  ...
  merged.labels = Array.isArray(merged.labels) && merged.labels.length ? merged.labels : ['Y1','Y2','Y3'];
  merged.datasets = Array.isArray(merged.datasets) && merged.datasets.length ? merged.datasets : [{ label:'Predicted', data:[600000,1100000,1650000] }];
  return merged;
}

// RENDER predictive candidate → <figure data-chart="...">
function renderCandidateToFragment(section, c){
  // preserve provided values but inject presentation defaults that match screenshot
  const first = (Array.isArray(c.datasets) && c.datasets[0]) ? c.datasets[0] : {};
  const styled = {
    label: first.label || 'Projected',
    data: Array.isArray(first.data) ? first.data : [600000, 1100000, 1650000],
  ...
  const spec = {
    ...
    xTitle: c.xTitle || 'Years',
    yTitle: c.yTitle || (section === 'financials' ? 'CAD' : 'Percent'),
    labels: Array.isArray(c.labels) && c.labels.length ? c.labels : ['Y1','Y2','Y3'],
    datasets: [ styled ],
    options: {
      responsive: true,
...
export async function POST(req){
  ...
  // enforce total floor allocation
  ...
  // Table title injection (OOM-safe) + legacy cleanup + standard chart notes
