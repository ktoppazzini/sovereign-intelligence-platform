...
export async function POST(req) {
  ...
  for (const key of order) {
    ...
    // [KT:VIZ] ==== injection branch + breadcrumbs (restored) ====
    ...
    if (key !== 'exec' && !preHas) {
      ...
      try {
        ...
        console.log(TAG, 'section.viz.mined.result', key, {
          found: !!mined,
          len: mined ? mined.length : 0,
        });
      } catch (e) {
      ...
      if (mined) {
        ...
        try {
          ...
          console.log(TAG, 'section.viz.derived.result', key, {
            found: !!derived,
            len: derived ? derived.length : 0,
          });
        } catch (e) {
        ...
        if (derived) {
          ...
          try {
            ...
            console.log(TAG, 'section.viz.advice.result', key, {
              found: !!advice,
              len: advice ? advice.length : 0,
            });
          } catch (e) {
    ...
    // [KT:DBG][predictive] compact predictive block per major section (exec excluded)
    if (key !== 'exec') {
      let pred = '';
      try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'Return ONE compact predictive HTML block (<p> and/or a tiny <table>). No extra text.',
            },
            {
              role: 'user',
              content: `
Return ONE compact predictive HTML block for "${key}" using numbers in the section (or defensible derivation from canon).
Keep it ~80–120 words max. Include units (local currency or %). No headings above <h4>, no lists.
--- CANON ---
...
${html}`.trim(),
      ...
      console.log(TAG, 'section.predictive.result', key, {
        found: !!pred,
        len: pred ? pred.length : 0,
      });
      if (pred) {
        html += '\n' + pred;
