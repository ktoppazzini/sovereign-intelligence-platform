// NCE / Strategic Advisory// [KT:SURGICAL:APPENDIX-GRAPH-RENDER-FIX] (moved to POST handler)
...
import buildReformReportHTML, { DEFAULT_LABELS } from '@/lib/reportTemplate.js' // [KT:SURGICAL] explicit .js extension to guarantee default export resolution;
...
import { 
 ...
 waterfallChartHTML, 
 executiveSummaryInfographicHTML 
} from '@/lib/worldClassVisuals.js'; // [MCKINSEY-BEATING] 5 world-class visualizations [KT:LEGACY-IMPORT]

// [KT:SURGICAL:WC-VISUALS-INJECT-TRANSLATOR]


// [KT:I18N-TABLE-TITLES-LANG-GUARD] — soft check for English-only AI retitling
async function __isEnglishReportLangSoft(lang) {
  const v = String(lang || '').toLowerCase();
  return v === 'english' || v === 'en' || v.startsWith('en-');

// try {
//   const translate = require(process.cwd() + '/lib/translate.js');
// } catch (e) {
//   console.warn('[KT:WARN] translate.js not found, skipping translation');
// }
// import translate from '../../../lib/translate.js'

// [KT:SURGICAL:OPENAI-HELPER-FIX]
// Helper to lazily load OpenAI instance (prevents ReferenceError in topUpSection)
let __openaiInstance;
async function getOpenAI() {
  if (!__openaiInstance) {
    // Use already imported OpenAI if available, else dynamic import
    if (typeof OpenAI !== 'undefined') {
      __openaiInstance = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    } else {
...
// [KT:SURGICAL:JSON-SAFE-PARSE]
function __kt_safeParseJSON(label, raw) {
  try {
    if (!raw) return null;
    let text = String(raw).trim();

    // Try to isolate the main JSON block
    ...
    console.log('[SR:REFORM] json.safeParse.error', {
      label,
...
`;
    ...
    try {
      ...
      if (!parsed ||
        ...
        const values = labels.map((_, i) => Math.round(base * (0.5 + i / (labels.length * 1.2))));
        parsed = {
          labels,
          values,
          xTitle: 'Time',
...
// Reference to ensure these are included for side effects (if needed)
[lineChartHTML, barChartHTML, heatmapHTML, GRAPH_CSS];
...
/*function __kt_dedupeVisuals(html = '') {
  try {
    ...
    out += src.slice(last);
    console.log('[SR:REFORM] visuals.dedupe', { kept: kept, dropped: dropped, unique: seen.size });
    return out;
  } catch (e) {
    console.log('[SR:REFORM] visuals.dedupe.error', String(e?.message || e));
    return html;
...
// === [KT:SURGICAL:VIZ-TRANSLATE] ============================================
// Dynamically translate chart/benchmark/heatmap specs inside data-* attributes
// so titles, axes and labels are in canon.lang instead of hard-coded English.
// ============================================================================
async function __kt_translateVisualSpecs(html = '', canon = {}) {
  try {
    const langRaw = String(canon?.lang || 'English');
    const lang = langRaw.trim();
    // English or missing language -> no-op
    if (!lang || /^en(g(lish)?)?$/i.test(lang)) {
      console.log('[SR:REFORM] visuals.translateSpecs.skip', { lang });
      return html;
    }

    const src = String(html || '');
    // Same matcher as __kt_dedupeVisuals
    const re =
      /(<figure[^>]*data-chart=['"]([\s\S]*?)['"][^>]*>[\s\S]*?<\/figure>)|(<figure[^>]*data-widget=['"]benchmark['"][^>]*data-spec=['"]([\s\S]*?)['"][^>]*>[\s\S]*?<\/figure>)|(<div[^>]*data-heatmap=['"]([\s\S]*?)['"][^>]*>[\s\S]*?<\/div>)/gi;

    // 1) Collect all specs + all strings to translate
    const specs = []; // { kind, attrName, rawSpec, json, start, end }
    const textMap = {}; // id -> original string
    const pointers = []; // { id, specIndex, field, idx? }
    ...
    while ((m = re.exec(src)) !== null) {
      ...
      if (m[1] && m[2]) {
        ...
        // heatmap
        kind = 'heatmap';
        attrName = 'data-heatmap';
        rawSpec = m[6];
      } else {
        continue;
      }

      const json = safeParseAttrJSON(rawSpec) || {};
      const idx = specs.length;
      specs.push({ kind, attrName, rawSpec, json, start, end, full });
      matchIndex++;

      // Helper to register a string for translation
      const register = (val, field, extraIdx) => {
        const s = String(val || '').trim();
        if (!s) return;
        const id = `v${idx}_${field}${extraIdx != null ? '_' + extraIdx : ''}`;
        textMap[id] = s;
        pointers.push({ id, specIndex: idx, field, idx: extraIdx });
      };

      // Common fields
      register(json.title, 'title');
      if (json.xTitle) register(json.xTitle, 'xTitle');
      if (json.yTitle) register(json.yTitle, 'yTitle');

      if (kind === 'chart') {
        // Dataset labels, if any
        if (Array.isArray(json.datasets)) {
          json.datasets.forEach((ds, j) => {
            if (ds && ds.label) register(ds.label, 'datasetLabel', j);
          });
        }
        // Optional category labels
        if (Array.isArray(json.labels)) {
          json.labels.forEach((lab, j) => {
            register(lab, 'label', j);
          });
        }
      } else if (kind === 'heatmap') {
        if (Array.isArray(json.rows)) {
          json.rows.forEach((r, j) => register(r, 'row', j));
        }
        if (Array.isArray(json.cols)) {
          json.cols.forEach((c, j) => register(c, 'col', j));
        }
      } else if (kind === 'benchmark') {
        // Benchmarks usually only have title + yTitle; already covered
      }
    }

    const keys = Object.keys(textMap);
    if (!keys.length || !specs.length) {
      console.log('[SR:REFORM] visuals.translateSpecs.none', {
        lang,
        specs: specs.length,
        strings: keys.length,
      });
    ...
    // 2) Call OpenAI once to translate all strings
    let translated = {};
    try {
      const openai = await getOpenAI();
      const payload = {};
      for (const id of keys) payload[id] = textMap[id];

      const userPrompt = `
Translate the following short chart labels into ${lang}.
...
Do NOT change keys, do NOT add or remove keys.
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a precise UI translator. Return only valid JSON. Do not explain.',
          },
          {
            role: 'user',
      ...
      try {
        translated = JSON.parse(res.choices?.[0]?.message?.content || '{}');
      } catch (e) {
        console.log('[SR:REFORM] visuals.translateSpecs.jsonError', String(e?.message || e));
        translated = {};
      }
    } catch (e) {
      console.log('[SR:REFORM] visuals.translateSpecs.callError', String(e?.message || e));
      translated = {};
    }

    // 3) Apply translations back into specs
    for (const ptr of pointers) {
      const { id, specIndex, field, idx } = ptr;
      const spec = specs[specIndex];
      if (!spec) continue;
      const srcVal = textMap[id];
      const dstVal = (translated && translated[id]) || srcVal || '';

      if (field === 'title') {
        spec.json.title = dstVal;
      } else if (field === 'xTitle') {
        spec.json.xTitle = dstVal;
      } else if (field === 'yTitle') {
        spec.json.yTitle = dstVal;
      } else if (field === 'datasetLabel' && Array.isArray(spec.json.datasets)) {
        if (spec.json.datasets[idx]) {
          spec.json.datasets[idx].label = dstVal;
        }
      } else if (field === 'label' && Array.isArray(spec.json.labels)) {
        spec.json.labels[idx] = dstVal;
      } else if (field === 'row' && Array.isArray(spec.json.rows)) {
        spec.json.rows[idx] = dstVal;
      } else if (field === 'col' && Array.isArray(spec.json.cols)) {
        spec.json.cols[idx] = dstVal;
      }
    }

    // 4) Rebuild HTML with updated specs
    let out = '';
    let last = 0;
    let specCursor = 0;

    // Reuse same regex to walk and replace
    re.lastIndex = 0;
    while ((m = re.exec(src)) !== null) {
      const start = m.index;
      const end = re.lastIndex;
      ...
      // Find matching spec object
      const spec = specs[specCursor];
      if (!spec || spec.start !== start || spec.end !== end) {
        // If something drifted, just keep original
        out += orig;
      } else {
        // Rebuild attribute with translated JSON
        const attrJSON = toAttrJSON(spec.json);

        if (spec.kind === 'chart') {
          // replace data-chart="...old..." with new encoded JSON
          const replaced = orig.replace(
            /data-chart=['"][\s\S]*?['"]/i,
            `data-chart="${attrJSON}"`,
          );
          out += replaced;
        } else if (spec.kind === 'benchmark') {
          const replaced = orig.replace(
            /data-spec=['"][\s\S]*?['"]/i,
            `data-spec="${attrJSON}"`,
          );
    ...
    console.log('[SR:REFORM] visuals.translateSpecs.done', {
      lang,
      specs: specs.length,
      strings: keys.length,
    });
    ...
    console.log('[SR:REFORM] visuals.translateSpecs.error', String(e?.message || e));
    return html;
  }
}
...
async function aiSuggestKeywords(section, canon, sectionHtml) {
  ...
  try {
    ...
    let arr;
    try {
      // [KT:SURGICAL:I18N-PREDICTIVE-KEYWORDS]
      // If canon.lang is not English, translate keywords array to target language
      arr = JSON.parse(txt);
      if (canon && canon.lang && !/^english$/i.test(canon.lang) && Array.isArray(arr) && arr.length > 0) {
      try {
        const openai = await getOpenAI();
        const translationPrompt = `
  You are a professional business translator. Translate the following array of predictive analytics keywords/phrases to ${canon.lang}.
  - Translate each keyword/phrase to ${canon.lang}.
  - Do NOT translate numbers, company names, or technical acronyms.
  - Return ONLY the translated JSON array. No markdown, no prose.
  ...
  ${JSON.stringify(arr)}
        ...
        let translatedArr = [];
        try {
        translatedArr = JSON.parse(translationRes.choices?.[0]?.message?.content || '[]');
        if (Array.isArray(translatedArr) && translatedArr.length === arr.length) {
          arr = translatedArr;
          console.log(TAG, 'predictive.keywords.translated', { lang: canon.lang, count: arr.length });
        }
        } catch (e) {
        console.log(TAG, 'predictive.keywords.translation.parse.error', String(e?.message || e));
        }
      } catch (e) {
        console.log(TAG, 'predictive.keywords.translation.error', String(e?.message || e));
      }
      }
    } catch {
      arr = [];
...
/**
 * Purpose: Wrap any legacy, non-<code>chart-note</code> paragraphs that directly follow a visual
 * (chart, benchmark, or heatmap) in an HTML comment, to hide outdated descriptions.
 * This runs before injecting new descriptions, preventing duplicate notes.
 */
...
*/
function injectTableTitles(html) {// [KT:SURGICAL:I18N-TABLE-TITLES]

  const translatedTitle = translate.sync
  ? translate.sync(title, canon.lang)
  : title;
  <div class="table-title" style="font-weight:bold;color:#fff;">
  ${translatedTableTitle}
</div>
 ...
 /* [KT:LEGACY:TABLE-TITLES]
function injectTableTitles(html) {// [KT:SURGICAL:I18N-TABLE-TITLES]
  // ... legacy logic using first-row headers and "(Untitled Table)" ...
}
[KT:LEGACY:TABLE-TITLES] */

// [KT:SURGICAL:TABLE-TITLES-V2]
// Deterministic pass: ensure every <table class="report-table"> has a
// <h3 class="table-title"> wrapper BEFORE the table, but DO NOT derive a
// title from the first row. Let AI name the table later.
function injectTableTitles(html) {
  ...
  try {
    ...
    // Find all <table ... class="report-table"...>...</table> occurrences
    const tableRe =
      /<table\b[^>]*class=(["'])(?:(?:(?!\1).)*\s)?report-table(?:(?:(?!\1).)*)\1[^>]*>[\s\S]*?<\/table>/gi;
    ...
    while ((match = tableRe.exec(out)) !== null) {
      ...
      // Check immediate previous content for an existing table-title
      const beforeSnippet = out.slice(Math.max(0, tableStart - 400), tableStart);
      if (/\btable-title\b/i.test(beforeSnippet)) {
        // Already has a title; leave table unchanged
        parts.push(tableStr);
      ...
      // DO NOT derive from first row or headers anymore.
      // Just create a neutral placeholder; AI will rename it.
      let title = 'Table'; // neutral placeholder, gets replaced by AI

      __kt_tbl_scanned++;
      __kt_tbl_titled++;
      if (__kt_tbl_samples.length < 3) __kt_tbl_samples.push(title);

      // Build the title h3 (bold white)
      const titleDiv =
        `<h3 class="table-title" ` +
        `style="font-weight:bold;color:#FFFFFF;margin-bottom:.4rem;">` +
        `${title}</h3>`;
...
/*async function injectTableTitlesAI(html) {
  try {
    ...
    while ((m = tblRe.exec(out)) !== null) {
      ...
      let newTitle = translatedTableTitle || oldTitle;
      try {
...
/*async function injectTableTitlesAI(html, canon = {}) {
  try {
    // Reuse deterministic titles first
    let out = injectTableTitles(html);

    // [KT:SURGICAL:I18N-TABLE-TITLES]
    // Use the report language name in the prompt (default English)
    const langName = canon?.lang || 'English';

    // Updated regex to match <h3 class="table-title"> instead of <div>
    const tblRe =
      /(<h3 class="table-title"[^>]*>)([\s\S]*?)(<\/h3>\s*<table\b[^>]*class=["'][^"']*report-table[^"']*["'][^>]*>[\s\S]*?<\/table>)/gi;
    ...
    while ((m = tblRe.exec(out)) !== null) {
      ...
      // Heuristic: only improve obviously-generic titles
      const looksGeneric =
        /untitled/i.test(oldTitle) || // "(Untitled Table)" needs AI improvement
        /metric\b/i.test(oldTitle) ||
        /baseline|target|rationale/i.test(oldTitle) ||
      ...
      if (!looksGeneric) {
        // leave high-quality titles alone
        parts.push(block);
        last = end;
        continue;
      }

      // Default to the old title if AI fails
      ...
      try {
        ...
        const prompt = [
          `You are naming a business table for executives in ${langName}.`,
          'Return ONLY a short title (<= 7 words) in that language.',
          'Describe what the table is about (not the exact headers).',
          'No surrounding quotes, markdown, or explanations.',
          'Allowed characters: letters, numbers, spaces, & and % only.',
          '',
          'HTML of the table:',
          tblOnly,
        ].join('\n');
        ...
        const r = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'Return only the title text. No quotes, no markdown, no extra words.',
            },
            { role: 'user', content: prompt },
          ],
        });
        ...
        console.log(
          '[SR:REFORM] table.title.ai.translate.error',
          String(e?.message || e),
        );
      }

      // Preserve the style attribute when replacing title content
      const upgraded = block.replace(
        /(<h3 class="table-title"[^>]*>)[\s\S]*?(<\/h3>)/i,
        `$1${newTitle}$2`,
      );
      ...
      console.log('[SR:REFORM] table.title.ai', {
        lang: langName,
        was: oldTitle,
        now: newTitle,
      });
...
async function injectTableTitlesAI(html, canon = {}) {
  try {
    // 1) First pass: deterministic titles from thead (your existing helper)
    let out = injectTableTitles(html);

    // 2) Second pass: only refine obviously generic / untitled tables with AI
    //    Pattern: <h3 class="table-title">...</h3><table class="report-table">...</table>
    const tblRe = /(<h3 class="table-title"[^>]*>)([\s\S]*?)(<\/h3>\s*<table\b[^>]*class=["'][^"']*report-table[^"']*["'][^>]*>[\s\S]*?<\/table>)/gi;
    ...
    while ((m = tblRe.exec(out)) !== null) {
      ...
      // Heuristic: only improve generic titles
      const looksGeneric =
        /untitled/i.test(oldTitle) ||
        /metric\b/i.test(oldTitle) ||
        /baseline|target|rationale/i.test(oldTitle) ||
        oldTitle.split('•').length >= 3;
      ...
      try {
        // Extract just the table HTML so the model can infer meaning
        const tblOnly = (block.match(/<table[\s\S]*<\/table>/i) || [])[0] || '';

        const prompt = [
          'You are naming a business table for executives.',
          'Return ONLY a short title (<= 7 words).',
          'Describe what the table is about (not headers).',
          'No quotes, no markdown, no punctuation except letters/numbers/&/%.',
          'HTML of the table:',
          tblOnly
        ].join('\n');
        ...
        console.log('[SR:REFORM] table.title.ai.inner.error', String(e?.message || e));
      }

      // Preserve <h3> attributes and style; replace only the text content
      const upgraded = block.replace(
        /(<h3 class="table-title"[^>]*>)[\s\S]*?(<\/h3>)/i,
        `$1${newTitle}$2`
      );
...
/* [KT:SURGICAL:I18N-TABLE-TITLES-NEXT]
 * Legacy experimental block that tried to re-translate titles per table.
 * Kept for history but COMMENTED OUT to avoid top-level `await` errors.
 *
 * NOTE: The full report HTML (including <h3 class="table-title">text</h3>)
 *       is already translated later by translateHTML, so additional per-table
...
//function commentOutLegacyVisualNotes(html) {
 // try {
   ...
   // let count = 0;
    ...
    // For each visual, comment out ALL immediately-following <p> blocks
    // that are not already <p class="chart-note"> … stopping at the first non-<p>.
    //const visRe = /((?:<figure[^>]*data-chart=['"][\s\S]*?<\/figure>)|(?:<figure[^>]*data-widget=['"]benchmark['"][\s\S]*?<\/figure>)|(?:<div[^>]*data-heatmap=['"][\s\S]*?<\/div>))/gi;

    //out = out.replace(visRe, (visual, _1, offset) => {
      //let tail = out.slice(offset + visual.length);
      //let consumed = 0;
      //let replaced = '';

      // eat whitespace/newlines
      //const ws = (tail.match(/^\s+/) || [''])[0];
     // replaced += ws;
      //consumed += ws.length;
      //tail = tail.slice(ws.length);

      // repeatedly comment paragraphs that are NOT class="chart-note"
      //const paraRe = /^<(?!!--)p\b(?![^>]*class=["']chart-note["'])[\s\S]*?<\/p>\s*/i;
      //let m; let didAny = false;
      //while ((m = tail.match(paraRe))) {
        //didAny = true; count += 1;
        //replaced += `<!-- [KT:SURGICAL LEGACY NOTE COMMENTED OUT]\n${m[0]}\n-->`;
        //consumed += m[0].length;
        //tail = tail.slice(m[0].length);
      //}

      //if (!didAny) return visual;       // nothing to comment
      // splice: visual + commented block(s)
     // return visual + replaced;
    //});

    //console.log('[SR:REFORM] chart.notes.legacy.commented', { count });
   // return out;
  //} catch (e) {
    //console.log('[SR:REFORM] chart.notes.legacy.error', String(e?.message || e));
    //return html;
  //}
//}
...
//async function injectVisualDescriptions(html, canon) {
  ...
  //}
//}
// [KT:SURGICAL GRAPH NOTES] — universal description injector (fixed)
//async function injectVisualDescriptions(html, canon) {
 // try {
   // const src = String(html || '');
   // const re = /(<figure[^>]*data-chart=['"][\s\S]*?<\/figure>)|(<figure[^>]*data-widget=['"]benchmark['"][\s\S]*?<\/figure>)|(<div[^>]*data-heatmap=['"][\s\S]*?<\/div>)/gi;

    //let match, cursor = 0, parts = [], injectedCount = 0;
   ...
   // while ((match = re.exec(src)) !== null) {
     ...
     // const frag  = match[0];
      ...
      // does a chart-note already follow (allowing whitespace)?
      //const after = src.slice(end, end + 700).replace(/^\s+/, '');
      //const hasNote = /^<p\b[^>]*class=["']chart-note["'][^>]*>[\s\S]*?<\/p>/i.test(after);
    ...
    //parts.push(src.slice(cursor));
    //const out = parts.join('');
    //console.log('[SR:REFORM] chart.notes.injected', { count: injectedCount });
    //return out;
  //} catch (e) {
   // console.log('[SR:REFORM] chart.notes.error', String(e?.message || e));
    //return html;
  //}
//}
...
// [KT:SURGICAL:EXEC-METRICS] Derive executive summary metrics dynamically from report content
/*async function deriveExecSummaryMetrics(canon, sections, phases) {
   const defaults = {
    roi: String(canon?.roi || '').trim() || '',
    payback: canon?.paybackMonths
      ? `${canon.paybackMonths} ${canon.paybackUnit || 'months'}`
      : '',
    savings: String(canon?.savings || '').trim() || '',
    timeline: String(canon?.timeline || canon?.timeFrame || '').trim() || '',
    confidence: String(canon?.confidence || '').trim() || '',
    impact: String(canon?.impact || '').trim() || '',
    quickWins: Number.isFinite(canon?.quickWins)
      ? canon.quickWins
      : (phases && phases.length) ? Math.min(phases.length, 6) : 0,
    phases: (phases && phases.length)
      ? phases.length
      : (Number.isFinite(canon?.phases) ? canon.phases : 0),
  };
  // Language-aware instruction so metrics themselves come back in the target language
  const languageInstruction =
    canon.lang && !/^english$/i.test(canon.lang)
      ? `\n\nIMPORTANT: Return ALL strings already written in ${canon.lang}.`
      : `\n\nIMPORTANT: Return ALL strings in English.`;

  // Compact snapshot of key sections (exec + financials + ROI) – HTML is fine
  const snap = [
    sections?.exec,
    sections?.current,
    sections?.financials,
    ...
    .filter(Boolean)
    .join('\n\n---\n\n')
    .slice(0, 12000); // guard token size

  /*const defaults = {
    roi: canon.roi || '267%',
    payback: canon.paybackMonths ? `${canon.paybackMonths} months` : '8 months',
    savings: canon.savings || '$2.4M annually',
    timeline: canon.timeline || (canon.timeFrame || '18 months'),
    confidence: '92%',
    impact: 'High',
    quickWins: 3,
    phases: (phases && phases.length) || 4,
  };

    [KT:LEGACY:EXEC-DEFAULTS] Old hard-coded demo defaults (commented out – DO NOT USE IN PROD)
  [KT:SURGICAL:EXEC-DEFAULTS-DYNAMIC-I18N]
  Dynamically translate all default values if a non-English language is requested.
  Renamed to avoid redeclaration error
  const defaults = { ... }  <-- Removed duplicate declaration to avoid error

If the requested language is not English, translate all default values (keys and values)
  if (canon.lang && !/^english$/i.test(canon.lang)) {
    try {
      const translationPrompt = `
You are a professional business translator. Translate the following JSON object to ${canon.lang}.
- Translate ALL keys and ALL values, including units and phrases.
- Do NOT translate numbers, company names, or currency codes.
- Return ONLY the translated JSON object. No markdown, no prose.
...
${JSON.stringify(defaults, null, 2)}
      ...
      let translatedObj = {};
      try {
        translatedObj = JSON.parse(translationRes.choices?.[0]?.message?.content || '{}');
        // Defensive: If translation failed, fallback to original
        if (Object.keys(translatedObj).length === 8) {
          Object.assign(defaults, translatedObj);
        } else {
          console.log(TAG, 'defaults.i18n.translation.incomplete', { keys: Object.keys(translatedObj) });
        }
      } catch (e) {
        console.log(TAG, 'defaults.i18n.translation.parse.error', String(e?.message || e));
      }
    } catch (e) {
      console.log(TAG, 'defaults.i18n.translation.error', String(e?.message || e));
    }
  }
  const out = { ...defaults, ...(obj || {}) };*/
// [KT:SURGICAL:EXEC-METRICS-I18N]
async function deriveExecSummaryMetrics(canon, SEC, L, lang) {
  const execDefaults = {
    roi: canon?.roi || 'n/a',
    payback: canon?.payback || 'n/a',
    savings: canon?.savings || 'n/a',
    timeline: canon?.timeFrame || 'n/a',
    confidence: 'n/a',
    impact: 'n/a'
  };

  // existing deterministic calculations here (use SEC, ROI numbers, etc.)
  let out = { ...execDefaults };

  // === AI refinement (keep, but make safe + no dangling "defaults" or "obj") ===
  try {
        const translationPrompt = `
    You are a senior transformation consultant building an "Executive Summary at a Glance" infographic.
    
    Given:
    - CANON (baseline numeric assumptions)
    - KEY SECTIONS of the report (HTML)
    you must choose decisive, defensible executive metrics.
    
    Use the content to refine or override defaults where possible.
    Always keep values realistic and internally consistent.
    
    Return STRICT JSON ONLY, with:
    
    {
      "roi": "xxx",          // ROI percentage, short string, e.g. "245%"
      "payback": "xxx",      // Payback period, short string with units, e.g. "14 months"
      "savings": "xxx",      // Annual savings, short string, e.g. "C$2.3M annually"
      "timeline": "xxx",     // Transformation horizon, short string, e.g. "24 months"
      "confidence": "xxx",   // Confidence level, short string, e.g. "90%"
      "impact": "xxx",       // Strategic impact phrase, e.g. "High" or "Transformational"
      "quickWins": 3,        // Integer (count of quick wins)
      "phases": 4            // Integer (number of phases)
    }
    
    Rules:
    - Use the report content to choose or adjust the numbers.
    - If the report is vague, fall back to sensible defaults.
    - "payback" and "timeline" MUST include units (months/years).
    - quickWins and phases MUST be integers, not strings.
    ${lang && !/^english$/i.test(lang) ? `- If the requested language is not English, translate ALL keys and ALL values in the JSON to ${lang}. This includes keys like "roi", "payback", "savings", "timeline", "confidence", "impact", "quickWins", "phases", and all their values and units.` : ''}
    - If the requested language is English, keep keys and values in English.
    - Do not add any explanations or markdown formatting. Return only the JSON object.
    CANON:
    ${JSON.stringify(
      {
        ...
        timeFrame: canon.timeFrame,
        roi: canon.roi,
        paybackMonths: canon.paybackMonths,
        savings: canon.savings,
        costSavingsGoal: canon.costSavingsGoal,
      },
      null,
      2,
    )}
    
    REPORT SECTIONS (HTML snippets):
    ${SEC && typeof SEC === 'object' ? Object.values(SEC).join('\n\n---\n\n').slice(0, 12000) : ''}
    `.trim();
    const r = await openai.chat.completions.create({
      model: MODEL,
      temperature: 1,
    ...
    const parsed = __kt_safeParseJSON('execMetrics', raw) || {};*/
    // [KT:SURGICAL:I18N-LABELS]
const raw = completion.choices?.[0]?.message?.content || '{}';
// report raw once if needed
// console.log('[SR:REFORM] labels.translate.raw', String(raw).slice(0, 200));

const parsed = __kt_safeParseJSON('labels', raw);
if (!parsed || typeof parsed !== 'object') {
  console.log('[SR:REFORM] labels.translate.fallback', { lang });
  // fall back to identity mapping (no translation) instead of blowing up
  return labels; // whatever your original label array/map is
}

// use "parsed" as your translated map

    if (parsed && typeof parsed === 'object') {
      out = { ...out, ...parsed };
    }
  } catch (e) {
    console.log('[SR:REFORM] execMetrics.translate.error', String(e?.message || e));
  }

  // normalize
  out.roi = String(out.roi || execDefaults.roi).trim();
  out.payback = String(out.payback || execDefaults.payback).trim();
  out.savings = String(out.savings || execDefaults.savings).trim();
  out.timeline = String(out.timeline || execDefaults.timeline).trim();
  out.confidence = String(out.confidence || execDefaults.confidence).trim();
  out.impact = String(out.impact || execDefaults.impact).trim();
...
out.impact = String(out.impact || execDefaults.impact).trim();
  ...
  // [KT:SURGICAL:EXEC-METRICS-I18N]
  // Dynamically translate all keys and values in the JSON output if a non-English language is requested.
  // This ensures the infographic is fully localized, including keys and units.
  // [KT:SURGICAL:EXEC-METRICS-I18N]
  // Dynamically translate all keys and values in the JSON output if a non-English language is requested.
  // This ensures the infographic is fully localized, including keys and units.
  const user = `
...
- quickWins and phases MUST be integers, not strings.
${languageInstruction}
- If the requested language is not English, translate ALL keys and ALL values in the JSON to ${canon.lang}. This includes keys like "roi", "payback", "savings", "timeline", "confidence", "impact", "quickWins", "phases", and all their values and units.
- If the requested language is English, keep keys and values in English.
- Do not add any explanations or markdown formatting. Return only the JSON object.
CANON:
${JSON.stringify(
...
`.trim();
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Return *only* valid JSON with the 8 keys: roi, payback, savings, timeline, confidence, impact, quickWins, phases. No prose, no markdown.',
        },
        { role: 'user', content: user },
      ],
    });
    ...
    try {
      ...
      console.log(TAG, 'execMetrics.json.parse.error', String(e?.message || e));
      obj = {};
    }

    // If the language is English, use as-is. If not, translate keys/values to target language.
    let out = { ...defaults, ...(obj || {}) };

    // Type + sanity guard
    out.roi = String(out.roi || defaults.roi).trim();
    out.payback = String(out.payback || defaults.payback).trim();
    out.savings = String(out.savings || defaults.savings).trim();
    out.timeline = String(out.timeline || defaults.timeline).trim();
    out.confidence = String(out.confidence || defaults.confidence).trim();
    out.impact = String(out.impact || defaults.impact).trim();

    const qw = parseInt(out.quickWins, 10);
    const ph = parseInt(out.phases, 10);
    out.quickWins = Number.isFinite(qw) && qw > 0 ? qw : defaults.quickWins;
    out.phases = Number.isFinite(ph) && ph > 0 ? ph : defaults.phases;

    // [KT:SURGICAL:EXEC-METRICS-I18N:TRANSLATE-KEYS-AND-VALUES]
    // If the requested language is not English, translate both keys and values
    if (canon.lang && !/^english$/i.test(canon.lang)) {
      try {
        // Compose a translation prompt for the keys and values
        const translationPrompt = `
You are a professional business translator. Translate the following JSON object to ${canon.lang}.
- Translate ALL keys and ALL values, including units and phrases.
- Do NOT translate numbers, company names, or currency codes.
...
${JSON.stringify(out, null, 2)}
        ...
        // Wrap the await in an async IIFE to allow usage of await in this block
        await (async () => {
          const translationRes = await openai.chat.completions.create({
            model: MODEL,
            temperature: 1,
          ...
          try {
            ...
            if (Object.keys(translatedObj).length === 8) {
              ...
              console.log(TAG, 'execMetrics.i18n.translation.incomplete', { keys: Object.keys(translatedObj) });
            }
          } catch (e) {
            console.log(TAG, 'execMetrics.i18n.translation.parse.error', String(e?.message || e));
          }
        })();
      } catch (e) {
        console.log(TAG, 'execMetrics.i18n.translation.error', String(e?.message || e));
      }
    }

    console.log(TAG, 'execMetrics.derived', out);
    return out;
  } catch (e) {
    console.log(TAG, 'execMetrics.error', String(e?.message || e));
    return defaults;
  }
}

// [KT:SURGICAL:TIMEFRAME-PARSE] Convert canon timeframe into approximate weeks (no hard-coded 18m/6w)
function parseTimeFrameToWeeks(canon) {
  const tf = String(canon.timeline || canon.timeFrame || '').toLowerCase();
  if (!tf) return null;

  // rough parse: "<number> <unit>"
  const m = tf.match(/(\d+(\.\d+)?)[^\d]*(month|months|mo|year|years|yr|yrs|week|weeks)/);
  if (!m) return null;
  ...
  if (/month/.test(unit) || /mo/.test(unit)) {
    return Math.round(num * 4);   // approximate 4 weeks per month
  }
  if (/year/.test(unit) || /yr/.test(unit)) {
    return Math.round(num * 52);  // 52 weeks per year
  }
...
if (key === 'nextSteps') {
  ...
  html = await genSectionFirstPass(key, canon, floor);
    ...
    if (key === 'kpis') {
      try {
        ...
        const savingsChart = barChartHTML({
        ...
        const paybackChart = lineChartHTML({
        ...
        for (let i = 0; i < benchmarks.length; i++) {
          ...
          const bmChart = benchmarkHTML(bm);
          kpiCharts += bmChart + '\n';
          console.log(TAG, `✅ [KPI-FIX] AI benchmark ${i + 1} generated:`, bmChart.length, 'chars');
        }
      ...
      messages: [
        {
          role: 'system',
          content:
            `Return HTML only, a single <section id="conclusion">…</section> with <h3> subheadings to organize the content. Include 6-8 short paragraphs, 3-4 sentences each.${languageInstruction}`,
        },
        {
          role: 'user',
          content: `Write a "Conclusion" section for ${canon.orgName} based on the preceding sections. Structure with 2-3 <h3> subheadings (e.g., "Key Takeaways", "Strategic Imperatives", "Next Steps"). Under each subheading, write 2-3 paragraphs, max 3-4 sentences per paragraph, no run-on sentences.`,
        },
      ],
    });
  ...
  /*const execInfographic = executiveSummaryInfographicHTML({
    ...
    // 1. Executive Summary Infographic - extract key metrics (dynamic + language-aware)
  // [KT:LEGACY:EXEC-INFOGRAPHIC] Original fixed metrics kept for audit:
  /*
  const execInfographic = executiveSummaryInfographicHTML({
    roi: canon.roi || '267%',
    payback: canon.paybackMonths ? `${canon.paybackMonths} months` : '8 months',
  ...
  */
  ...
  /*let execInfographic = '';
  try {
    const summaryMetrics = await deriveExecSummaryMetrics(canon, sections, phases);
    execInfographic = executiveSummaryInfographicHTML(summaryMetrics);
  } catch (e) {
    console.log(TAG, 'execInfographic.dynamic.error', String(e?.message || e));
    // [KT:SURGICAL:FALLBACK] Preserve original static behavior if AI step fails
    execInfographic = executiveSummaryInfographicHTML({
     roi: summaryMetrics?.roi || canon.roi || 'n/a',
    payback: summaryMetrics?.payback
      || (canon.paybackMonths ? `${canon.paybackMonths} ${canon.paybackUnit || 'months'}` : 'n/a'),
    savings: summaryMetrics?.savings || canon.savings || 'n/a',
    timeline: summaryMetrics?.timeline || canon.timeline || canon.timeFrame || 'n/a',
    confidence: summaryMetrics?.confidence || canon.confidence || 'n/a',
    impact: summaryMetrics?.impact || canon.impact || 'n/a',
    quickWins: Number.isFinite(summaryMetrics?.quickWins)
      ? summaryMetrics.quickWins
      : (Number.isFinite(canon.quickWins) ? canon.quickWins : 0),
    phases: Number.isFinite(summaryMetrics?.phases)
      ? summaryMetrics.phases
      : (phases?.length || Number(canon.phases) || 0),
    });
  }*/
...
try {
  summaryMetrics = await deriveExecSummaryMetrics(canon, sections, phases);
  execInfographic = executiveSummaryInfographicHTML(summaryMetrics);
} catch (e) {
  console.log(TAG, 'execInfographic.dynamic.error', String(e?.message || e));

  // [KT:SURGICAL:FALLBACK] No hard-coded demo numbers, only canon + whatever we already derived
  execInfographic = executiveSummaryInfographicHTML({
    roi:
      (summaryMetrics && summaryMetrics.roi)
      || canon.roi
      || 'n/a',
    payback:
      (summaryMetrics && summaryMetrics.payback)
      || (canon.paybackMonths
        ? `${canon.paybackMonths} ${canon.paybackUnit || 'months'}`
        : 'n/a'),
    savings:
      (summaryMetrics && summaryMetrics.savings)
      || canon.savings
      || 'n/a',
    timeline:
      (summaryMetrics && summaryMetrics.timeline)
      || canon.timeline
      || canon.timeFrame
      || 'n/a',
    confidence:
      (summaryMetrics && summaryMetrics.confidence)
      || canon.confidence
      || 'n/a',
    impact:
      (summaryMetrics && summaryMetrics.impact)
      || canon.impact
      || 'n/a',
    quickWins: Number.isFinite(summaryMetrics?.quickWins)
      ? summaryMetrics.quickWins
  ...
  const visualTimeline = visualTimelineHTML(timelinePhases);*/
    // 2. Visual Timeline - convert phases to timeline format (dynamic + AI-derived)
  // [KT:LEGACY:TIMELINE] Original fixed mapping kept for audit:
  /*
  const timelinePhases = (phases || []).map((p, i) => ({
    name: p.title || `Phase ${i + 1}`,
    startWeek: i * 6,
  ...
  */
  ...
  /*let visualTimeline = '';
  try {
    const timelinePhases = await deriveTimelinePhases(canon, sections, phases);
    visualTimeline = visualTimelineHTML(timelinePhases);
  } catch (e)*//* {
    console.log(TAG, 'visualTimeline.dynamic.error', String(e?.message || e));
    // [KT:SURGICAL:FALLBACK] Preserve original fixed behavior on error
    const fallbackTimelinePhases = (phases || []).map((p, i) => ({
      name: p.title || `Phase ${i + 1}`,
      startWeek: i * 6,
      endWeek: (i + 1) * 6,
      color: ['#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EF4444'][i % 5],
    ...
    visualTimeline = visualTimelineHTML(fallbackTimelinePhases);
  }*/

    // [KT:SURGICAL:TIMELINE-DERIVE] Derive visual timeline phases dynamically from report content
async function deriveTimelinePhases(canon, sections, phases) {
  const basePhases = (phases || []).map((p, i) => ({
    name: p.title || `Phase ${i + 1}`,
    caption: p.caption || '',
    index: i,
  }));
  ...
  if (!basePhases.length) {
    console.log(TAG, 'timeline.basePhases.empty');
    return [];
  }

  // Approximate total weeks from canon, then allocate across phases
  const parsedWeeks = parseTimeFrameToWeeks(canon);
  const totalWeeks = parsedWeeks && parsedWeeks > 0
    ? parsedWeeks
    : basePhases.length * 6; // last-resort structural fallback

  const segment = Math.max(4, Math.floor(totalWeeks / basePhases.length));

  const structuralDefaults = basePhases.map((bp, i) => {
    const startWeek = i * segment;
    const endWeek = (i + 1) * segment;
    return {
      name: bp.name,
      startWeek,
      endWeek,
      milestones: [
        {
          week: Math.floor((startWeek + endWeek) / 2),
          label: bp.caption || 'Milestone',
        },
      ],
      deliverables: [bp.caption || 'Deliverables'],
    };
  });
  ...
  const user = `
You are structuring a transformation roadmap timeline for a reform program.

Use:
- CANON (organization + overall timeframe)
- REPORT SECTIONS (HTML snippets)
- BASE_PHASES (titles and order)
to create a concise phase-based implementation timeline.
...

{
  "phases": [
    {
      "name": "Phase 1 – Stabilize core operations",
      "startWeek": 0,
      "endWeek": 6,
      "milestones": [
        { "week": 3, "label": "Baseline diagnostic complete" }
      ],
      "deliverables": [
        "Diagnostic findings documented",
        "Priority issues agreed with leadership"
      ]
    },
    ...
  ]
...
Rules:
- Number of items in "phases" MUST equal BASE_PHASES length (${basePhases.length}).
- Preserve the order of BASE_PHASES; you may refine names, but keep intent.
- startWeek and endWeek are integers (weeks), non-negative, and strictly increasing.
- Each phase must have at least ONE milestone and ONE deliverable.
- Milestone weeks must fall between startWeek and endWeek (inclusive).
- Use report content and canon timeframe to shape durations, milestones, and deliverables.
- If content is vague, fall back to structuralDefaults.
${languageInstruction}
...
BASE_PHASES:
${JSON.stringify(basePhases, null, 2)}
...
`.trim();
  ...
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            'Return ONLY valid JSON with property "phases". No prose, no markdown, no comments.',
        },
        { role: 'user', content: user },
      ],
    });
    ...
    try {
      ...
      console.log(TAG, 'timeline.json.parse.error', String(e?.message || e));
      obj = {};
    }

    let out = Array.isArray(obj.phases) ? obj.phases : [];

    if (out.length !== structuralDefaults.length) {
      console.log(TAG, 'timeline.length.mismatch', {
        expected: structuralDefaults.length,
        got: out.length,
      });
      out = structuralDefaults;
    }

    const colors = ['#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EF4444'];

    const sanitized = out.map((p, i) => {
      const d = structuralDefaults[i];

      let startWeek = Number.isFinite(+p.startWeek) ? +p.startWeek : d.startWeek;
      let endWeek = Number.isFinite(+p.endWeek) ? +p.endWeek : d.endWeek;
      if (endWeek <= startWeek) {
        endWeek = startWeek + segment;
      }
      ...
      const safeMilestones = milestones.map((m) => {
        let w = Number.isFinite(+m.week) ? +m.week : Math.floor((startWeek + endWeek) / 2);
        if (w < startWeek) w = startWeek;
        if (w > endWeek) w = Math.floor((startWeek + endWeek) / 2);
        return {
          week: w,
          label: String(m.label || 'Milestone').trim(),
        };
      });

      const deliverables = Array.isArray(p.deliverables) && p.deliverables.length
        ? p.deliverables.map((d) => String(d || '').trim()).filter(Boolean)
        : d.deliverables;

      return {
        name: String(p.name || d.name).trim(),
        startWeek,
        endWeek,
    ...
    console.log(TAG, 'timeline.derived', sanitized);
    return sanitized;
  } catch (e) {
    console.log(TAG, 'timeline.derive.error', String(e?.message || e));
...
const visualTimeline = visualTimelineHTML(timelinePhases);
  ...
  let riskHeatmap = '';
  try {
    // [KT:LEGACY:RISK-HEATMAP] original minimal mapping
    /*
    const raidRisks = implKit?.raid?.items?.filter(r => r.type === 'Risk') || [];
    if (raidRisks.length > 0) {
      const risks = raidRisks.slice(0, 10).map((r, i) => ({
        id: i + 1,
        name: r.item || r.name || `Risk ${i + 1}`,
        severity: r.severity || (r.impact === 'High' ? 4 : r.impact === 'Medium' ? 3 : 2),
  ...
  // [KT:SURGICAL:WORLD-CLASS-VISUALS]
const worldClassVisuals = await prepareWorldClassVisuals({
  canon,
  SEC,
  lang: canon.lang,
  labels: L,
...
async function prepareWorldClassVisuals(opts) {
  const { canon, SEC, lang, labels: L, summaryMetrics } = opts || {};
  // use L in riskHeatmapHTML / waterfallChartHTML instead of referencing a global
}
...
try {
  ...
  html =
    ...
    conclusionHTML;
  */
  // [KT:SURGICAL:APPENDICES-MERGE] preserve template appendices, then append diagnostics + conclusion
  // [KT:LEGACY:APPENDICES-MERGE] keep for audit only
  /*
  {
    const re = /(<section id=["']appendices["'][\s\S]*?<\/section>)/i;
    if (re.test(html)) {
      html = String(html).replace(re, (m) =>
        appendicesHTML ? `${m}\n${appendicesHTML}` : m
      );
      html = `${html}\n${conclusionHTML}`;
    } else {
  ...
  */
  ...
  // [KT:SURGICAL:APPENDICES-MERGE:DISABLE-DIAGNOSTICS]
  /* [KT:SURGICAL:APPENDIX-ORDER-FIX] Kept logic commented out. Conclusion is now rendered before appendices. */
  // {
  //   const re = /(<section id=["']appendices["'][\s\S]*?<\/section>)/i;
  //   if (re.test(html)) {
  //     // Keep the template-built appendices as-is and just append the conclusion.
  //     html = String(html).replace(re, (m) => m);
  //     html = `${html}\n${conclusionHTML}`;
  //   } else {
  //     // No appendices section found – just tack the conclusion at the end.
  //     html = `${html || ''}\n${conclusionHTML}`;
  //   }
  // }
...
// === end injection point ===
 ...
 // }
  ...
  if (_useAITitles && typeof injectTableTitlesAI === 'function') {
    ...
    console.log('[SR:REFORM] table.titles.ai.injected', { lang: canon?.lang || 'English' })
  } else if (typeof injectTableTitles === 'function') {
    // Non-English (or no AI fn): deterministic only, no language drift
    html = injectTableTitles(html);
    html = fixUntitledTables(html, _langForTitles);
    html = injectTableTitles(html);
    html = fixUntitledTables(html, _langForTitles);
    console.log('[SR:REFORM] table.titles.injected', {
      mode: 'deterministic',
      lang: _langForTitles,
      reason: _useAITitles ? 'no-ai-fn' : 'non-english'
    });
  } else {
    // [KT:SURGICAL:TABLE-TITLES-AWAIT-FIX]
    // If we are in a non-async context, we cannot use await.
    // Fallback: inject deterministic titles only, log that AI titles are skipped.
    // Await is not allowed in non-async context, so just call injectTableTitles synchronously.
    html = injectTableTitles(html);
    html = fixUntitledTables(html, _langForTitles);
    html = injectTableTitles(html);
    html = fixUntitledTables(html, _langForTitles);
    console.log('[SR:REFORM] table.titles.injected', {
      ...
      lang: _langForTitles,
      reason: 'await not allowed in non-async context'
    });
...
// === [KT:SURGICAL GRAPH NOTES] ===
// Fix #2: Ensure EVERY visual has an executive note below it (charts, benchmarks, heatmaps).
//try {
 // html = await injectVisualDescriptions(html, canon);
//} catch (e) {
  //console.log(TAG, 'chart.notes.invoke.error', String(e?.message || e));
//}
  ...
  //if (canon.lang && !/^english$/i.test(canon.lang)) {
    //console.log(TAG, 'translation.starting', { lang: canon.lang, htmlSize: html.length });
    //html = await translateHTML(html, canon.lang);
    //console.log(TAG, 'translation.complete', { lang: canon.lang, translatedSize: html.length });
  //} else {
   // console.log(TAG, 'translation.skipped', { reason: 'English or no language specified' });
  //}
  ...
  try {
    if (canon.lang && !/^english$/i.test(canon.lang)) {
      ...
      console.log(TAG, 'translation.complete', { lang: canon.lang, translatedSize: translatedHtml.length });
    } else {
      console.log(TAG, 'translation.skipped', { reason: 'English or no language specified' });
    }
  } catch (e) {
    console.error('[AUDIT] [KT:SURGICAL:TRANSLATION-GPT5NANO] Translation error:', String(e?.message || e));
  ...
  return NextResponse.json({ ok: true, html: finalizedHtml, wordCount: sectionsWords, htmlWordCount: htmlWords });
}
// [KT:SURGICAL:UNTITLED-TABLE-FIX:GLOBAL-COPY]
// Global helper so POST() can safely call fixUntitledTables without scope issues.
// The original version remains inside __kt_buildAppendices_FromImplementationKit
// to preserve historical behavior there.
function fixUntitledTables(html, lang) {
  const langPrefix =
    lang && typeof lang === 'string' ? `[${lang.toUpperCase()}] ` : '';
  const titlePrefix = `${langPrefix}Table: `;

  let replacements = 0;

  const result = String(html || '').replace(
    /(<p>\s*\(Untitled Table\)\s*<\/p>\s*)(<table[\s\S]*?<\/table>)/g,
    (_, untitledPara, tableHtml) => {
      const titleHtml = `<p style="font-weight:bold; color:#FFFFFF;">${titlePrefix}(auto-generated)</p>`;
      replacements += 1;
      // We deliberately DROP the "(Untitled Table)" paragraph
      return titleHtml + tableHtml;
    },
  );

  if (replacements > 0) {
    console.log('[SR:REFORM] table.untitled.fixed', {
      lang,
      replacements,
    });
  }
...
// Example JSON schema for appendices (for reference only):
/*
{
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: 'system', 
          content: `You are an implementation consultant. Generate detailed resource requirements. Return ONLY valid JSON.${languageInstruction}` 
        },
        { 
          role: 'user', 
          content: `Create resource requirements for ${canon.orgName || 'the client'} to execute this ${canon.timeFrame || '24 month'} transformation.

Industry: ${canon.industry || 'N/A'}
Company Size: ${canon.companySize || 'N/A'}
Budget: Targeting ${canon.costSavingsGoal ? '$' + (canon.costSavingsGoal/1000000).toFixed(1) + 'M' : 'significant'} value

Generate:
1. 6 core team roles with FTE%, duration, responsibilities
2. Budget breakdown with 4-6 categories and amounts
3. 5-6 essential tools/software
...
Return JSON:
{
  ...
  "tools": [
    ...
    const parsed = JSON.parse(raw);
    console.log(TAG, 'resourceRequirements.generated', { 
      team: parsed?.team?.length || 0,
      budget: parsed?.budget?.length || 0,
      tools: parsed?.tools?.length || 0
    });
    return parsed;
  } catch (err) {
    console.error(TAG, 'resourceRequirements.error', String(err?.message || err));
    return { team: [], budget: [], tools: [] };
  }
}
...
async function getOpenAI(){ // ← must be async because we await import()
  if (!__openai) {
    const { OpenAI } = await import('openai');
...
/* ORIGINAL (commented) */
// const openai = new OpenAI({ apiKey: process.en
// --- END OCT-10-5:15 SOURCE EXCERPT ---

export async function __kt_buildAppendices_FromImplementationKit(reportHTML, canon, sections){
    // [KT:SURGICAL:APPENDIX-ENHANCEMENT] Strictly additive: legacy code below is commented, new logic is additive only.
    /*
    // Legacy appendix logic (commented out, not deleted)
    try{
      ...
      // ...existing code for HTML assembly and embedding...
    }catch(e){ 
    ...
    */
    ...
    //try {
      ...
      //if (decisionFramework && Array.isArray(decisionFramework.scenarios)) {
        //implKITHTML.J = decisionFramework.scenarios;
        //console.log('[APPENDIX-J] ✔️ Data found: scenarios =', decisionFramework.scenarios.length);
      //} else {
          //const scenariosOrItems = Array.isArray(model?.scenarios)
    //? model.scenarios
    //: Array.isArray(model?.items)
     // ? model.items
//: [];

  //if (!scenariosOrItems.length) {
    //console.log('[APPENDIX-J] Not added: no scenarios/items array', { model });
  //} else {
   // console.log(
     // '[APPENDIX-J] ✔️ Data found: scenarios/items = %d',
     // scenariosOrItems.length
    //);

       // console.warn('[APPENDIX-J] ❌ decisionFramework.scenarios missing or not array', decisionFramework);
     // }
    //} catch (e) {
     // console.warn('[APPENDIX-J] ❌ Error generating decision framework:', e);
    //}
    // [KT:APPENDIX-J:NEW-BLOCK]
try {
  decisionFramework = await genDecisionFramework(canon, sections);
...
// Second-pass fixer: replace "(Untitled Table)" captions with AI titles
// inferred from the first header row of the following <table>.
/*function fixUntitledTables(html, lang) {
  ...
  const result = html.replace(
    /<p[^>]*>\s*\(Untitled Table\)\s*<\/p>\s*(<table[\s\S]*?<\/table>)/gi,
    (match, tableHtml) => {
      // Try to read the first header row (th or first tr)
      const rowMatch = tableHtml.match(
        /<tr[^>]*>\s*<t[dh][^>]*>([\s\S]*?)<\/t[dh]>\s*<t[dh][^>]*>([\s\S]*?)<\/t[dh]>[\s\S]*?<\/tr>/i,
      );

      // Default, language-aware fallback title
      let title =
        (lang || "").toLowerCase().startsWith("it")
          ? "Sintesi degli indicatori chiave"
      ...
      if (rowMatch) {
        ...
        const col1 = clean(rowMatch[1]);
        const col2 = clean(rowMatch[2]);

        if (col1 && col2) {
          // Example: "Indicatore vs Valore" / "Indicator vs Value"
          title = `${col1} vs ${col2}`;
        } else if (col1) {
          title = col1;
        }
