...
// [KT:SURGICAL DEDUPE + GUARDS] — add-only
const VISUAL_PLAN = true; // guard for legacy references
...
function __kt_sigFromFrag(frag='') {
  try {
    ...
    if (m) {
      ...
      return `chart|${(obj.title||'').toLowerCase().trim()}|${(obj.type||'').toLowerCase().trim()}`;
    }
    // benchmarks
...
function __kt_dedupeVisuals(html='') {
  try {
    ...
    let out = '', last = 0, m, kept = 0, dropped=0;
    const seen = new Set();
...
async function injectTableTitles(html, canon){
  try{
    ...
    // [KT:SURGICAL DEDUPE] remove repeated visuals by title/kind
  try {
