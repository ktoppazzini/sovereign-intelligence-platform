...
function attachChartTypeControls(html=''){
  ...
  out = out.replace(/(<figure\b[^>]*data-chart=(["'])([\s\S]*?)\2[^>]*>)(?![\s\S]*?data-kt-type)/ig,
    (m, openTag)=>{
      const ctl = '<div class="chart-type-ctl" data-kt-typectl><label>Type:</label><select data-kt-type><option value="line">Line</option><option value="bar">Bar</option><option value="area">Area</option></select></div>';
...
(function(){
  ...
  function parseSpec(el){
    try{ return JSON.parse(el.getAttribute('data-chart')||'{}'); }catch{return null;}
  }
  function render(fig){
  ...
  function init(){
    document.querySelectorAll('figure[data-chart]').forEach(function(fig){
      ...
      if(!sel){
        var d=document.createElement('div'); d.className='chart-type-ctl'; d.innerHTML='<label>Type:</label><select data-kt-type><option value="line">Line</option><option value="bar">Bar</option><option value="area">Area</option></select>';
...
if (typeof html === "string" && /<head>/i.test(html)) { html = html.replace(/<\/head>/i, __KT_AESTHETIC_PLUS + "</head>"); }
...
if (typeof html === "string" && /<head>/i.test(html)) {
  html = html.replace(/<\/head>/i, __KT_AESTHETIC_PLUS + "</head>");
// [KT-PATCH] commented out to fix unterminated/stray HTML: 
} else {
  html = __KT_MARGIN_CSS_2 + html; // [KT-PATCH] replaced; original below commented
