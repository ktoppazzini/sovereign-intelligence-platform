...
// [KT:COMMENTED-OUT] This block led the section writer to inject generic charts/benchmarks.
// const DATA_BACKED_VIZ_GUIDANCE = ` ... `;

// [KT:NO-VISUALS] replace guidance: the section writer must NOT emit visuals.
// Predictive charts are injected by a separate, controlled step.
...
- "type" in ["line","bar","pie","doughnut"]  // [KT:BLOCK] no "benchmark"
- "title" (clear and specific; if omitted, the system synthesizes),
...
// Render candidate to fragment
function renderCandidateToFragment(section, c) {
  // [KT:BLOCKED] predictive benchmark renderer
  /*
...
Paragraph discipline: 3–4 sentences per paragraph (≈4–6 lines). Split any longer blocks.

When you include a table, ALWAYS render it as <table class="report-table">...</table> so styles apply.
${NO_VISUALS_GUIDANCE}  <!-- [KT:REPLACED] previously injected DATA_BACKED_VIZ_GUIDANCE -->
