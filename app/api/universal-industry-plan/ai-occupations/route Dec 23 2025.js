...
// ============================================================================
// Translate Occupations to Target Language
// ============================================================================
async function translateOccupations(occupations, targetLang) {
  ...
  try {
    ...
    // Translate categories (major groups)
    const catPrompt = `Translate these ISCO-08 occupation category names to ${targetLang}. Keep the number code prefix (e.g., "1 - ", "2 - "). Return as JSON array in same order.
...
Return JSON: { "translations": ["translated name 1", "translated name 2", ...] }`;
    ...
    // Translate sub-occupations in batches (to avoid token limits)
    const BATCH_SIZE = 60;
    const translatedSubOccupations = [];
    
    for (let i = 0; i < allSubOccupations.length; i += BATCH_SIZE) {
      const batch = allSubOccupations.slice(i, i + BATCH_SIZE);
      
      const subPrompt = `Translate these ISCO-08 occupation names to ${targetLang}. Keep numeric codes (e.g., "11 - ", "111 - ", "1111 - "). Return as JSON array in same order.
...
Return JSON: { "translations": ["translated 1", "translated 2", ...] }`;
      ...
      translatedSubOccupations.push(...(subParsed.translations || batch));
    }
