// app/api/universal-industry-plan/ai-occupations/route.js
// [KT:AI-OCCUPATIONS-v1.0] ISO ISCO-08 Occupation Classification
// Based on ILO ISCO-08 (International Standard Classification of Occupations)
...
const TAG = '[SR:AI-OCCUPATIONS]';

// [KT:UTF8-SANITIZER] Fix encoding issues in AI JSON responses
function sanitizeJSONForParsing(raw) {
  if (!raw || typeof raw !== 'string') return '';
  
  let jsonStr = raw;
  
  // Remove embedded newlines that break JSON
  ...
  // Fix UTF-8 corruption patterns (German, French, etc.)
  const utf8Fixes = [
    [/├⌐/g, 'é'], [/├Â/g, 'ê'], [/├ç/g, 'è'], [/├á/g, 'à'], [/├â/g, 'â'],
    [/├╝/g, 'ú'], [/├╖/g, 'ö'], [/├½/g, 'ü'], [/├ª/g, 'ª'], [/├ñ/g, 'ä'],
    [/├ó/g, 'ó'], [/├│/g, 'í'], [/├ü/g, 'ü'], [/├À/g, 'À'], [/├È/g, 'È'],
    [/├Ñ/g, 'ø'], [/├å/g, 'å'], [/├Æ/g, 'Æ'], [/├ß/g, 'ß'],
    [/Γî/g, 'î'], [/Γò/g, 'ò'], [/Γñ/g, 'ñ'], [/Γü/g, 'ü'],
  ];
...
// Cache for occupations
let __occupationsCache = null;
let __occupationsCacheTime = 0;
const CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days

// ============================================================================
// ISCO-08 - International Standard Classification of Occupations
// ILO (International Labour Organization) - Complete 10 Major Groups
// ============================================================================
const ISCO_OCCUPATIONS = {
  '1 - Managers': {
    icon: '👔',
    code: '1',
    level: 'strategic',
    reportFocus: ['strategy', 'governance', 'risk', 'investment', 'stakeholders'],
    subOccupations: [
  ...
  '2 - Professionals': {
    ...
    code: '2',
    level: 'expert',
    reportFocus: ['technical-depth', 'best-practices', 'innovation', 'methodology', 'standards'],
    subOccupations: [
  ...
  '3 - Technicians and Associate Professionals': {
    ...
    code: '3',
    level: 'tactical',
    reportFocus: ['implementation', 'processes', 'quality', 'compliance', 'operations'],
    subOccupations: [
  ...
  '4 - Clerical Support Workers': {
    ...
    code: '4',
    level: 'operational',
    reportFocus: ['efficiency', 'workflows', 'documentation', 'data-entry', 'coordination'],
    subOccupations: [
  ...
  '5 - Service and Sales Workers': {
    ...
    code: '5',
    level: 'frontline',
    reportFocus: ['customer-experience', 'service-delivery', 'sales', 'satisfaction', 'retention'],
    subOccupations: [
  ...
  '6 - Skilled Agricultural, Forestry and Fishery Workers': {
    ...
    code: '6',
    level: 'specialized',
    reportFocus: ['sustainability', 'yield', 'resource-management', 'seasonal-planning', 'equipment'],
    subOccupations: [
  ...
  '7 - Craft and Related Trades Workers': {
    ...
    code: '7',
    level: 'skilled-trade',
    reportFocus: ['craftsmanship', 'safety', 'tools', 'materials', 'apprenticeship'],
    subOccupations: [
      ...
      '7119 - Building Frame and Related Trades Workers NEC',
  ...
  '8 - Plant and Machine Operators and Assemblers': {
    ...
    code: '8',
    level: 'operational',
    reportFocus: ['productivity', 'safety', 'maintenance', 'throughput', 'quality-control'],
    subOccupations: [
  ...
  '9 - Elementary Occupations': {
    ...
    code: '9',
    level: 'entry',
    reportFocus: ['basic-tasks', 'physical-work', 'supervision', 'safety', 'onboarding'],
    subOccupations: [
  ...
  '0 - Armed Forces Occupations': {
    ...
    code: '0',
    level: 'military',
    reportFocus: ['defense', 'security', 'logistics', 'chain-of-command', 'readiness'],
    subOccupations: [
...
// ============================================================================
// [KT:SOVEREIGN-AI] Self-Learning AI Setup for Translation
// ============================================================================
...
async function getOpenAI() {
  ...
  // Return Sovereign AI compatible wrapper
  __openaiClient = {
    chat: {
      completions: {
        create: async (params) => {
          ...
          console.log(TAG, 'Calling Sovereign AI for occupation translation');
          const result = await SovereignAI.call({
            prompt: userPrompt,
            systemPrompt,
            callType: 'occupation-translation',
            vertical: 'universal-industry-plan',
            expectedFormat: params.response_format?.type === 'json_object' ? 'json' : 'text',
            maxTokens: params.max_completion_tokens || params.max_tokens || 16000,
          });
          
          return {
            choices: [{
              message: { content: result.response || result.content || '', role: 'assistant' },
              finish_reason: 'stop'
            }],
...
// ============================================================================
// Translate Occupations to Target Language (OPTIMIZED - Parallel Processing)
// ============================================================================
async function translateOccupations(occupations, targetLang) {
  const cacheKey = `occupations_${targetLang}`;
  ...
  try {
    ...
    const categoryNames = Object.keys(occupations);
    const allSubOccupations = [];
    categoryNames.forEach(cat => {
      occupations[cat].subOccupations.forEach(sub => allSubOccupations.push(sub));
    });

    console.log(TAG, 'translation.start', { lang: targetLang, categories: categoryNames.length, subOccupations: allSubOccupations.length });
...
Return: { "translations": ["1 - Name", ...] }`;
    ...
    translationPromises.push(
      openai.chat.completions.create({
        ...
        messages: [
          { role: 'system', content: `You are a professional translator for ${targetLang}. ALWAYS return valid JSON. NEVER ask clarifying questions. NEVER provide explanations. If uncertain, use best judgment immediately.` },
          { role: 'user', content: catPrompt }
        ],
      }).then(res => {
        const rawContent = res.choices[0].message.content || '{}';
        
        // [KT:FIX-ALL-LANGUAGES] Extract JSON from any conversational wrapper
        let jsonText = rawContent;
        const jsonMatch = rawContent.match(/\{[\s\S]*"translations"[\s\S]*\}/); 
        if (jsonMatch) {
          jsonText = jsonMatch[0];
        }
        
        const sanitized = sanitizeJSONForParsing(jsonText);
        try {
          ...
          return { type: 'categories', data: parsed };
        } catch (e) {
          console.error(TAG, '[JSON-PARSE] Categories failed for', targetLang, ':', e.message, 'Response:', rawContent.substring(0, 200));
          return { type: 'categories', data: { translations: categoryNames } };
        }
      })
    );
    ...
    for (let i = 0; i < allSubOccupations.length; i += BATCH_SIZE) {
      const batch = allSubOccupations.slice(i, i + BATCH_SIZE);
...
Return: { "translations": [...] }`;
      ...
      translationPromises.push(
        openai.chat.completions.create({
          ...
          response_format: { type: 'json_object' },
          messages: [
            { role: 'system', content: `You are a professional translator for ${targetLang}. ALWAYS return valid JSON immediately. NEVER ask questions. NEVER provide explanations outside JSON. If uncertain about any term, use best professional judgment and proceed.` },
            { role: 'user', content: subPrompt }
          ],
        }).then(res => {
          const rawContent = res.choices[0].message.content || '{}';
          
          // [KT:FIX-DYNAMIC-LANGUAGE] Extract JSON from any language response
          let jsonText = rawContent;
          const jsonMatch = rawContent.match(/\{[\s\S]*"translations"[\s\S]*\}/);
          if (jsonMatch) {
            jsonText = jsonMatch[0];
          ...
          try {
            const parsed = JSON.parse(sanitized);
            if (!parsed.translations || !Array.isArray(parsed.translations)) {
              console.warn(TAG, `[FALLBACK] Invalid structure for ${targetLang}, using fallback`);
              return { type: 'batch', batchIdx, data: { translations: batch }, fallback: batch };
            }
            return { type: 'batch', batchIdx, data: parsed, fallback: batch };
          } catch (e) {
            console.error(TAG, '[JSON-PARSE] Batch', batchIdx, 'failed for', targetLang, ':', e.message, 'Response preview:', rawContent.substring(0, 200));
            return { type: 'batch', batchIdx, data: { translations: batch }, fallback: batch };
          }
        })
      );
    ...
    console.log(TAG, 'translation.parallel', { totalCalls: translationPromises.length });
    const results = await Promise.all(translationPromises);

    // Process results
    const catResult = results.find(r => r.type === 'categories');
    const translatedCategories = catResult?.data?.translations || categoryNames;
    
    // Rebuild sub-occupations in order
    const translatedSubOccupations = new Array(allSubOccupations.length);
    results.filter(r => r.type === 'batch').forEach(r => {
      const translations = r.data?.translations || r.fallback;
      const { start, end } = batchIndices[r.batchIdx];
      translations.forEach((t, idx) => {
        translatedSubOccupations[start + idx] = t;
      });
    });
    ...
    categoryNames.forEach((origCat, catIdx) => {
      ...
      const origData = occupations[origCat];
      const subCount = origData.subOccupations.length;
      
      translated[newCatName] = {
        icon: origData.icon,
        code: origData.code,
        level: origData.level,
        reportFocus: origData.reportFocus,
        subOccupations: translatedSubOccupations.slice(subIdx, subIdx + subCount),
      };
      subIdx += subCount;
    });

    __translationCache.set(cacheKey, translated);
    console.log(TAG, 'translation.done', { lang: targetLang, timeMs: Date.now() - startTime });
    
    return translated;
  } catch (e) {
    console.error(TAG, 'translation.error', e.message);
    return occupations; // Fallback to English
  }
}
...
export async function GET(req) {
  ...
  console.log(TAG, 'GET', { lang, source: 'ISCO-08' });
  
  // For English, use cache directly
  if (isEnglish(lang)) {
    if (__occupationsCache && Date.now() - __occupationsCacheTime < CACHE_TTL) {
      ...
      return NextResponse.json({
        ...
        standard: 'ISCO-08',
        lang: 'English',
        occupations: __occupationsCache,
        majorGroupCount: Object.keys(__occupationsCache).length,
        totalOccupations: Object.values(__occupationsCache).reduce((sum, cat) => sum + cat.subOccupations.length, 0),
      });
    }
    
    __occupationsCache = ISCO_OCCUPATIONS;
    __occupationsCacheTime = Date.now();
    ...
    return NextResponse.json({
      ...
      standard: 'ISCO-08',
      lang: 'English',
      occupations: ISCO_OCCUPATIONS,
      majorGroupCount: Object.keys(ISCO_OCCUPATIONS).length,
      totalOccupations: Object.values(ISCO_OCCUPATIONS).reduce((sum, cat) => sum + cat.subOccupations.length, 0),
    });
  }
  
  // For non-English, translate
  try {
    const translated = await translateOccupations(ISCO_OCCUPATIONS, lang);
    ...
    return NextResponse.json({
      ...
      majorGroupCount: Object.keys(translated).length,
      totalOccupations: Object.values(translated).reduce((sum, cat) => sum + cat.subOccupations.length, 0),
    });
