...
// ---------- Version ----------
const CLIENT_VERSION = 'users-ui v1.0.5';
...
// **From** must come from Users → lookup label: "Roles (from Role)"
function extractRoleFromUserRecord(rec) {
  const f = rec?.fields || rec || {};
  const v =
    f[USERS_ROLE_LOOKUP_FIELD] || // primary (lookup)
    rec[USERS_ROLE_LOOKUP_FIELD] || // sometimes not nested
    f.Role || // very soft fallbacks
    rec.Role;
...
// Build dropdown from Roles table → Roles field **options**.
// 1) Prefer schema.fields[].options.choices for ROLES_FIELD_NAME
// 2) Fallback: scan roles records and take fields[ROLES_FIELD_NAME] only
function choicesFromRolesResponse(payload, fieldName = ROLES_FIELD_NAME) {
  const out = new Set();

  // Preferred: schema choices
  ...
  // Fallback: records’ fields[fieldName] only (ignore names/labels/recIDs)
  if (!out.size) {
    const arrays = [payload?.records, payload?.data].filter(Array.isArray);
    for (const arr of arrays) {
      for (const r of arr) {
        const f = r?.fields || {};
        const val = f[fieldName];
        if (typeof val === 'string' && val.trim() && !REC_ID_RE.test(val)) out.add(val.trim());
      }
    }
  }
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    const labels = { ...t, title: 'Users' }; // stable base
    ...
    try {
      ...
      if (!ok || !json?.ok) {
        ...
        const normalized = (json.records || []).map((rec) => ({
          id: rec.id,
          // Name/Email: support both fielded and top-level shapes
          name: rec?.fields?.Name ?? rec?.Name ?? '',
          email: rec?.fields?.Email ?? rec?.Email ?? '',
  ...
  // ---------- Load roles options (schema → fallback) ----------
  async function loadRoles() {
    ...
    try {
      ...
      if (!list.length) throw new Error(`No options found for Roles → “${ROLES_FIELD_NAME}”`);
