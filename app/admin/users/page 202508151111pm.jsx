...
// v1.0.14-role-post-guard — force mode, include roleRecId, lowercase emails; louder logs

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.14-role-post-guard';
...
export default function AdminUsersPage() {
  ...
  // Helper: find a role record id by the visible label/value we show in the dropdown
  function findRoleRecIdByChoice(choice) {
    if (!choice) return null;
    const match = roles.find((r) => r.label === choice || r.value === choice || r.id === choice);
  ...
  async function callUsers(body) {
    // Normalize email to lowercase before POST
    const send = { ...body };
    if (send.email) send.email = String(send.email).trim().toLowerCase();

    console.log('➡️ POST /api/admin/users body:', send);
  ...
  async function saveRoleModal() {
    ...
    // Force a mode so the backend always gets one
    const finalMode = roleModal.mode || (roleModal.from ? 'change' : 'assign');

    // Map visible “to” value to a Roles record id to help the backend
    const roleRecId = findRoleRecIdByChoice(roleModal.to);

    console.log('📝 saveRoleModal ->', {
    ...
    try {
      await callUsers({
        email: roleModal.email,
        role: roleModal.to, // pass the role NAME for readability
        roleRecId, // pass ROLE RECORD ID to help the route update the linked field
        from: roleModal.from || undefined,
        mode: finalMode,
        _client: CLIENT_VERSION,
      });
      await loadUsers();
