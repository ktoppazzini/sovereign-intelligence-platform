...
// v1.0.15-roles-linkedi18n-fix — keep stable UI/logic, add i18n via /api/gptTranslation (expects {prompt})

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.15-roles-linkedi18n-fix';
...
// ---------- Default translation dictionary (English) ----------
const DEFAULT_T = {
  title: 'Users',
  email: 'Email',
  name: 'Name',
...
// ---------- i18n helper that matches /api/gptTranslation route contract ----------
async function fetchI18n(lang, template) {
  // Your route expects { prompt: string }
  const prompt =
    `Translate ONLY the values (not the keys) of this JSON into ${lang}. ` +
    `Return valid JSON with the SAME KEYS and NO extra text:\n` +
  ...
  if (!res.ok) {
    throw new Error(j?.error || j?.message || 'i18n request failed');
  }
  // Route returns { translation: {...} }
  const dict = j.translation || j.result || j.data || j;
  if (!dict || typeof dict !== 'object') {
    throw new Error('i18n response missing translation dictionary');
  }
  return dict;
}
...
export default function AdminUsersPage() {
  useEffect(() => {
    ...
    // Optional: expose template for quick console tests
    try {
      window.__DEFAULT_T = DEFAULT_T;
    } catch {}
  }, []);

  // ---- Translations (initialized to English, then replaced if ?lang= is set) ----
  const [t, setT] = useState(DEFAULT_T);

  // Load i18n once (and whenever ?lang changes)
  useEffect(() => {
    const lang = new URLSearchParams(location.search).get('lang') || 'English';
    (async () => {
      try {
        const translated = await fetchI18n(lang, DEFAULT_T);
        const safe = Object.fromEntries(
          Object.keys(DEFAULT_T).map((k) => [k, translated[k] ?? DEFAULT_T[k]]),
        );
        setT(safe);
      } catch (e) {
        console.warn('🌐 i18n fallback to English:', e?.message || e);
        setT(DEFAULT_T);
      }
    })();
  }, []);
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        ...
        const normalized = (j.records || []).map((rec) => {
          ...
          // Prefer server-provided RoleNames array & RawRole if present
          ...
          // For table display we show the first role, if any
  ...
  async function addUser() {
    ...
    try {
      await callUsers({
        ...
        name: name.trim(),
        role: firstRole, // optional — server maps name→rec id
        orgId: orgId.trim(),
        active: true,
  ...
  async function saveRoleModal() {
    ...
    try {
      await callUsers({
        ...
        role: roleModal.to, // pass role NAME; server will map to the linked record id
        from: roleModal.from,
        mode: roleModal.mode,
        _client: CLIENT_VERSION,
      });
