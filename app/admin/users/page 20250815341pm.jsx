...
// users-ui v1.0.11-roles-formula — From dropdown, Roles options from Roles table, robust role mapping, louder logs

import { useEffect, useMemo, useState } from 'react';

/* ---------------- Version info (persists to localStorage) ---------------- */
const CLIENT_VERSION = 'users-ui v1.0.11-roles-formula';
...
export default function AdminUsersPage() {
  ...
  // ---- Translations (driven by /api/gptTranslation) ----
  ...
  const [roleIds, setRoleIds] = useState(''); // raw input (comma-separated) only used by the add form
  ...
  const [roles, setRoles] = useState([]); // [{id,label,value}]
  const [rolesErr, setRolesErr] = useState('');
  const [rolesLoading, setRolesLoading] = useState(false);

  // firstRole helper for Add User form (if roleIds typed)
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        ...
        const normalized = (j.records || []).map((rec) => {
          // Prefer your new Users.Role (formula/text), then the lookup, then any other aliases
          const roleLabel =
            rec.Role ?? // <-- your new formula field (string)
            rec['Roles (from Role)'] ?? // legacy lookup label
            rec.RoleName ??
            rec.roleName ??
            rec.role_label ??
  ...
  /* ---------------- Load roles options (To) from schema/options ---------------- */
  async function loadRoles() {
    setRolesLoading(true);
    setRolesErr('');
    try {
      // Primary: dedicated endpoint that returns single-select choices from Roles table/field.
      // Your server already supports this (schema/options), but we'll hit ?options=1 which you've seen working.
      ...
      console.log('✅ roles options:', list);
      setRoles(list);
    } catch (e) {
  ...
  async function addUser() {
    ...
    try {
      await callUsers({
        ...
        name: name.trim(),
        role: firstRole, // optional, if you typed a role into the "Role Record IDs" box
        orgId: orgId.trim(),
        active: true,
  ...
  const [roleModal, setRoleModal] = useState({
    open: false,
    email: '',
    from: '', // current role (string)
    to: '',
    mode: 'assign', // 'assign' | 'change'
    fromOptions: [], // dropdown content for "From" (comes from Users.Role for that user)
  });

  function openAssignModal(row) {
    console.log('🪄 openAssignModal row:', row);
    setRoleModal({
      open: true,
      email: row.email,
      from: '', // no "from" in assign
      to: '',
      mode: 'assign',
      fromOptions: [],
    });
  ...
  function openChangeModal(row) {
    console.log('🪄 openChangeModal row:', row);
    const current = (row.role || '').toString().trim();
    setRoleModal({
      open: true,
      email: row.email,
      from: current || '-', // show '-' if blank
      to: '',
      mode: 'change',
      fromOptions: current ? [current] : ['-'],
    });
  }
  ...
  async function saveRoleModal() {
    ...
    // Build a rich body so the existing route can accept any of the common keys
    const body = {
      email: roleModal.email,
      // what we're changing "to"
      role: roleModal.to,
      roleName: roleModal.to,
      newRole: roleModal.to,
      role_label: roleModal.to,
      // additional context
      from: roleModal.from || '',
      mode: roleModal.mode,
      _client: CLIENT_VERSION,
        ...
        {roleModal.open && (
          ...
          >
            ...
            >
              ...
              {/* FROM — dropdown fed from this user's Role (formula). Shows one option if present. */}
              ...
              >
                {roleModal.fromOptions.length === 0 ? (
                  <option value="">-</option>
                ) : (
                  roleModal.fromOptions.map((opt, idx) => (
                    <option key={idx} value={opt} style={{ color: '#000' }}>
                      {opt}
                    </option>
                  ))
                )}
              ...
              {/* TO — Roles table → Roles (single-select) options */}
