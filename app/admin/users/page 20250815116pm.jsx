...
// v1.0.6-fix-roles — roles from schema, better name/email mapping, louder logs

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.6-fix-roles';
...
export default function AdminUsersPage() {
  ...
  // ---- Translations (kept as-is; your /api/gptTranslation drives this) ----
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        ...
        const normalized = (j.records || []).map((rec) => {
          // Name mapping (avoid grabbing the Role accidentally)
          const theName =
          ...
          // Human-role label comes from lookup first: "Roles (from Role)"
          const roleLabel =
            rec['Roles (from Role)'] ??
            rec.RoleName ??
            rec.roleName ??
  ...
  async function loadRoles() {
    ...
    setRolesErr('');
    try {
      // Ask dedicated endpoint for single-select choices
