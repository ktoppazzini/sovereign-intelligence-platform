...
// v1.0.18-i18n-cache-rtl — persist i18n to localStorage, set RTL when needed,
// keep v1.0.16 role-id mapping and layout

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.18-i18n-cache-rtl';
...
function getLangFromURL() {
  try {
    ...
    return normalizeLang((sp.get('lang') || '').trim());
  } catch {
    return '';
  }
}
...
function tCacheKey(lang) {
  return `SI_USERS_T_${(lang || 'English').toLowerCase()}`;
}
...
export default function AdminUsersPage() {
  ...
  // —— i18n loader with cache & RTL (now reuses last language if URL lacks ?lang=)
  async function loadI18n() {
    ...
    try {
      ...
      if (!lang && stored) lang = stored;
    } catch {}
    lang = normalizeLang(lang || 'English');

    // 2) persist and set text direction
    try {
      localStorage.setItem('SI_USERS_LANG', lang);
    } catch {}
    try {
    ...
    if (/^english$/i.test(lang)) {
      console.log('🌐 i18n: English (effective lang =', lang, ')');
      return;
    }

    // 3) apply cached translation immediately
    try {
      const cached = localStorage.getItem(tCacheKey(lang));
      if (cached) {
        const obj = JSON.parse(cached);
        if (obj && typeof obj === 'object') {
          setT((prev) => ({ ...prev, ...obj }));
          console.log('🌐 i18n cache hit for', lang);
        }
      }
    } catch {}

    // 4) fetch fresh translation from /api/gptTranslation
    const base = { ...t }; // send English keys
    const prompt = `Translate the following UI labels for an administrative dashboard into ${lang}.
Keep the JSON keys exactly the same and return ONLY JSON.`;
    ...
    try {
      ...
      // Accept multiple shapes: {translation:{...}} OR the object itself
      const translated =
      ...
      if (res.ok && translated) {
        ...
        try {
          localStorage.setItem(tCacheKey(lang), JSON.stringify(translated));
        } catch {}
        console.log('🌐 i18n applied for', lang);
      } else {
        console.warn('🌐 i18n: response not OK or missing translation. Raw:', raw);
      }
    } catch (e) {
  ...
  useEffect(() => {
    loadI18n(); // <— now also uses last language if URL has none
    loadUsers();
    loadRoles();
  }, []);
        ...
        {showDebug && (
          ...
          >
            {JSON.stringify(
              {
                version: readVersion(),
                urlLang: getLangFromURL(),
                storedLang:
                  typeof localStorage !== 'undefined'
                    ? localStorage.getItem('SI_USERS_LANG')
                    : 'n/a',
                dir:
