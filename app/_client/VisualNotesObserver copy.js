...
// [KT:SURGICAL:AI-NOTE] Ask server endpoint for GPT-5-Nano summary, fallback to statAnalysis
async function aiNote(meta){
  try{
    ...
    if (out?.html) return out.html; // already styled white
  }catch(e){ /* swallow */ }
  // fallback to local analysis if API unavailable
  return statAnalysis({
    title: meta?.title, xTitle: meta?.xTitle, yTitle: meta?.yTitle,
    labels: [], series: []
  });
}
...
export default function VisualNotesObserver() {
  useEffect(() => {
    const effect = async () => {
      ...
      async function ensureNoteForVisual(visualEl, phase) {
...
const noteHTML = await aiNote(meta);
...
container.parentNode?.insertBefore(note, container.nextSibling);
    ...
    let injected = 0;
    await Promise.all(
Array.from(document.querySelectorAll('figure canvas, figure svg, .chart-card canvas, .chart-card svg'))
 .map(async v => { if (await ensureNoteForVisual(v, 'initial')) injected++; })
    );
