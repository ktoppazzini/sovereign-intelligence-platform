...
function ensureControls(fig) {
  ...
  label.setAttribute('for', 'sr-chart-type-select');

  const select = document.createElement('select');
  select.className = 'sr-chart-type';
  select.id = 'sr-chart-type-select';
  select.setAttribute('data-role', 'sr-chart-type');
  // Improved style for consistency and accessibility
  select.style.background = '#181f2a';
  select.style.color = '#e8eefb';
  select.style.border = '1px solid #66e6b0';
...
function hydrateOne(fig) {
  ...
  // [KT:SURGICAL:ATTR-MIRROR] mirror the current type for QA
fig.setAttribute('data-type', select.value);
