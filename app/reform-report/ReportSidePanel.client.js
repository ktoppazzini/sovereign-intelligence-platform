...
(function () {
  ...
  // [KT:LINK-BASED] Finalize first to store HTML in Airtable
  async function finalizeReport(payload) {
    var r = await fetch('/api/reform/finalize', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(payload),
    });
  ...
  var UI_EN = {
    ...
    ridLabel: 'RID:',
    ridFallback: '(generate report to get RID)',
    notesPlaceholder: 'Notes (optional)',
    submitButton: 'Finalize & Submit',
    alertMissingRid: 'Missing RID. Please generate a report first.',
    msgUploadingRevision: 'Uploading revision…',
    msgRevisionUploaded: 'Revision {{rev}} uploaded — requesting approval…',
    msgFinalizing: 'Finalizing report…',
    msgRequestingApproval: 'Sending approval request…',
    msgSubmitted: 'Submitted ✔ Email sent!',
  ...
  elBtn.addEventListener('click', async function () {
    ...
    if (f) {
      ...
      setMsg(UI.msgFinalizing || 'Finalizing report…');
    }
    ...
    try {
      ...
      if (!reportHtml || reportHtml.length < 100) {
        setMsg('Error: No report content found. Please generate a report first.');
        setBusy(false);
        return;
      }
      ...
      // [KT:LINK-BASED] Step 1: Finalize - store HTML in Airtable
      setMsg(UI.msgFinalizing || 'Finalizing report…');
      var finalizePayload = {
        rid: currentRid,
        html: reportHtml,
        lang: lang,
        orgName: '', // Could be retrieved from form if needed
        preparedFor: (elFor.value || '').trim(),
        preparedBy: (elBy.value || '').trim(),
      };
      
      var finalizeResult = await finalizeReport(finalizePayload);
      
      if (!finalizeResult.ok) {
        setMsg('Error finalizing: ' + (finalizeResult.error || 'Unknown error'));
        setBusy(false);
        return;
      }
      
      console.log('[SI:PANEL] Finalized OK, now submitting for approval');
      
      // [KT:LINK-BASED] Step 2: Submit - send approval email with link
      setMsg(UI.msgRequestingApproval || 'Requesting approval…');
      
      // Get approver email - must have preparedForEmail
      var approverEmail = preparedForEmail || elFrom.value;
      if (!approverEmail) {
        setMsg('Error: Missing approver email. Please enter a valid email.');
        setBusy(false);
        return;
      }
      ...
      var submitPayload = {
        ...
        requesterEmail: preparedByEmail || elFrom.value || '',
        notes: (elNote.value || '').trim(),
        
        // If resubmitting after "changes requested"
        changeReqId: changeContext ? changeContext.changeReqId : undefined,
        requesterEmail: changeContext ? changeContext.requesterEmail : (preparedByEmail || elFrom.value || ''),
