...
export default function EnterpriseReportForm({ verticalId, lang, ui, onReportGenerated }) {
  ...
  const [reportingPeriod, setReportingPeriod] = useState('Q4');
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      const payload = { 
        ...
        // Include burning issues context
    ...
    setLoading(false);
  }, [lang, verticalId, reportType, organizationName, preparedBy, reportingPeriod, selectedCategories, selectedRole, selectedRoleId, context, workflowState, approvals, comments, uploadedData, surveyData, interviewData, burningIssues, onReportGenerated]);

  const generateDemoReport = () => {
    const reportTypeLabel = config.reportTypes.find(r => r.id === reportType)?.label || 'Report';
    const categoryAnalysis = selectedCategories.map(catId => {
      const cat = config.categories.find(c => c.id === catId);
      const score = Math.floor(Math.random() * 25) + 70;
      const color = score >= 85 ? '#10b981' : score >= 70 ? '#f59e0b' : '#ef4444';
      return `<div style="background: rgba(0,0,0,0.3); border-radius: 16px; padding: 24px; border-left: 4px solid ${config.color};">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          <h3 style="font-size: 18px; color: #fff; margin: 0;">${cat?.label || catId}</h3>
          <span style="background: ${color}20; color: ${color}; padding: 6px 16px; border-radius: 20px; font-size: 13px; font-weight: 600;">${score}/100</span>
        </div>
        <div style="background: rgba(255,255,255,0.1); height: 8px; border-radius: 4px; overflow: hidden;"><div style="background: ${color}; height: 100%; width: ${score}%;"></div></div>
        <ul style="margin: 16px 0 0 0; padding-left: 20px; color: rgba(255,255,255,0.8);">
          <li style="margin-bottom: 8px;">AI analysis indicates ${score >= 80 ? 'strong' : 'moderate'} performance in this area</li>
          <li style="margin-bottom: 8px;">Automated monitoring detected ${Math.floor(Math.random() * 10) + 1} actionable insights</li>
          <li>Predictive models forecast ${score >= 75 ? 'positive' : 'mixed'} trends for next quarter</li>
        </ul>
      </div>`;
    }).join('');
    ...
    return `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #e8eefb;">
      ...
      <div style="margin-bottom: 48px;">
        ...
        <div style="background: ${config.color}10; border-radius: 16px; padding: 28px; border: 1px solid ${config.color}20;">
          <p style="font-size: 16px; line-height: 1.9; color: rgba(255,255,255,0.85); margin: 0;">
            ...
            covering <strong style="color: ${config.color};">${selectedCategories.length}</strong> key areas for <strong style="color: ${config.color};">${reportingPeriod} 2025</strong>.
      ...
      <div style="margin-bottom: 48px;">
        <h2 style="font-size: 24px; color: ${config.color}; margin-bottom: 24px;">🔍 Category Analysis</h2>
        <div style="display: grid; gap: 20px;">${categoryAnalysis}</div>
      </div>
    ...
    <div style={{ display: 'grid', gap: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 4, background: 'rgba(0,0,0,0.3)', padding: 6, borderRadius: 12, flexWrap: 'wrap' }}>
        {[
          ...
          { id: 'interviews', label: `🎤 Interviews${interviewsCompleted > 0 ? ` (${interviewsCompleted})` : ''}` },
          { id: 'survey', label: `📋 Survey${surveyCompleted ? ' ✓' : ''}` },
          { id: 'data', label: `📊 Data${uploadedData ? ' ✓' : ''}` }, 
          { id: 'progress', label: '📈 Monitor' },
      ...
      {activeTab === 'config' && (
        <>
          <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 16, padding: 24, border: `1px solid ${config.color}20` }}>
            <h3 style={{ color: '#fff', margin: '0 0 20px 0', fontSize: 18 }}>Report Configuration</h3>
            <div style={{ display: 'grid', gap: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                <div>
                  ...
                  <input type="text" value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} placeholder="Enter organization name" style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Prepared By</label>
                  <input type="text" value={preparedBy} onChange={(e) => setPreparedBy(e.target.value)} placeholder="Your name" style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Your Role</label>
                  <select value={selectedRoleId} onChange={(e) => handleRoleChange(e.target.value)} style={inputStyle}>
                    <optgroup label="Executive">
                      {verticalRoles.roles.filter(r => r.level === 'executive').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                    <optgroup label="Senior Leadership">
                      {verticalRoles.roles.filter(r => r.level === 'senior').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                    <optgroup label="Management">
                      {verticalRoles.roles.filter(r => r.level === 'management').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                    <optgroup label="Specialist">
                      {verticalRoles.roles.filter(r => r.level === 'specialist').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                    <optgroup label="Operational">
                      {verticalRoles.roles.filter(r => r.level === 'operational').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                    <optgroup label="Frontline">
                      {verticalRoles.roles.filter(r => r.level === 'frontline').map(r => (
                        <option key={r.id} value={r.id}>{r.title}</option>
                      ))}
                    </optgroup>
                  </select>
                ...
                <div>
                  ...
                  <select value={reportingPeriod} onChange={(e) => setReportingPeriod(e.target.value)} style={inputStyle}>
                    <option value="Q1">Q1 2025</option><option value="Q2">Q2 2025</option><option value="Q3">Q3 2025</option><option value="Q4">Q4 2025</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 16, padding: 24, border: `1px solid ${config.color}20` }}>
            <h3 style={{ color: '#fff', margin: '0 0 20px 0', fontSize: 18 }}>Analysis Categories</h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
              {config.categories.map(cat => (
                <button key={cat.id} onClick={() => toggleCategory(cat.id)} style={{ padding: '10px 18px', borderRadius: 20, background: selectedCategories.includes(cat.id) ? config.color : `${config.color}15`, color: selectedCategories.includes(cat.id) ? '#000' : config.color, border: `1px solid ${config.color}40`, cursor: 'pointer', fontWeight: 500, fontSize: 14 }}>{cat.label}</button>
              ))}
            </div>
          </div>

          <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 16, padding: 24, border: `1px solid ${config.color}20` }}>
            <h3 style={{ color: '#fff', margin: '0 0 20px 0', fontSize: 18 }}>Additional Context</h3>
            <textarea value={context} onChange={(e) => setContext(e.target.value)} placeholder="Any specific focus areas, concerns, or context for this report..." rows={4} style={{ ...inputStyle, resize: 'vertical' }} />
          </div>
        </>
      )}
      ...
      <button onClick={generateReport} disabled={loading} style={{ padding: '16px 32px', borderRadius: 12, background: loading ? 'rgba(255,255,255,0.2)' : `linear-gradient(135deg, ${config.color}, ${config.color}cc)`, color: loading ? 'rgba(255,255,255,0.5)' : '#fff', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 16 }}>
        {loading ? 'Generating Report...' : `Generate ${config.name}`}
      </button>
