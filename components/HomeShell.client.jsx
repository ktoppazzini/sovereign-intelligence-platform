...
/**
 * useLang with optional initialLang (from server / cookies).
 * - Prefers ?lang= from URL
 * - Falls back to initialLang prop
 * - Defaults to English
 */
function useLang(initialLang) {
  const safeInitial = normalizeLang(initialLang || 'English');
  const [lang, setLang] = useState(safeInitial);

  useEffect(() => {
    try {
      const sp = new URLSearchParams(window.location.search);
      const fromQuery = sp.get('lang');
      const normalized = normalizeLang(fromQuery || safeInitial || 'English');
      setLang(normalized);
      document.documentElement.setAttribute('lang', normalized);
    } catch {
      setLang(safeInitial || 'English');
      document.documentElement.setAttribute('lang', safeInitial || 'English');
    }
  }, [safeInitial]);
...
export default function HomeShell({
  ...
  children,
  lang: initialLang,
  // role prop is accepted but not yet used (future-proof)
  role,
}) {
