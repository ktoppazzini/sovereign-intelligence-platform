...
`;
    ...
    // Translate KPI names and descriptions
    const translatedKPIs = (parsed.kpis || []).slice(0, 4).map((kpi) => {
      try {
        return {
          ...kpi,
          nameTranslated: translatedLabels[kpi.name] || kpi.name,
          descriptionTranslated: translatedLabels[kpi.description] || kpi.description,
          xTitleTranslated: translatedLabels[kpi.xTitle] || kpi.xTitle,
          yTitleTranslated: translatedLabels[kpi.yTitle] || kpi.yTitle
        };
      } catch (e) {
        return kpi;
      }
    ...
    console.log(TAG, '[KT:KPI-DASHBOARD-INTEGRATED]', {
      kpiCount: translatedKPIs.length,
      kpiNames: translatedKPIs.map(k => k.name),
      dataQuality: translatedKPIs.map(k => ({
        name: k.name,
        labelsLen: k.labels?.length || 0,
        valuesLen: k.values?.length || 0
      }))
    });

    return translatedKPIs;
  } catch (e) {
    console.error(TAG, '[KT:KPI-DASHBOARD-INTEGRATED] error', String(e?.message || e));
    return null;
  }
}
  ...
  const sectionResults = await Promise.all(order.map(async (key) => {
    ...
    if (key === 'kpis') {
      try {
        ...
        console.log(TAG, '🔧 [KPI-PREDICTIVE] Calling buildKPIDashboardSpec to identify and extract KPI data...');
        const selectedKPIs = await buildKPIDashboardSpec(canon, sections, html, translatedLabels);
        
        if (!selectedKPIs || selectedKPIs.length < 4) {
          console.error(TAG, '❌ [KPI-PREDICTIVE] KPI dashboard extraction returned < 4 KPIs');
          throw new Error('KPI dashboard extraction failed');
        }
        
        console.log(TAG, 'kpi.dashboard.completed', {
        ...
        for (let i = 0; i < selectedKPIs.length; i++) {
          ...
          console.log(TAG, `🔧 [KPI-PREDICTIVE] KPI ${i + 1}: Rendering "${kpiName}"...`);
          ...
          try {
            ...
            if (kpiData && kpiData.labels && kpiData.values) {
              ...
              console.log(TAG, `✅ [KPI-PREDICTIVE] KPI ${i + 1} (${kpiName}) rendered:`, kpiChart.length, 'chars');
            } else {
              console.log(TAG, `⚠️ [KPI-PREDICTIVE] KPI ${i + 1} (${kpiName}) missing data, skipping`);
              kpiResults.failed++;
            }
          } catch (e) {
            console.error(TAG, `❌ [KPI-PREDICTIVE] KPI ${i + 1} render error:`, String(e?.message || e));
            kpiResults.failed++;
          }
        }
...
console.log(TAG, 'labels.translated', {
  ...
  // 4. Risk Heatmap - extract from RAID if available
    ...
    // [KT:SURGICAL:PREDICTIVE:ATTACH]
    try {
      // Build reportHTML from available sections for context
  ...
  try {
    ...
    // Build reportHTML from available sections: business case + implementation + timeline content
    const reportContentForRisks = [
      sections.business || '',
      sections.implementation || '',
      sections.timeline || '',
      sections.risksAndMitigations || ''
    ].filter(Boolean).join('\n\n').slice(0, 3000); // Use first 3000 chars for context
  ...
  try {
    // Build reportHTML from available sections for context
    const reportContentForPredictive2 = [
      sections.exec || '',
      sections.business || '',
      sections.financial || ''
    ].filter(Boolean).join('\n\n').slice(0, 3000);
    const predictiveModel = await buildPredictiveModelSpec(canon, sections, reportContentForPredictive2, DEFAULT_LABELS);
    sections.predictiveModel = predictiveModel;
    console.log(TAG, 'predictiveModel.generated', {
      labels: predictiveModel?.labels?.length || 0,
      values: predictiveModel?.values?.length || 0,
