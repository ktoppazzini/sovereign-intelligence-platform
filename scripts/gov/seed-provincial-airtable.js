// scripts/gov/seed-provincial-airtable.js
...
const AIRTABLE_MODE = (process.env.AIRTABLE_MODE || 'append').toLowerCase(); // append | replace
...
// Countries to run (leave most commented for speed while testing)
const COUNTRIES = [
...
// ───────────────────────────────────────────────────────────
// Translation cache (same pattern as national)
...
// 1) Get first-level administrative divisions (states/provinces)
async function fetchLevel1Divisions(country) {
  // Q10864048 = first-level administrative division
  const endpoint = 'https://query.wikidata.org/sparql';
  const query = `
    SELECT ?div ?divLabel WHERE {
      ?div wdt:P31/wdt:P279* wd:Q10864048 .
      ?div wdt:P17 wd:${country.wikidata} .
  ...
  return rows
    .map((b) => ({ id: b?.div?.value || '', label: b?.divLabel?.value || '' }))
    .filter((x) => x.label);
}

// 2) For each division, grab “ministries”/agencies located in that division
async function fetchProvincialMinistries(division) {
  const endpoint = 'https://query.wikidata.org/sparql';
  const query = `
    SELECT ?item ?itemLabel ?article WHERE {
      {
        ...
        ?item wdt:P31/wdt:P279* wd:Q327333 .     # government agency (covers many subnational depts)
      }
      ?item wdt:P131 wd:${division.id} .         # located in this division
      OPTIONAL {
...
async function buildRowsForCountry(country) {
  const divisions = await fetchLevel1Divisions(country);
  if (!divisions.length) {
    console.warn(`(No first-level divisions for ${country.name})`);
    return [];
  }
  ...
  for (const d of divisions) {
    const orgs = await fetchProvincialMinistries(d);
...
async function airtableListAllIds(tableName) {
  const ids = [];
  let offset = undefined;
  do {
    const u = new URL(`${AIRTABLE_ENDPOINT}/${encodeURIComponent(tableName)}`);
...
async function airtableDeleteBatch(tableName, recordIds) {
  if (!recordIds.length) return;
  const url = `${AIRTABLE_ENDPOINT}/${encodeURIComponent(tableName)}`;
  const chunks = chunk(recordIds, 10);
...
async function airtableCreateRows(tableName, rows) {
  const url = `${AIRTABLE_ENDPOINT}/${encodeURIComponent(tableName)}`;
...
async function main() {
  ...
  for (const c of COUNTRIES) {
    console.log(`→ Building PROV rows for ${c.name}`);
    const rows = await buildRowsForCountry(c);
    allRows.push(...rows);
  }
  console.log(`Built ${allRows.length} provincial/state rows.`);

  ensureDir(path.join(process.cwd(), 'data', 'out'));
  const outFile = path.join(process.cwd(), 'data', 'out', 'provincial_government.csv');
