// scripts/gov/seed-municipal-airtable.js
...
// ───────────────────────────────────────────────────────────
// Wikidata
// Get municipalities (Q15284) OR big cities (Q515) for coverage.
// We prioritize municipalities; if sparse, cities will still pull agencies.
async function fetchMunicipalities(country) {
  const endpoint = 'https://query.wikidata.org/sparql';
  const query = `
    SELECT ?muni ?muniLabel WHERE {
      {
        ?muni wdt:P31/wdt:P279* wd:Q15284 .     # municipality
      } UNION {
        ?muni wdt:P31/wdt:P279* wd:Q515 .       # city (fallback)
      }
      ?muni wdt:P17 wd:${country.wikidata} .
  ...
  return rows
    .map((b) => ({ id: b?.muni?.value || '', label: b?.muniLabel?.value || '' }))
    .filter((x) => x.label);
}

// Municipal departments/agencies located in municipality
async function fetchMunicipalAgencies(muni) {
  const endpoint = 'https://query.wikidata.org/sparql';
  const query = `
    SELECT ?item ?itemLabel ?article WHERE {
      {
        ?item wdt:P31/wdt:P279* wd:Q327333 .  # government agency
      } UNION {
        ?item wdt:P31/wdt:P279* wd:Q644371 .  # municipal department
      } UNION {
        ?item wdt:P31/wdt:P279* wd:Q1234894 . # ministry (rare but included)
      }
      ?item wdt:P131 wd:${muni.id} .
      OPTIONAL {
...
async function buildRowsForCountry(country) {
  const munis = await fetchMunicipalities(country);
  if (!munis.length) {
    console.warn(`(No municipalities/cities for ${country.name})`);
    return [];
  }
  ...
  for (const m of munis) {
    const orgs = await fetchMunicipalAgencies(m);
    for (const org of orgs) {
      for (const roleEN of ROLE_LADDER) {
        rows.push({
          Country: country.name,
          Level: 'Municipal',
          Region: m.label, // municipality/city name
          Executive: execLabel,
...
async function airtableListAllIds(name) {
  ...
  do {
    const u = new URL(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(name)}`,
    );
...
async function airtableDeleteBatch(name, recordIds) {
  ...
  const url = `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${encodeURIComponent(name)}`;
...
async function main() {
  ...
  for (const c of COUNTRIES) {
    console.log(`→ Building MUNI rows for ${c.name}`);
    const rows = await buildRowsForCountry(c);
    allRows.push(...rows);
  }
  console.log(`Built ${allRows.length} municipal rows.`);

  ensureDir(path.join(process.cwd(), 'data', 'out'));
  const outFile = path.join(process.cwd(), 'data', 'out', 'municipal_government.csv');
