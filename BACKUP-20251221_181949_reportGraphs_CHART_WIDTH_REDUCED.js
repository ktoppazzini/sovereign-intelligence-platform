...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]], lang = 'English' }) {
  ...
  const leftPadding = Math.min(Math.max(100 + extraPadding, 100), 180); // [KT:FIX-v6] Reduced max from 200 to 180
  ...
  // [KT:FIX-v6] Reduced baseWidth from 760 to 720, cap total at 840 max to prevent overflow
  const baseWidth = 720;
  const w = Math.min(baseWidth + (leftPadding - 100), 840); // Cap at 840px max
  const h = 320;
...
export function prioritizationMatrixHTML({ title = 'Initiative Prioritization', initiatives = [], nano5 = false, lang = 'English' } = {}) {
  ...
  // [KT:FIX-v6] Reduced from 800 to 760 to match other charts and prevent viewer overflow
  const w = 760, h = 560, p = 90;
...
export function prioritizationMatrixHTMLNano5({ title = 'Initiative Prioritization (Nano-5)' , initiatives = [] } = {}) {
  // [KT:FIX-v6] Reduced from 900 to 850 to fit within report max-width of 900px with margin
  const w = 850, h = 650, p = 70;
  const cellW = (w - 2 * p) / 5;
  const cellH = (h - 2 * p) / 5;
