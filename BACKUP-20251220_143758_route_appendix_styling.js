...
function renderAppendicesHTML(appendixSpec, L = {}) {
  ...
  try {
    const appendicesHTML = appendixSpec.appendices.map((app, idx) => {
      ...
      // Build table header row with column widths - McKinsey dark style matching Annexe format
      const headerCells = app.columns.map(col => {
        const width = col.width || `${Math.round(100 / app.columns.length)}%`;
        return `<th style="width:${width}; padding:14px 16px; text-align:left; background:#0B2E54; color:#FFFFFF; font-weight:700; border:none; font-size:0.9rem; font-family:'Merriweather', Georgia, serif;">${sanitize(col.name || '')}</th>`;
      }).join('');

      // Build data rows - map each row object to cells based on column names with alternating colors
      const dataRows = app.rows.map((rowData, rIdx) => {
        const isEvenRow = rIdx % 2 === 0;
        const bgColor = isEvenRow ? '#F5F7FA' : '#FFFFFF';
        const cells = app.columns.map((col, cIdx) => {
          ...
          return `<td style="padding:12px 16px; border-bottom:1px solid #E5E7EB; font-size:0.9rem; color:#1F2937; background:${bgColor}; font-family:'Merriweather', Georgia, serif;">${sanitize(value)}</td>`;
        }).join('');
        return `<tr>${cells}</tr>`;
      }).join('');

      // Build complete appendix section with McKinsey styling matching old Annexe tables
      const appendixHTML = `
        <section id="${appendixID}" style="margin: 2.5rem 0; padding: 0; background:#FFFFFF; page-break-inside:avoid;">
          <h3 style="margin:0 0 1rem 0; color:#000000; font-size:1.3rem; font-weight:700; font-family:'Merriweather', Georgia, serif; border-bottom:2px solid #0B2E54; padding-bottom:0.5rem;">Annexe ${sanitize(app.id || String.fromCharCode(65 + idx))} — ${sanitize(app.title)}</h3>
          ${app.description ? `<p style="margin:0 0 1rem 0; color:#4B5563; font-size:0.9rem; line-height:1.6; font-family:'Merriweather', Georgia, serif;">${sanitize(app.description)}</p>` : ''}
          
          <table class="report-table" style="width:100%; border-collapse:collapse; background:#FFFFFF; border:1px solid #D1D5DB; font-family:'Merriweather', Georgia, serif;">
            <thead>
              <tr>${headerCells}</tr>
            </thead>
            <tbody>
