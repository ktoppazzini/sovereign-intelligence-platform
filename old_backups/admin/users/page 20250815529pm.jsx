...
// v1.0.13-roles-linked — use Roles options for “To”, show current “From” as a dropdown, louder logs, safer role mapping

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.13-roles-linked';
...
export default function AdminUsersPage() {
  ...
  async function callUsers(body) {
    ...
    if (!r.ok || !j?.ok) {
      ...
      setErrText(`${msg}${j?.status ? `. Status: ${j.status}` : ''}`);
      if (j?.debug) setDebug(j.debug);
      // Throw to let callers show the red overlay you saw
      throw new Error(msg);
    } else {
      if (j.debug) setDebug(j.debug);
    }
