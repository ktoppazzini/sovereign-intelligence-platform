...
const CLIENT_VERSION = 'users-ui v1.0.1';
...
// ---------- Minimal inline styles (keeps your layout) ----------
...
function normalizeRolesPayload(payload) {
  // Accept a bunch of possible shapes and return [{id,label,value,raw}]
  let arr = [];
  if (Array.isArray(payload)) arr = payload;
  else if (Array.isArray(payload?.roles)) arr = payload.roles;
  else if (Array.isArray(payload?.records)) arr = payload.records;
  else if (Array.isArray(payload?.data)) arr = payload.data;

  const list = arr.map((r, i) => {
    const id = r.id || r.value || r.recordId || r.record_id || r.Name || r.name || `idx-${i}`;
    const label = r.label || r.Name || r.name || r.value || r.RoleName || r.roleName || String(id);
    const value = r.value || r.id || r.recordId || r.record_id || label || id;
    return { id, label, value, raw: r };
  });

  // Deduplicate by value/label
  const seen = new Set();
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    rememberVersion();
    const v = readVersion();
    console.log('📦 Users page version:', v);
  }, []);

  // ---- Translations (kept minimal here) ----
  const [t] = useState({
    title: 'Users',
    email: 'Email',
    name: 'Name',
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        ...
        const normalized = (j.records || []).map((rec) => {
          // Try to derive a human role label
          const roleName =
            rec.RoleName ||
            rec.roleName ||
            rec.role_label ||
          ...
          return {
            ...
            role: roleName,
            roleId:
              rec.RoleId || rec.roleId || (Array.isArray(rec.Role) && rec.Role[0]) || rec.Role,
  ...
  async function loadRoles() {
    ...
    try {
      // Primary endpoint
      let { ok, json } = await fetchJson('/api/admin/users?roles=1&debug=1');
      console.log('🎭 /api/admin/users?roles=1 response:', json);
      let list = normalizeRolesPayload(json);

      // Fallbacks if we only got 0–1 item
      if (!ok || list.length <= 1) {
        const tries = ['/api/admin/roles?debug=1', '/api/roles?debug=1'];
        for (const url of tries) {
          const res2 = await fetchJson(url);
          console.log(`🎭 fallback roles ${url}:`, res2.json);
          const add = normalizeRolesPayload(res2.json);
      ...
      if (!list.length) {
        setRolesErr('Failed to load roles');
        setRoles([]);
        return;
      }
  ...
  async function fetchUserRole(email) {
    try {
      ...
      const candidate =
        ...
        json.RoleName ||
        json.roleName ||
        (json.user && (json.user.RoleName || json.user.roleName || json.user.role)) ||
        (Array.isArray(json.records) &&
          (json.records[0]?.RoleName || json.records[0]?.role || json.records[0]?.Role)) ||
        '';
      return candidate || '';
    } catch (e) {
      console.warn('current role fetch failed', e);
      return '';
    }
  }

  function openRoleModal(row) {
    // open immediately, then try to upgrade the "from" value via API
    setRoleModal({
      open: true,
      email: row.email,
      from: row.role || '-',
  ...
  // ---------- UsersTable with bullet-proof returns ----------
  function UsersTable() {
    if (errText) {
      return (
        <div
    ...
    if (loading) {
      return (
        <div style={{ marginTop: 20, color: '#fff', opacity: 0.8, padding: 16 }}>Loading…</div>
      );
    }
    if (rows.length === 0) {
      return (
        ...
        {rows.map((row) => (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              {/* Both buttons open the same single-dropdown modal */}
              <button onClick={() => openRoleModal(row)} style={smallBtn('#ffffff', '#082b52')}>
                {t.assignRole}
              </button>

              <button onClick={() => openRoleModal(row)} style={smallBtn('#dbeafe', '#082b52')}>
                {t.changeRole}
              </button>
  ...
  return (
    <div style={wrapStyle}>
      {/* Global placeholder color to make top row inputs legible */}
      <style jsx global>{`
        .admin-input::placeholder {
          color: #ffffff; /* solid white */
          opacity: 1; /* ensure visible in chromium */
          font-weight: 800; /* bold placeholder */
        }
      `}</style>
