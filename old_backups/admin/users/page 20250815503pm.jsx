...
// users-ui v1.0.12-roles-formula — From dropdown from Users.RawRole/RoleNames, To from Roles table options, send role + roleRecId, louder logs

import { useEffect, useMemo, useState } from 'react';

/* ---------- Version info ---------- */
const CLIENT_VERSION = 'users-ui v1.0.12-roles-formula';
...
export default function AdminUsersPage() {
  ...
  // ---- Translations (driven via /api/gptTranslation) ----
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        ...
        const normalized = (j.records || []).map((rec) => {
          // Your API (per your debug) returns RoleNames[] and RawRole
          // Build a deduped list of current role labels
          const roleCandidates = [];
          if (Array.isArray(rec.RoleNames)) roleCandidates.push(...rec.RoleNames);
          if (rec.RawRole) roleCandidates.push(rec.RawRole);
          // also check some historical keys just in case
          if (rec.Role) roleCandidates.push(rec.Role);
          if (rec['Roles (from Role)']) roleCandidates.push(rec['Roles (from Role)']);
          if (rec.role) roleCandidates.push(rec.role);

          const clean = roleCandidates
            .flat()
            .filter(Boolean)
            .map((s) => String(s).trim());
          const dedup = Array.from(new Set(clean));
          const displayRole = dedup[0] || '';
          ...
          return {
            ...
            email: theEmail,
            role: displayRole,
            currentRoles: dedup, // keep all we found for the "From" dropdown
  ...
  function openChangeModal(row) {
    ...
    const fromOpts = (
      row.currentRoles && row.currentRoles.length ? row.currentRoles : [row.role || '-']
    ).filter(Boolean);
    setRoleModal({
      open: true,
      email: row.email,
  ...
  async function saveRoleModal() {
    if (!roleModal.email) return alert('Missing email.');
    if (!roleModal.to) return alert('Select a role.');

    const chosen = findRoleByLabel(roleModal.to);
    const roleRecId = chosen?.id || null;

    // send both label & record id (plus common aliases) so the route can accept whatever it wants
    const body = {
      ...
      roleId: roleRecId,
      roleRecId: roleRecId,
      role_record_id: roleRecId,
      role_ids: roleRecId ? [roleRecId] : undefined,
      roleIdList: roleRecId ? [roleRecId] : undefined,
      // context
      from: roleModal.from || '',
      mode: roleModal.mode,
      _client: CLIENT_VERSION,
        ...
        {roleModal.open && (
          ...
          >
            ...
            >
              ...
              >
                {(roleModal.fromOptions.length ? roleModal.fromOptions : ['-']).map((opt, idx) => (
