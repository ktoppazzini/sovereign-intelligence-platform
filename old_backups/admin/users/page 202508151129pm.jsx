...
// v1.0.16-role-idmap — keep layout; map role NAME -> Role record ID before POST; hide "From" on Assign

import { useEffect, useMemo, useState } from 'react';

// ---------- Version info (persists to localStorage) ----------
const CLIENT_VERSION = 'users-ui v1.0.16-role-idmap';
...
export default function AdminUsersPage() {
  ...
  // ---- Translations (local defaults; server i18n can override externally) ----
  ...
  // ---------- Load roles options (for To/Assign) via schema/choices ----------
  ...
  // ---------- Map Role NAME -> Role record ID (client-side belt & suspenders) ----------
  ...
  // ---------- POST helper (now enriches payload with role record id) ----------
  async function callUsers(body) {
    ...
    // If a role NAME is present, map to record id and include common id keys
  ...
  async function addUser() {
    ...
    try {
      await callUsers({
        ...
        name: name.trim(),
        role: firstRole, // optional — callUsers will map name → id if provided
        orgId: orgId.trim(),
        active: true,
