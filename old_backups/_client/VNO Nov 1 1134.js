...
/* [KT:SURGICAL:CLIENT-NOTES:v2 + TELEMETRY]
   ...
   No style/look/feel changes. Safe with type switchers and canvas.
*/
...
//function buildNote(meta) {
  ...
  // Short, AI-friendly analysis hint (exact phrasing as requested)
...
/* ============================================================================
   ...
   No deletions: prior return kept but commented as [KT:COMMENT-OUT].
   ========================================================================== */
...
function statAnalysis({ title, xTitle, yTitle, labels=[], series=[] }){
  ...
  if(!vals.length){
    return `<p class="chart-note">${t} shows performance with ${x} on the horizontal axis and ${y} on the vertical axis. </p>`;
  }
  ...
  const returns=[];
  for(let i=1;i<n;i++){ const prev=vals[i-1]||0; returns.push(prev===0?0:(vals[i]-prev)/Math.abs(prev)); }
  ...
  return `<p class="chart-note">${s1} ${s2} ${s3}</p>`;
}
...
function extractMeta(el) {
  // Prefer normalized data-* if present (predictive or server-upgraded)
  ...
  if (bench) {
    ...
    return {
      kind: 'benchmark',
      title: json.title || 'Benchmark',
      xTitle: 'Measure',
      yTitle: json.yTitle || 'Percent',
      type: 'benchmark',
      labelsLen: 2,
      valuesLen: 2
    };
  ...
  if (heat) {
    ...
    return {
      kind: 'heatmap',
      title: json.title || 'Risk Heat Map',
      xTitle: 'Columns',
      yTitle: 'Rows',
      type: 'heatmap',
      labelsLen: cols,
  ...
  // Legacy/standard canvas wrapped in figure/div
  ...
  const dsLen = (() => {
    // try to read a data-len marker if present; otherwise fallback to 0
    const q = fig?.getAttribute?.('data-len');
  ...
  return {
    ...
    title,
    xTitle: 'Months',
    yTitle: looksPercent ? 'Percent' : 'Index',
    type: 'line',
    labelsLen: 0,
    valuesLen: dsLen
  };
...
function sendTelemetry(payload) {
  try {
    ...
    // fire-and-forget; server returns 204
...
export default function VisualNotesObserver() {
  useEffect(() => {
    ...
    function ensureNoteForVisual(visualEl, phase) {
      ...
      // flush immediately in small batches to ensure delivery
      if (batch.length >= 5) {
    ...
    // Watch for dynamic replacements (type switchers / Fast Refresh)
