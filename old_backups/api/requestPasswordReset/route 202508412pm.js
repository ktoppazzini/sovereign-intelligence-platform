 ...
 *
 * - Generates a token (future: store with expiry).
 * - Translates subject/body to `lang` via /api/gptTranslation (6s timeout).
 * - Sends from no-reply with the translated content.
 *
 * ENV:
...
{"subject":"${subject.replace(/"/g, '\\"')}","body":"${body.replace(/"/g, '\\"').replace(/\n/g, '\\n')}"}`;
        ...
        if (gptRes.ok) {
          ...
          // Accept either { translation: { subject, body } } or { subject, body }
          const obj =
        ...
        // timeout or translation failure -> keep English fallback
      }
    }
    ...
    // (Next task) persist token + expiry to Airtable/DB for reset verification
