...
Paragraph discipline: max 2 sentences per paragraph, <= 20 words each. Split any longer blocks.
...
/* ========================================================================== */
/* Appendices — REINSTATED (kept as in Oct 8)                                  */
/* ========================================================================== */
function buildAppendicesHTML(canon, sections) {
  // [KT:SURGICAL] $appendix placeholder commented; safe fallback added
  /* $appendix */
  try {
    const totals = {
      exec: (sections.exec || '').length,
      current: (sections.current || '').length,
      financials: (sections.financials || '').length,
      kpis: (sections.kpis || '').length,
      timeline: (sections.timeline || '').length,
      ops: (sections.ops || '').length,
      risk: (sections.risk || '').length,
      roi: (sections.roi || '').length,
    };
    return [
      '<section id="appendices">',
      '<h3>Appendices</h3>',
      '<p class="note">Auto-generated appendix (safe fallback). This preserves structure and prevents runtime errors.</p>',
      '<table class="report-table"><thead><tr><th>Section</th><th>Bytes</th></tr></thead><tbody>',
      '<tr><td>Executive Summary</td><td>' + String(totals.exec) + '</td></tr>',
      '<tr><td>Current State</td><td>' + String(totals.current) + '</td></tr>',
      '<tr><td>Financials</td><td>' + String(totals.financials) + '</td></tr>',
      '<tr><td>KPIs</td><td>' + String(totals.kpis) + '</td></tr>',
      '<tr><td>Timeline</td><td>' + String(totals.timeline) + '</td></tr>',
      '<tr><td>Operations</td><td>' + String(totals.ops) + '</td></tr>',
      '<tr><td>Risk</td><td>' + String(totals.risk) + '</td></tr>',
      '<tr><td>ROI</td><td>' + String(totals.roi) + '</td></tr>',
      '</tbody></table>',
      '</section>',
    ].join('\n');
  } catch (e) {
  ...
  for (const key of order) {
    ...
    if (key !== 'exec') {
      try {
        ...
        if (key === 'current') {
          try {
            const labels = /year/i.test(String(canon.timeFrame || ''))
              ? ['Y1', 'Y2', 'Y3']
              : ['M1', 'M2', 'M3', 'M4', 'M5', 'M6'];
            const spec = {
              type: 'line',
              title: 'Current Performance Projection',
              xTitle: /year/i.test(String(canon.timeFrame || '')) ? 'Years' : 'Months',
              yTitle: 'Index',
              labels,
              datasets: [
                { label: 'Predicted', data: labels.map((_, i) => (i + 1) * (i < 3 ? 2 : 3)) },
              ],
            };
            // build the fragment without template literals (avoids stray backtick issues)
            const frag =
              '<figure data-chart="' + toAttrJSON(spec) + '" data-origin="predictive"></figure>';
            html += '\n' + frag;

            const meta = parseVizMeta(frag);
            console.log(TAG, 'section.vizInjected.fallback', key, {
              kind: meta.kind,
              type: meta.type,
              title: meta.title,
              xTitle: meta.xTitle,
              ...
              labelsLen: meta.labelsLen,
              dataLen: meta.dataLen,
              canvas: /<figure[^>]*data-chart=/.test(frag),
            });
          } catch (e) {
            console.log(TAG, 'section.viz.fallback.error', key, String(e?.message || e));
          }
        }
      } catch (e) {
    ...
    sections[key] = html;
    const finalCharts = (html.match(/data-chart=/g) || []).length;
    const finalHeat = (html.match(/data-heatmap=/g) || []).length;
    const finalBench = (html.match(/data-widget=['"]benchmark['"]/g) || []).length;
    const finalTables = (html.match(/<table\b/gi) || []).length;
    console.log(TAG, 'section.viz.final', key, { finalCharts, finalHeat, finalBench, finalTables });
  }
