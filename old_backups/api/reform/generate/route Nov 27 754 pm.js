...
if (key === 'nextSteps') {
  ...
  // [KT:SURGICAL:WORLD-CLASS-VIZ-I18N-CALL]
// Run language-aware localization over world-class visuals (titles, labels, axis text).
if (canon.lang && !/^english$/i.test(String(canon.lang || ''))) {
  try {
    await __kt_translateWorldClassVisuals(
      worldClassVisuals,
      canon.lang,
      openai,
      L,
  ...
  // [KT:SURGICAL:ASYNC-BUILD-REPORT-FIX]
  // Wrap buildReformReportHTML in an async IIFE if it ever returns a Promise (future-proofing)
  let html;
  if (buildReformReportHTML.constructor.name === 'AsyncFunction') {
    html = await buildReformReportHTML({ 
      canon, 
      sections, 
      phases, 
      planDiagramHTML, 
...
async function fixUntitledTables(html, lang) {
  const langPrefix =
