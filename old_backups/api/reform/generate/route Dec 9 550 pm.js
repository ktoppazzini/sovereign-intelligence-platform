...
async function __kt_aiTableTitle(sectionHTML, tableHTML, canon) {
  try {
    ...
    // Build prompt for GPT - CRITICAL: Generate title directly in target language
    const targetLang = canon.lang || 'English';
    const languageInstruction = targetLang && !/^english$/i.test(targetLang)
      ? `\n\nCRITICAL: Write the table title DIRECTLY in ${targetLang}. Do NOT write in English first. Use professional business terminology appropriate for ${targetLang}.`
      : '';
    
    const prompt = `
You generate SHORT, EXECUTIVE-LEVEL table titles by analyzing table content.

TABLE HEADERS:
${headerText}

REQUIREMENTS:
- Analyze what the table represents (NOT just list the headers)
- Return ONLY the title text in ${targetLang}
- Max length: 7 words
- No quotes, no punctuation except letters/numbers
- Describe the table's PURPOSE, not its structure
${languageInstruction}
    `.trim();
    ...
    const res = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: 'system', 
          content: `You are a business analyst. Generate a concise table title that describes what the table shows. Write DIRECTLY in ${targetLang}. No markdown, no JSON, no English if target language is not English.${languageInstruction}` 
        },
        { role: 'user', content: prompt }
      ]
    });
  ...
  "labels": ["[${lang} translation]", "[${lang} translation]", ...]
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a translator. Translate all text to ${lang}. Return valid JSON only, no markdown or explanations.`
        },
        {
          role: 'user',
          content: prompt
    ...
    try {
      ...
      console.log(TAG, 'chart.labels.translate.json.error', { lang, error: String(err?.message || err) });
...
async function translateLabels(targetLang) {
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_completion_tokens: 16000,  // Increased from 4096 to handle all labels
    });
  ...
  html = await genSectionFirstPass(key, canon, floor);
    ...
    if (key === 'kpis') {
      try {
        ...
        if (canon.lang && !/^english$/i.test(canon.lang)) {
          // Build translation cache for chart summary phrases
          const summaryTranslations = await buildChartSummaryTranslations(canon.lang);
          __chartSummaryTranslationCache[canon.lang] = summaryTranslations;
          
          // Set up synchronous translator using the cache for both reportGraphs and worldClassVisuals
          const translatorFn = (s) => {
            const cached = __chartSummaryTranslationCache[canon.lang]?.[s];
            return cached || s; // Fall back to original if not in cache
          };
          
          setGraphTranslator(translatorFn);
          setWorldClassVisualsTranslator(translatorFn);
          console.log(TAG, '✅ [KPI-FIX] Graph translators enabled for:', canon.lang, 'with', Object.keys(summaryTranslations).length, 'phrases');
          console.log(TAG, '🔍 [KPI-FIX] Sample translations:', {
            'Overall': summaryTranslations['Overall'],
            'Key Insights': summaryTranslations['Key Insights'],
            'increasing': summaryTranslations['increasing'],
            'Peak value of': summaryTranslations['Peak value of']
          });
          console.log(TAG, '📋 [TRANSLATION-CACHE] Full cache for', canon.lang + ':', JSON.stringify(summaryTranslations, null, 2));
        }
        ...
        const savingsChart = barChartHTML({ ...savingsSpec, lang: canon.lang });
kpiCharts += savingsChart + '\n';
        ...
        const paybackChart = lineChartHTML({ ...paybackSpec, lang: canon.lang });
kpiCharts += paybackChart + '\n';
        ...
        for (let i = 0; i < benchmarks.length; i++) {
          ...
          // Translate the benchmark spec itself when non-English
          if (canon.lang && !/^english$/i.test(canon.lang)) {
            try {
  ...
  console.log(TAG, `📊 [KPI-FIX] Benchmark ${i + 1} VALUES:`, {
    ...
    yTitle: bm.yTitle,
          ...
          const bmChart = benchmarkHTML({ ...bm, lang: canon.lang });
          kpiCharts += bmChart + '\n';
...
console.log(TAG, 'labels.translated', {
  ...
  // 4. Risk Heatmap - extract from RAID if available
    ...
    // [KT:SURGICAL:PREDICTIVE:ATTACH]
    try {
      let predictiveModel = await buildPredictiveModelSpec(canon, sections, '', L);
  ...
  try {
    ...
    // [KT:SURGICAL:PREDICTIVE:ATTACH]
    try {
      const predictiveModel = await buildPredictiveModelSpec(canon, sections, '', L);
      sections.predictiveModel = predictiveModel;
      console.log(TAG, 'predictiveModel.generated', {
        labels: predictiveModel?.labels?.length || 0,
        values: predictiveModel?.values?.length || 0,
  ...
  */
  ...
  let html = await buildReformReportHTMLWithTranslation({
    ...
    planDiagramHTML,
    implKit,
    labelMap: translatedLabels,        // Primary translated labels
