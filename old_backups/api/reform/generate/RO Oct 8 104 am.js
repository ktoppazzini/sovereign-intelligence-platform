...
// [KT:COMMENTED-OUT] prior generic-visual guidance removed

// [KT:NO-VISUALS] replace guidance: the section writer must NOT emit visuals.
...
- "title" (clear and specific; **must be present**),
- "xTitle" ("${years ? 'Years' : 'Months'}")  // **must be present**
- "yTitle" ("${section === 'financials' || section === 'exec' ? yCur : section === 'kpis' || section === 'roi' || section === 'current' ? 'Percent' : 'Index'}")  // **must be present**
...
function isGraphCandidateValid(section, c) {
  ...
  // Require explicit titles/axes — NO DEFAULTS/FALLBACKS
  if (!String(c.title || '').trim()) return false;
  if (!String(c.xTitle || '').trim()) return false;
  if (!String(c.yTitle || '').trim()) return false;
...
// Render candidate to fragment (NO default titles/axes)
function renderCandidateToFragment(section, c) {
  if (String(c.type || '').toLowerCase() === 'heatmap' && section === 'risk') {
    ...
    console.log(TAG, 'predictive.render.heatmap', section, {
      title: spec.title,
      rows: spec.rows.length,
      cols: spec.cols.length,
    });
  ...
  const spec = {
    ...
    datasets: Array.isArray(c.datasets) ? c.datasets : [{ label: 'Predicted', data: [] }],
  };
  ...
  console.log(TAG, 'predictive.render.chart', section, {
    type: spec.type,
    title: spec.title,
    xTitle: spec.xTitle,
    yTitle: spec.yTitle,
    labelsLen: spec.labels.length,
    valuesSample: spec.datasets?.[0]?.data?.slice?.(0, 3) || [],
  });
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Write a crisp 1–2 sentence description (≤20–25 words each) of the predictive chart for "${section}" titled "${meta.title}".
Mention X and Y meanings and the executive decision it informs.`,
        },
      ],
    });
...
async function choosePredictiveFragment(section, canon, sectionHtml, existingSigSet) {
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    // De-dupe by title BEFORE rendering
    const preTitle = String(c.title || '')
      .trim()
      .toLowerCase();
    if (existingSigSet.has(`chart|${preTitle}`) || seenLocal.has(`chart|${preTitle}`)) {
      console.log(TAG, 'predictive.reject.duplicate.pre', section, { idx: i + 1, title: preTitle });
      continue;
    }
    ...
    if (existingSigSet.has(sig) || seenLocal.has(sig)) {
      console.log(TAG, 'predictive.reject.duplicate.post', section, { idx: i + 1, sig });
      continue;
    }
...
/* ========================================================================== */
/* [KT:ITER] legacy strategy list (kept; filtered)                             */
/* ========================================================================== */
...
/* ========================================================================== */
/* mining / derived / advice (helpers retained; left disabled)                 */
/* ========================================================================== */
...
Paragraph discipline: 2–3 sentences per paragraph. Keep each sentence ≤ 20–25 words.
Split any longer blocks. Avoid run-ons and filler.
...
Write 2–3 short paragraphs (2–3 sentences each; ≤20–25 words per sentence). Avoid repetition and filler.
...
`.trim();
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      temperature: 0.2,
      messages: [
        { role: 'system', content: 'Return STRICT JSON that exactly matches the requested shape.' },
        {
          role: 'user',
          content:
            body +
            `\n\n--- CONTEXT ---\nOrg:${canon.orgName} (${canon.country}) Time frame:${canon.timeFrame}\nSections:\n${Object.entries(
              sections,
            )
              .map(([k, v]) => `[${k}] ${v.substring(0, 4000)}`)
              .join('\n\n')}`,
        },
      ],
    });
    ...
    console.log(TAG, 'appendix.implKit.generated', {
      ok: true,
      counts: {
        charters: obj?.charters?.length || 0,
        raci: obj?.raci?.items?.length || 0,
        raid: obj?.raid?.items?.length || 0,
        benefits: obj?.benefits?.lines?.length || 0,
        plan100: obj?.plan100?.weeks?.length || 0,
      },
    });
    return obj;
  } catch (e) {
...
- Paragraph discipline: limit to 2–3 sentences per paragraph; split long blocks; avoid orphans/widows.
- Sentences should be concise (≤20–25 words).
...
async function genSectionFirstPass(section, canon, floor) {
  ...
  html = sanitizeNonPredictiveVisuals(section, html); // [KT:ADD]
...
async function topUpSection(section, canon, currentHTML, floor) {
  ...
  while (words < floor && guard < 8) {
    ...
    html += '\n' + sanitizeNonPredictiveVisuals(section, add); // [KT:ADD]
    words = wc(html);
    guard += 1;
...
// Small helper to craft a brief conclusion
      ...
      messages: [
        {
          role: 'system',
          content:
            'Return HTML only for a short Conclusion (<section id="conclusion">…</section>).',
        },
        {
          role: 'user',
          content: `Write a concise Conclusion for ${canon.orgName}.
Two short paragraphs, 2–3 sentences each (≤20–25 words per sentence).
Summarize ROI, operating model, risk posture, and next steps.`,
        },
      ],
    });
...
export async function POST(req) {
  ...
  // lock total per-section floors to 20,000
  ...
  // Add a concise Conclusion at end (outside audit’s anchor constraints)
  const conclusion = await genConclusion(canon, sections);
  if (conclusion) {
    html += `\n${conclusion}`;
    console.log(TAG, 'conclusion.appended', { ok: true, len: wc(conclusion) });
  }
