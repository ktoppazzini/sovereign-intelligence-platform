...
// Default floors (words) — tune as needed
...
function sectionPrompt(section, canon, minWords) {
  ...
  const asks = {
    exec: 'Write an executive summary with 4–6 bullet takeaways and 2–3 tight paragraphs. Quantify headline savings and payback. Avoid generic filler.',
    current:
      'Describe current state, constraints, and pain points. Use concrete examples. Include a compact comparative table if useful.',
    financials:
    ...
    timeline:
      'Describe a 12–24 month plan grouped into exactly five phases with milestones and owners. Keep paragraphs short.',
...
- <figure data-chart='{"type":"bar|line|pie|doughnut|radar","title":"...","labels":[...],"datasets":[{"label":"...","data":[...]}]}'><canvas></canvas></figure>
...
async function topUpSection(section, canon, currentHTML, floor) {
  ...
  while (words < floor && guard < 8) {
    const remaining = Math.max(200, floor - words); // nudge model
    const res = await openai.chat.completions.create({
      model: MODEL,
      messages: [
        {
...
export async function POST(req) {
  ...
  // 1) Per-section loop: finish each section before moving on
  ...
  // 2) If the whole report is still short, do light balanced top-ups
  ...
  // 3) SSR fallback trend only (derived from inputs)
  const years = Math.max(1, Number(String(canon.timeFrame).match(/\d+/)?.[0] || 2));
  const points = Math.min(36, Math.max(6, years * 6)); // bi-monthly
  const base = Math.max(1, canon.costSavingsGoal || 2_000_000);
  const trend = Array.from({ length: points }, (_, i) => Math.round((base * (i + 1)) / points));

  // 4) Derive phases for the PNG overlay
  ...
  // 5) Build final HTML with your template
  const html = buildReformReportHTML({
    charts: {
      trend: { labels: ['Y1', 'Y2', 'Y3'], series: [200000, 420000, 700000] },
      spend: { labels: ['Y1', 'Y2', 'Y3'], series: [60000, 320000, 700000] },
      kpi: {
        labels: ['Ops', 'Digital', 'Supply', 'Pricing'],
        series: [420000, 680000, 310000, 260000],
      },
      mix: { labels: ['Dine-in', 'Takeout', 'Delivery', 'Digital'], series: [25, 35, 20, 20] },
      risk: {
        matrix: [
          [1, 2, 3, 2, 1],
          [2, 3, 4, 3, 2],
      ...
      roi: { labels: ['Q1', 'Q2', 'Q3', 'Q4', 'Q5', 'Q6'], series: [0, 2, 5, 8, 13, 18] },
      benchmark: {
        matrix: [
          [0.2, 0.4, 0.6, 0.3],
          [0.5, 0.7, 0.9, 0.2],
    ...
    planImage: '/images/implementation/implementation-plan.png',
    debug: false, // set true to see render logs
  });
