...
async function choosePredictiveFragment(section, canon, sectionHtml, existingSigSet) {
  // inside choosePredictiveFragment(...) where you iterate batch
  for (let i = 0; i < batch.length; i++) {
    // 1) merge synth fields BEFORE any validation or render
    const merged = mergeSynth(section, canon, batch[i]); // <-- make sure mergeSynth is defined top-level

    // 2) validate the *merged* spec, not the raw candidate
    ...
    // 3) render from the merged spec (so titles/axes/labels are present)
    const frag = renderCandidateToFragment(section, merged);

    // 4) pull meta *after* render to log exactly what will appear
    ...
    console.log(TAG, 'predictive.select', section, {
      ...
      valuesSample: Array.isArray(merged.datasets?.[0]?.data)
        ? merged.datasets[0].data.slice(0, 3)
        : [],
    });

    return { html: frag, meta }; // (or your existing return/accumulation)
  }
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    if (!isGraphCandidateValid(section, c)) {
      console.log(TAG, 'predictive.reject.invalid', section, {
        ...
        type: c.type,
        kind,
        sample: Array.isArray(c.datasets?.[0]?.data) ? c.datasets[0].data.slice(0, 3) : [],
      });
      continue;
    }
    ...
    // As a last resort, synthesize axes if any are missing (rare now)
