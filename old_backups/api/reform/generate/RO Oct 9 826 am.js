/* ===============================
   ...
   (Corrected per Rules; surgical changes only)
   =============================== */
...
const extractVizFragment = (s = '') => {
  ...
  const m1 = str.match(
    new RegExp('<figure[^>]*data-chart=["\\\'][\\\\s\\\\S]*?<\\\\/figure>', 'i'),
  );
  if (m1) return m1[0];

  /* const m2 = str.match(/<figure[^>]*data-widget=['\"]benchmark['\"][\s\S]*?<\/figure>/i); */
  const m2 = str.match(
    new RegExp('<figure[^>]*data-widget=["\\\']benchmark["\\\'][\\\\s\\\\S]*?<\\\\/figure>', 'i'),
  );
  if (m2) return m2[0];

  /* const m3 = str.match(/<div[^>]*data-heatmap=['"][\s\S]*?<\/div>/i); */
  const m3 = str.match(new RegExp('<div[^>]*data-heatmap=["\\\'][\\\\s\\\\S]*?<\\\\/div>', 'i'));
  if (m3) return m3[0];
...
function parseVizMeta(frag = '') {
  ...
  const chart = frag.match(
    new RegExp('<figure[^>]*data-chart=["\\\']([\\\\s\\\\S]*?)[\"\\\']', 'i'),
  );
  if (chart) {
    out.kind = 'chart';
    try {
  ...
  const heat = frag.match(
    new RegExp('<div[^>]*data-heatmap=[\"\\\']([\\\\s\\\\S]*?)[\"\\\']', 'i'),
  );
  if (heat) {
    out.kind = 'heatmap';
    try {
...
function synthTitleIfMissing(section, frag) {
  // --- ORIGINAL (commented) ---
  // return String(frag).replace(/(<figure[^>]*data-chart=['"])([\s\S]*?)(['"][^>]*>[\s\S]*?<\/figure>)/i, ...
  return String(frag).replace(
    new RegExp(
      '(<figure[^>]*data-chart=["\\\'])([\\\\s\\\\S]*?)(["\\\'][^>]*>[\\\\s\\\\S]*?<\\\\/figure>)',
      'i',
    ),
    (m, pre, jsonRaw, post) => {
...
async function annotateAllVisuals(html, canon) {
  let out = html;
  /* const re = /(<figure[^>]*data-chart[^>]*>[\s\S]*?<\/figure>|<div[^>]*data-heatmap=['"][\s\S]*?<\/div>|<figure[^>]*data-widget=['"]benchmark['"][\s\S]*?<\/figure>)/ig; */
  const re = new RegExp(
    '(<figure[^>]*data-chart[^>]*>[\\\\s\\\\S]*?<\\\\/figure>|<div[^>]*data-heatmap=[\"\\\'][\\\\s\\\\S]*?[\"\\\'][\\\\s\\\\S]*?<\\\\/div>|<figure[^>]*data-widget=[\"\\\']benchmark[\"\\\'][\\\\s\\\\S]*?<\\\\/figure>)',
    'ig',
  );
...
`.trim();
  ...
  try {
    ...
    if (!obj.applied || !newHtml) {
      // Deterministic, structure-safe polish (ALWAYS apply; no skip)
      ...
      console.log(TAG, '[KT:AUDIT] applied.safe');
      return safe;
    }
    const keyIds = [
      'exec',
    ...
    const lengthOK = Math.abs(driftPct) <= 5; // strict guard restored to ±5%
    ...
    if (okIds && lengthOK) {
      ...
      console.log(TAG, '[KT:AUDIT] applied.safe.bounds', {
        okIds,
        lengthOK,
        wordDeltaPct: +driftPct.toFixed(1),
      });
      // Fall back to original to preserve structure/length within bounds
      return html;
    }
  } catch (e) {
    console.log(TAG, '[KT:AUDIT] applied.safe.error', String(e?.message || e));
    // Fall back with the deterministic polish
    let safe = String(html || '');
    safe = safe.replace(/\s{2,}/g, ' ').replace(/<br\s*\/?>\s*<br\s*\/?>/gi, '<br>');
    safe = safe.replace(/<table(?![^>]*class=)/gi, '<table class="report-table"');
    safe = safe.replace(
...
function ensureStandardVisuals(html, canon) {
  ...
  if (!hasKpiCharts) {
    // --- ORIGINAL (string-with-placeholder; commented) ---
    // const DASHBOARD_HTML = ` ... figure data-chart="&quot;{...}&quot;" ... `;

    // --- SURGICAL: build real spec objects and write with toAttrJSON() so they actually render ---
    const dash1 = {
      type: 'line',
      title: 'Savings Over Time',
      xTitle: 'Months',
      yTitle: canon.currencyCode || 'USD',
    ...
    const DASHBOARD_HTML = `<section id="dashboard">
  <div class="chart-card">
    <figure data-chart="${toAttrJSON(dash1)}" data-origin="standard"></figure>
  </div>
  <div class="chart-card">
    <figure data-chart="${toAttrJSON(dash2)}" data-origin="standard"></figure>
  </div>
</section>`;
  ...
  if (!hasHeatmap) {
    ...
    const HEATMAP = `<div class="chart-card"><div data-heatmap="${toAttrJSON(heatSpec)}" data-origin="standard"></div></div>`;
...
export async function POST(req) {
  ...
  // McKinsey-style polish (now guaranteed to apply or apply.safe)
