...
function toAttrJSON_RAW(obj) {
  try {
    ...
    return escapeForSingleQuotedAttr(json);
  } catch {
    return '{}';
  }
}
...
Allowed improvements (style & microcopy only):
...
export async function POST(req) {
    ...
    if (key !== 'exec') {
      try {
        ...
        if (predFrag) {
          ...
          LOGV(TAG, 'section.vizInjected.fragment', key, predFrag.slice(0, 1400));
        } else {
  ...
  // [KT-ADD] Verify predictive visuals made it into the assembled report
  ...
  // If counts are zero but we injected during sections, re-inject now
...
/* ========================================================================== */
/* [KT PATCH] ensureStandardVisuals — re-emit RAW JSON (single-quoted attrs)   */
/* ========================================================================== */
function ensureStandardVisuals(html, canon) {
  var out = String(html || '');
  ...
  if (!hasKpi) {
    ...
    var dash =
      ...
      '    <figure data-chart=\'' +
      toAttrJSON_RAW(spec1) +
      '\' data-origin="standard"></figure>' +
      '  </div>' +
      '  <div class="chart-card">' +
      '    <figure data-chart=\'' +
      toAttrJSON_RAW(spec2) +
      '\' data-origin="standard"></figure>' +
      '  </div>' +
      '</section>';
