...
function quoteSafeChartAttrs(html = '') {
  // [KT-REMOVED]return String(html||''); // ← NO-OP to avoid double-encoding JSON in attributes
  return String(html || '');
}
...
function safeParseAttrJSON(s = '') {
  try {
    ...
    if (raw === '{') {
      console.error(TAG, 'predictive.parse.error', { kind: 'chart', snippet: '{', len: 1 });
      return null;
    }
    return JSON.parse(
      String(s)
...
function toAttrJSON_RAW(obj) {
  // [KT-REMOVED]
  // [KT-REMOVED]  try {
  // [KT-REMOVED]    const json = JSON.stringify(obj);
  // [KT-REMOVED]    // keep JSON as a single line and escape apostrophes for single-quoted attributes
  // [KT-REMOVED]    return json
  // [KT-REMOVED]      .replace(/'/g, '&#39;')
  // [KT-REMOVED]      .replace(/\r\n|\n|\r/g, ' ');
  // [KT-REMOVED]  } catch {
  // [KT-REMOVED]    return '{}';
  // [KT-REMOVED]  }
...
function parseVizMeta(frag = '') {
  ...
  if (chart) {
    out.kind = 'chart';
    /* [KT-REMOVED] try { */
    /* [KT-REMOVED]       const captured = chart[2]; */
    /* [KT-REMOVED]       const json = safeParseAttrJSON(captured); */
    /* [KT-REMOVED]       if (!json) { */
    /* [KT-REMOVED]         console.log(TAG, 'predictive.parse.error', { kind:'chart', snippet: String(captured).slice(0,120), len: String(captured).length }); */
    /* [KT-REMOVED]       } */
    /* [KT-REMOVED]       out.type  = String(json?.type||''); */
    /* [KT-REMOVED]       out.title = String(json?.title||''); */
    /* [KT-REMOVED]       out.xTitle = String(json?.xTitle||''); */
    /* [KT-REMOVED]       out.yTitle = String(json?.yTitle||''); */
    /* [KT-REMOVED]       out.labelsLen = Array.isArray(json?.labels) ? json.labels.length : 0; */
    /* [KT-REMOVED]       out.dataLen   = Array.isArray(json?.datasets?.[0]?.data) ? json.datasets[0].data.length : 0; */
    /* [KT-REMOVED]     } catch (e) { */
    /* [KT-REMOVED]       console.log(TAG, 'predictive.parse.exception', String(e?.message||e)); */
    /* [KT-REMOVED]     } */
    try {
...
async function predictiveDescription(section, canon, meta) {
  const openai = await getOpenAI();
  try {
    // [KT-REMOVED] const res = await openai.chat.completions.create({
    // [KT-REMOVED]       model: MODEL,
    // [KT-REMOVED]       temperature: 1,
    // [KT-REMOVED]       messages: [
    // [KT-REMOVED]         { role:'system', content:'Return one short HTML paragraph only (<p class="chart-note">…</p>).' },
    // [KT-REMOVED]         { role:'user', content:
    // [KT-REMOVED] `Explain "${meta.title}" for ${canon.orgName}. State X and Y meanings, the trend, one decision it informs this quarter, and one risk to watch.` } ],
    // [KT-REMOVED]     });
    // [KT-REMOVED]
    const auditCall = openai.chat.completions.create({
      model: MODEL,
      temperature: 1,
      messages: [
    ...
    const timeout = new Promise((_, rej) =>
      setTimeout(() => rej(new Error('audit-timeout')), 90_000),
    );
    const res = await Promise.race([auditCall, timeout]);
...
async function annotateAllVisuals(html, canon) {
  ...
  const MAX_ANNOTS = 8; // [KT-SPEED]
  const found = [...out.matchAll(re)];
  const queue = found.slice(0, MAX_ANNOTS);
  // [KT-REMOVED-DUP] const found = [...out.matchAll(re)];
...
async function annotateTablesWithTitles(html, canon) {
  ...
  let last = 0,
    m;
  const MAX_TABLES = 6; // [KT-SPEED]
  let count = 0;
  while ((m = re.exec(out)) !== null && count < MAX_TABLES) {
...
/* ========================================================================== */
/* [KT-ADD] Server-side upgrade: data-* → full HTML (uses reportGraphs.js)     */
/* ========================================================================== */
function upgradeChartsToHTML(html = '') {
  ...
  // data-chart figures
  out = out.replace(
    /<figure[^>]*data-chart=(["'])([\s\S]*?)\1[^>]*>([\s\S]*?)<\/figure>/gi,
    (m, q, specRaw) => {
      ...
      try {
        ...
        if (t === 'pie' || t === 'doughnut') return lineChartHTML(spec);
      } catch (e) {
        console.log(TAG, 'render.error.chart', String(e?.message || e));
      }
      return m;
    },
  );
  ...
  // data-heatmap divs
  out = out.replace(
    /<div[^>]*data-heatmap=(["'])([\s\S]*?)\1[^>]*>([\s\S]*?)<\/div>/gi,
    (m, q, specRaw) => {
      const spec = safeParseAttrJSON(specRaw) || {};
      try {
        return heatmapHTML(spec);
      } catch (e) {
        console.log(TAG, 'render.error.heatmap', String(e?.message || e));
        return m;
      }
    },
  );
...
function isGraphCandidateValid(section, c) {
  ...
  const xOk = String(c.xTitle || '').trim().length > 0 || type === 'heatmap';
  const yOk = String(c.yTitle || '').trim().length > 0 || type === 'heatmap';

  const labels = Array.isArray(c.labels) ? c.labels : [];
  const data = Array.isArray(c.datasets && c.datasets[0] && c.datasets[0].data)
    ? c.datasets[0].data
    : [];

  const nums = data.map((x) => Number(x)).filter((n) => Number.isFinite(n));
  const dataOk = nums.length === data.length && !nums.every((n) => n === 0) && data.length > 0;

  // ✅ IMPORTANT: use JS logical OR, not "or"
...
export async function POST(req) {
  try {
    ...
    // [KT:MINWORDS] Enforce global minimum of 20,000 after audit as well
    if (canon.minWords < 20000) canon.minWords = 20000;
  ...
  <style id="kt-theme">
    ...
    // [KT-ADD] render charts server-side so the UI shows real plots
    ...
    (function ensureAnchors() {
      ...
      for (const id of required) {
        if (!new RegExp(`id=["']${id}["']`).test(html)) {
          console.log(TAG, 'anchor.missing', id, '— restoring placeholder');
          html = html.replace(
            '</body>',
            `<section id="${id}"><h2>${id.toUpperCase()}</h2></section></body>`,
          );
        }
      }
    })();

    // ===================== POST-AUDIT GRAPH COUNTS =====================
    const __postChartCount = (String(html).match(/data-chart=/g) || []).length;
    const __postHeatCount = (String(html).match(/data-heatmap=/g) || []).length;
    const __graphsAfter = __postChartCount + __postHeatCount;
    console.log(TAG, 'viz.counts.postAudit', {
      charts: __postChartCount,
      heatmaps: __postHeatCount,
      total: __graphsAfter,
    });
    ...
    return NextResponse.json({
      ok: true,
      html,
      graphsBeforeAudit: __graphsBefore,
      graphsAfterAudit: __graphsAfter,
