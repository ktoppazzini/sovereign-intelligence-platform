...
/* ===============================
   ...
   2025-10-20 [KT:SURGICAL:HEURISTICS+AUDIT]
   - Heuristic table titles (avoids “Year 1 • …”).
   - Smarter double-note guard (scan until next visual or 1200 chars).
   - Visuals audit summary logging for meta proof across all visuals.
   =============================== */
...
/*
  ...
  - Heuristic improvement (Fix A): Avoid building titles that are just column names like
    "Metric • Year 1 • Year 2 • ...". Prefer nearest heading (<h3>/<h2>/<strong> within ~600 chars)
    or <caption>. Otherwise synthesize "N-Year Metrics (Summary)" or "N-Month Metrics (Summary)".
*/
function injectTableTitles(html) {
  try {
    ...
    while ((match = tableRe.exec(out)) !== null) {
      ...
      // Already titled? look back a short window for .table-title
      const beforeSnippet = out.slice(Math.max(0, tableStart - 600), tableStart);
      ...
      // ---- derive default title from first header row (legacy path) ----
      let derivedFromThead = '(Untitled Table)';
      try {
        ...
        if (theadMatch) {
          ...
          if (firstRowMatch) {
            ...
            if (ths.length) derivedFromThead = ths.join(' • ');
            else {
              const tds = Array.from(firstRowMatch[0].matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/gi)).map(
                m => m[1].replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim()
              ).filter(Boolean);
              if (tds.length) derivedFromThead = tds.join(' • ');
            }
          }
        } else {
          ...
          if (firstRow) {
            ...
            if (cells.length) derivedFromThead = cells.join(' • ');
          }
        }
      } catch { derivedFromThead = '(Untitled Table)'; }

      // ---- NEW heuristic: detect Year/Month column-name patterns ----
      const tokens = (derivedFromThead || '').split('•').map(s => s.trim());
      const yearTokens = tokens.filter(t => /\b(?:y(?:ear)?\s*\d+|\d+\s*year)\b/i.test(t) || /^y\d+$/i.test(t));
      const monthTokens = tokens.filter(t => /\b(?:m(?:onth)?\s*\d+|\d+\s*month)\b/i.test(t) || /^m\d+$/i.test(t));
      const looksLikeColumnsOnly = (yearTokens.length >= 2) || (monthTokens.length >= 3);

      // Prefer preceding heading if columns-only pattern detected
      let title = derivedFromThead;
      let strategy = 'thead-joined';
      if (looksLikeColumnsOnly) {
        // Try nearest preceding heading or strong within ~600 chars
        const back = out.slice(Math.max(0, tableStart - 600), tableStart);
        const h3 = back.match(/<h3[^>]*>([\s\S]*?)<\/h3>(?![\s\S]*<h3)/i);
        const h2 = back.match(/<h2[^>]*>([\s\S]*?)<\/h2>(?![\s\S]*<h2)/i);
        const strong = back.match(/<strong[^>]*>([\s\S]*?)<\/strong>(?![\s\S]*<strong)/i);

        const heading = (h3?.[1] || h2?.[1] || strong?.[1] || '').replace(/<[^>]*>/g,'').trim();
        if (heading) {
          title = heading;
          strategy = 'preceding-heading';
        } else {
          // else try caption inside table
          const cap = (tableStr.match(/<caption[^>]*>([\s\S]*?)<\/caption>/i)||[])[1];
          if (cap) {
            title = String(cap).replace(/<[^>]*>/g,'').trim();
            strategy = 'caption';
          } else {
            // synthesize compact fallback
            const nY = yearTokens.length || 0;
            const nM = monthTokens.length || 0;
            if (nY >= 2) { title = `${nY}-Year Metrics (Summary)`; strategy = 'fallback-years'; }
            else if (nM >= 3) { title = `${nM}-Month Metrics (Summary)`; strategy = 'fallback-months'; }
            else { title = 'Metrics (Summary)'; strategy = 'fallback-generic'; }
          }
        }
      }
      ...
      // [KT:SURGICAL LOG] per-table title decision
      console.log(TAG, 'table.title.heuristic', { idx, strategy, title, yearTokens: yearTokens.length, monthTokens: monthTokens.length });
...
/* ========================================================================== */
/* [KT:SURGICAL GRAPH NOTES] — Universal description injector (improved)       */
/* ========================================================================== */
/*
  Fix (B): "already has a note" now scans until next visual (or 1200 chars),
  allowing wrappers/spacers between the visual and the note.
*/
async function injectVisualDescriptions(html, canon) {
  try {
    ...
    while ((match = re.exec(src)) !== null) {
      ...
      // append pre-frag content + the frag
      parts.push(src.slice(cursor, start));
      parts.push(frag);

      // compute lookahead window up to next visual (or 1200 chars max)
      const afterAll = src.slice(end);
      const nextVisualRel = afterAll.search(/(<figure[^>]*data-chart=|<figure[^>]*data-widget=['"]benchmark['"]|<div[^>]*data-heatmap=)/i);
      const lookEndAbs = nextVisualRel >= 0 ? (end + nextVisualRel) : Math.min(src.length, end + 1200);
      const windowTail = src.slice(end, lookEndAbs);

      // old immediate check (kept as history)
      // const after = src.slice(end, end + 600).replace(/^\s+/, '');
      // const hasNote = /^<p\b[^>]*class=["']chart-note["'][^>]*>[\s\S]*?<\/p>/i.test(after);

      const noteMatch = windowTail.match(/<p\b[^>]*class=["']chart-note["'][^>]*>[\s\S]*?<\/p>/i);
      if (noteMatch) {
        console.log(TAG, 'chart.notes.skipped.existingNote', { distanceChars: noteMatch.index ?? 0 });
        cursor = end;
        continue;
      }

      // otherwise inject a concise note
...
// [KT:SURGICAL] Visuals audit — prove meta for all visuals post-assembly
function logAllVisualsMeta(html) {
  try {
    ...
    for (const frag of all) {
      const m = parseVizMeta(frag);
      items.push({ kind: m.kind, type: m.type, title: m.title, xTitle: m.xTitle, yTitle: m.yTitle, labelsLen: m.labelsLen, dataLen: m.dataLen });
    }
    console.log(TAG, 'chart.audit.summary', { count: items.length, items: items.slice(0, 24) });
  } catch (e) {
    console.log(TAG, 'chart.audit.error', String(e?.message || e));
  }
}
...
export async function POST(req) {
          ...
          try {
            ...
            if (lastFragMatch) {
              ...
              const flags = {
                canvas: /<figure[^>]*data-chart=/.test(lastFragMatch),
                hasTitle: !!(meta.title && meta.title.trim()),
                hasX: !!(meta.xTitle && meta.title?.trim()),
                hasY: !!(meta.yTitle && meta.title?.trim()),
                hasVals: meta.labelsLen > 0 && meta.dataLen > 0,
              };
              console.log(TAG, 'chart.debug.injected', key, {
                kind: meta.kind,
  ...
  // [KT:SURGICAL] Visuals audit — prove meta across ALL visuals
  try { logAllVisualsMeta(html); } catch {}

  // [KT:SURGICAL:RUN-LOCK]
  try {
    logRun();
    html = markRun(html);
  } catch (e) {
  ...
  // Audit kept commented per instruction:
