...
export async function POST(req) {
  ...
  for (const key of order) {
    ...
    try{
      ...
      console.log(TAG, 'section.viz.counts', key, debugCounts);
    }catch(e){ console.log(TAG, 'section.viz.counts.error', key, String(e?.message||e)); }
  }
  ...
  try{
    const pre = { charts: (String(Object.values(sections).join('')).match(/data-chart=/g)||[]).length, heatmaps:(String(Object.values(sections).join('')).match(/data-heatmap=/g)||[]).length, tables:(String(Object.values(sections).join('')).match(/<table\b/gi)||[]).length };
    console.log(TAG,'viz.counts.preTemplate', pre);
  }catch(e){ console.log(TAG,'viz.counts.preTemplate.error', String(e?.message||e)); }
html =
    String(html).replace(
  ...
  // [KT:SURGICAL] Audit disabled to prevent visual removal & reduce runtime
// html = await polishMcKinsey(html, canon);
const sectionsWords = wc(Object.values(sections).join(' '));
  const htmlWords = wc(html);
  try{
    const post = { charts: (String(html).match(/data-chart=/g)||[]).length, heatmaps:(String(html).match(/data-heatmap=/g)||[]).length, tables:(String(html).match(/<table/gi)||[]).length };
    console.log(TAG,'viz.counts.postTemplate', post);
  }catch(e){ console.log(TAG,'viz.counts.postTemplate.error', String(e?.message||e)); }
