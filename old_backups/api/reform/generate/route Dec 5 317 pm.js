...
// NOTE: Temporarily disabled to avoid runtime 500 when mainTranslateHTMLFragment is undefined.
...
// Fallback: always inject the original note (untranslated) to guarantee stability
out += '\n' + note + '\n';
...
${sectionHtml}`.trim();
const safeCostGoal = Number(canon.costSavingsGoal || 0);
...
Cost savings target: ${yCur} ${safeCostGoal.toLocaleString()}
...
async function translateLabels(targetLang) {
  ...
  try {
    ...
    // Return English on any unexpected error
    return DEFAULT_LABELS;
  }
}


// [KT:SURGICAL:CHART-LABEL-TRANSLATOR]
// Minimal helper so KPI + benchmark charts can localize titles/axes
// without touching numbers or structure.
async function translateChartLabels(spec, targetLang) {
  // Fast path: English or empty lang → return spec unchanged
  if (!targetLang || __isEnglishReportLangSoft(targetLang)) {
    return spec;
  }
  ...
  try {
    ...
    // Only send human-readable bits; never send full data arrays
    const payload = {
      title: String(spec.title || ''),
      xTitle: String(spec.xTitle || ''),
      yTitle: String(spec.yTitle || ''),
      // Labels are optional and truncated to keep JSON small/safe
      labels: Array.isArray(spec.labels)
        ? spec.labels.slice(0, 24).map((v) => String(v ?? ''))
        : [],
    };
      ...
      messages: [
        {
          role: 'system',
          content:
            'You translate chart labels for a business report. ' +
            `Target language: ${targetLang}. ` +
            'Return ONLY valid JSON with the SAME KEYS as the input ' +
            '("title", "xTitle", "yTitle", "labels"). ' +
            'Do NOT change numbers or add extra keys. ' +
    ...
    try {
      ...
      return spec; // Safe fallback: keep original English labels
    }

    // Merge back into the original spec, only overriding label strings
    return {
      ...spec,
      title: translated.title || spec.title,
      xTitle: translated.xTitle || spec.xTitle,
      yTitle: translated.yTitle || spec.yTitle,
      labels:
        Array.isArray(spec.labels) &&
        Array.isArray(translated.labels) &&
        translated.labels.length === spec.labels.length
          ? translated.labels
          : spec.labels,
    };
  } catch (err) {
    console.log(TAG, 'chart.labels.translate.error', {
      lang: targetLang,
      error: String(err?.message || err),
    });
    // On any failure, keep the English spec – charts still render
    return spec;
  }
}
