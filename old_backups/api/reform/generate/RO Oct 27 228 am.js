...
/* [KT:PREDICTIVE:parse] accept Oct-22/23 formats like:
   M1: 4, M2: 7, M3: 12 ...
   or {"labels":["M1","M2"],"data":[...]} or raw "4,7,12"
*/
function normalizeLabel(v) {
  const s = String(v || '').trim();
  if (/^m\d+$/i.test(s)) return s.toUpperCase();        // M1
  if (/^y\d+$/i.test(s)) return s.toUpperCase();        // Y1
  return s;
}
...
function extractMSeries(text) {
  // finds "M1: 4" style and returns [{label:'M1', value:4}, ...]
  const out = [];
  const r = /(?:^|\b)(M|Y)(\d+)\s*:\s*([-\d.,]+)/gi;
  let m;
  while ((m = r.exec(text))) out.push({ label: (m[1] + m[2]).toUpperCase(), value: Number(m[3].replace(/,/g,'')) });
  return out;
}
function extractArraySeries(text) {
  // finds "data: 4,7,12" or "[4,7,12]"
  const m = text.match(/(?:data\s*:\s*)?\[?([-\d.,\s]+)\]?/i);
  if (!m) return [];
  return coerceNumbers(m[1].split(/[,\s]+/));
}
function toChartSpec(candidate, { type='line', xTitle='', yTitle='', signature='', title='' } = {}) {
  const labels = (candidate.labels || []).map(normalizeLabel);
  const data = coerceNumbers(candidate.data || []);
  if (!labels.length && candidate.series) {
    // if we only had M1:.. pairs
    const pairs = candidate.series;
    return {
      type, xTitle, yTitle, title,
      labels: pairs.map(p => normalizeLabel(p.label)),
      datasets: [{ data: pairs.map(p => p.value) }],
      signature
    };
  }
  return { type, xTitle, yTitle, title, labels, datasets: [{ data }], signature };
}
/* [KT:PREDICTIVE:build] returns {chosen:[], rejected:[{reason,...}]} */
function buildPredictiveBatchFromLegacy(sectionKey, sectionText, plan) {
  const rejected = [];
  const chosen = [];

  try {
    // 1) Try M1:/Y1: pairs
    const pairs = extractMSeries(sectionText);
    if (pairs.length >= 3) {
      const spec = toChartSpec({ series: pairs }, {
        type: plan?.type === 'bar' ? 'bar' : 'line',
        xTitle: plan?.xTitle || (pairs[0]?.label?.startsWith('Y') ? 'Years' : 'Months'),
        yTitle: plan?.yTitle || 'Index',
        signature: `${sectionKey}:pred:m-series`,
        title: 'Predictive Outlook'
      });
      if (spec.datasets[0].data.length === spec.labels.length) chosen.push(spec);
      else rejected.push({ sig: 'm-series', reason: 'labels/data length mismatch' });
    } else {
      rejected.push({ sig: 'm-series', reason: 'insufficient points' });
    }

    // 2) Try array forms
    const arr = extractArraySeries(sectionText);
    if (arr.length >= 3) {
      const labels = Array.from({length: arr.length}, (_,i)=> (plan?.xTitle === 'Years' ? `Y${i+1}` : `M${i+1}`));
      chosen.push({
        type: plan?.type || 'line',
        xTitle: plan?.xTitle || (plan?.key?.includes('roi') ? 'Years' : 'Months'),
        yTitle: plan?.yTitle || 'Index',
        title: 'Predictive Trend',
        labels,
        datasets: [{ data: arr }],
        signature: `${sectionKey}:pred:array`
      });
    } else {
      rejected.push({ sig: 'array', reason: 'insufficient points' });
    }

    // 3) De-dup by signature
    const sigSeen = new Set();
    const unique = [];
    for (const c of chosen) {
      if (sigSeen.has(c.signature)) continue;
      sigSeen.add(c.signature);
      unique.push(c);
    }
    return { chosen: unique, rejected };
  } catch (e) {
    return { chosen: [], rejected: [{ sig: 'builder', reason: String(e?.message||e) }] };
  }
}
