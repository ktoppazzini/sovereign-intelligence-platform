...
export async function __kt_buildAppendices_FromImplementationKit(reportHTML, canon, sections){
  ...
  try {
    ...
    // Try to extract the predictive model from sections
    let predictiveGraphHTML = '';
    try {
      if (sections && sections.predictiveModel && Array.isArray(sections.predictiveModel.labels) && Array.isArray(sections.predictiveModel.values)) {
        // Render a single predictive chart using the same style as main report graphs
        const chartSpec = {
          type: 'line',
          title: sections.predictiveModel.title || 'Predictive Analytics Forecast',
          xTitle: sections.predictiveModel.xTitle || 'Time',
          yTitle: sections.predictiveModel.yTitle || 'Value',
          labels: sections.predictiveModel.labels,
          datasets: [{
            label: 'Predicted',
            data: sections.predictiveModel.values
          }]
        };
        predictiveGraphHTML = `<figure data-chart="${toAttrJSON(chartSpec)}" class="appendix-graph"></figure>`;
      }
    } catch (err) {
      console.warn('[KT] predictive appendix graph error', err);
    }
