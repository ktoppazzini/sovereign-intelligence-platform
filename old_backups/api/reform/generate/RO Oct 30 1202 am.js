...
/* ============================================================================
   [KT:SURGICAL][2025-10-29_23-00-38_EDT] — Appendices restoration (Implementation Kit JSON, Oct 10 5:15)
   - No deletions; legacy appendix code is preserved. This block adds a safe builder.
   - Uses the Oct 10 5:15 "Implementation Kit JSON" prompt & shape as guidance.
============================================================================ */
export async function __kt_buildAppendices_FromImplementationKit(reportHTML, canon, sections){ try{
  // Extract minimal plain text per section for context
  const plain = s => String(s||'').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').slice(0, 8000);
  const ctx = {
    exec: plain(sections.exec), current: plain(sections.current), financials: plain(sections.financials),
    kpis: plain(sections.kpis), timeline: plain(sections.timeline), ops: plain(sections.ops),
    risk: plain(sections.risk), roi: plain(sections.roi), conclusion: plain(sections.conclusion||'')
  };

  // Oct 10 5:15 declaration (comment-only for traceability)
...
function quoteSafeChartAttrs(ht
     --- END OCT-10-5:15 SOURCE --- */

  // Build JSON kit (placeholder logic; actual AI call occurs upstream if available)
  let kit = (sections && sections.__implementationKitJSON) || { "charters": [], "raci": {"items":[]}, "raid":{"items":[]},
    "benefits":{"lines":[]}, "plan100":{"weeks":[]}, "pilot":{"name":"", "locations":0, "successKPIs":[], "thresholds":[], "sampleDesign":"", "rollbackCriteria":""},
    "assumptions":{"items":[]}, "methods":{"benchmarks":[], "sources":[]} };

  // Render to HTML Appendix section
  const section = [
    ...
    '  <div class="card">',
    '    <h4>Embedded JSON</h4>',
    '    <pre class="json-block">', JSON.stringify(kit, null, 2).replace(/[<]/g,'&lt;'), '</pre>',
    '  </div>',
    '</section>'
  ].join('\n');

  // Also embed raw JSON for programmatic export
  const embed = '<script type="application/json" id="implementation-kit-json">' + JSON.stringify(kit) + '</script>';
  return section + "\n" + embed;
}catch(e){ console.warn('KT appendices error', e); return '<section id="appendices"><h2>Appendices</h2><p>Appendix generation failed gracefully.</p></section>'; }}

// Wire-up: export a post-finalizer that runs template postprocess and appendix builder if available
export async function __kt_finalize_report(html, canon, sections){ try{
  let out = String(html||'');
  try { if (typeof __kt_postprocess_report_html === 'function') out = __kt_postprocess_report_html(out); } catch(_){}
