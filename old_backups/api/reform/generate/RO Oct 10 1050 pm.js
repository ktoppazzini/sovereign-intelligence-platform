...
function toAttrJSON_RAW(obj) {
  // [KT-REMOVED] // [KT-REMOVED]
  // [KT-REMOVED] // [KT-REMOVED]  try {
  // [KT-REMOVED] // [KT-REMOVED]    const json = JSON.stringify(obj);
  // [KT-REMOVED] // [KT-REMOVED]    // keep JSON as a single line and escape apostrophes for single-quoted attributes
  // [KT-REMOVED] // [KT-REMOVED]    return json
  // [KT-REMOVED] // [KT-REMOVED]      .replace(/'/g, '&#39;')
  // [KT-REMOVED] // [KT-REMOVED]      .replace(/\r\n|\n|\r/g, ' ');
  // [KT-REMOVED] // [KT-REMOVED]
...
/* [KT-COMMENTED-OUT] prior predictiveDescription (heavy) for traceability
...
const timeout = new Promise((_, rej)=>setTimeout(()=>rej(new Error('audit-timeout')), 90_000));
const res = await Promise.race([auditCall, timeout]);const p = (res.choices?.[0]?.message?.content || '').trim();
    return /^<p/i.test(p) ? p : `<p class="chart-note">${p}</p>`;
  }catch{ return ''; }
}

    */
/* [KT-FIX] predictiveDescription — small note generator (Nano 5, temp=1) */
async function predictiveDescription(section, canon, meta) {
  const openai = await getOpenAI();
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: 'user',
          content: `Explain "${meta.title}" (X:${meta.xTitle} / Y:${meta.yTitle}) for ${canon.orgName}. Give 2 insights and 1 action.`,
        },
      ],
    });
...
`.trim();
  ...
  // [KT-REMOVED]       temperature: 1,
  // [KT-REMOVED]       messages: [
  // [KT-REMOVED]         { role:'system', content:'Return JSON only with keys "applied","notes","html". Obey all constraints.' },
  // [KT-REMOVED]         { role:'user',   content: prompt + `\n\n--- HTML ---\n${ frozen }` }
  // [KT-REMOVED]       ]
  // [KT-REMOVED]     });
  // [KT-REMOVED]
  try {
    // [KT-SPEED] timebox audit to 90s
    const auditCall = openai.chat.completions.create({
      model: MODEL,
      temperature: 1,
      messages: [
