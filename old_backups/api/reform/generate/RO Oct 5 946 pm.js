...
// ---- currency units for yTitle hints (T4 currency correctness) ----
...
/* ========================================================================== */
/* [KT:VIZ] detection + extraction helpers (restored)                          */
/* ========================================================================== */
...
/* ========================================================================== */
/* [KT:T4] Guidance for numeric, standardized visuals                          */
/* ========================================================================== */
...
/* ========================================================================== */
/* [KT:ITER] Iterative predictive strategies                                   */
/* ========================================================================== */

// Candidate predictive strategies to try in order
const PREDICTIVE_STRATEGIES = [
...
function predictiveStrategyPrompt(section, canon, sectionHtml, strategy) {
  ...
  // strategy-specific shape requirements
  const shape =
...
- Titles must include the section name.
...
// Try strategies in order until one yields a valid fragment
async function tryPredictiveStrategies(section, canon, sectionHtml) {
  for (const strat of PREDICTIVE_STRATEGIES) {
    console.log(TAG, 'predictive.try', section, { strategy: strat.id, type: strat.type });
    try {
      ...
      if (frag) {
        console.log(TAG, 'predictive.hit', section, { strategy: strat.id, len: frag.length });
        return frag;
      }
      console.log(TAG, 'predictive.miss', section, { strategy: strat.id });
    } catch (e) {
  ...
  console.log(TAG, 'predictive.fail', section); // <-- explicit failure log requested
  return '';
}

/* ========================================================================== */
/* [KT:T4] mining / derived / advice (kept helpers; may be disabled below)     */
/* ========================================================================== */
...
async function addDerivedVizIfNeeded(section, canon) {
  ...
  // simple defensible derivation from costSavingsGoal across horizon
...
/* ========================================================================== */
/* [KT:T3] Text prompts with strict grouping                                   */
/* ========================================================================== */
...
export async function POST(req) {
  ...
  // [KT:T3] lock total per-section floors to 20,000 and keep growth in-place
  ...
  for (const key of order) {
    ...
    // [KT:ITER] NEW: iterative predictive analytics
    if (key !== 'exec') {
      if (!preHas) {
        const iterFrag = await tryPredictiveStrategies(key, canon, html);
        if (iterFrag) {
          html += '\n' + iterFrag;
          console.log(TAG, 'section.vizInjected', key, { mode: 'iterative' });
        } else {
          // As requested: if strategies fail, log and insert NOTHING
          console.log(TAG, 'section.viz.predictive.failNoInsert', key);
        }
      } else {
        console.log(TAG, 'section.viz.skip', key, { reason: 'already-has-fragment' });
      }
    } else {
      console.log(TAG, 'section.viz.skip', key, { reason: 'exec' });
    }

    // [KT:ITER:DISABLED] The older mined/derived/advice path kept here but commented to avoid double-injection.
    /*
    if (key !== 'exec' && !preHas) {
      let mined = '';
      try {
        mined = await addVizIfData(key, canon, html);
        console.log(TAG, 'section.viz.mined.result', key, { found: !!mined, len: mined ? mined.length : 0 });
      } catch (e) {
        console.log(TAG, 'section.viz.mined.error', key, String(e?.message || e));
      }

      if (mined) {
        html += '\n' + mined;
        console.log(TAG, 'section.vizInjected', key, { mode: 'mined' });
      } else {
        let derived = '';
        try {
          derived = await addDerivedVizIfNeeded(key, canon, html);
          console.log(TAG, 'section.viz.derived.result', key, { found: !!derived, len: derived ? derived.length : 0 });
        } catch (e) {
          console.log(TAG, 'section.viz.derived.error', key, String(e?.message || e));
        }

        if (derived) {
          html += '\n' + derived;
          console.log(TAG, 'section.vizInjected', key, { mode: 'derived' });
        } else {
          console.log(TAG, 'section.viz.none', key, { reason: 'no-mined-and-no-derived' });
          let advice = '';
          try {
            advice = await addDataAdviceParagraph(key, canon, html);
            console.log(TAG, 'section.viz.advice.result', key, { found: !!advice, len: advice ? advice.length : 0 });
          } catch (e) {
            console.log(TAG, 'section.viz.advice.error', key, String(e?.message || e));
          }
          if (advice) {
            html += '\n' + advice;
    ...
    */
    ...
    // [KT:ITER:DISABLED] The compact predictive text block (not charts) left disabled per your instruction.
    /*
    if (key !== 'exec') {
      let pred = '';
      try {
        console.log(TAG, 'predictive.start', key, { tf: String(canon.timeFrame||'') });
        const res = await openai.chat.completions.create({
          model: MODEL,
          messages: [
            { role:'system', content:'Return ONE compact predictive HTML block (<p> and/or a tiny <table>). No extra text.' },
            { role:'user',   content: `...` },
          ],
        });
        pred = (res.choices?.[0]?.message?.content || '').trim();
        console.log(TAG, 'predictive.ok', key, { len: pred.length });
      } catch (e) {
        console.log(TAG, 'predictive.error', key, String(e?.message || e));
      }
      if (pred) {
        html += '\n' + pred;
        console.log(TAG, 'section.predictiveInjected', key);
      } else {
        console.log(TAG, 'section.predictive.none', key);
      }
    }
    */
  ...
  // [KT:T3:DISABLED] global “balancing” loop left in file for auditability, but disabled to avoid drift.
  /*
  for (let pass = 0; pass < 2; pass++) {
    // (previous cross-section top-up logic was here; disabled per corrective action)
  }
  */
