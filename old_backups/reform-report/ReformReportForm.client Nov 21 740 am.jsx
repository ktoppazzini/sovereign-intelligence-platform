...
export default function ReformReportForm({ setGenDoneExternal, setGeneratedHtmlExternal }) {
  ...
  const { t: ui, rtl } = useTranslation(BASE_UI, langResolved);
  ...
  async function onExportPdfServer() {
    ...
    try {
      ...
      if (j.error) throw new Error(j.error || `HTTP ${res.status}`);
  ...
  async function onFinalize() {
    ...
    try {
      ...
      if (j?.error || j?.ok === false) throw new Error(j?.error || `HTTP ${res.status}`);
