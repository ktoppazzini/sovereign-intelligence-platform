...
async function getOrGenerateChartSummary(chartSpec, lang, context = '') {
  ...
  const summary = await generateAIChartSummary(chartSpec, lang, context);
  if (summary) {
...
${reportContext}
` : attempt === 2 ? `
Extract 5-8 risks from this report. Return JSON only:
{"risks": [{"name": "...", "description": "...", "severity": 1-5, "likelihood": 1-5}]}

Look for: challenges, concerns, obstacles, issues, threats, barriers mentioned.

Report:
${reportContext.slice(0, 4000)}
` : `
List business risks from this text as JSON array:
...
async function __kt_generateReportTitle(canon = {}, sections = {}) {
  try {
    ...
    const resp = await openai.chat.completions.create({
      ...
      temperature: 1,
      max_completion_tokens: 2000,  // [KT:FIX-TOKEN-LIMIT] Increased from 32 for GPT-5-nano reasoning tokens
      messages: [
        {
          role: 'system',
          content:
    ...
    if (!title) {
      ...
      try {
        const fallbackResp = await openai.chat.completions.create({
          model: MODEL,
          temperature: 1,
          max_completion_tokens: 2000,  // [KT:FIX-TOKEN-LIMIT] Increased from 40 for GPT-5-nano reasoning tokens
          messages: [
...
async function translateLabels(targetLang) {
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_completion_tokens: 64000,  // [KT:FIX-TOKEN-LIMIT] Increased from 32000 for GPT-5-nano reasoning tokens
    });
