'use client';
// import { getUiTranslations, normalizeLang } from '@/lib/i18nClient'; // [KT:SURGICAL:I18N-LEGACY]
...
// import { useTranslation } from "../hooks/useTranslation";  // adjust ../ as needed

// Add normalizeLang utility (simple implementation)
...
const BASE_UI = {
  ...
  pdfDownloaded: 'PDF downloaded successfully!', // [KT:WORKFLOW-FIX] Add download success text
  submitShort: 'Submit',
  submitting: 'Submitting…', // [KT:WORKFLOW-FIX] Add submitting text

  // [KT:SURGICAL:I18N-CHART-TYPES] Chart-type labels for dynamically injected dropdowns
  chartTypeLabel: 'Type',
  chartTypeLine: 'Line',
  chartTypeBar: 'Bar',
  chartTypePie: 'Pie',
...
async function translateMapClient(map, targetLang) {
  ...
  const payload = {
    mode: 'json',
    map: clonePlainObject(map),
    ui: clonePlainObject(map), // 🔥 REQUIRED – this is what GPT expects
    targetLang,
  };
...
export default function ReformReportForm({ setGenDoneExternal, setGeneratedHtmlExternal }) {
  ...
  const { t, rtl, loading } = useTranslation(BASE_UI, langParam);
  // const [activeLang, setActiveLang] = useState(detectLanguage());
  const [activeLang, setActiveLang] = useState(langParam || detectLanguage());
  // Fast Mode toggle state
  const [fastMode, setFastMode] = useState(false);
  const langResolved = useMemo(
    // () => normalizeLang(activeLang || 'English'),
    () => normalizeLang(langParam || activeLang || 'English'),
    [langParam, activeLang],
  );

  // [KT:SURGICAL:I18N-HOOK-APPLIED] ReformReportForm now uses shared Nano-based translation.
  const ui = t;

  // [KT:SURGICAL:I18N-LEGACY-STATE]
  // const [ui, setUi] = useState(BASE_UI);
  // const [loadingUI, setLoadingUI] = useState(true);
  const loadingUI = false; // hook handles translation; keep flag shape for aria-busy
  ...
  const [downloadDone, setDownloadDone] = useState(false); // [KT:WORKFLOW-FIX] Track download completion
  ...
  useEffect(() => {
    ...
    setActiveLang(langParam || detectLanguage());
...
// [KT:SURGICAL:I18N-DROPDOWNS] Identity map for dropdowns – no GPT calls.
/*async function translateList(values, langTarget, cacheKey) {
  const vals = uniq(values);
  if (!vals.length) return {};

  // Just echo the original labels back
  const map = Object.fromEntries(vals.map((v) => [v, v]));

  // Keep the cache write so callers depending on it don’t break
  try {
    if (cacheKey) {
      localStorage.setItem(cacheKey, JSON.stringify(map));
    }
  } catch (e) {
    dbgWarn('translateList.cache.error', {
      cacheKey,
...
// [KT:SURGICAL:I18N-DROPDOWNS:GPT]
// Translate dropdown lists via /api/gptTranslation (JSON mode) with caching.
// This will translate Country / Tier / Company Size / Time Frame option text.
async function translateList(values, langTarget, cacheKey) {
  const vals = uniq(values);
  if (!vals.length) return {};

  const langNorm = String(langTarget || 'English').toLowerCase();

  // English → identity, no GPT cost
  if (langNorm === 'english' || langNorm === 'en') {
    const idMap = Object.fromEntries(vals.map((v) => [v, v]));
    try {
      if (cacheKey) localStorage.setItem(cacheKey, JSON.stringify(idMap));
    } catch (e) {
      dbgWarn('translateList.cache.write.en.error', {
        cacheKey,
        message: String(e?.message || e),
      });
    }
  ...
  if (cacheKey) {
    try {
      ...
      if (raw) {
        ...
        // Only use cache if it covers all current values
        if (parsed && vals.every((v) => parsed[v])) {
          dbg('[opts] cache hit →', cacheKey);
          return parsed;
        }
      }
    } catch (e) {
      dbgWarn('translateList.cache.read.error', {
        cacheKey,
        message: String(e?.message || e),
      });
    }
  ...
  // Build base map for GPT
  const baseMap = Object.fromEntries(vals.map((v) => [v, v]));

  let translated = baseMap;
  try {
    // Uses translateMapClient above → /api/gptTranslation JSON mode (Nano 5)
    translated = await translateMapClient(baseMap, langTarget);
  } catch (e) {
    dbgWarn('translateList.gpt.error', {
      langTarget,
      message: String(e?.message || e),
    });
  }
  ...
  // Cache for this lang + field
  if (cacheKey) {
    try {
      localStorage.setItem(cacheKey, JSON.stringify(finalMap));
    } catch (e) {
      dbgWarn('translateList.cache.write.error', {
        cacheKey,
        message: String(e?.message || e),
      });
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(`Failed to load options: HTTP ${res.status}`);
        }
        let j = {};
        try {
          j = await res.json();
        } catch (jsonErr) {
          dbgErr('options → invalid JSON', { message: String(jsonErr?.message || jsonErr) });
          j = {};
        }
        if (cancelled) return;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          dbgErr('generate → invalid JSON', { message: String(parseErr?.message || parseErr) });
          j = null;
        }
        ...
        if (j.rid || j.report || j.wordCount) {
          ...
          if (j.report) {
            ...
            if (typeof window !== 'undefined') {
              // [KT:PDF-EXPORT-FIX] Store HTML in window for PdfExportWire to access
              window.__REFORM_REPORT_HTML__ = j.report;
        ...
        if (j.recordId || j.html || j.status) {
          ...
          if (j.html) {
            ...
            if (typeof window !== 'undefined') {
              // [KT:PDF-EXPORT-FIX] Store HTML in window for PdfExportWire to access
              window.__REFORM_REPORT_HTML__ = j.html;
              window.dispatchEvent(
                new CustomEvent('si-reform-generated', {
  ...
  async function onExportPdfServer() {
    ...
    try {
      ...
      const { max_tokens, ...formWithoutMaxTokens } = form;
      const exportPayload = {
        html: generatedHtml,
        lang: langResolved,
        recordId: recordId || rid || null,
        orgName: formWithoutMaxTokens.orgName,
        country: formWithoutMaxTokens.country,
        tier: formWithoutMaxTokens.tier,
        companySize: formWithoutMaxTokens.companySize,
        timeFrame: formWithoutMaxTokens.timeFrame,
        costSavingsGoal: formWithoutMaxTokens.costSavingsGoal,
        strategicGoal: formWithoutMaxTokens.strategicGoal,
        desiredOutcome: formWithoutMaxTokens.desiredOutcome,
        preparedFor: formWithoutMaxTokens.preparedFor,
        preparedForEmail: formWithoutMaxTokens.preparedForEmail,
        preparedBy: formWithoutMaxTokens.preparedBy,
        preparedByEmail: formWithoutMaxTokens.preparedByEmail,
        // add other fields as needed
        // add other fields as needed
      };

      // Remove unsupported parameters if present
      // (No max_tokens or similar should be sent)
      ...
      // [KT:PDF-EXPORT-FIX] The API returns binary PDF, not JSON with URL
      // Check content type to determine response format
      const contentType = res.headers.get('content-type') || '';
      
      if (contentType.includes('application/pdf')) {
        // Binary PDF response - download directly
        dbg('export → received PDF binary', { rid: ridExp, status: res.status, ms: Math.round(now() - t0) });
        const blob = await res.blob();
        console.log('[PDF-DOWNLOAD] Blob received:', { size: blob.size, type: blob.type });
        
        // Verify blob size matches expected PDF size
        if (blob.size < 100000) {
          console.warn('[PDF-DOWNLOAD] WARNING: Blob is suspiciously small:', blob.size, 'bytes');
        }
        
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `Reform_Report_${form.orgName || 'export'}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        setTimeout(() => URL.revokeObjectURL(url), 2000);
        
        setSubmitMsg('PDF downloaded successfully!');
        setExportDone(true);
      } else {
        // JSON response with URL (legacy/alternative format)
        const raw = await readSafeText(res);
        let j = {};
        try {
          j = safeParseJSON(raw) || {};
        } catch (parseErr) {
          dbgErr('export → invalid JSON', { message: String(parseErr?.message || parseErr) });
          j = {};
        }
        ...
        if (j.error && typeof j.error === 'object') {
          const errMsg = j.error.message || j.error.code || j.error.type || 'Export failed';
          throw new Error(errMsg);
        }
      ...
      document.querySelectorAll('canvas[id]').forEach((canvas) => {
        ...
        try {
          ...
          if (parent && !parent.querySelector('.chart-type-select')) {
            ...
            // [KT:SURGICAL:I18N-CHART-DROPDOWN] Text is now translated; values stay canonical
            ['line', 'bar', 'pie'].forEach((type) => {
              const opt = document.createElement('option');
              opt.value = type;
              // opt.text = type;
              let label = type;
              if (type === 'line') label = ui.chartTypeLine || 'Line';
              else if (type === 'bar') label = ui.chartTypeBar || 'Bar';
              else if (type === 'pie') label = ui.chartTypePie || 'Pie';
              opt.text = label;
  ...
  }, [generatedHtml, langResolved]);

  // Derived flags
  const hasHtml = !!generatedHtml;
  const hasRecord = !!(recordId || rid); // used for Finalize/Submit

  // [KT:WORKFLOW-FIX] Finalize handler - calls the finalize API
  async function onFinalize() {
    setFinalizing(true);
    setFinalizeDone(false);
    setSubmitMsg('');
    try {
      const finalizeRecordId = recordId || rid;
      
      // Finalize endpoint needs HTML, not just recordId
      const res = await fetch('/api/reform/finalize', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          html: generatedHtml || '',
          rid: finalizeRecordId || Math.random().toString(36).slice(2, 8),
        }),
      });
      
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.error || `Finalize failed (${res.status})`);
      }
      ...
      setSubmitMsg('Report finalized successfully!');
    } catch (e) {
      dbgErr('finalize → error', { message: String(e?.message || e) });
      // Still mark as done to allow workflow to continue
      setFinalizeDone(true);
      setSubmitMsg('Finalized (with warnings)');
    } finally {
      setFinalizing(false);
    }
  }

  // [KT:WORKFLOW-FIX] Unified download click: use server PDF if present; else fallback
  ...
  function onSubmit(e) {
    e.preventDefault();
    // You can add your submit logic here, e.g. call onGenerate or onFinalize
    // For now, let's just call onGenerate as a default action
    onGenerate();
  }
        ...
        {/* Left column */}
          ...
          <div className={styles.actions}>
            ...
            <button
              ...
              disabled={!exportDone && !hasHtml}
              aria-label={`3 - ${ui.downloadPdf}`}
              className={styles.navButton}
              style={{
                ...(downloadDone ? { ...navBtnBase, ...navBtnDone } : navBtnBase),
                ...(!exportDone && !hasHtml ? navBtnDisabled : null),
              }}
              onMouseEnter={(e) => {
                if (!downloadDone && (exportDone || hasHtml))
                  e.currentTarget.style.backgroundColor = pillHover;
              }}
              onMouseLeave={(e) => {
                if (!downloadDone && (exportDone || hasHtml))
            ...
            >
              {downloadDone ? `3- ${ui.pdfDownloaded || 'PDF downloaded successfully!'}` : `3- ${ui.downloadPdf}`}
            </button>
          ...
          {/* Result preview box - removed duplicate/narrow report rendering */}
        </form>
