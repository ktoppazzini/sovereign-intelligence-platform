...
export function riskHeatmapHTML(risks = []) {
  ...
  // Deduplicate and use already-translated names/descriptions from route.js
  const uniqueRisks = [];
  const seen = new Set();
  for (const r of risks) {
    if (!seen.has(r.name)) {
  ...
  // Risk names and descriptions are already translated by buildRiskHeatmapSpec in route.js
  // No further translation needed - just ensure they're properly formatted
          ...
          .map(
            (r) =>
              `<li style="margin-bottom:4px;"><span style="font-weight:bold;">${esc(
                ...
                r.descriptionTranslated || r.description
                  ? esc(r.descriptionTranslated || r.description)
 ...
 * ========================================================= */
export function waterfallChartHTML(components = [], currency = "$", aiSummary = null) {
  ...
  console.log(
    ...
    currency,
    "aiSummary:",
    aiSummary ? 'provided' : 'not provided'
  );
  ...
  function chartSummaryRowValueBridge({ components, currency, aiSummary }) {
    // [KT:AI-SUMMARY] Use AI-generated summary if provided
    if (aiSummary && aiSummary.overall && aiSummary.keyInsights) {
      console.log('[KT:AI-SUMMARY] Using AI-generated summary for waterfall chart');
      return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
  ...
  // Chart type dropdown (translated for all languages dynamically)
  // [KT:FIX] Use uppercase keys to match GPT translation dictionary (Type, Line, Bar, etc.)
  const typeDropdown = `
  ...
  const barElements = bars
    .map((bar, i) => {
      ...
      return `
        ...
        x + barWidth / 2
      } ${h - p + 32})">${esc(bar.nameTranslated || t(bar.name) || bar.name)}</text>
        ${connector}
      `;
    })
    .join("");
  ...
  const xLabels = components.map((c, i) => {
    ...
    return `<text x="${x}" y="${h - p + 8}" text-anchor="middle" fill="#FFFFFF" font-size="11" transform="rotate(-45 ${x} ${
      h - p + 8
    })">${esc(c.nameTranslated || t(c.name) || c.name || `T${i + 1}`)}</text>`;
  });
  ...
  return `
    <figure class="chart-card waterfall-card" data-type="line" data-widget="series">
      ${chartSummaryRowValueBridge({ components, currency, aiSummary })}
      <header class="chart-head">
...
// Export missing functions for universal-industry-plan
export function renderWaterfallChart(data) {
  // Simplified waterfall for compatibility
  return `<div style="padding: 20px; background: #1a1a2e; color: #fff;">Waterfall Chart: ${JSON.stringify(data)}</div>`;
}

export function renderLineChart(data) {
  // Simplified line chart for compatibility
  return `<div style="padding: 20px; background: #1a1a2e; color: #fff;">Line Chart: ${JSON.stringify(data)}</div>`;
}

export function renderBarChart(data) {
  // Simplified bar chart for compatibility
  return `<div style="padding: 20px; background: #1a1a2e; color: #fff;">Bar Chart: ${JSON.stringify(data)}</div>`;
}

export function renderPieChart(data) {
  // Simplified pie chart for compatibility
  return `<div style="padding: 20px; background: #1a1a2e; color: #fff;">Pie Chart: ${JSON.stringify(data)}</div>`;
}
