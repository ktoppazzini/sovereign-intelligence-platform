...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]], lang = 'English' }) {
  ...
  const xlab = c.map((label, i) => `<text x="${leftPadding + (i + .5) * cw}" y="${h - 6}" text-anchor="middle" fill="#FFFFFF" font-size="18" font-weight="700">${esc(t(label))}</text>`).join('');
  const ylab = r.map((label, i) => `<text x="${leftPadding - 8}" y="${p + (i + .5) * ch + 6}" text-anchor="end" fill="#FFFFFF" font-size="22" font-weight="700">${esc(t(label))}</text>`).join('');
