  ...
  //try{
   ...
   // let sample = [];
    ...
    //while((m = re.exec(src))!==null){
      //scanned++;
      //console.log('[SR:REFORM] std.charts.upgrade', { title, type, xTitle, yTitle });
     ...
     // }
//console.log('[SR:REFORM] std.charts.skip', { reason: 'already-tagged', open: open.slice(0,120) });
   ...
   // out += src.slice(last);
   //console.log('[SR:REFORM] std.charts.upgrade', { title, type, xTitle, yTitle });
// [KT:S] Safe footer log (kept for audit; values emitted per-upgrade inside loop)
...
//console.log('[SR:REFORM] std.charts.tagged', { scanned, upgraded, sample });
 ...
 // }catch(e){
  ...
  //}
//}
/* [KT:SURGICAL:STANDARD-DESC:A — FIXED, no out-of-scope log] */
function __kt_tagStandardCharts(html = '', labels = [], canon = {}) {
  try {
    const src = String(html || '');
    const re  = /<figure\b([^>]*class=["'][^"']*chart-card[^"']*["'][^>]*)>([\s\S]*?)<\/figure>/gi;
    ...
    while ((m = re.exec(src)) !== null) {
      ...
      // already tagged (data-*)? keep as-is
      if (/(data-chart|data-widget\s*=\s*["']benchmark["']|data-heatmap)/i.test(open)) {
      ...
      // derive minimal spec from heading + data-type
      const h3 = (inner.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i) || [,''])[1];
      const title  = h3.replace(/<[^>]*>/g, '').trim() || 'Chart';
      const type   = ((open.match(/data-type=["']([^"']+)["']/i) || [,'line'])[1]).toLowerCase();
      const xTitle = labels[0]?.startsWith('Y') ? 'Years' : 'Months';
      const yTitle = /percent|%/i.test(title) ? 'Percent'
                   : /payback/i.test(title) ? 'Cumulative'
                   : /savings|revenue|spend|capex|opex/i.test(title) ? 'Value'
...
+ `>
  ...
  /* [KT:SURGICAL:KPI FULL-BLEED] */
:root { --page-hpad: .45in; }
.kpi-dash {
  margin-left: calc(-1 * var(--page-hpad));
  margin-right: calc(-1 * var(--page-hpad));
}

/* make the four KPI cards taller & more legible on desktop */
.kpi-dash .chart-card { min-height: 280px; }
@media (min-width: 1100px){
  .kpi-grid{ grid-template-columns: 1fr 1fr; }
  .kpi-dash .chart-card { min-height: 360px; }
}
