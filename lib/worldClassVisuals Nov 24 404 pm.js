...
export function setWorldClassVisualsTranslator(fn) {
  if (typeof fn === 'function') {
    ...
    console.warn('setWorldClassVisualsTranslator: Provided argument is not a function. Translator not updated.');
  }
}
// Ensure all strings are translated
function ensureTranslation(strings) {
  if (!Array.isArray(strings)) {
    console.warn('ensureTranslation: Expected an array, received:', strings || 'undefined or null');
    return [];
  }
  return strings.filter((s) => typeof s === 'string').map((s) => _translator(s));
}
// Define a default translator function
const mainTranslatorFn = (s) => (typeof s === 'string' ? s : ''); // Ensure it handles undefined, null, or non-string values gracefully
setWorldClassVisualsTranslator(mainTranslatorFn); // Set as the default translator

const t = (s) => _translator(typeof s === 'string' ? s : '');

const esc = (str) =>
  String(typeof str === 'string' ? str : '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
 ...
 * All user-facing strings are wrapped in t() for translation/localization.
 */
export function visualTimelineHTML(phases = []) {
  if (!phases || phases.length === 0) {
    phases = [
      { name: t('Discovery'), startWeek: 0, endWeek: 4, color: '#3B82F6', milestones: [{week: 2, title: t('Baseline')}, {week: 4, title: t('Analysis Complete')}], deliverables: [t('Current State'), t('Gap Analysis')] },
      { name: t('Design'), startWeek: 4, endWeek: 8, color: '#10B981', milestones: [{week: 6, title: t('Blueprint')}, {week: 8, title: t('Sign-off')}], deliverables: [t('Target Operating Model'), t('Implementation Plan')] },
      { name: t('Pilot'), startWeek: 8, endWeek: 12, color: '#F59E0B', milestones: [{week: 10, title: t('Launch')}, {week: 12, title: t('Review')}], deliverables: [t('Pilot Results'), t('Lessons Learned')] },
      { name: t('Scale'), startWeek: 12, endWeek: 24, color: '#10B981', milestones: [{week: 16, title: t('Phase 1')}, {week: 20, title: t('Phase 2')}, {week: 24, title: t('Full Rollout')}], deliverables: [t('Training Complete'), t('Go-Live'), t('Benefits Tracking')] }
    ];
  }
    // [KT:SURGICAL] Removed stray 'o' for clarity
  ...
  const riskList = `
    <div class="risk-list" style="margin-top:16px; color:#fff;">
      ...
      <ol style="margin:8px 0 0 20px; padding:0;">
        ${uniqueRisks.map((r) => `<li style="margin-bottom:4px;"><span style="font-weight:bold;">${esc(r.name)}</span>: ${r.description ? esc(r.description) : t('No description provided.')}</li>`).join('')}
      </ol>
    </div>
  `;
  return `
...
export function waterfallChartHTML(components = [], currency = '$') {
  ...
  const cur = t((currency && String(currency).trim()) || '$');
  // helper used for y-ticks and labels
  const fmt = (n) => `${t(cur)}${Number(n || 0).toLocaleString()}`;
  if (typeof process !== 'undefined' && process.env && process.env.NODE_ENV === 'development') {
