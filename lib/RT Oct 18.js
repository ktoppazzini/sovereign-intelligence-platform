...
// Adds a tiny JSON writer (for HTML attributes) + a transformer that tags
// standard <figure class="chart-card"> visuals so /api/reform/generate can
// inject executive notes under them exactly like predictive visuals.
...
/**
 * Tag any standard chart-card figures that are missing data-* spec.
 * We infer a minimal spec (type, title, axes, labels length) so the
 * description injector can generate correct, concise analysis.
 */
function tagStandardVisualsWithDataAttrs(html='', labels=[], canon={}){
  try{
    ...
    while((m = re.exec(src))!==null){
      ...
      // If this figure already has a predictive spec, pass through.
      if (/(data-chart|data-widget\s*=\s*["']benchmark["'])/i.test(open)) {
        out += src.slice(last, end); last = end; continue;
      }

      // Extract a title from the <h3> if present.
      let title = ((inner.match(/<h3[^>]*>([\s\S]*?)<\/h3>/i)||[])[1]||'Chart')
                  .replace(/<[^>]*>/g,'').trim() || 'Chart';

      // Use the card's data-type="line|bar|pie|doughnut" if present.
      const type = ((open.match(/data-type=["']([^"']+)["']/i)||[])[1]||'line').toLowerCase();

      // X/Y hints (match your template behavior)
      const xTitle = (labels[0] && labels[0].startsWith('Y')) ? 'Years' : 'Months';
      let yTitle = /savings|payback|roi/i.test(title) ? ( /roi/i.test(title) ? 'ROI Index' :
                     /payback/i.test(title) ? 'Cumulative' : 'Savings' )
                   : /percent|%/i.test(title) ? 'Percent'
                   : 'Value';

      // We don’t try to reverse-engineer series from SVG; just provide a
      // sane placeholder so the meta has lengths and axis names.
      const specObj = {
        type, title, xTitle, yTitle,
        labels, datasets: [{ label: title, data: Array.from({length:labels.length}, (_,i)=>i+1) }]
      };

      const annotatedOpen =
        open.replace(/^/,'') + " data-chart='" + toAttrJSON(specObj) + "' data-origin=\"standard\"";

      out += src.slice(last, start)
           + '<figure' + annotatedOpen + '>' + inner + '</figure>';
      last = end;
    }
    out += src.slice(last);
    return out;
...
// [KT:SURGICAL:STANDARD-DESC:A] — tag standard charts so they receive exec notes
function __kt_toAttrJSON(obj){
  try { return JSON.stringify(obj).replace(/"/g, '&quot;'); } catch { return '{}'; }
}

/**
 * Finds <figure class="chart-card" ...>…</figure> (line/bar/pie/doughnut)
 * that do NOT already have data-chart / data-widget / data-heatmap,
 * and adds a minimal data-chart spec so the downstream description injector
 * can attach a <p class="chart-note"> under them.
 */
...
export default function buildReformReportHTML(opts) {
  ...
  const prep = (s) => {
    ...
    // [KT:SURGICAL:STANDARD-DESC:B] — tag standard charts so they get descriptions
    let postHTML = withWidgets; // (mutable successor of const)
    try {
      postHTML = __kt_tagStandardCharts(postHTML, labels, canon);
      console.log('[SR:REFORM] std.charts.tagged');
    } catch(e) {
      console.log('[SR:REFORM] std.charts.tag.error', String(e?.message||e));
    }

    // [KT:SURGICAL:WIDTH-GUARD] — moved to final assembly to avoid const reassign
    // html += `\n<style>.report{max-width:100% !important;width:100% !important}</style>`;
    // console.log('[SR:REFORM] width.guard.injected@template');
  ...
  `;
// [KT:SURGICAL:STANDARD-DESC:B] — run after widgets/tables are in place
