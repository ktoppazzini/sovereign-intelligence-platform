...
.table thKey Takeaways{background:#0E2A44;text-align:left}
...
function resourceRequirementsHTML(model = {}) {
  ...
  <section class="appendix">
    ...
    <table class= <table class="table small">
      <thead>
        <tr>
          <th>Resource</th>
          <th>Role</th>
...
function benefitsHTML(model) {
  // [KT:SURGICAL:APPENDIX-D-CHART-BLOCK-REWRITE]
  // Use standardized .chart-card markup and dropdown for Appendix D chart
  ...
  const rows = lines.map(l=>`
    <tr>
      ...
      <td style="text-align:right">${esc(l && l.volume || "")}</td><td style="text-align:right">${esc(l && l.rate || "")}</td><td style="text-align:right"><strong>${esc(l && l.monthlyImpact || "")}</strong></td><td>${esc(l && l.confidence || "")}</td><td>${esc(l && l.startMonth || "")}</td><td>${esc(l && l.runRateMonth || "")}</td><td style="text-align:right">${esc(l && l.oneOffCost || "")}</td>
    </tr>
  `).join("");
  ...
  <section class="appendix">
    ...
    <div class="chart-card" data-type="line">
      ...
      >
        <!-- NOTE: the TypeSwitch script only needs .plot SVG groups.
             We keep simple placeholders; reportGraphs.js will still
             toggle visibility correctly by class. -->
...
function buildAllAppendices({charters, raci, raid, benefits, plan100, pilot, assumptions, methodsSources, implementationChecklist, decisionFramework, resourceRequirements}) {
  ...
  // Inject TYPE_SWITCH_SCRIPT once at the end for appendix dropdowns
  return content + `\n<script>${TYPE_SWITCH_SCRIPT}</script>`;
}

/* [KT:SURGICAL][LEGACY_APPENDICES_G_TO_J]
   Keeping these for reference only; no longer injected into the report.
   They caused Appendices I/J/K to be duplicated and appear at the very end. */
const LEGACY_EXTRA_APPENDICES_HTML = `
    <!-- ========================================================= -->
    <!-- Appendix G — Change Management Plan -->
    <section class="appendix">
      <h2>Appendix G — Change Management Plan</h2>
      ...
    </section>

    <!-- Appendix H — Risk & Mitigation Summary -->
    <section class="appendix">
      <h2>Appendix H — Risk &amp; Mitigation Summary</h2>
      ...
    </section>

    <!-- Appendix I — Governance & Decision Rights -->
    <section class="appendix">
      <h2>Appendix I — Governance &amp; Decision Rights</h2>
      ...
    </section>

    <!-- Appendix J — Pilot Evaluation Criteria -->
    <section class="appendix">
      <h2>Appendix J — Pilot Evaluation Criteria</h2>
      ...
    </section>
`;
