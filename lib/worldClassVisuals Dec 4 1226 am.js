...
// [KT:SAFE-L] Safe fallback for L to avoid ReferenceError. Allows
// optional label overrides from the main report label map.
const L = (typeof window !== 'undefined' && window.L) ? window.L : {};
...
export function topInsightsHeaderHTML() {
  return `
    <div class="top-insights-header" style="text-align:left; padding: 8px 12px; font-family: Georgia, 'Times New Roman', serif; font-weight:700; color:#fff;">
      <span style="background:#FF8C00; color:#000; padding:4px 8px; border-radius:4px; font-size:14px;">${t(
        'Overall'
      )}</span>
      <span style="margin-left:8px; font-size:14px; color:#fff;">&amp; ${t(
        'Key Insights'
      )}</span>
    </div>
  `;
...
export function riskHeatmapHTML(risks = []) {
  ...
  function chartSummaryRowRisk({ risks }) {
    ...
    if (typeof t === 'function') {
      uniqueRisks.forEach((r) => {
        try {
          ...
          console.log('[KT:I18N-RISK] translation.error', String(e?.message || e));
        }
      });
    }
    ...
    const overallLabel = L.overall || t('Overall');
    const keyInsightsLabel = L.keyInsights || t('Key Insights');
...
export function waterfallChartHTML(components = [], currency = '$') {
  ...
  function chartSummaryRowValueBridge({ components, currency }) {
    ...
    return `
      ...
      firstVal
    )} ${t('to')} ${fmt(lastVal)} (${changePercent > 0 ? '+' : ''}${changePercent}%).
          </span>
        </div>
        <div>
          <span style="font-weight:bold;">${keyInsightsLabel}:</span>
...
`;
  ...
  const xAxisLabel = `<text x="${w / 2}" y="${
    h - p + 70
  }" text-anchor="middle" fill="#B6C2E2" font-size="15">${t('Component')}</text>`;
