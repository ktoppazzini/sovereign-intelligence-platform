...
export function riskHeatmapHTML(risks = []) {
  ...
  function chartSummaryRowRisk({ risks }) {
    // [KT:TRANSLATION-DEBUG] Log risk matrix translations
    console.log('[KT:TRANSLATE-RISK] Overall:', t('Overall'));
    console.log('[KT:TRANSLATE-RISK] Key Insights:', t('Key Insights'));
    console.log('[KT:TRANSLATE-RISK] Risk levels distributed:', t('Risk levels are distributed across the matrix, highlighting areas of concern.'));
    console.log('[KT:TRANSLATE-RISK] Top risk:', t('Top risk'));
    console.log('[KT:TRANSLATE-RISK] N/A:', t('N/A'));
    ...
    const riskSummary = {
      overall: t('Risk levels are distributed across the matrix, highlighting areas of concern.'),
      keyInsights: `${t('Top risk')}: ${topRisk}. ${topDesc}`
    };
    
    // [KT:TRANSLATION-DEBUG] Log final risk summary
    console.log('[KT:TRANSLATE-RISK] Final Risk Summary:', riskSummary);
    
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">${t('Overall')}:</span> <span style="color:#fff;">${riskSummary.overall}</span></div>
        <div><span style="font-weight:bold;">${t('Key Insights')}:</span> <span style="color:#fff;">${riskSummary.keyInsights}</span></div>
      </div>
    `;
  }
