...
/* [KT:SURGICAL:STANDARD-DESC:A2] — begin
   Broaden tagging for standard figures that are not "chart-card" but appear
   as "viz-card" or generic "chart" class. Add data-chart and log meta. */
function __kt_tagStandardCharts2(html='', labels=[], canon={}) {
  try {
    const src = String(html||'');
    const re  = /<figure\b([^>]*class=["'][^"']*(?:viz-card|(?:^|\s)chart(?:\s|$))[^"']*["'][^>]*)>([\s\S]*?)<\/figure>/gi;
    let out = '', last = 0, m;
    let scanned = 0, upgraded = 0;
    const details = [];
    ...
    while((m = re.exec(src))!==null){
      ...
      // skip if already a data-* viz or was handled by the first pass chart-card
      if (/(data-chart|data-widget\s*=\s*["']benchmark["']|data-heatmap|chart-card)/i.test(open)) {
        out += src.slice(last, end); last = end; continue;
      }
      ...
      const upgradedOpen = open + " data-chart='" + __kt_toAttrJSON(spec) + "' data-origin=\"standard\"";
      out += src.slice(last, start) + '<figure' + upgradedOpen + '>' + inner + '</figure>';
      last = end;
      upgraded++;
      if (details.length < 8) {
        details.push({ title, type, xTitle, yTitle, labelsLen: (labels||[]).length, dataLen: (labels||[]).length });
      }
    }
    out += src.slice(last);
    console.log('[SR:REFORM] std.charts2.tagged', { scanned, upgraded, detailsSample: details, samples: details.map(d=>d.title).slice(0,3) });
    return out;
  } catch(e) {
    console.log('[SR:REFORM] std.charts2.tag.error', String(e?.message||e));
    return html;
  }
}
// [KT:SURGICAL:STANDARD-DESC:A2] — end
...
export default function buildReformReportHTML(opts) {
  ...
  const prep = (s) => {
    ...
    try {
      ...
      // [KT:SURGICAL:STANDARD-DESC:A2] — second pass for viz-card/generic chart
      postHTML = __kt_tagStandardCharts2(postHTML, labels, canon);
    } catch(e) {
      console.log('[SR:REFORM] std.charts.tag.error', String(e?.message||e));
    }
