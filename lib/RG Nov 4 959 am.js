...
/* ===========================================================
   ...
   [KT:SURGICAL:GLOBAL-TYPE-SWITCHER:v3]
   Fixes:
   - Defines a safe debug flag (no ReferenceError).
...
(function () {
  ...
  // Safe debug guard (prevents ReferenceError if not defined elsewhere)
  var __DEBUG_TYPESWITCH__ =
    (typeof window !== 'undefined' && window.__DEBUG_TYPESWITCH__) || false;

  function applyType(card) {
    try {
      if (!card) return;
      var sel =
        card.querySelector && card.querySelector('.chart-type select');
      if (!sel) return;
      ...
      card.dataset.typeAppliedBy = 'global-switcher-v3';
    } catch (e) {
      console.warn('[TypeSwitch] apply error', e);
    }
  }
  ...
  function initTypes(root) {
    try {
      (root || document)
        ...
        .forEach(applyType);
      if (__DEBUG_TYPESWITCH__)
        console.log('[TypeSwitch] init scan complete');
    } catch (e) {
      console.warn('[TypeSwitch] init error', e);
    }
  }
    ...
    function onChange(e) {
      try {
        var sel =
          e.target &&
          e.target.closest &&
          e.target.closest('.chart-type select');
        if (!sel) return;
        ...
        console.warn('[TypeSwitch] onChange error', err);
      }
    },
    /* capture */ true
  );
...
.kpi-grid .chart-card{ min-height:320px; }

.kpi-grid .chart-card canvas{ height:260px !important; width:100% !important; display:block; }

@media (min-width:1280px){

  .kpi-grid .chart-card{ min-height:360px; }

  .kpi-grid .chart-card canvas{ height:300px !important; }
