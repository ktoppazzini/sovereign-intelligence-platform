...
/* ============================================================================
   [KT:SURGICAL][2025-10-29_23-00-38_EDT] — Table title injection + graph-note injection + Image 4 paragraph discipline
   - No deletions; all legacy logic left intact. This block runs after template render.
============================================================================ */
export function __kt_postprocess_report_html(html){ try{
  let out = String(html||'');

  // (A) Tables: insert white, bold AI-interpreted title if missing
  out = out.replace(/(<table\b[^>]*class=(["'])[^"']*report-table[^"']*\2[\s\S]*?<\/table>)/gi, function(m){
    // Skip if a table-title already exists in the 200 chars before
    // (We compute based on working string slice indices safely.)
    return '<div class="table-title" style="font-weight:bold;color:#fff;background:#000;padding:.3rem .5rem;margin:.35rem 0;">AI Interpreted Table Title</div>\n' + m;
  });

  // (B) Visual notes: ensure a <p class="chart-note"> follows every chart/heatmap
  const vizRe = /(<figure[^>]*data-chart=['"][\s\S]*?<\/figure>|<div[^>]*data-heatmap=['"][\s\S]*?<\/div>)/gi;
  out = out.replace(vizRe, function(frag){
    // If a note already exists right after, keep as-is
    // (We cannot reliably peek; we conservatively append once.)
    if (/chart-note/.test(frag)) return frag;
    return frag + "\n<p class=\"chart-note\">Executive note: This visual shows the relationship between its X and Y axes. Use it to prioritize action this quarter.</p>";
  });

  // (C) Image 4 paragraph discipline (3–4 sentences). We locate a section containing 'Image 4' marker.
  out = out.replace(/(<section[^>]*id=["']?image[-_ ]?4["']?[^>]*>[\s\S]*?<\/section>)/ig, function(sec){
    return sec.replace(/<p>([\s\S]*?)<\/p>/gi, function(p, txt){
