...
function styles(){
  ...
  <style>
    ...
    html,body{
      ...
      overflow-x:hidden; /* prevent horizontal scroll from outer wrappers */
    }
    a{color:#0f62fe;text-decoration:none}

    /* Letter portrait comfortable width (≈ 8.5in usable with margins) */
    .report{
      max-width:816px;
      width:100%;
      margin:0 auto;
    ...
    /* Headings & text force dark-on-light regardless of outer theme */
    ...
    .cover-inner{max-width:816px;margin:0 auto;padding:0 24px}
    ...
    /* Zebra tables preserved */
    .report-table{width:100%;border-collapse:collapse}
    .report-table th,.report-table td{border-bottom:1px solid var(--rule);padding:8px 10px;text-align:left}
    .report-table tr:nth-child(even){background:#fbfdff}

    /* Charts responsive in preview */
    figure{margin:16px 0}
    figure canvas{display:block;max-width:100%;height:auto}
    ...
    .impl-labels{position:absolute;left:0;right:0;top:38%;display:grid;grid-template-columns:repeat(5,1fr);gap:8px;padding:0 6%}
  ...
  <section class="cover">
    <div class="cover-inner">
      ...
      <div class="cover-meta">
        ...
        <div><strong>Date:</strong> ${esc(canon.reportDate)}</div>
      </div>
    </div>
  </section>`;
}
...
function toc(items){
  return `<nav class="toc"><h2>Contents</h2><ol>${items.map((it,i)=>`<li><span style="color:var(--muted);font-weight:700">${i+1}.</span> <a href="#${it.id}">${esc(it.title)}</a></li>`).join('')}</ol></nav>`;
}
      ...
      document.querySelectorAll('[data-heatmap]').forEach(function(box){
        ...
        for(var r=0;r<rows.length;r++){
          for(var c=0;c<cols.length;c++){
            ...
            ctx.fillStyle='rgba(220,38,38,'+a+')'; ctx.fillRect(pad+c*cw, pad+r*ch, cw, ch); ctx.strokeStyle='#fff'; ctx.strokeRect(pad+c*cw, pad+r*ch, cw, ch);
          }
        }
...
function planOverlayHTML({ phases = [], imageSrc = '/images/Implementation-roadmap.png' } = {}){
...
export default function buildReformReportHTML({ canon, sections, charts = {}, phases = [], Graphs: G = Graphs, planDiagramHTML }){
  ...
  // FIX: use the correct function name exposed by reportGraphs (heatmapHTML)
    ...
    `${benchmarkSpec ? `
      <div class="viz-wrap">
        <figure data-widget="benchmark" data-spec='${esc(JSON.stringify(benchmarkSpec))}'>
          ...
          <div data-fallback>${Graphs.barChartHTML([benchmarkSpec.current, benchmarkSpec.benchmark],{ labels:['Current','Benchmark'], title:benchmarkSpec.title })}</div>
        </figure>
      </div>` : ''}
     <div class="viz-wrap">
  ...
  const impl = (typeof planDiagramHTML === 'function')
    ...
    : planOverlayHTML({ phases, imageSrc:'/images/Implementation-roadmap.png' });
