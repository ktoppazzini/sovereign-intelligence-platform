...
.report .table .table-title{
  ...
  caption-side:top;
    ...
    /* [KT:SURGICAL] Report tables — black background with refined white grid */
    ...
    .report .report-table th, .report .report-table td{
      border:1px solid #fff !important; padding:.55rem .6rem;
    }
  ...
  const __kt_styled = `
    <style>
      ...
      .report-table th, .report-table td { border: 1px solid #fff; padding:.55rem .6rem; }
  ...
  if (implKit) {
    reportHtml += [
      ...
      implKit.benefits ? benefitsHTML(implKit.benefits, currencySymbol) : '',
      implKit.plan100 ? plan100HTML(implKit.plan100, currencySymbol) : '',
      implKit.pilot ? pilotHTML(implKit.pilot, currencySymbol) : '',
      implKit.assumptions ? assumptionsHTML(implKit.assumptions, currencySymbol) : '',
      implKit.methods ? methodsSourcesHTML(implKit.methods, currencySymbol) : ''
    ].join('');
  }
  ...
  // Verify the script has the fixed querySelector
  if (TYPE_SWITCH_SCRIPT.includes("querySelector('.chart-type select, .chart-type-select'")) {
    console.log('[SR:REFORM] ✅ [DROPDOWN-FIX] Script contains FIXED querySelector (comma-separated selectors)');
    ...
    console.warn('[SR:REFORM] ⚠️ [DROPDOWN-FIX] WARNING: Cannot verify querySelector pattern in script');
  }
