...
setGraphTranslator((s) => translateToReportLanguage(s));
...
export const DEFAULT_LABELS = {
  ...
  riskHeatmap: "Risk Heatmap",
    ...
    // --- Chart UI / axes (NEW for ReportGraphs i18n) ---
  chartTypeLabel: "Type",
  chartTypeLine: "Line",
  chartTypeBar: "Bar",
...
function injectSmartWidgets(html, { labels, canon, L }) {
  ...
  const LL = L || (typeof DEFAULT_LABELS !== 'undefined' ? DEFAULT_LABELS : {}) || {};

  // Chart titles
  const projectedSavingsTitle =
    LL.projectedSavings || 'Projected Savings Over Time';
  const implementationSpendTitle =
    LL.operatingExpenses || 'Implementation Expenditures';
  ...
  const make = (kind) => {
    ...
    switch (kind) {
      case 'bar': {
        ...
        return barChartHTML({
          title: implementationSpendTitle,
          labels,
          // NOTE: ReportGraphs barChartHTML already expects a simple numeric series
          series: spendSeries,
          xTitle: labels[0]?.startsWith('Y') ? yearsAxis : monthsAxis,
          yTitle: spendAxis,
        });
      }
...
`;
  ...
  // [KT:SURGICAL:I18N-TOC] — use translated labels instead of hard-coded English
const tocItems = [
  L.executiveSummary,      // 1. Executive Summary
  L.currentState,          // 2. Current State
  L.financialAnalysis,     // 3. Financials
  L.kpiAnalysis,           // 4. KPIs & Targets
  L.timeline,              // 5. Implementation Plan / Timeline
  L.operationalAnalysis,   // 6. Operating Model / Ops
  L.riskAssessment,        // 7. Risks & Mitigations
  L.roiProjections,        // 8. ROI
  L.nextSteps              // 9. Next Steps
].map((t) => `<li>${t}</li>`).join('');
  ...
  ${esc(canon?.aiTitle || 'Strategic Transformation & Value Creation Plan')}
</h2>
  ...
  <div class="meta-item">
    <!-- [KT:SURGICAL:I18N-TITLE-PAGE-META] Prepared by label translated -->
    <span class="meta-label">${L.preparedBy}</span>
...
export async function buildReformReportHTMLWithTranslation(opts) {
  ...
  if (!translatedLabels || typeof translatedLabels !== 'object') {
    translatedLabels = await translateReportLabels(DEFAULT_LABELS, lang);
  }
