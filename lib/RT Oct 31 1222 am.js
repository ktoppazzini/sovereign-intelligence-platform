...
============================================================================ */
export 
/* [KT:SURGICAL][POSTPROCESS] — Move full paragraph above each chart and inject a white “Type” dropdown
   - For each <figure data-chart> block:
     * If the next sibling is a plain <p> paragraph, move its text into <p class="chart-note"> INSIDE the figure, ABOVE the canvas.
     * Add a header row with a white “Type:” label and a <select class="chart-type-select"> (Line/Area/Bar).
     * Skip if .chart-head already exists to avoid duplication.
*/
function __kt_postprocess_report_html(html){
  try{
    let src = String(html||'');
    ...
    src = src.replace(/<figure([^>]*data-chart=['"][^'"]+['"][^>]*)>([\s\S]*?)<\/figure>(\s*<p>([\s\S]*?)<\/p>)?/gi,
      function(_m, open, inner, afterPWhole, afterPText){
        ...
        if (/<div[^>]*class=['\"][^'\"]*chart-head[^'\"]*['\"]/.test(inner) && /<p[^>]*class=['\"][^'\"]*chart-note[^'\"]*['\"]/.test(inner)){
        ...
        const head = ''
          ...
          +   '<div class="chart-title" style="font-weight:700;color:#E6F0FF;">'+ title.replace(/</g,'&lt;').replace(/>/g,'&gt;') +'</div>'
        ...
        if (!noteText){
          ...
          if (inlineNote) noteText = inlineNote[1].replace(/<[^>]*>/g,'').trim();
        }
