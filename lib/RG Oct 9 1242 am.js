...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      <line x1="${p}" y1="${y}" x2="${w - p}" x2="${w - p}" stroke="rgba(255,255,255,.35)"/>
      <!-- was font-size:16 / y+8; keeping bigger for readability -->
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  ...
  <figure class="chart-card" data-type="line">
    <header class="chart-head">
      <h3>${esc(nice)}</h3>
      <!-- Duplicate title in legend commented out to avoid repeated title text -->
...
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  // const w = 760, h = 240, p = 92; /* too compressed for Y axis */
  const w = 760, h = 320, p = 92; /* ↑ taller for legibility */
  ...
  <figure class="chart-card" data-type="bar">
    <header class="chart-head">
      <h3>${esc(nice)}</h3>
      <!-- Add a non-functional Type control for visual consistency -->
      <label class="chart-type"><span>Type:</span>
        <select aria-label="Chart type (benchmark)"><option selected>Bar</option></select>
      </label>
    </header>
    <div class="canvas-wrap">
      <svg class="plot bar visible" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  <section class="chart-card chart-heat" data-type="heat">
    ...
    <div class="canvas-wrap">
      <svg class="plot heat visible" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
        <rect x="0" y="0" width="${w}" height="${h}" rx="12" ry="12" fill="#0E2A44"/>
...
.plot.line{ display:block; }  /* Fallback: show Line when JS is blocked */
.plot.visible{ display:block; } /* Fallback for single-variant plots */
