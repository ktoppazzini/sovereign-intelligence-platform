...
// Surgical updates for T1 (tables only):
// - Add [KT: TABLE THEME] CSS so ALL .table elements (in any section) match Appendix style.
// - Keep unifyTables() so every <table> becomes <table class="table">.
// No other changes beyond table styling.
...
// Keep this: forces Appendix class onto ALL tables in sections.
...
export default function buildReformReportHTML(opts) {
  ...
  <style>
    ...
    /* ==== [KT: TABLE THEME] START =======================================
       Force Appendix table styling globally for all .table (sections + appendices)
       to match your TO-BE sample (dark header, light-on-dark body, grid lines). */
