...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  // Truncate VERY long Y-axis titles only (35 char limit to accommodate translations)
  const truncatedYTitle = yTitle && yTitle.length > 35 ? yTitle.slice(0, 32) + '...' : yTitle;
  ...
  const xTitleText = xTitle
    ...
    : '';
  // Y-axis title moved further left (p - 24 instead of p - 16) to prevent cutoff with longer labels
  const yTitleText = truncatedYTitle
    ? `<text x="${p - 24}" y="${p - 14}" text-anchor="end" fill="#FFFFFF" font-size="20" font-weight="700">${esc(truncatedYTitle)}</text>`
    : '';
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '', lang = 'English', ui } = {}) 
  ...
  const extraPadding = Math.max(0, Math.ceil((yTitleLength - 8) * 8));
  const p = Math.min(Math.max(basePadding + extraPadding, 110), 200); // cap at 200px max
  ...
  <figure class="chart-card" data-type="line" data-widget="series">
    ...
    <div class="canvas-wrap">
      ...
      <svg class="plot line" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ${svgAxes({ w, h, p, xTitle: xTitleT, yTitle: yTitleT, maxY: M, labels: lbl })}
...
/* ===========================================================
   ...
   ===========================================================*/
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '', lang = 'English' }) {
  const w = 760, h = 320;
  ...
  // Each character averages ~8px width at font-size 20px
  const extraPadding = Math.max(0, Math.ceil((yTitleLength - 8) * 8));
  const p = Math.min(Math.max(basePadding + extraPadding, 92), 200); // cap at 200px max
