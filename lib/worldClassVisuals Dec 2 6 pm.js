...
// [KT:TRANSLATION:TAGGED] All user-facing strings in this module are wrapped in t() for translation.
let _translator = (s) => s;

// [KT:SAFE-L] Safe fallback for L to avoid ReferenceError
...
export function topInsightsHeaderHTML() {
  ...
  <span style="margin-left:8px; font-size:14px; color:#fff;">&amp; ${(L.keyInsights || 'Key Insights')}
</span>
    </div>
  `;
}
...
export function riskHeatmapHTML(risks = []) {
  ...
  function chartSummaryRowRisk({ risks }) {
    ...
    // [KT:SURGICAL:I18N-RISK] translate risk names + descriptions
if (typeof t === "function") {
  uniqueRisks.forEach(r => {
    try {
      r.name = t(r.name || '');
      r.description = r.description ? t(r.description) : t('No description provided.');
    } catch (e) {
    ...
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">${L.overall || 'Overall'}:</span> <span style="color:#fff;">${t('Risk levels are distributed across the matrix, highlighting areas of concern.')}</span></div>
        <div><span style="font-weight:bold;">${L.keyInsights || 'Key Insights'}
:</span> <span style="color:#fff;">${t('Top risk')}: ${esc(topRisk)}. ${esc(topDesc)}</span></div>
      </div>
    `;
  }
  ...
  const riskBubbles = uniqueRisks.map((r, i) => {
    ...
    return `
      ...
      <text x="${x}" y="${y}" text-anchor="middle" fill="#FFFFFF" font-size="10" font-weight="700" dy="4">${t(String(i+1))}</text>
  ...
  `<text x="20" y="${20 + i*20}" fill="#FFFFFF" font-size="11">${t(String(i+1))}. ${esc(r.name)}</text>`
).join('');
  ...
  const riskList = `
    <div class="risk-list" style="margin-top:16px; color:#fff;">
      ...
      <ol style="margin:8px 0 0 20px; padding:0;">
        ${uniqueRisks.map((r, i) => `<li style="margin-bottom:4px;"><span style="font-weight:bold;">${esc(r.name)}</span>: ${r.description ? esc(r.description) : t('No description provided.')}</li>
`).join('')}
      </ol>
    </div>
  `;
...
export function waterfallChartHTML(components = [], currency = '$') {
  ...
  function chartSummaryRowValueBridge({ components, currency }) {
    ...
    // Old summary row:
    // <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;"></div>
    ...
    //   <div><span style="font-weight:bold;">${t('Key Insights')}:</span> <span style="color:#fff;">${t('Peak value of')} ${t(currency)}${t(maxVal)} ${t('achieved. Average performance:')} ${t(currency)}${t(Math.round(avgVal))}.</span></div>
    // </div>
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <span style="font-weight:bold;">${L.overall || t('Overall')}</span>:</span> <span style="color:#fff;">${icon} ${t('The chart shows a')} ${trend} ${t('trend from')} ${currency}${firstVal} ${t('to')} ${currency}${lastVal} (${changePercent > 0 ? '+' : ''}${changePercent}%).</span></div>
        <div><span style="font-weight:bold;">${L.keyInsights || t('Key Insights')}</span>:</span> <span style="color:#fff;">${t('Peak value reached:')} ${currency}${maxVal}. ${t('Average:')} ${currency}${Math.round(avgVal)}.</span></div>
      </div>
    `;
  }
  ...
  const typeDropdown = `
    <label class="chart-type"><span>${L('Type')}:</span>
      <select class="chart-type-select" aria-label="${L('Chart type')}">
        <option value="line" selected>${L('Line')}</option>
        <option value="bar">${L('Bar')}</option>
        <option value="pie">${L('Pie')}</option>
        <option value="doughnut">${L('Doughnut')}</option>
      </select>
    </label>
  `;
