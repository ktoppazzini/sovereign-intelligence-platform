...
export function topInsightsHeaderHTML() {
  ...
  // Use shared label map (L) with translator fallback
  const overallLabel = t(L.overall || 'Overall');
  const keyInsightsLabel = t(L.keyInsights || 'Key Insights');
    ...
    <figure class="chart-card radar-card">
      ...
      <div class="canvas-wrap">
        <svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" style="background:#000000; border-radius:12px;">
          ...
          <text x="${w - 65}" y="55" fill="#FFFFFF" font-size="12">${t(
            "Benchmark"
          )}</text>
        </svg>
      </div>
...
export function riskHeatmapHTML(risks = []) {
    ...
    if (typeof t === "function") {
      uniqueRisks.forEach((r) => {
        try {
          ...
          console.log(
            "[KT:I18N-RISK] translation.error",
            String(e?.message || e)
          );
        }
      });
  ...
  if (!risks || risks.length === 0) {
    risks = [
      ...
      },
      {
        ...
        severity: 4,
        likelihood: 3,
        description: "Project phases may take longer than planned.",
      },
      {
        name: "Scope Creep",
        severity: 3,
  ...
  const yLabels = ["", "Negligible", "Minor", "Moderate", "Major", "Critical"]
    .map((label, i) => {
      ...
      return `<text x="${p - 10}" y="${y}" text-anchor="end" fill="#FFFFFF" font-size="12" dy="4">${t(
        label
      )}</text>`;
    })
    .join("");
  ...
  `;
  // [KT:SURGICAL:I18N-RISK-LABELS] Replace static labels with translated UI map if available
const uiRisk = (typeof window !== "undefined" && window.__REPORT_GRAPHS_UI__) || {};
//const translatedOverall = uiRisk.overallLabel || t("Overall");//
const translatedOverall = "Overall";
const translatedKeyInsights = "Key Insights";
  ...
  return `
    <figure class="chart-card risk-heatmap-card">
      ...
      <div class="canvas-wrap">
        <svg width="${w}" height="${h + 100}" viewBox="0 0 ${w} ${
    h + 100
    ...
    h / 2
      ...
      ${riskList}
      <p class="chart-note">
        ${t("Risk heatmap visualizes")} ${uniqueRisks.length} ${t(
    "identified risks by severity (vertical) and likelihood (horizontal)."
  )}
        <span style="font-weight:bold;">${t("Color legend:")}</span>
        <span style="color:#EF4444;">${t("Red = critical")}</span>,
        <span style="color:#F59E0B;">${t("Orange = high")}</span>,
        <span style="color:#10B981;">${t("Green = medium")}</span>,
        <span style="color:#3B82F6;">${t("Blue = low")}</span>.
      </p>
    </figure>
  `;
}
...
export function waterfallChartHTML(components = [], currency = "$") {
  ...
  const keyInsightsLabel = t(L.keyInsights || 'Key Insights');
    ...
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div>
         <span style="font-weight:bold;">${translatedOverall}:</span>
      ...
      changePercent > 0 ? "+" : ""
        ...
        <div>
          <span style="font-weight:bold;">${translatedKeyInsights}:</span>
          <span style="color:#fff;">
  ...
  // Each chart instance gets a stable ID for the Type dropdown wiring
  const chartId =
    "value-bridge-" +
  ...
  <div class="chart-type">
    <label for="${chartId}-type">${t("Type")}</label>
    <select id="${chartId}-type" data-chart-id="${chartId}">
      <option value="waterfall" selected>${t("Waterfall")}</option>
...
`;
  ...
  const barElements = bars
    .map((bar, i) => {
      ...
      return `
        ...
        <text x="${x + barWidth / 2}" y="${
        h - p + 32
      }" text-anchor="middle" fill="#FFFFFF" font-size="18" font-weight="700" transform="rotate(-45 ${
        x + barWidth / 2
  ...
  const xLabels = components.map((c, i) => {
    ...
    return `<text x="${x}" y="${h - p + 18}" text-anchor="middle" fill="#FFFFFF" font-size="11" transform="rotate(-45 ${x} ${
      h - p + 18
  ...
  const yAxisTicks = Array.from({ length: yTicksBar }, (_, i) => {
    ...
    const y = yScale(v);
    return `<text x="${p - 16}" y="${y + 4}" text-anchor="end" fill="#B6C2E2" font-size="16" font-weight="bold">${fmt(
      v
    )}</text>`;
  }).join("");
  ...
  return `
    <figure class="chart-card waterfall-card" data-type="line" data-widget="series" data-chart-id="${chartId}">
...
export function executiveSummaryInfographicHTML(summary = {}) {
  ...
  return `
    <div class="executive-infographic" style="background: linear-gradient(135deg, #0B1A3D 0%, #1e3a5f 100%); padding: 2rem 1.5rem; border-radius: 16px; margin: 2rem 0; max-width: 100%; box-sizing: border-box;">
      <h2 style="text-align: center; color: #10B981; font-size: 2rem; margin-bottom: 2rem; font-weight: 700;">${t(
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 2rem; max-width: 100%;">
        <div style="background: rgba(16,185,129,0.1); border: 2px solid #10B981; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          <div style="font-size: 0.75rem; color: rgba(255,255,255,0.7); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem;">${t(
            "Projected ROI"
          )}</div>
          <div style="font-size: 2.5rem; color: #10B981; font-weight: 700; font-family: Georgia, serif; line-height: 1;">${esc(
            roi
          )}</div>
        </div>
        <div style="background: rgba(59,130,246,0.1); border: 2px solid #3B82F6; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          ...
          <div style="font-size: 2.5rem; color: #3B82F6; font-weight: 700; font-family: Georgia, serif; line-height: 1;">${esc(
            payback
          )}</div>
        </div>
        <div style="background: rgba(245,158,11,0.1); border: 2px solid #F59E0B; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          ...
          <div style="font-size: 2.5rem; color: #F59E0B; font-weight: 700; font-family: Georgia, serif; line-height: 1;">${esc(
            savings
          )}</div>
        </div>
      </div>
      ...
      <div style="background: rgba(0,0,0,0.3); border-radius: 8px; padding: 1.5rem 1rem; margin-bottom: 1.5rem;">
        <h3 style="color: #FFFFFF; font-size: 1.2rem; margin-bottom: 1rem; text-align: center;">${t(
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; max-width: 100%;">
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: #FFFFFF; font-size: 0.75rem; font-weight: 600;">${t(
              "Discovery"
            )}</div>
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.5rem;">4 ${t(
              "weeks"
            )}</div>
          </div>
          ...
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.25rem;">4 ${t(
              "weeks"
            )}</div>
          </div>
          <div style="flex: 0 0 30px; text-align: center;">
          ...
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.25rem;">12 ${t(
              "weeks"
            )}</div>
          </div>
        </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; max-width: 100%;">
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
          ...
          <div style="color: rgba(255,255,255,0.8); font-size: 0.75rem; margin-top: 0.5rem;">${t(
            "Quick Wins"
          )}</div>
        </div>
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
        ...
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
          <div style="font-size: 2rem; color: #F59E0B; font-weight: 700; line-height: 1;">${esc(
            timeline
          )}</div>
          <div style="color: rgba(255,255,255,0.8); font-size: 0.75rem; margin-top: 0.5rem;">${t(
            "Timeline"
        ...
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 8px;">
          <div style="font-size: 2rem; color: #10B981; font-weight: 700; line-height: 1;">${esc(
            confidence
          )}</div>
          <div style="color: rgba(255,255,255,0.8); font-size: 0.75rem; margin-top: 0.5rem;">${t(
            "Confidence"
