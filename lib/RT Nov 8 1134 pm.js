...
function unifyTables(html = '') {
  ...
  // Reverted horizon to 4 weeks for readability
  const roadmap = planDiagramHTML(phases || [], 'Implementation Plan (4 weeks)');
  let html = '';
  html += `
  <section>
...
.report .table .table-title{
  ...
  if (implKit) {
    reportHtml += [
      ...
      implKit.pilot ? pilotHTML(implKit.pilot, currencySymbol, L) : '',
