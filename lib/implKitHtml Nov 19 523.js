...
function benefitsHTML(model) {
  ...
  // [KT:SURGICAL:APPENDIX-TYPE-SWITCH-FIX] Updated type switch to match main report style and functionality
  const bridgeChartId = "monthly-value-bridge";
  const bridgeTypeSelect = `
    <div class="chart-ui" style="display:inline-block;vertical-align:middle;margin-bottom:8px;">
      <label for="${bridgeChartId}-type" class="small" style="margin-right:8px;">TYPE:</label>
      <select id="${bridgeChartId}-type" class="chart-type-select" style="background:#111;color:#fff;border:1px solid #444;border-radius:8px;padding:.2rem .4rem;">
