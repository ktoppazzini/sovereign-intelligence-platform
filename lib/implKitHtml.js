...
function chartersHTML(model, L = {}) {
  ...
  const blocks = items
    .map((c) => {
      ...
      <table class="table small">
        <tr>
          <th style="width:22%" data-i18n-key="appendixA.objective">${L.objective || "Objective"}</th>
          <td>${esc(c.objective || "")}</td>
        </tr>
        <tr>
          <th data-i18n-key="appendixA.scopeIn">${L.scopeIn || "Scope (In)"}</th>
          <td>${esc(c.scopeIn || "")}</td>
        </tr>
        <tr>
          <th data-i18n-key="appendixA.scopeOut">${L.scopeOut || "Scope (Out)"}</th>
          <td>${esc(c.scopeOut || "")}</td>
        </tr>
        <tr>
          <th data-i18n-key="appendixA.ownerStakeholders">${L.ownerStakeholders || "Owner & Stakeholders"}</th>
          <td><strong>${esc(c.owner || "")}</strong>; ${esc((c.stakeholders || []).join(", "))}</td>
        </tr>
        <tr>
          <th data-i18n-key="appendixA.milestones">${L.milestones || "Milestones"}</th>
          <td>${(c.milestones || [])
            .map(
              (m) =>
        ...
        <tr>
          <th data-i18n-key="appendixA.kpis">${L.kpis || "KPIs"}</th>
          <td>${(c.kpis || [])
            .map((k) => {
        ...
        <tr>
          <th data-i18n-key="appendixA.risks">${L.risks || "Risks"}</th>
          <td>${(c.risks || [])
            .map(
              (r) =>
        ...
        <tr>
          <th data-i18n-key="appendixA.budget">${L.budget || "Budget"}</th>
          <td>${esc(c.budgetSummary || "")}</td>
        </tr>
        <tr>
          <th data-i18n-key="appendixA.acceptance">${L.acceptance || "Acceptance"}</th>
          <td>${esc(c.acceptanceCriteria || "")}</td>
        </tr>
      </table>
      <div class="note" data-i18n-key="appendixA.tip">
        ${L.charterTip || "Tip: copy as a one-pager per store/workstream."}
      </div>
  ...
  <section class="appendix">
    <h3 data-i18n-key="appendixA.title">${L.appendix || "Appendix"} A — ${L.projectCharters || "Initiative Charters"}</h3>
    ${blocks}
  </section>`;
}
...
function raciHTML(model, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixB.title">${L.appendix || "Appendix"} B — ${L.raciMatrix || "RACI & Decision SLAs"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixB.decision">${L.decision || "Decision"}</th>
...
function raidHTML(model, L = {}) {
  ...
  <section class="appendix">
    <h3 data-i18n-key="appendixC.title">
      ${L.appendix || "Appendix"} C — ${L.raidLog || "RAID (Risks, Assumptions, Issues, Dependencies)"}
    </h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixC.type">${L.type || "Type"}</th>
          <th data-i18n-key="appendixC.description">${L.description || "Description"}</th>
          <th data-i18n-key="appendixC.owner">${L.owner || "Owner"}</th>
          <th data-i18n-key="appendixC.impact">${L.impactHeader || "Impact"}</th>
          <th data-i18n-key="appendixC.probability">${L.probability || "Prob."}</th>
          <th data-i18n-key="appendixC.trigger">${L.trigger || "Trigger"}</th>
          <th data-i18n-key="appendixC.mitigation">${L.mitigation || "Mitigation"}</th>
          <th data-i18n-key="appendixC.status">${L.status || "Status"}</th>
          <th data-i18n-key="appendixC.nextReview">${L.nextReview || "Next review"}</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
function benefitsHTML(model, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixD.title">${L.appendix || "Appendix"} D — ${L.benefitsRealization || "Benefits Realization Model"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th style="width:10%; white-space: nowrap;" data-i18n-key="appendixD.workstream">${L.workstream || "Workstream"}</th>
          <th style="width:10%; white-space: nowrap;" data-i18n-key="appendixD.lever">${L.lever || "Lever"}</th>
          <th style="width:11%; white-space: nowrap;" data-i18n-key="appendixD.unitAssumption">${L.unitAssumption || "Unit Assumption"}</th>
          <th style="width:9%; white-space: nowrap;" data-i18n-key="appendixD.source">${L.source || "Source"}</th>
          <th style="width:7%; white-space: nowrap;" data-i18n-key="appendixD.volume">${L.volume || "Volume"}</th>
          <th style="width:7%; white-space: nowrap;" data-i18n-key="appendixD.rate">${L.rate || "Rate"}</th>
          <th style="width:9%; white-space: nowrap;" data-i18n-key="appendixD.monthly">${L.monthlySavings || "Monthly $"}</th>
          <th style="width:8%; white-space: nowrap;" data-i18n-key="appendixD.confidence">${L.confidence || "Confidence"}</th>
          <th style="width:7%; white-space: nowrap;" data-i18n-key="appendixD.start">${L.startMonth || "Start"}</th>
          <th style="width:8%; white-space: nowrap;" data-i18n-key="appendixD.runRate">${L.runRate || "Run-rate"}</th>
          <th style="width:8%; white-space: nowrap;" data-i18n-key="appendixD.oneOff">${L.oneOffCost || "One-off $"}</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
    <p class="small" data-i18n-key="appendixD.totalMonthly">
      <strong>${L.totalMonthlyImpact || "Total monthly impact (estimated)"}:</strong> $${totals.toLocaleString()}
    </p>
  </section>`;
}
...
function plan100HTML(model, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixE.title">${L.appendix || "Appendix"} E — ${L.hundredDayPlan || "100-Day Plan (Week by Week)"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixE.week">${L.week || "Week"}</th>
          <th data-i18n-key="appendixE.workstream">${L.workstream || "Workstream"}</th>
          <th data-i18n-key="appendixE.task">${L.task || "Task"}</th>
          <th data-i18n-key="appendixE.owner">${L.owner || "Owner"}</th>
          <th data-i18n-key="appendixE.status">${L.status || "Status"}</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
// ---------- Appendix F – Pilot Design ----------

function pilotHTML(model = {}, L = {}) {
  const hasContent =
    !!(model &&
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixF.title">${L.appendix || "Appendix"} F — ${L.pilotCharter || "Pilot Charter"}</h3>
    <table class="table small">
      <tr>
        <th style="width:22%" data-i18n-key="appendixF.pilot">${L.pilot || "Pilot"}</th>
        <td>${esc(model.name || "")}</td>
      </tr>
      <tr>
        <th data-i18n-key="appendixF.locations">${L.locations || "Locations"}</th>
        <td>${esc(String(model.locations || ""))}</td>
      </tr>
      <tr>
        <th data-i18n-key="appendixF.successKpis">${L.successKPIs || "Success KPIs"}</th>
        <td>${esc((model.successKPIs || []).join(", "))}</td>
      </tr>
      <tr>
        <th data-i18n-key="appendixF.thresholds">${L.thresholds || "Thresholds"}</th>
        <td>${esc((model.thresholds || []).join("; "))}</td>
      </tr>
      <tr>
        <th data-i18n-key="appendixF.sampleDesign">${L.sampleDesign || "Sample Design"}</th>
        <td>${esc(model.sampleDesign || "")}</td>
      </tr>
      <tr>
        <th data-i18n-key="appendixF.rollback">${L.rollbackCriteria || "Rollback Criteria"}</th>
...
function assumptionsHTML(model = {}, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixG.title">${L.appendix || "Appendix"} G — ${L.assumptionsRanges || "Assumptions & Ranges"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixG.assumption">${L.assumption || "Assumption"}</th>
          <th data-i18n-key="appendixG.low">${L.low || "Low"}</th>
          <th data-i18n-key="appendixG.value">${L.value || "Value"}</th>
          <th data-i18n-key="appendixG.high">${L.high || "High"}</th>
          <th data-i18n-key="appendixG.note">${L.note || "Note"}</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
function methodsSourcesHTML(input = {}, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixH.title">${L.appendix || "Appendix"} H — ${L.methodsSources || "Methods & Sources"}</h3>
    <p class="small" data-i18n-key="appendixH.lead">
      ${L.benchmarksNormalized || "Benchmarks were normalized for region and time; assumptions have documented ranges and sensitivity reviewed by Finance."}
    </p>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixH.benchmark">${L.benchmark || "Benchmark"}</th>
          <th data-i18n-key="appendixH.source">${L.source || "Source"}</th>
          <th data-i18n-key="appendixH.date">${L.date || "Date"}</th>
          <th data-i18n-key="appendixH.notes">${L.notes || "Notes"}</th>
        </tr>
      </thead>
      <tbody>
        ${benchmarks
    ...
    ${
      sources.length
        ? `<p class="small" data-i18n-key="appendixH.additionalSources">
             ${L.additionalSources || "Additional sources"}: ${sources.map((s) => esc(s)).join("; ")}
           </p>`
        : ""
    }
  </section>`;
...
function implementationChecklistHTML(model = {}, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixI.title">${L.appendix || "Appendix"} I — ${L.implementationChecklist || "Implementation Checklist"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixI.phase">${L.phase || "Phase"}</th>
          <th data-i18n-key="appendixI.activities">${L.activities || "Activities"}</th>
          <th data-i18n-key="appendixI.owner">${L.owner || "Owner"}</th>
          <th data-i18n-key="appendixI.successCriteria">${L.successCriteria || "Success Criteria"}</th>
        </tr>
      </thead>
      <tbody>
        ${rows}
    ...
    <p class="small" data-i18n-key="appendixI.summary">
      ${L.totalTasks || "Total tasks"}: <strong>${totalTasks}</strong>
      &nbsp;|&nbsp; ${L.completed || "Completed"}: <strong>${completedTasks}</strong>
    </p>
  </section>`;
}
...
function decisionFrameworkHTML(model = {}, L = {}) {
  // API returns { scenarios: [...] }, so use scenarios instead of items
  const items = Array.isArray(model.scenarios) && model.scenarios.length > 0 ? model.scenarios : 
                Array.isArray(model.items) && model.items.length > 0 ? model.items : [];
  if (!items.length) {
    if (typeof console !== "undefined") {
      if (!Array.isArray(model.scenarios) && !Array.isArray(model.items)) {
        console.log("[APPENDIX-J] Not added: model.scenarios/items is not an array", { model });
      } else {
  ...
  const rows = items
    .map(
      (item) => `
    <tr>
      <td>${esc((item && item.scenario) || (item && item.decision) || "")}</td>
      <td>${esc((item && item.symptom) || (item && item.criteria) || "")}</td>
      <td>${esc((item && item.rootCause) || (item && item.options) || "")}</td>
      <td>${esc((item && item.action) || (item && item.recommendation) || "")}</td>
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixJ.title">${L.appendix || "Appendix"} J — ${L.decisionFramework || "Decision Framework"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th style="width:12%" data-i18n-key="appendixJ.decision">${L.decision || "Decision"}</th>
          <th style="width:13%" data-i18n-key="appendixJ.criteria">${L.criteria || "Criteria"}</th>
          <th style="width:14%" data-i18n-key="appendixJ.rootCause">${L.rootCause || "Root Cause"}</th>
          <th style="width:16%" data-i18n-key="appendixJ.recommendedAction">${L.recommendedAction || "Recommended Action"}</th>
          <th style="width:16%" data-i18n-key="appendixJ.rationale">${L.rationale || "Rationale"}</th>
          <th style="width:12%" data-i18n-key="appendixJ.owner">${L.owner || "Owner"}</th>
          <th style="width:12%" data-i18n-key="appendixJ.status">${L.status || "Status"}</th>
        </tr>
      </thead>
      <tbody>
        ${rows}
...
function resourceRequirementsHTML(model = {}, L = {}) {
  ...
  return `
  <section class="appendix">
    <h3 data-i18n-key="appendixK.title">${L.appendix || "Appendix"} K — ${L.resourceRequirements || "Resource Requirements"}</h3>
    <table class="table small">
      <thead>
        <tr>
          <th data-i18n-key="appendixK.role">${L.role || "Role"}</th>
          <th data-i18n-key="appendixK.fte">${L.fte || "FTE %"}</th>
          <th data-i18n-key="appendixK.duration">${L.duration || "Duration"}</th>
          <th data-i18n-key="appendixK.responsibilities">${L.responsibilities || "Responsibilities"}</th>
        </tr>
      </thead>
      <tbody>
        ${rows}
...
function buildAllAppendices({
  ...
  console.log(
    "[APPENDIX DEBUG ORDER]",
    "I:", implementationChecklist?.items?.length,
    "J:", decisionFramework?.scenarios?.length
  );

  const content = [
    chartersHTML(charters, L),
    raciHTML(raci, L),
    raidHTML(raid, L),
    benefitsHTML(benefits, L),
    plan100HTML(plan100, L),
    pilotHTML(pilot, L),
    assumptionsHTML(assumptions, L),
    // [KT:SURGICAL] Appendix order: I → J → K (enforced order to prevent rendering swap)
    // [ISSUE-1-FIX] Correct order: Implementation Checklist is Appendix I, Decision Framework is Appendix J, Methods is Appendix J
    implementationChecklistHTML(implementationChecklist, L), // Appendix I
    methodsSourcesHTML(methodsSources, L),                   // Appendix J
    decisionFrameworkHTML(decisionFramework, L),              // Appendix K - REORDERED TO FIX SEQUENCE
    resourceRequirementsHTML(resourceRequirements, L),        // Appendix L
  ]
    .filter(Boolean)
    .join("\n");
