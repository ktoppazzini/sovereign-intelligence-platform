/* lib/reportGraphs.js
   T4: Unified chart theming + accessibility + print
   - Blue canvas (#0E2A44), white grid & labels
   - Horizontal Y-axis title at top-left (prevents cut-offs)
   - Currency-aware tick formatting (CAD/USD/GBP/EUR/INR) with commas
   - Dropdown to switch types (Line/Bar/Pie/Doughnut)
   - Print-safe: Letter portrait; no horizontal scrollbar from charts

   NOTE: This module guarantees NAMED exports used elsewhere:
         lineChartHTML, barChartHTML, heatmapHTML, benchmarkHTML, GRAPH_CSS
         and also provides a default export object.
*/
...
function detectCurrency(yTitle='') {
  ...
  if (t.includes('USD') || t.includes('$'))  return { code: 'USD', locale: 'en-US', sym: '$' };
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      ...
      <text x="${p - 14}" y="${y + 8}" text-anchor="end" fill="#FFFFFF" font-size="16">${fmtTick(val, yTitle)}</text>
    `);
  }
  ...
  const xTicks = labels.map((t, i) => {
    ...
    return `<text x="${x}" y="${h - p + 18}" text-anchor="middle" fill="#FFFFFF" font-size="16">${esc(t)}</text>`;
  }).join('');

  const axes = `<polyline points="${p},${p} ${p},${h - p} ${w - p},${h - p}" fill="none" stroke="rgba(255,255,255,.65)"/>`;

  const xTitleText = xTitle
    ? `<text x="${w / 2}" y="${h - 6}" text-anchor="middle" fill="#9AD0FF" font-size="18" font-weight="700">${esc(xTitle)}</text>`
    : '';
  const yTitleText = yTitle
    ? `<text x="${p - 16}" y="${p - 14}" text-anchor="end" fill="#9AD0FF" font-size="18" font-weight="700">${esc(yTitle)}</text>`
    : '';
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  const w = 720, h = 300, p = 92;  // slightly narrower to avoid horizontal scroll in letter portrait
    ...
    <div class="canvas-wrap">
      ...
      <svg class="plot bar" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ...
        ${data.map((v, i) => {
          ...
          const bw = slot * 0.90; // wider bars so plot looks bigger
...
// Keep a **named** export exactly called `barChartHTML` for back-compat.
export function barChartHTML(args = {}) {
  return lineChartHTML({ ...args, title: args?.title });
}

// Dashboard / benchmark card — NAMED export for reportTemplate/implKitHtml
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  const w = 720, h = 220, p = 92;
...
// Shared chart CSS (includes print letter portrait + avoid horizontal scroll)
...
/* Pure CSS switcher (if :has unsupported, .plot.line stays visible by default) */
...
/* Print & page-fit rules */
@media print {
  @page { size: Letter portrait; margin: 0.75in; }
  .chart-card{ break-inside: avoid; -webkit-region-break-inside: avoid; page-break-inside: avoid; }
  svg.plot{ max-width:100%; height:auto; }
}
`;

// Back-compat default export (optional but harmless)
const _default = { lineChartHTML, barChartHTML, heatmapHTML, benchmarkHTML, GRAPH_CSS };
