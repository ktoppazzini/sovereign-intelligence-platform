...
export function visualTimelineHTML(phases = []) {
  ...
  // [KT:SURGICAL:PHASE-LABELS] Show 'Phase X - [Phase Name]' above each bar, dynamic and auditable
  const lanes = phases.map((phase, i) => {
    ...
    // Make the "Scale" label larger if this is the last phase
    const isScale = /scale/i.test(phase.name);
    const labelFontSize = isScale ? 24 : 16;
    // Show Phase X - [Phase Name] above the bar
    const phaseLabel = `<text x="${x1 + phaseWidth/2}" y="${y + laneHeight*0.18}" text-anchor="middle" fill="#FF3333" font-size="15" font-weight="bold">Phase ${i+1} - ${esc(phase.name)}</text>`;
    const label = `<text x="${x1 + phaseWidth/2}" y="${y + laneHeight*0.5}" text-anchor="middle" fill="#FFFFFF" font-size="${labelFontSize}" font-weight="700">${esc(phase.name)}</text>`;
...
export function topInsightsHeaderHTML() {
  ...
  <span style="background:#FF8C00; color:#000; padding:4px 8px; border-radius:4px; font-size:14px;">Overall</span>
  <span style="margin-left:8px; font-size:14px; color:#fff;">&amp; Key Insights</span>
    </div>
  `;
}
...
export function riskHeatmapHTML(risks = []) {
  ...
  function chartSummaryRowRisk({ risks }) {
    ...
    const topRisk = uniqueRisks && uniqueRisks.length ? uniqueRisks[0].name : 'N/A';
    const topDesc = uniqueRisks && uniqueRisks.length ? (uniqueRisks[0].description || 'No description provided.') : '';
    // Remove checkbox and delete label (surgical)
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">Overall:</span> <span style="color:#fff;">Risk levels are distributed across the matrix, highlighting areas of concern.</span></div>
        <div><span style="font-weight:bold;">Key Insights:</span> <span style="color:#fff;">Top risk: ${topRisk}. ${topDesc}</span></div>
      </div>
    `;
  }
  ...
  const riskList = `
    <div class="risk-list" style="margin-top:16px; color:#fff;">
      ...
      <ol style="margin:8px 0 0 20px; padding:0;">
        ${uniqueRisks.map((r, i) => `<li style="margin-bottom:4px;"><span style="font-weight:bold;">${esc(r.name)}</span>: ${r.description ? esc(r.description) : 'No description provided.'}</li>`).join('')}
      </ol>
    </div>
  `;
  return `
...
export function waterfallChartHTML(components = [], currency = '$') {
  ...
  function chartSummaryRowValueBridge({ components, currency }) {
    ...
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">Overall:</span> <span style="color:#fff;">${icon} Values show ${trend} trend from ${currency}${firstVal} to ${currency}${lastVal} (${changePercent > 0 ? '+' : ''}${changePercent}%).</span></div>
        <div><span style="font-weight:bold;">Key Insights:</span> <span style="color:#fff;">Peak value of ${currency}${maxVal} achieved. Average performance: ${currency}${Math.round(avgVal)}.</span></div>
      </div>
    `;
  }
  ...
  const yAxisTicks = Array.from({length: yTicks}, (_, i) => {
    const v = minValue + (range * (yTicks - 1 - i)) / (yTicks - 1);
    const y = yScale(v);
    return `<text x="${p - 16}" y="${y + 4}" text-anchor="end" fill="#B6C2E2" font-size="13">${currency}${Math.round(v)}</text>`;
  }).join('');
  // Y Axis line
  const yAxisLine = `<line x1="${p - 8}" y1="${p}" x2="${p - 8}" y2="${h - p}" stroke="#B6C2E2" stroke-width="2"/>`;
  // Y Axis label
  const yAxisLabel = `<text x="${p - 48}" y="${(h) / 2}" text-anchor="middle" fill="#B6C2E2" font-size="15" transform="rotate(-90 ${p - 48},${(h) / 2})">Value</text>`;
  // X Axis label
  ...
  return `
    <figure class="chart-card waterfall-card" data-type="line" data-widget="series">
      ...
      <header class="chart-head">
        <h3 class="chart-title">Value Bridge Analysis Graph</h3>
        ${typeDropdown}
      </header>
      <div class="canvas-wrap">
        ${lineSVG}
    ...
    <div class="executive-infographic" style="background: linear-gradient(135deg, #0B1A3D 0%, #1e3a5f 100%); padding: 2rem 1.5rem; border-radius: 16px; margin: 2rem 0; max-width: 100%; box-sizing: border-box;">
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 2rem; max-width: 100%;">
        <div style="background: rgba(16,185,129,0.1); border: 2px solid #10B981; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          ...
          <div style="font-size: 2.5rem; color: #10B981; font-weight: 700; font-family: 'Merriweather', 'Georgia', serif; line-height: 1;">${esc(roi)}</div>
        </div>
        <div style="background: rgba(59,130,246,0.1); border: 2px solid #3B82F6; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          ...
          <div style="font-size: 2.5rem; color: #3B82F6; font-weight: 700; font-family: 'Merriweather', 'Georgia', serif; line-height: 1;">${esc(payback)}</div>
        </div>
        <div style="background: rgba(245,158,11,0.1); border: 2px solid #F59E0B; border-radius: 8px; padding: 1.25rem 1rem; text-align: center;">
          ...
          <div style="font-size: 2.5rem; color: #F59E0B; font-weight: 700; font-family: 'Merriweather', 'Georgia', serif; line-height: 1;">${esc(savings)}</div>
        </div>
      </div>
      ...
      <div style="background: rgba(0,0,0,0.3); border-radius: 8px; padding: 1.5rem 1rem; margin-bottom: 1.5rem;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; max-width: 100%;">
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.5rem;">4 weeks</div>
          </div>
          ...
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.25rem;">4 weeks</div>
          </div>
          <div style="flex: 0 0 30px; text-align: center;">
            <svg width="30" height="16" viewBox="0 0 30 16">
              <path d="M 0 8 L 25 8 M 20 4 L 25 8 L 20 12" stroke="#10B981" stroke-width="2" fill="none"/>
          ...
          <div style="flex: 1; text-align: center; min-width: 0;">
            ...
            <div style="color: #FFFFFF; font-size: 0.75rem; font-weight: 600;">Scale</div>
            <div style="color: rgba(255,255,255,0.6); font-size: 0.95rem; margin-top: 0.25rem;">12 weeks</div>
          </div>
        </div>
      </div>
