...
// 1) Y-axis title is now HORIZONTAL and sits to the LEFT of the axis (near your arrow).
// 2) Y-axis tick values are much larger (remain white).
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  // grid + Y tick values (bigger font, keep white)
  const grid = [];
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      ...
      <text x="${p - 14}" y="${y + 8}" text-anchor="end" fill="#FFFFFF" font-size="16">${val}</text>
    `);
  }

  // X tick labels (unchanged)
  const slot = (w - 2 * p) / Math.max(labels.length, 1);
  const xTicks = labels.map((t, i) => {
    const x = p + i * slot + slot / 2;
    return `<text x="${x}" y="${h - p + 16}" text-anchor="middle" fill="#FFFFFF" font-size="16">${esc(t)}</text>`;
  }).join('');
  ...
  // Y-axis title — HORIZONTAL and to the LEFT of the ticks
  // 'end' anchor lines the right edge of the text with the axis area.
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  const w = 760, h = 320, p = 92;  // left padding already ample (prevents clipping)
...
// Added yTitle passthrough so these two cards can show the horizontal Y axis title identically.
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  const w = 760, h = 240, p = 92;
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  // axis labels for heatmap (kept as before)
...
// CSS (unchanged; does not modify axis text)
