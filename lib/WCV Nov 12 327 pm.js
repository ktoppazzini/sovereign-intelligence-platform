...
export function visualTimelineHTML(phases = []) {
  ...
  // [KT:SURGICAL:PHASE-LABELS] Add Phase 1-5 labels above each phase bar (visible, reversible, and commented)
  /*
    To restore the original (no Phase labels), comment out the block below and uncomment the original.
    The Phase label is only added to the main roadmap, not appendix/implHTMLKit.
  */
  const lanes = phases.map((phase, i) => {
    ...
    // Add Phase 1-5 label above the bar
    const phaseLabel = `<text x="${x1 + phaseWidth/2}" y="${y + laneHeight*0.18}" text-anchor="middle" fill="#FF3333" font-size="14" font-weight="bold">Phase ${i+1}</text>`;
...
export function waterfallChartHTML(components = [], currency = '$') {
  // Unified summary row for value bridge (only render the second/bottom summary row, not the first/top one)
  function chartSummaryRowValueBridge({ components, currency }) {
    ...
    // Only render the summary row once (bottom/after chart head)
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
