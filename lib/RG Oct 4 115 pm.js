// lib/reportGraphs.js
// Surgical update: (1) bigger, bold, colored axis titles and larger tick values
//                   (2) more left padding so Y-axis numbers never clip.
// Also keeps: blue canvas look, dropdown type switch (CSS :has) and
//             “NPS” -> “Net Promoter Score” title expansion.
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      ...
      <text x="${p - 12}" y="${y + 6}" text-anchor="end" fill="#FFFFFF" font-size="16">${val}</text>
    `);
  }
  ...
  const xTicks = labels.map((t, i) => {
    ...
    return `<text x="${x}" y="${h - p + 24}" text-anchor="middle" fill="#FFFFFF" font-size="16">${esc(t)}</text>`;
  }).join('');

  // axes
  const axes = `<polyline points="${p},${p} ${p},${h - p} ${w - p},${h - p}" fill="none" stroke="rgba(255,255,255,.65)"/>`;

  // axis titles (bigger, bold, contrasting color)
  ...
  const yTitleText = yTitle
    ? `<text transform="translate(28 ${h / 2}) rotate(-90)" text-anchor="middle" fill="#9AD0FF" font-size="18" font-weight="700">${esc(yTitle)}</text>`
    : '';
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  const w = 760, h = 320, p = 64;               // <- extra left padding to avoid clipped numbers
  ...
  <figure class="chart-card">
    ...
    <!-- NOTE: no footer captions -> avoids duplicate "Years"/"Savings" under charts -->
  </figure>`;
}

export function barChartHTML(args) {
  // still render all four modes; "bar" shows when selected from the dropdown
  return lineChartHTML({ ...args, title: args?.title });
}

export function benchmarkHTML({ title, current = 0, benchmark = 0 }) {
  const w = 760, h = 240, p = 64; // padding aligned with line/bar charts
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  const w = 760, h = 320, p = 64; // more left space so row labels never clip
  ...
  const ylab = r.map((t, i) => `<text x="${p - 8}" y="${p + (i + .5) * ch + 6}" text-anchor="end" fill="#9AD0FF" font-size="18" font-weight="700">${esc(t)}</text>`).join('');
...
// CSS (unchanged except: hide .chart-axes in case older templates still render it)
