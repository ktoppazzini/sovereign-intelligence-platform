...
.ssr-svg{ position:absolute; inset:0; pointer-events:none; border-radius:12px; }
...
export const HYDRATE_INLINE_SCRIPT = `(() => {
  ...
  function drawAxes(ctx, W, H, pad, xTitle, yTitle, maxVal, steps=5){
    ...
    ctx.strokeStyle = 'rgba(255,255,255,.35)'; ctx.fillStyle = '#E6F0FF'; ctx.lineWidth = 1;
  ...
  function drawPie(ctx, labels, data, doughnut=false){
    const W = ctx.canvas.width, H = ctx.canvas.height, cx = W/2, cy = H/2, R = Math.min(W,H)*0.35;
  ...
  function drawHeatmap(ctx, rows, cols, matrix){
    ...
    let min=Infinity, max=-Infinity; matrix.forEach(r=>r.forEach(v=>{ if(v<min)min=v; if(v>max)max=v; })); if (!Number.isFinite(min)) { min=0; max=1; }
    const color = (v)=>{ const t = (v-min)/Math.max(1e-6, (max-min)); const h = 200 - t*140; return 'hsl('+h+',80%,55%)'; };
    for(let r=0;r<nR;r++){ for(let c=0;c<nC;c++){ const x = pad + c*cw, y = pad + r*ch; ctx.fillStyle = color(Number(matrix[r][c])||0); ctx.fillRect(x,y,cw,ch); } }
    ...
    ctx.restore();
    ctx.save(); ctx.fillStyle='#E6F0FF'; ctx.textAlign='center'; cols.forEach((t,i)=>{ const x = pad + (i+.5)*cw; ctx.fillText(String(t), x, H-6); });
    ctx.textAlign='right'; rows.forEach((t,i)=>{ const y = pad + (i+.5)*ch; ctx.fillText(String(t), pad-6, y+3); }); ctx.restore();
  }
  ...
  function resizeCanvasToFigure(canvas){
    ...
    const target = Math.max(600, Math.round(rect.width - 24));
  ...
  function renderChart(fig){
    try{
      ...
      const xTitle = obj.xTitle || ''; const yTitle = obj.yTitle || '';
      ...
      const typeSel = fig.querySelector('.chart-type select'); const type = (typeSel && typeSel.value) || obj.type || 'line';
  ...
  function renderHeat(block){
    try{
      ...
      const obj = JSON.parse(spec); const rows = obj.rows||[]; const cols = obj.cols||[]; const data = obj.data||[];
  ...
  function renderBenchmark(fig){
    try{
      ...
      const current = Number(obj.current)||0; const benchmark = Number(obj.benchmark)||1;
      const max = Math.max(current, benchmark) * 1.2 || 1;
      ctx.fillStyle='#0E2A44'; ctx.fillRect(0,0,W,H);
      ctx.save(); ctx.strokeStyle='rgba(255,255,255,.35)'; ctx.fillStyle='#E6F0FF';
      ctx.beginPath(); ctx.moveTo(pad,H/2); ctx.lineTo(W-pad,H/2); ctx.stroke();
      ctx.textAlign='center'; ctx.fillText(title, W/2, pad-10); ctx.restore();
      const slot = (W-2*pad)/2, bw=slot*0.7;
      ctx.save(); ctx.fillStyle='#8bc6ff';
      const h1 = (current/max)*(H-2*pad); ctx.fillRect(pad+(slot-bw)/2, H/2 - h1, bw, h1);
      ctx.fillStyle='#b3ffa8';
      const h2 = (benchmark/max)*(H-2*pad); ctx.fillRect(pad+slot+(slot-bw)/2, H/2 - h2, bw, h2);
      ctx.restore();
    }catch(e){}
  }
  ...
  function hydrate(root=document){
    ...
    document.querySelectorAll('[data-heatmap]').forEach(renderHeat);
    document.querySelectorAll('figure[data-widget="benchmark"]').forEach(renderBenchmark);
  ...
  setTimeout(()=>hydrate(document), 400);
})();`;
