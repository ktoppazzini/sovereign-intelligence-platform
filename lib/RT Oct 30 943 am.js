...
/* ============================================================================
   ...
   - Constrain report to centered column; hide horizontal overflow.
   - Chart cards get dark background, subtle border, and room for pinned 'Type' UI.
   - Tables: bold white title above table; 2px white grid lines; white header text.
============================================================================ */
// [KT:SURGICAL] Legacy attempt (invalid top-level return). Keeping for history, commented-out.
// const styled = `
//   <style>
//     #report { max-width: 900px; margin: 0 auto; padding: 0 8px; overflow-x: hidden; }
//     .report { box-sizing: border-box; background:#000; color:#fff; padding: 16px; border-radius: 16px; }
...
//   </style>
// `;
// return wrapReport(html + styled); // ← INVALID at top-level; kept commented.
// // If this file builds the HTML string named `html`, make sure the return adds the style://
// // (If you don’t have wrapReport, then do: return html + styled;)//




/* ============================================================================
   [KT:SURGICAL][2025-10-30_00-11-29_EDT] — Post-process: table titles, chart notes, Image 4 paragraph discipline
============================================================================ */
export function __kt_postprocess_report_html(html){
  try{
    let out = String(html||'');

    // (1) Insert bold white titles above tables if missing
    out = out.replace(/(<table\b[^>]*class=(["'])[^"']*report-table[^"']*\2[\s\S]*?<\/table>)/gi, function(m){
      return '<div class="table-title">AI Interpreted Table Title</div>\n' + m;
    });

    // (2) Ensure a <p class="chart-note"> follows every chart/heatmap
    const vizRe = /(<figure[^>]*data-chart=['"][\s\S]*?<\/figure>|<div[^>]*data-heatmap=['"][\s\S]*?<\/div>)/gi;
    out = out.replace(vizRe, function(frag){ if(/class=["']chart-note["']/.test(frag)) return frag;
      return frag + '\n<p class="chart-note">This visual measures performance along its principal axes. Use the trend and variance to guide next‑quarter decisions.</p>'; });

    // (3) Image 4 paragraphs: enforce 3–4 sentences per <p>
    out = out.replace(/(<section[^>]*id=["']?image[-_ ]?4["']?[^>]*>[\s\S]*?<\/section>)/ig, function(sec){
      return sec.replace(/<p>([\s\S]*?)<\/p>/gi, function(_, txt){
        const sentences = String(txt).split(/(?<=[.!?])\s+/).filter(Boolean);
        if (sentences.length <= 4) return '<p>'+String(txt).trim()+'</p>';
        const first = sentences.slice(0,4).join(' ');
        const rest  = sentences.slice(4).join(' ');
        return '<p>'+first+'</p>' + (rest ? '<p>'+rest+'</p>' : '');
      });
    });
    return out;
  }catch(e){ console.warn('[KT] postprocess error', e); return html; }
}
