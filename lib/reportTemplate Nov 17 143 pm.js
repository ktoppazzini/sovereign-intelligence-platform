...
// [KT:SURGICAL:APPX-PREDICTIVE] Build Predictive Appendix chart (uses global TYPE switcher)
function predictiveAppendixHTML(report, sections, currency) {
  const pm = sections?.predictiveModel || {};
  const labels = Array.isArray(pm.labels) && pm.labels.length ? pm.labels : ['T1','T2','T3','T4','T5','T6'];
  const values = Array.isArray(pm.values) && pm.values.length === labels.length
    ? pm.values : [10,14,17,20,23,26];

  const spec = {
    type: 'line',
    title: 'Predictive Outcome (Projected)',
    xTitle: pm.xTitle || 'Time',
    yTitle: pm.yTitle || `Value (${currency || '$'})`,
    labels,
    datasets: [{ label: 'Projection', data: values }]
  };

  // Encode JSON with &quot; exactly as required by your renderer
  const encoded = String(JSON.stringify(spec)).replace(/"/g, '&quot;');

  return `
  <section class="report-section">
    <h3>Appendix — Predictive Analytics</h3>
     <figure data-chart="&quot;${encoded}&quot;">
      <div class="chart-ui"><label>TYPE:</label>
    ...
    <p class="chart-note">Projection derived from report context by AI. Use TYPE to compare views.</p>
  </section>`;
}
/* ============================================================================
...
function unifyTables(html = '') {
  ...
  html += `
  <section>
    <h2 id="timeline">Implementation Plan</h2>
  ...
  <article class="report" dir="${direction}" lang="${esc(canon?.lang || 'en')}">
    <section class="title-page">
      <div class="title-header">
        <div class="title-logo">
          ...
          <div class="brand-text">
            ...
            <div class="brand-tagline">Strategic Advisory</div>
          </div>
        </div>
      ...
      <div class="title-main">
        ...
        <h2 class="title-subtitle">Strategic Transformation &amp; Value Creation Plan</h2>
      ...
      <div class="title-footer">
        <div class="title-meta-grid">
          <div class="meta-item">
            <span class="meta-label">${L.preparedFor}</span>
          ...
          <div class="meta-item">
            <span class="meta-label">${L.timeline}</span>
            <span class="meta-value">${esc(canon?.timeFrame || '—')}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">${L.date}</span>
            <span class="meta-value">${esc(canon?.reportDate || new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }))}</span>
          </div>
        </div>
        <div class="title-confidential">${L.confidential} | FOR INTERNAL USE ONLY</div>
      </div>
    </section>
`;
  ...
  reportHtml += `
    ...
    <section>
      ...
      ${worldClassVisuals.waterfallChart ? (() => {
        // [KT:AUDIT:VALUE-BRIDGE-INJECT] Logging Value Bridge chart injection for auditability and debugging
        console.log('[KT:AUDIT][reportTemplate] worldClassVisuals.waterfallChart:', worldClassVisuals.waterfallChart);
        return `
        <div class="chart-desc" style="font-size:14px; color:#e5e7eb; margin:0 0 6px 0;">Description: Waterfall / Value Bridge overview</div>
        <h3>Value Bridge Analysis</h3>
        ${worldClassVisuals.waterfallChart}
        <h3>Value Bridge Analysis</h3>
        `;
      })() : ''}
    ...
    <section>
      ...
      ${prioritizationMatrixHTML({ title: L.prioritizationMatrix })}
...
.report .table .table-title{
  ...
  if (implKit) {
    reportHtml += [
      implKit.charters?.length ? chartersHTML(implKit.charters, L) : '',
      implKit.raci ? raciHTML(implKit.raci, L) : '',
      implKit.raid ? raidHTML(implKit.raid, L) : '',
      implKit.benefits ? benefitsHTML(implKit.benefits, currencySymbol, L) : '',

      // [KT:SURGICAL:APPX-PREDICTIVE:INCLUDE]
      predictiveAppendixHTML(canon, sections, currencySymbol),

      /* [KT:LEGACY:APPX-PREDICTIVE-PLACEHOLDER] Commented out static/dummy predictive appendix graph (no deletions) */
      // predictiveAppendixHTML({}, {}, '$'),

      implKit.plan100 ? plan100HTML(implKit.plan100, currencySymbol, L) : '',
      // [KT:AUDIT] Pilot appendix audit log and fallback
      (implKit.pilot && Object.keys(implKit.pilot).some(k => implKit.pilot[k]))
        ? pilotHTML(implKit.pilot, currencySymbol, L)
        : '<section class="report-table"><h2>Appendix F — Pilot</h2><div style="color:#ffb300; font-weight:bold;">[KT:AUDIT] No pilot data available for this report run.</div></section>',
      implKit.assumptions ? assumptionsHTML(implKit.assumptions, currencySymbol, L) : '',
      implKit.methods ? methodsSourcesHTML(implKit.methods, currencySymbol, L) : '',
      // BEYOND MCKINSEY: Self-implementation tools
      implementationChecklistHTML(canon, L),
      decisionFrameworkHTML(L),
      resourceRequirementsHTML(canon, L)
    ].join('');
  }

  /* [KT:SURGICAL:PORT-DARK] Add dark-theme overrides with white borders */
  reportHtml += `
  <style>
