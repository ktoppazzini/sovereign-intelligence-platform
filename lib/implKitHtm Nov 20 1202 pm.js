...
function chartersHTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-A] Not added: model.items is empty or missing', { model });
  }
  return `
  <section class="appendix">
    ...
    <table class="table small">
      ...
      <tbody>${rows || '<tr><td colspan="5" class="small">No data available</td></tr>'}</tbody>
    </table>
  </section>`;
}
...
function raciHTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-B] Not added: model.items is empty or missing', { model });
  }
  return `
  <section class="appendix">
...
function raidHTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-C] Not added: model.items is empty or missing', { model });
  }
  return `
  <section class="appendix">
    ...
    <table class="table small">
      ...
      <tbody>${rows || '<tr><td colspan="8" class="small">No data available</td></tr>'}</tbody>
    </table>
  </section>`;
}
...
function benefitsHTML(model) {
  ...
  if (!lines.length && typeof console !== 'undefined') {
    console.log('[APPENDIX-D] Not added: model.lines is empty or missing', { model });
  }
  ...
  <section class="appendix">
    ...
    <table class="table small">
      ...
      <tbody>${rows || '<tr><td colspan="11" class="small">No data available</td></tr>'}</tbody>
    </table>
...
function plan100HTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-E] Not added: model.weeks is empty or missing', { model });
  }
  return `
  <section class="appendix">
...
function pilotHTML(model = {}) {
  const hasData = model && (model.objective || model.scope || model.metrics || model.timeline || model.sites);
  if (!hasData && typeof console !== 'undefined') {
    console.log('[APPENDIX-F] Not added: model is empty or missing', { model });
  }
  return `
  <section class="appendix">
    <h3>Appendix F — Pilot Charter</h3>
    <table class="table small">
      <tr><th>Objective</th><td>${esc(model.objective) || '<span class="small">No data available</span>'}</td></tr>
      <tr><th>Scope</th><td>${esc(model.scope) || '<span class="small">No data available</span>'}</td></tr>
      <tr><th>Success metrics</th><td>${esc(model.metrics) || '<span class="small">No data available</span>'}</td></tr>
      <tr><th>Timeline</th><td>${esc(model.timeline) || '<span class="small">No data available</span>'}</td></tr>
      <tr><th>Sites</th><td>${esc(model.sites) || '<span class="small">No data available</span>'}</td></tr>
    </table>
  </section>`;
}
...
function assumptionsHTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-G] Not added: model.items is empty or missing', { model });
  }
  return `
  <section class="appendix">
    ...
    <table class="table small">
      ...
      <tbody>${rows || '<tr><td colspan="4" class="small">No data available</td></tr>'}</tbody>
    </table>
  </section>`;
}
...
function methodsHTML(model) {
  ...
  `).join("");
  if (!rows && typeof console !== 'undefined') {
    console.log('[APPENDIX-H] Not added: model.items is empty or missing', { model });
  }
  return `
  <section class="appendix">
...
function implementationChecklistHTML(model = {}) {
  ...
  if (!items.length && typeof console !== 'undefined') {
    console.log('[APPENDIX-I] Not added: model.items is empty or missing', { model });
  }
...
function decisionFrameworkHTML(model = {}) {
  ...
  if (!scenarios.length && typeof console !== 'undefined') {
    console.log('[APPENDIX-J] Not added: model.scenarios is empty or missing', { model });
  }
...
function resourceRequirementsHTML(model = {}) {
  const items = safeArray(model.items);
  if (!items.length && typeof console !== 'undefined') {
    console.log('[APPENDIX-K] Not added: model.items is empty or missing', { model });
  }
  const rows = items.map(item => `
    <tr>
  ...
  <section class="appendix">
    ...
    <table class="table small">
      ...
      <tbody>${rows || '<tr><td colspan="6" class="small">No data available</td></tr>'}</tbody>
    </table>
  </section>`;
}
