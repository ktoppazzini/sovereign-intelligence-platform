...
/* ---------------------------------------------------------------------------
   lib/reportGraphs.js
   Unified chart styling — UPDATED (KT: Black card + Amber series + Area type)

   - Card background switched to near-black (#0b0f14).
   - Series color switched to amber (#ff7a00) for line/area/bar.
   - Type dropdown now supports Line / Area / Bar (pie/doughnut retained but hidden by default).
   - No deletions: prior blue theme is commented out with [KT:COMMENT-OUT].
   - SVG axes/title colors normalized to white for readability (blue titles commented).
   - Toggle script now also handles "area" plot.
--------------------------------------------------------------------------- */
...
// NOTE: Colors updated to neutral/white; previous blue tokens commented.
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  return `
    <!-- [KT:COMMENT-OUT] blue card -->
    <!-- <rect x="0" y="0" width="${w}" height="${h}" rx="12" ry="12" fill="#000000"/> -->
    <!-- [KT:NEW] black plot background to match black card -->
    <rect x="0" y="0" width="${w}" height="${h}" rx="12" ry="12" fill="#000000"/>
    ${grid.join('')}
    ${axes}
    ${xTicks}
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  ...
  // Build area polygon (amber with soft opacity)
  const areaPoly = (() => {
    ...
    <header class="chart-head">
      ...
      <label class="chart-type"><span>Type:</span>
        <select aria-label="Chart type">
          ...
          <!-- keep legacy options but not default -->
  ...
  <!-- [KT:COMMENT-OUT] blue line stroke -->
  <!-- <polyline points="${pts}" fill="none" stroke="#8bc6ff" stroke-width="2"/> -->
  <!-- [KT:NEW] amber line -->
  <polyline points="${pts}" fill="none" stroke="#ff7a00" stroke-width="3"/>
</svg>
<!-- LINE -->
<svg class="plot line" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
  ${svgAxes({ w, h, p, xTitle, yTitle, maxY: M, labels: lbl })}

  <!-- [KT:SURGICAL:BROWN-AREA] fill under line -->
  <polygon points="${areaPts}" fill="#5a3a18" opacity="0.65"></polygon>
...
/>
      ...
      <svg class="plot area" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ${svgAxes({ w, h, p, xTitle, yTitle, maxY: M, labels: lbl })}
        <!-- [KT:NEW] amber line + soft amber fill -->
  ...
  return { x: Number(x.toFixed(1)), y: Number(y.toFixed(1)) };
      ...
      <svg class="plot pie" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ...
        ${(() => {
          ...
          return data.map((v, i) => {
            ...
            // keep colorful wedges; not part of blue canvas
    ...
    <!-- UPDATED: switcher includes 'area' and never appends duplicate charts -->
    <script>
      (function(){
  ...
  (function(){
    ...
    function apply(){
      var value = sel.value;
      // Set the active type on the card
      card.setAttribute('data-type', value);
      // Toggle which SVG is visible (the rest hidden)
      card.querySelectorAll('.plot').forEach(function(svg){
        svg.style.display = svg.classList.contains(value) ? 'block' : 'none';
      });
      // For "area", ensure both LINE polyline and the brown polygon show (they live in the same LINE svg)
      if(value === 'area'){
        card.querySelector('.plot.line')?.style.setProperty('display','block');
      }
    }

    sel.addEventListener('change', apply);
    apply(); // initialize based on default/selected
  })();
</script>
...
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  ...
  <figure class="chart-card" data-type="bar">
    <header class="chart-head"><h3>${esc(nice)}</h3>
    <label class="chart-type"><span>Type:</span>
  <select aria-label="Chart type">
    <option value="line" selected>Line</option>
    <option value="area">Area</option>
    ...
    <div class="canvas-wrap">
      <svg class="plot bar" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
        ...
        <rect x="${(p + (slot - bw) / 2).toFixed(1)}" y="${(h - p - h1).toFixed(1)}" width="${bw.toFixed(1)}" height="${h1.toFixed(1)}" fill="#ff7a00"/>
        <rect x="${(p + slot + (slot - bw) / 2).toFixed(1)}" y="${(h - p - h2).toFixed(1)}" width="${bw.toFixed(1)}" height="${h2.toFixed(1)}" fill="#ff7a00"/>
      </svg>
    </div>
  </figure>`;
}
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  const cells = r.flatMap((_, ri) => c.map((_, ci) => {
    ...
    // keep blue→green heat scale (independent of card color)
    const hue = 200 - t * 140;
...
/* [KT:COMMENT-OUT] blue select */
.chart-type select{
  background:#0E2A44;   /* dropdown bg */
  color:#E6F0FF;        /* text color */
  border:1px solid #4b6a8d;
  border-radius:8px;
  padding:4px 8px;
  outline:none;
}
.chart-type select:focus{ box-shadow:0 0 0 2px rgba(155,208,255,.25); }
...
/* [KT:COMMENT-OUT] blue legend swatch */
...
/* Pure CSS switcher (kept commented to avoid double-behavior)
