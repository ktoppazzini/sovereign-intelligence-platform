...
// [KT:I18N:INJECTABLE-IMPLKIT]
// Allows the reform-report translator to localize this module.
let __implKitTranslator = (s) => s;

export function setImplementationKitTranslator(fn) {
  __implKitTranslator = typeof fn === 'function' ? fn : (s) => s;
}
function t(s) {
  try {
    return __implKitTranslator ? __implKitTranslator(s) : s;
  } catch {
    return s;
  }
}
...
function chartersHTML(model) {
  ...
  const blocks = items
    .map((c) => {
      ...
      return `
      <h4>${esc(c.name || "")}</h4>
      <table class="report-table table small">
...
function implementationChecklistHTML(model = {}) {
  ...
  if (!items.length && Array.isArray(model.phases)) {
    items = model.phases;
  }

  if (!items.length && Array.isArray(model)) {
    items = model;
  }
  ...
  if (!items.length) {
    debugAppendix("APPENDIX-I", "Not added: items array is empty or missing", {
      hasItemsArray: Array.isArray(model.items),
      hasPhasesArray: Array.isArray(model.phases),
      isArrayModel: Array.isArray(model),
    });
    return "";
  }
...
function decisionFrameworkHTML(model = {}) {
  ...
  if (!items.length && Array.isArray(model.scenarios) && model.scenarios.length > 0) {
    items = model.scenarios;
  }

  if (!items.length) {
    if (typeof console !== "undefined") {
      if (!Array.isArray(model.items) && !Array.isArray(model.scenarios)) {
        console.log("[APPENDIX-J] Not added: model.items/scenarios not arrays", {
          modelShape: Object.keys(model || {}),
        });
      } else {
        console.log("[APPENDIX-J] Not added: items/scenarios array empty", {
          itemsLen: Array.isArray(model.items) ? model.items.length : 0,
          scenariosLen: Array.isArray(model.scenarios)
...
function resourceRequirementsHTML(model = {}) {
  ...
  if (!items.length) {
    const collected = [];
    ["team", "budget", "tools"].forEach((key) => {
      if (Array.isArray(model[key]) && model[key].length > 0) {
        collected.push(...model[key]);
      }
    });
  ...
  if (!items.length) {
    if (typeof console !== "undefined") {
      if (!Array.isArray(model.items)) {
        console.log("[APPENDIX-K] Not added: model.items is not an array", {
          modelShape: Object.keys(model || {}),
        });
      } else {
        console.log("[APPENDIX-K] Not added: items array is empty", {
          itemsLen: model.items.length,
        });
      }
    }
    return "";
