...
// ---------- HTML helpers (SSR fallback visible even if JS blocked) ----------
  ...
  <figure class="chart-card" data-chart='${JSON.stringify({type:'line', title, labels: lbl, datasets:[{label:title||'Series', data: arr}], xTitle, yTitle})}'>
    ...
    <div class="canvas-wrap">
      ...
      <svg class="ssr-svg" width="${w}" height="${h}" role="img" aria-label="fallback">
        <rect x="0" y="0" width="${w}" height="${h}" rx="12" ry="12" fill="#193A60"/>
        <path d="${path}" fill="none" stroke="#8bc6ff" stroke-width="2"/>
      </svg>
    </div>
    <footer class="chart-axes">
      <div class="x">${xTitle||''}</div><div class="y">${yTitle||''}</div>
    </footer>
  </figure>`;
}
...
export function heatmapHTML({ id, title, rows = [], cols = [], data = [[]] }) {
  ...
  <section class="chart-card chart-heat" data-heatmap='${JSON.stringify({rows:r, cols:c, data:m})}'>
    ...
    <div class="canvas-wrap"><canvas width="900" height="320" aria-label="${title||'Heat Map'}"></canvas></div>
  </section>`;
}
...
.chart-head{ display:flex; align-items:center; justify-content:space-between; gap:12px; margin:4px 4px 8px; }
...
.canvas-wrap canvas{ display:block; width:100%; height:auto; border-radius:12px; }
...
export const HYDRATE_INLINE_SCRIPT = `(() => {
  ...
  function drawLine(ctx, labels, data, xTitle, yTitle){
    ...
    ctx.save();
    ctx.strokeStyle = '#8bc6ff'; ctx.lineWidth = 2;
    data.forEach((v,i)=>{
  ...
  function renderChart(fig){
    try{
      ...
      const xTitle = obj.xTitle || '';
      const yTitle = obj.yTitle || '';

      const canvas = fig.querySelector('canvas');
      const svgFallback = fig.querySelector('.ssr-svg');
      if (!canvas) return;
    ...
    }catch(e){ if (DEBUG) console.error('renderChart failed', e); }
  }

  function renderHeat(block){
    try{
      const spec = block.getAttribute('data-heatmap');
      if (!spec) return;
      const obj = JSON.parse(spec);
      const rows = Array.isArray(obj.rows)? obj.rows : [];
      const cols = Array.isArray(obj.cols)? obj.cols : [];
    ...
    }catch(e){ if (DEBUG) console.error('renderHeat failed', e); }
  }

  function renderBenchmark(fig){
    try{
      const spec = fig.getAttribute('data-spec');
      if (!spec) return;
      const obj = JSON.parse(spec);
      const canvas = fig.querySelector('canvas');
      if (!canvas) return;
      ...
      const benchmark = Number(obj.benchmark)||1;
      ...
      ctx.restore();
    }catch(e){ if (DEBUG) console.error('renderBenchmark failed', e); }
  }

  function hydrate(root=document){
    // Render explicit AI blocks
    $('[data-chart]', root).forEach(renderChart);
    $('[data-heatmap]', root).forEach(renderHeat);
    $('figure[data-widget="benchmark"]', root).forEach(renderBenchmark);

    // Wire dropdowns
    $('.chart-type select', root).forEach(sel=>{
  ...
  // Run now, then again after load, plus a short retry (for iframe srcdoc timing).
