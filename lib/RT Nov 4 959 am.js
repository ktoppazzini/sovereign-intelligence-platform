...
`;
  ...
  reportHtml += `
    ...
    <section>
      ...
      ${SEC.kpis.html}
      <section class="kpi-dash dashboard-wide">
  <div class="kpi-grid">
    ...
  </div>
</section>
...
<script>
(function(){<script>
/* [KT:SURGICAL] Rebuild Chart.js when the "Type" dropdown changes */
(function typeSwitcher(){
  ...
  document.addEventListener('change', function(e){
    ...
    var fig = sel.closest('figure[data-chart]');
    if(!fig) return;

    var spec = parseSpec(fig);
    if(!spec) return;

    var newType = String(sel.value || '').toLowerCase();
    if(!newType) return;

    // Update spec + persist back onto the element so exports keep the choice
    spec.type = newType;
    try{
      fig.setAttribute('data-chart', JSON.stringify(spec).replace(/"/g,'&quot;'));
    }catch{}

    // (Re)build chart
    var canvas = fig.querySelector('canvas');
    if(!canvas){ canvas = document.createElement('canvas'); fig.appendChild(canvas); }
    var ctx = canvas.getContext('2d');

    // Destroy the old instance if present
    if(fig.__chart && typeof fig.__chart.destroy === 'function'){
      try{ fig.__chart.destroy(); }catch(_){}
    }

    // Create a fresh chart with the new type
    try{
      fig.__chart = new Chart(ctx, {
        type: spec.type || 'line',
      ...
      console.warn('[TypeSwitch] Chart rebuild failed:', err);
    }
  }, true);
})();
</script>
...
<script>
/* Global Type switcher for Chart.js figures */
(function(){
  // Destroy & rebuild with new type (reads original data-chart spec)
  function rehydrate(fig, nextType){
    try{
      var raw = (fig.getAttribute('data-chart')||'').replace(/&quot;/g,'"').replace(/&amp;/g,'&');
      var spec = JSON.parse(raw||'{}');
      if(!spec || !Array.isArray(spec.datasets)) return;

      // update type in a shallow copy to avoid mutating attribute JSON
      var cfg = {
        type: (nextType||spec.type||'line').toLowerCase(),
        data: { labels: spec.labels||[], datasets: spec.datasets||[] },
        options: Object.assign({
          responsive:true, maintainAspectRatio:false,
          plugins:{ legend:{display:false}, title:{display:false} },
      ...
      // remove old chart instance if present
      if (fig.__chart) { try { fig.__chart.destroy(); } catch(_){} }

      fig.__chart = new Chart(ctx, cfg);
    }catch(e){ console.warn('[TypeSwitch] rehydrate error', e); }
  }

  // Event delegation so it also works for dynamically injected figures
  document.addEventListener('change', function(e){
    var sel = e.target && e.target.closest && e.target.closest('.chart-type-select, .chart-type select');
    if(!sel) return;
    var fig = sel.closest('figure[data-chart]');
    if(!fig) return;
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    if(e.stopPropagation) e.stopPropagation();
    rehydrate(fig, sel.value);
  }, true);
})();
</script>
