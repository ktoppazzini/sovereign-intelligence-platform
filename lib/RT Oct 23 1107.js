...
function __kt_tagStandardCharts(html = '', labels = [], canon = {}) {
  try {
    ...
    while ((match = re.exec(src)) !== null) {
      ...
      // Skip if already tagged or special widgets
      ...
      const title = titleMatch ? titleMatch[1].replace(/<[^>]*>/g, '').trim() : 'Chart';
      const typeMatch = open.match(/data-type=["']([^"']+)["']/i);
      const type  = (typeMatch ? typeMatch[1] : 'line').toLowerCase();

      // Axis heuristics
      const xTitle = labels && labels[0]?.startsWith('Y') ? 'Years' : 'Months';
      ...
      const attr = JSON.stringify(spec).replace(/"/g, '&quot;');
      const upgradedOpen = open + " data-chart='" + attr + "' data-origin=\"standard\"";
    ...
    console.log('[SR:REFORM] std.charts.tag.error', String(err?.message || err));
    return html;
  }
}
...
export default function buildReformReportHTML(opts) {
  ...
  const kpiDash = `
    <section class="kpi-dash">
     ...
     /* [KT:SURGICAL:KPI FULL-BLEED + LARGE CHARTS] */
/* Make KPI dashboard container span full width */
.kpi-dash {
  margin-left: -0.5in;
  margin-right: -0.5in;
}
/* Enlarge the KPI cards so text and axes are more readable */
.kpi-dash .chart-card {
  min-height: 260px;
  font-size: 1rem;
}
/* Adjust grid to two columns at wider widths */
@media (min-width: 1000px) {
  .kpi-grid { grid-template-columns: 1fr 1fr; }
  .kpi-dash .chart-card { min-height: 340px; }
}
...
<div class="phase">
  <div class="phase-title">Phase 1: Readiness & Data Setup</div>
  <div class="phase-description">Mobilize leadership, ...</div>
</div>
...
+ `>
  ...
  <style>
    ...
    @media print {
      ...
      /* Full-bleed KPI dashboard and larger cards */
.kpi-dash {
  margin-left: -0.5in;
  margin-right: -0.5in;
}
...
/* Full-bleed KPI dashboard and larger chart cards */
.kpi-dash {
  margin-left:-0.5in;
  margin-right:-0.5in;
}
