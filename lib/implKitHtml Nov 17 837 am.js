...
function resourceRequirementsHTML(model = {}) {
  // Build rows for the resource requirements table
  const items = (model && Array.isArray(model.items) ? model.items : []);
  const rows = items.map(item => `
    <tr>
      <td>${esc(item && item.role || "")}</td>
      <td style="text-align:right">${esc(item && item.quantity || "")}</td>
      <td>${esc(item && item.duration || "")}</td>
      <td style="text-align:right"><strong>${esc(item && item.cost || "")}</strong></td>
      <td>${esc(item && item.notes || "")}</td>
    </tr>
  `).join("");
  // Simple bar chart for resource cost breakdown (like Appendix D)
  const chart = barChartHTML({
    title: "Resource Cost Breakdown",
    labels: items.map(i => i.role || "").slice(0, 6),
    series: items.map(i => +i.cost || 0).slice(0, 6),
    xTitle: "Role",
    yTitle: "Cost",
    type: "bar"
  });
  ...
  <section class="appendix">
    ...
    <p class="small">Total resource cost (estimated): <strong>${
      (model && Array.isArray(model.items) 
        ? model.items.reduce((sum, item) => sum + (+item.cost || 0), 0)
        : 0
      ).toLocaleString()
    }</strong></p>
  </section>`;
...
function decisionFrameworkHTML(model = {}) {
  const rows = (model && Array.isArray(model.items) ? model.items : []).map(item => `
    <tr>
...
// Resource Requirements Appendix
...
function raciHTML(model) {
  const rows = (model && Array.isArray(model.items) ? model.items : []).map(x=>`
    <tr><td>${esc(x && x.decision || "")}</td><td>${esc(x && x.R || "")}</td><td>${esc(x && x.A || "")}</td><td>${esc((x && x.C||[]).join(", "))}</td><td>${esc((x && x.I||[]).join(", "))}</td><td>${esc(x && x.SLA || "")}</td></tr>
  `).join("");
  return `
  <section class="appendix">
...
function benefitsHTML(model) {
  ...
  const bridge = barChartHTML({
    title:"Monthly Value Bridge (illustrative)",
    labels:lines.map(l=>l.lever || "").slice(0,6),
    series:lines.map(l=>+l.monthlyImpact||0).slice(0,6),
    xTitle:"Levers", yTitle:"$/month", type:"bar"
  });
...
function implementationChecklistHTML(model = {}) {
  ...
  const totalTasks = (model && Array.isArray(model.items) ? model.items.length : 0);
  const completedTasks = (model && Array.isArray(model.items) 
    ? model.items.filter(item => (item.status || "").toLowerCase() === "complete" || (item.status || "").toLowerCase() === "completed").length
    : 0
  );
  // Simple bar chart for progress (reuse barChartHTML)
  const progressChart = barChartHTML({
    title: "Implementation Progress",
    labels: ["Completed", "Remaining"],
    series: [completedTasks, Math.max(0, totalTasks - completedTasks)],
    xTitle: "Status",
    yTitle: "Tasks",
    type: "bar"
  });
