...
function normalizeLogoPath(raw) {
  ...
  // If starts with public/, add leading slash
  if (/^public\//i.test(s)) return '/' + s;
...
export default function buildReformReportHTML(opts) {
  ...
  function __kt_renderTitlePage(canon, L) {
    try {
      // [KT:I18N:TITLE-PAGE] Ensure all title page text is in target language
      const headline = `${canon?.orgName || "Organization"} ${L.transformationPlan || "Strategic Transformation & Value Creation Plan"}`;
...
<figure data-widget="series" data-chart-id="exec-savings">
  ...
  >
    ...
    <section>
      ...
      ${ensureOne(SEC.ops.injected, lineChartHTML({ title: L.opsThroughputTrend || 'Ops Throughput Trend', labels, series: mkDefaultSeries(labels.length, 100), xTitle: labels[0].startsWith('Y') ? (L.years || 'Years') : (L.months || 'Months'), yTitle: L.indexAxis || 'Index' }))}
      ${SEC.ops.html}
    </section>
