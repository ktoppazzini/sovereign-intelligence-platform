  ...
  function chartSummaryRow({ trend, firstVal, lastVal, changePercent, dataMax, dataAvg, yTitle }) {
    ...
    return `
      <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
        <div><span style="font-weight:bold;">Overall:</span> <span style="color:#fff;">${icon} Values show ${trend} trend from ${fmtTick(firstVal, yTitle)} to ${fmtTick(lastVal, yTitle)} (${changePercent > 0 ? '+' : ''}${changePercent}%).</span></div>
        <div><span style="font-weight:bold;">Key Insights:</span> <span style="color:#fff;">Peak value of ${fmtTick(dataMax, yTitle)} achieved. Average performance: ${fmtTick(Math.round(dataAvg), yTitle)}.</span></div>
      </div>
    `;
  }
