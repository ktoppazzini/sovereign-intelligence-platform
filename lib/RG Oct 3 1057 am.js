// lib/reportGraphs.js
// SSR-only chart HTML (SVG) that matches your blue canvas styling.
// Surgical changes: bigger axis titles/values, bold colored titles, no duplicate footer labels,
// and "NPS" -> "Net Promoter Score" title expansion only.
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  const xTicks = labels.map((t, i) => {
    ...
    return `<text x="${x}" y="${h - p + 22}" text-anchor="middle" fill="#FFFFFF" font-size="14">${esc(t)}</text>`;
  }).join('');

  // axes
  const axes = `<polyline points="${p},${p} ${p},${h - p} ${w - p},${h - p}" fill="none" stroke="rgba(255,255,255,.65)"/>`;

  // axis titles (bigger, bold, blue)
  const xTitleText = xTitle
    ? `<text x="${w / 2}" y="${h - 6}" text-anchor="middle" fill="#9AD0FF" font-size="16" font-weight="700">${esc(xTitle)}</text>`
    : '';
  const yTitleText = yTitle
    ? `<text transform="translate(16 ${h / 2}) rotate(-90)" text-anchor="middle" fill="#9AD0FF" font-size="16" font-weight="700">${esc(yTitle)}</text>`
    : '';
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  ...
  <figure class="chart-card">
    ...
    <!-- NOTE: removed <footer class="chart-axes"> to avoid duplicate captions under the card -->
  </figure>`;
}
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  const xlab = c.map((t, i) => `<text x="${p + (i + .5) * cw}" y="${h - 6}" text-anchor="middle" fill="#9AD0FF" font-size="16" font-weight="700">${esc(t)}</text>`).join('');
  const ylab = r.map((t, i) => `<text x="${p - 6}" y="${p + (i + .5) * ch + 5}" text-anchor="end" fill="#9AD0FF" font-size="16" font-weight="700">${esc(t)}</text>`).join('');
...
/* Pure CSS switcher; only one plot is ever displayed */
