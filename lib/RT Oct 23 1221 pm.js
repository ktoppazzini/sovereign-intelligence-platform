...
function __kt_tagStandardCharts(html='', labels=[], canon={}) {
  try {
    ...
    while ((match = re.exec(src)) !== null) {
      scanned++;
      const start = match.index, end = re.lastIndex;
      const open  = match[1] || '';
      const inner = match[2] || '';
      ...
      const upgradedOpen = open + " data-chart='" + specJSON + "' data-origin=\"standard\"";
