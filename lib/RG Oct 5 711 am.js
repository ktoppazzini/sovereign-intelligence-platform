// lib/reportGraphs.js
// Surgical: axis readability + less-compressed bars (no TS).
    ...
    <div class="canvas-wrap">
      ...
      <svg class="plot bar" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(nice)}">
        ...
        ${data.map((v, i) => {
          ...
          const bw = slot * 0.90; /* wider bars so plot looks bigger (per screenshot) */
...
// Dashboard / single benchmark card (e.g., Delivery vs Market, On-time, NPS)
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  const w = 760, h = 240, p = 92; // keep canvas size; widen *bars* instead
  const max = Math.max(1, current, benchmark);
  const slot = (w - 2 * p) / 2, bw = slot * 0.90; // <-- less compressed bars
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  // axis labels for heatmap (kept aligned with other charts)
...
// CSS (unchanged; axis text set inline)
