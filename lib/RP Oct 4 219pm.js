// lib/reportGraphs.js
// Surgical update (Oct 4, 2:11pm): per screenshots
// 1) Y-axis title is now HORIZONTAL and placed to the left of the axis.
// 2) Y-axis tick values are much larger (white) for readability.
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  // grid + Y tick values (bigger font, stay white)
  const grid = [];
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      ...
      <text x="${p - 14}" y="${y + 8}" text-anchor="end" fill="#FFFFFF" font-size="28">${val}</text>
    `);
  }

  // X tick labels (unchanged size, still white)
  const slot = (w - 2 * p) / Math.max(labels.length, 1);
  const xTicks = labels.map((t, i) => {
    const x = p + i * slot + slot / 2;
    return `<text x="${x}" y="${h - p + 28}" text-anchor="middle" fill="#FFFFFF" font-size="22">${esc(t)}</text>`;
  }).join('');
  ...
  // Axis titles:
  // - X stays centered at the bottom
  const xTitleText = xTitle
    ? `<text x="${w / 2}" y="${h - 6}" text-anchor="middle" fill="#9AD0FF" font-size="28" font-weight="700">${esc(xTitle)}</text>`
    : '';

  // - Y is now HORIZONTAL and positioned to the LEFT of the ticks (per green arrow)
  //   Using 'end' anchor so the text finishes at the inside edge next to the tick values.
  const yTitleText = yTitle
    ? `<text x="${p - 16}" y="${p - 14}" text-anchor="end" fill="#9AD0FF" font-size="28" font-weight="700">${esc(yTitle)}</text>`
    : '';
...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  const w = 760, h = 320, p = 92; // left padding preserved
...
// Dashboard "pair bar" cards (e.g., Order On-time %, Net Promoter Score)
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '' }) {
  const w = 760, h = 240, p = 92; // keep padding consistent with other charts
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  // Axis labels for heatmap (unchanged)
  const xlab = c.map((t, i) =>
    `<text x="${p + (i + .5) * cw}" y="${h - 6}" text-anchor="middle" fill="#9AD0FF" font-size="26" font-weight="700">${esc(t)}</text>`
  ).join('');
  const ylab = r.map((t, i) =>
    `<text x="${p - 8}" y="${p + (i + .5) * ch + 6}" text-anchor="end" fill="#9AD0FF" font-size="26" font-weight="700">${esc(t)}</text>`
  ).join('');
...
.chart-axes{ display:none !important; } /* remove duplicate captions under charts */
...
/* Pure CSS switcher; only one plot is displayed */
