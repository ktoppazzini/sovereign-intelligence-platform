...
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '' }) {
  ...
  const summary = `
    <div class="chart-summary">
      <p class="summary-result"><strong>Overall:</strong> ${trend === 'increasing' ? '📈' : trend === 'decreasing' ? '📉' : '➡️'} Values show ${trend} trend from ${fmtTick(firstVal, yTitle)} to ${fmtTick(lastVal, yTitle)} (${changePercent > 0 ? '+' : ''}${changePercent}%).</p>
      <p class="summary-highlights"><strong>Key Insights:</strong> Peak value of ${fmtTick(dataMax, yTitle)} achieved. Average performance: ${fmtTick(Math.round(dataAvg), yTitle)}.</p>
    </div>
  `;
...
export function barChartHTML(args) { 
  const html = lineChartHTML({ ...args, title: args?.title });
  // Change default data-type from "line" to "bar"
  return html.replace('data-type="line"', 'data-type="bar"');
}
