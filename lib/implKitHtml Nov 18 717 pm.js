// [KT:SURGICAL:APPENDIX-TYPE-SWITCH] Additive: Ensure appendix charts have a styled, working type switch dropdown matching main report charts.
// This block is additive and does not remove or replace any existing code.
// Usage: Call this function after rendering an appendix chart to inject the type switch and event logic.
function injectAppendixChartTypeSwitch(chartId) {
  // Add dropdown if not present
  const chartElem = document.getElementById(chartId);
  if (chartElem && !chartElem.querySelector('.chart-type-dropdown')) {
    ...
    dropdown.style.padding = '.2rem .4rem';
    chartElem.insertBefore(dropdown, chartElem.firstChild);
    // Add event listener for type switch
    dropdown.addEventListener('change', function(e) {
      const type = e.target.value;
      const event = new CustomEvent('appendix-chart-type-change', { detail: { id: chartId, type } });
      window.dispatchEvent(event);
    });
  }
}
// [KT:SURGICAL:APPENDIX-TYPE-SWITCH-END]
// /lib/implKitHtml.js
...
function implementationChecklistHTML(model = {}) {
  ...
  //   </tr>
  // `).join("");
  // Commented out to prevent blank appendix after conclusion.
  const rows = "";
