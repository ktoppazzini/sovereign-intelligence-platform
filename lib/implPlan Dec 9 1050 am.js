...
export function planDiagramHTML_TwoPhase(phasesIn = [], heading = "Implementation Plan (Phase 1-2)") {
  ...
  const phaseBox = (i, tPhase) => `
    <g transform="translate(${p + i * cellW}, ${p})">
      ...
      <text x="10" y="28" fill="#fff" font-weight="700" font-family="Arial">${esc(t(`Phase ${i+1}`))}</text>
      <text x="10" y="54" fill="#d6e4ff" font-family="Arial" font-size="12">${esc(t(tPhase?.title || (`Phase ${i+1}`)))}</text>
