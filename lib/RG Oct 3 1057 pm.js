...
// Exports: GRAPH_CSS, lineChartHTML(opts), barChartHTML(opts), heatmapHTML(opts),
...
.chart-type{font-size:.95rem;background:#0c2238;border:1px solid rgba(255,255,255,.12);
  color:#E6F0FF;border-radius:8px;padding:4px 10px}
.chart-canvas{width:100%;height:320px;display:block;border-radius:12px;background:#0b2034}
.chart-legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:8px;font-size:.95rem;opacity:.92}
...
export function lineChartHTML(opts={}){ return cardHTML({ type:'line', ...opts }); }
export function barChartHTML(opts={}) { return cardHTML({ type:'bar',  ...opts }); }
...
export function heatmapHTML({ id, title='Risk Heat Map', rows=[], cols=[], data=[] } = {}){
  ...
  const m = Array.isArray(data)&&data.length?data:
            Array.from({length:r.length},()=>Array.from({length:c.length},()=>1+Math.floor(Math.random()*5)));
  ...
  <figure class="chart-card" data-type="heatmap" data-heat="${payload}">
    ...
    <div class="chart-legend" aria-hidden="true">
      <span><span class="sw" style="background:#2ecc71"></span>Low</span>
      <span><span class="sw" style="background:#a3d977"></span></span>
      <span><span class="sw" style="background:#f1c40f"></span>Moderate</span>
      <span><span class="sw" style="background:#f39c12"></span></span>
      <span><span class="sw" style="background:#e74c3c"></span>High</span>
    </div>
  </figure>`;
}
...
(function(){
  ...
  function drawAxes(ctx, W, H, pad, min, max, xLabels, titles){
    ...
    for (let i=0;i<=yTicks;i++){
      ...
      ctx.fillStyle = '#E6F0FF'; ctx.font = '600 14px system-ui';
      ctx.textAlign = 'left';
      ctx.fillText(val.toLocaleString(), 8, y+5);
    }
    // y axis
    ...
    // axis titles (bold)
    ctx.fillStyle='#E6F0FF'; ctx.font='700 16px system-ui';
    if (titles.y){
  ...
  function drawSeries(ctx, kind, rect, labels, series, min, max){
    ...
    if (kind==='line'){
      ...
      series.forEach((v,i)=>{ const x=scaleX(i), y=scaleY(v); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); });
      ctx.stroke();
    } else {
      const w = (right-left)/Math.max(series.length,1)*0.65;
      ctx.fillStyle = '#9cd1ff';
      series.forEach((v,i)=>{ const x = scaleX(i) - w/2, y = scaleY(v); ctx.fillRect(x, y, w, bottom-y); });
    }
  }
  ...
  function hydrateCard(el){
    ...
    if (type==='heatmap'){
      ...
      const W=canvas.width, H=canvas.height, pad=48, top=pad, left=pad+24, bottom=H-pad, right=W-pad;
      const cellW=(right-left)/cols.length, cellH=(bottom-top)/rows.length;
      const palette = ['#2ecc71','#a3d977','#f1c40f','#f39c12','#e74c3c']; // G-Y-R
      ctx.fillStyle='#0b2034'; ctx.fillRect(0,0,W,H);

      // axes
      ctx.fillStyle='#E6F0FF'; ctx.font='600 14px system-ui';
      rows.forEach((r,i)=>ctx.fillText(r, 10, top+cellH*(i+.65)));
      cols.forEach((c,i)=>ctx.fillText(c, left+cellW*(i+.5), H-10));
    ...
    // single-canvas switch (no extra elements added)
    const initial = (typeSel?.value || type || 'line');
    render(initial);
    if (typeSel){ typeSel.addEventListener('change', e=>render(e.target.value)); }
  }
