...
// New (Oct 3, interactive placeholders):
// - Detects AI-suggested “placeholder” text (e.g., “Notes on the chart below…”, “The following chart is presented…”)
//   and replaces it with a real, interactive widget (line/bar/benchmark/heatmap), based on keywords.
// - Ensures at least ONE interactive widget per major section (Exec, Current, Financials, KPIs, Ops, Risks, ROI).
// - Logo path normalized to /images/<file>, rendered at 200×200.
// - Roadmap image fixed to /images/implementation-roadmap.png with phase overlays.
...
function normalizeLogoPath(raw) {
  ...
  if (!name) return '/images/secure.png';
  if (name.startsWith('/')) return name;
  if (/^images\//i.test(name)) return '/' + name.replace(/^images\//i,'images/');
  return '/images/' + name;
}
...
function mkDefaultSeries(len, total=100) {
  return Array.from({length:len}, (_,i)=>Math.round((i+1)*total/len));
}

// --- Smart placeholder injector -------------------------------------------
// Replaces common AI “placeholder” sentences with interactive widgets.
// Heuristics: looks for keywords to choose widget type.
function injectSmartWidgets(html, { labels, canon }) {
  ...
  const pick = (text) => {
    ...
    if (/\bheat\s*map\b/.test(t)) return 'heat';
    if (/\bimplementation\s+expenditure|\bspend\b/.test(t)) return 'bar';
    if (/\bprojected\s+savings|\bover\s+time\b/.test(t)) return 'line';
    if (/\bdelivery\b.*\bmarket share|\bmarket\b.*\bdelivery/.test(t)) return 'benchmark';
    if (/\broi\b|\bpayback\b/.test(t)) return 'line';
    return 'line';
  };

  const make = (kind) => {
    const series = mkDefaultSeries(labels.length, (canon?.costSavingsGoal || 100));
    if (kind === 'bar') return barChartHTML({ title: 'Implementation Expenditures', labels, series, xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle: 'Spend' });
    if (kind === 'heat') return heatmapHTML({ id:'auto_heat', title:'Risk Heat Map', rows:['Ops','Sales','CX','IT','People'], cols:['Q1','Q2','Q3','Q4','Q5'], data: Array.from({length:5},()=>Array.from({length:5},()=>Math.floor(Math.random()*5)+1)) });
    if (kind === 'benchmark') {
      const spec = { title: "Delivery Penetration vs Market", current: Number(canon?.metrics?.deliveryShare ?? 28), benchmark: Number(canon?.metrics?.marketDelivery ?? 34) };
    ...
    // default: line
    return lineChartHTML({ title: 'Projected Savings Over Time', labels, series, xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle: 'Savings' });
  };

  const replaced = blocks.map((raw) => {
    const p = raw.replace(/\s+/g, ' ').trim();
    const isPlaceholder = /^notes\s+on\s+the\s+chart\s+below/i.test(p)
...
export default function buildReformReportHTML(opts) {
  ...
  const logo = normalizeLogoPath(canon?.logoUrl);
  const labels = labelsForPeriod(canon?.timeFrame||'2 years');

  // Normalize + inject placeholders per section
  const prep = (s) => {
  ...
  // Guaranteed widgets where missing (one per section)
  const ensureOne = (already, fallbackHTML) => already ? '' : fallbackHTML;

  const execFallback = lineChartHTML({ title: 'Projected Savings Over Time', labels, series: mkDefaultSeries(labels.length, canon?.costSavingsGoal||100), xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle:'Savings' });
  const currentFallback = `<figure class="chart-card" data-widget="benchmark" data-spec='${JSON.stringify({ title:"Delivery Penetration vs Market", current:Number(canon?.metrics?.deliveryShare ?? 28), benchmark:Number(canon?.metrics?.marketDelivery ?? 34) })}'><header class="chart-head"><h3>Interactive Benchmark</h3></header><div class="canvas-wrap"><canvas width="760" height="240"></canvas></div></figure>`;
  const finSpend = barChartHTML({ title:'Implementation Expenditures', labels, series: mkDefaultSeries(labels.length, (canon?.costSavingsGoal||100)*0.35), xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle:'Spend' });
  const finPayback = lineChartHTML({ title:'Cumulative Payback', labels, series: mkDefaultSeries(labels.length, (canon?.costSavingsGoal||100)), xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle:'Cumulative' });
  const kpiDash = `
  ...
  const riskHeat = heatmapHTML({ id:'risk_heat', title:'Risk Heat Map', rows:['Ops','Sales','CX','IT','People'], cols:['Q1','Q2','Q3','Q4','Q5'], data: Array.from({length:5},()=>Array.from({length:5},()=>Math.floor(Math.random()*5)+1)) });

  const roiMini = lineChartHTML({ title:'ROI Trend (AI)', labels, series: mkDefaultSeries(labels.length, 100), xTitle: labels[0].startsWith('Y')?'Years':'Months', yTitle:'ROI Index' });

  const roadmap = (planDiagramHTML || defaultPlanDiagramHTML)(phases || []);

  const tocItems = [
    'Executive Summary','Current State','Financials','KPIs & Targets','Implementation Timeline','Operating Model','Risks & Mitigations','ROI & Next Steps'
