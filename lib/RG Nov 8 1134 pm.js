...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  const xTicks = labels.map((t, i) => {
    ...
    const shouldRotate = labelText.length > 8 || labels.length > 6;
    if (shouldRotate) {
      return `<text x="${x}" y="${h - p + 18}" text-anchor="end" fill="#FFFFFF" font-size="14" transform="rotate(-45 ${x} ${h - p + 18})">${esc(labelText)}</text>`;
    }
...
/* ===========================================================
   ...
   ===========================================================*/
export function prioritizationMatrixHTML({ title = 'Initiative Prioritization', initiatives = [] }) {
  ...
  return `
  <figure class="chart-card priority-matrix" data-widget="matrix">
    <header class="chart-head">
      <h3>${esc(nice)}</h3>
