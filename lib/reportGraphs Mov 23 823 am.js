...
export function renderAllAppendixSections(appendixData, opts = {}) {
  ...
  return sections.map((entry, idx) => {
    ...
    if (typeof section === 'string' && /^[G-Z]$/i.test(section.trim().replace(/^Appendix\s*/i, ''))) {
      ...
      logAIPickup({ appendixSection: section, data, reason });
    }
    return `
      <section class="appendix-section" data-section="${section}">
        <h3 class="appendix-section-title">${section}</h3>
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  const xTicks = labels.map((t, i) => {
    ...
    // Rotate labels if they're long or there are many labels
  ...
  // Axis titles - Y-axis HORIZONTAL on the left, X-axis centered below
  // Truncate VERY long Y-axis titles only (30 char limit, not 20)
  ...
  const xTitleY = hasRotatedLabels ? h - 2 : h - 6; // Move down if labels rotated
  
  const xTitleText = xTitle
    ? `<text x="${w / 2}" y="${xTitleY}" text-anchor="middle" fill="#FFFFFF" font-size="20" font-weight="700">${esc(xTitle)}</text>`
    : '';
  const yTitleText = truncatedYTitle
    ? `<text x="${p - 16}" y="${p - 14}" text-anchor="end" fill="#FFFFFF" font-size="20" font-weight="700">${esc(truncatedYTitle)}</text>`
    : '';
...
export function prioritizationMatrixHTMLNano5({ title = 'Initiative Prioritization (Nano-5)' , initiatives = [] } = {}) {
  ...
  const header = `<header class="chart-head"><h3>${title}</h3></header>`;
  return `
