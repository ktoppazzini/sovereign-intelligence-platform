...
const t = (s) => {
  try { return __reportTranslator(s); }
  catch { return s; }
};
      ...
      <div class="title-header">
        <div class="title-logo">
          ...
          <div class="brand-text">
            <div class="brand-name">${t('SOVEREIGN INTELLIGENCE')}</div>
...
`;
  ...
  reportHtml += `
    <section class="toc">
      ...
      <ol class="toc-list">$t{tocItems}</ol>
    </section>
...
.report .table .table-title{
  ...
  if (implKit) {
    ...
    reportHtml += `\n<section id="appendices"><h2>${t(L.appendix)}</h2>\n` + [
      (() => {
        if (!implKit.charters || !Array.isArray(implKit.charters.items) || implKit.charters.items.length === 0) {
          console.log('[APPENDIX-A] Not injected: charters missing or empty', { charters: implKit?.charters || null });
