...
// Baseline + surgical additions: SSR charts, robust placeholder handling,
// and NEW Implementation Kit appendices + CSV export buttons (no TS).
...
// NEW: Implementation Kit HTML + CSS
import {
  IMPLKIT_CSS,
...
// Implementation roadmap overlays (image anchor)
function roadmapHTML(phases=[]) {
  ...
  while (heads.length < 5) heads.push({ title:`Phase ${heads.length+1}`, caption:'Short description' });
...
export default function buildReformReportHTML(opts) {
  ...
  // Render any explicit data-* blocks first; then wrap; then smart-inject.
  const prep = (s) => {
  ...
  <script>
  // CSV export buttons for every appendix table (no framework changes)
  (function csvExport(){
