 ...
 * @param {string} module - Module name (e.g., 'reform', 'clinical', 'energy')
 * @param {object|string} baseUIOrLang - BaseUI object OR language string (if called with 2 args)
 * @param {string} lang - Language (optional if baseUIOrLang is language string)
 */
export async function loadModuleTranslations(module, baseUIOrLang, lang = 'English') {
  // Handle both 2-parameter and 3-parameter calls
  // If baseUIOrLang is a string, treat it as the language
  const actualLang = typeof baseUIOrLang === 'string' ? baseUIOrLang : lang;
  const baseUI = typeof baseUIOrLang === 'object' ? baseUIOrLang : {};
