...
export async function initGraphUiTranslations(lang) {
  ...
  // Existing heatmap summary
  heatmapOverall: 'Risk levels are distributed across the matrix, highlighting areas of concern.',
  heatmapKeyInsights: 'Highest risk: Top-right quadrant. Lowest risk: Bottom-left quadrant.',

  // 🔥 NEW — AI summary templates for translation
  aiOverallTemplate: 'Values show {trend} trend from {first} to {last} ({percent}).',
  aiKeyInsightsTemplate: 'Peak value of {peak} achieved. Average performance: {avg}.'
...
(function () {
  ...
  // ENHANCED delegated change handler (CAPTURE PHASE)
...
/* ===========================================================
   SVG AXES HELPER
   [KT:SURGICAL:I18N-AXES]
   - Uses raw yTitle for numeric formatting (percent/currency)
   - Uses translated labels via t(...) for display
   ===========================================================*/
/* ===========================================================
   SVG AXES HELPER
   [KT:SURGICAL:I18N-AXES]
...
function svgAxes({ w, h, p, xTitle, yTitle, maxY, ticks = 5, labels = [] }) {
  ...
  // Keep the raw Y title for tick formatting (percent / currency logic)
  const rawYTitle = String(yTitle || '');

  // Display titles go through the graph translator
  const dispXTitle = xTitle ? t(String(xTitle)) : '';
  const dispYTitle = rawYTitle ? t(rawYTitle) : '';
  ...
  for (let i = 0; i <= ticks; i++) {
    ...
    grid.push(`
      ...
      <text x="${p - 14}" y="${y + 10}" text-anchor="end" fill="#FFFFFF" font-size="18">
        ${fmtTick(val, rawYTitle)}
      </text>
    `);
  }

  // X tick labels – run labels through t(...)
  const displayLabels = (Array.isArray(labels) ? labels : []).map(l =>
    t(String(l ?? ''))
  );
   const slot = (w - 2 * p) / Math.max(labels.length, 1);
  const xTicks = labels.map((lblText, i) => {
    const x = p + i * slot + slot / 2;
    const raw = String(lblText || '');
    const labelText = t(raw); // <— translation hook

    const shouldRotate = labelText.length > 10 || labels.length > 8;
    if (shouldRotate) {
      const angle = -50;
      return `<text x="${x}" y="${h - p + 18}" text-anchor="end" fill="#FFFFFF" font-size="14" transform="rotate(${angle} ${x} ${h - p + 18})">${esc(labelText)}</text>`;
    }
    return `<text x="${x}" y="${h - p + 22}" text-anchor="middle" fill="#FFFFFF" font-size="18">${esc(labelText)}</text>`;
  }).join('');

  // axes path
  const axes = `
    <polyline
      points="${p},${p} ${p},${h - p} ${w - p},${h - p}"
      fill="none"
      stroke="rgba(255,255,255,.65)"
    />
  `;

  // Axis titles – display versions are translated
  const truncatedYTitle = yTitle && yTitle.length > 30 ? yTitle.slice(0, 27) + '...' : yTitle;

  const hasRotatedLabels = labels.some(l => String(l).length > 8) || labels.length > 6;
  const xTitleY = hasRotatedLabels ? h - 2 : h - 6;

 const xTitleText = xTitle
    ? `<text x="${w / 2}" y="${xTitleY}" text-anchor="middle" fill="#FFFFFF" font-size="20" font-weight="700">${esc(t(xTitle))}</text>`
    : '';
  const yTitleText = truncatedYTitle
    ? `<text x="${p - 16}" y="${p - 14}" text-anchor="end" fill="#FFFFFF" font-size="20" font-weight="700">${esc(t(truncatedYTitle))}</text>`
    : '';
...
/* ===========================================================
   ...
   ===========================================================*/
export function lineChartHTML({ title, labels = [], series = [], xTitle = '', yTitle = '', ui } = {}) 
  ...
  console.log('[KT:AUDIT][lineChartHTML] args:', { title, labels, series, xTitle, yTitle });
  const w = 760, h = 320, p = 110;  // generous padding to avoid clipping
...
const changePercentNum = firstVal !== 0 ? (change / firstVal) * 100 : 0;
const changePercentStr = `${changePercentNum > 0 ? '+' : ''}${changePercentNum.toFixed(1)}%`;
const trend = change > 0 ? 'increasing' : change < 0 ? 'decreasing' : 'stable';
// [KT:SURGICAL:I18N-AI-SUMMARY] Use translated UI templates for Overall / Key Insights
const uiSafe = ui && typeof ui === 'object' ? ui : {};
const overallTemplate =
uiSafe.aiOverallTemplate || 'Values show {trend} trend from {first} to {last} ({percent}).';
const insightsTemplate =
uiSafe.aiKeyInsightsTemplate || 'Peak value of {peak} achieved. Average performance: {avg}.';	
const overallText = overallTemplate
.replace('{trend}', trend)
.replace('{first}', fmtTick(firstVal, yTitle))
.replace('{last}', fmtTick(lastVal, yTitle))
.replace('{percent}', changePercentStr);
const insightsText = insightsTemplate
.replace('{peak}', fmtTick(dataMax, yTitle))
.replace('{avg}', fmtTick(Math.round(dataAvg), yTitle));
let trendIcon = '✔️';
if (trend === 'increasing') trendIcon = '📈';
else if (trend === 'decreasing') trendIcon = '📉';
else if (trend === 'stable' || trend === 'flat') trendIcon = '➡️';
const overallLabel = uiSafe.overallLabel || 'Overall';
const keyInsightsLabel = uiSafe.keyInsightsLabel || 'Key Insights';
const summary = `
<div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
<span style="font-weight:bold; color:#fff; background:#222; padding:2px 8px; border-radius:4px; margin-right:8px;">
${overallLabel}:
</span>
<span class="ai-overall" style="color:#fff;">${trendIcon} ${overallText}</span><br/>
<span style="font-weight:bold; color:#fff; background:#222; padding:2px 8px; border-radius:4px; margin-right:8px;">
${keyInsightsLabel}:
</span>
<span class="ai-key-insights" style="color:#fff;">${insightsText}</span>
</div>
`;
// [KT:AUDIT] Log what gets inserted for Overall and Key Insights (final, translated)
console.log('[KT:AUDIT][lineChartHTML] Inserted summary:', {
overall: `${trendIcon} ${overallText}`,
...
title
  ...
  // Unified summary row generator for all chart types
   // Unified summary row generator for all chart types – now i18n-aware
  function chartSummaryRow({ trend, firstVal, lastVal, changePercent, dataMax, dataAvg, yTitle }) {
    // [KT:SURGICAL:I18N-AI-SUMMARY] Pull translated templates & labels if available
    const ui =
      (typeof window !== 'undefined' && window.__REPORT_GRAPHS_UI__) ||
      {};
    ...
    const overallText = (ui.aiOverallTemplate || 'Values show {trend} trend from {first} to {last} ({percent}).')
      .replace('{trend}', trendWord)
      .replace('{first}', fmtTick(firstVal, yTitle))
      .replace('{last}', fmtTick(lastVal, yTitle))
      .replace('{percent}', percentStr);

    const insightsText = (ui.aiKeyInsightsTemplate || 'Peak value of {peak} achieved. Average performance: {avg}.')
      .replace('{peak}', fmtTick(dataMax, yTitle))
      .replace('{avg}', fmtTick(Math.round(dataAvg), yTitle));
    ...
    const overallLabel = ui.overallLabel || 'Overall';
    const keyLabel     = ui.keyInsightsLabel || 'Key Insights';
...
export function barChartHTML(args) { 
  ...
  */
  ...
  let html = lineChartHTML({ ...args, title: args?.title });
...
/* ===========================================================
   ...
   ===========================================================*/
export function benchmarkHTML({ title, current = 0, benchmark = 0, xTitle = '', yTitle = '', ui } = {}) {
  ...
  const h1 = (currentNum / M) * (h - 2 * p);
  const h2 = (benchmarkNum / M) * (h - 2 * p);
  ...
  /*const aiSummary = `
    <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
      <span style="font-weight:bold; color:#fff; background:#222; padding:2px 8px; border-radius:4px; margin-right:8px;">Overall:</span>
      <span class="ai-overall" style="color:#fff;">${benchmarkNum > currentNum ? '📈' : benchmarkNum < currentNum ? '📉' : '➡️'} Values show ${benchmarkNum > currentNum ? 'increasing' : benchmarkNum < currentNum ? 'decreasing' : 'stable'} trend from ${currentNum} to ${benchmarkNum} (${((benchmarkNum-currentNum)/currentNum*100).toFixed(1)}%).</span><br/>
      <span style="font-weight:bold; color:#fff; background:#222; padding:2px 8px; border-radius:4px; margin-right:8px;">Key Insights:</span>
      <span class="ai-key-insights" style="color:#fff;">Peak value of ${benchmarkNum} achieved. Average performance: ${((currentNum+benchmarkNum)/2).toFixed(0)}.</span>
    </div>
  `;*/

  // [KT:AUDIT] Log what gets inserted for Overall and Key Insights
 /*const ui = window.__REPORT_GRAPHS_UI__ || {};

const overall = (ui.aiOverallTemplate || 'Values show {trend} trend from {first} to {last} ({percent}).')
  .replace('{trend}', benchmarkNum > currentNum ? 'increasing' : benchmarkNum < currentNum ? 'decreasing' : 'stable')
  .replace('{first}', fmtTick(currentNum, yTitle))
  .replace('{last}', fmtTick(benchmarkNum, yTitle))
  .replace('{percent}', `${((benchmarkNum - currentNum) / currentNum * 100).toFixed(1)}%`);

const insights = (ui.aiKeyInsightsTemplate || 'Peak value of {peak} achieved. Average performance: {avg}.')
  .replace('{peak}', fmtTick(benchmarkNum, yTitle))
  .replace('{avg}', ((currentNum + benchmarkNum) / 2).toFixed(0));

  

const trendIcon =
  benchmarkNum > currentNum ? '📈' :
  benchmarkNum < currentNum ? '📉' :
  '➡️';*/
    // [KT:SURGICAL:I18N-AI-SUMMARY] Dynamic translated AI summary row for benchmarks
  const uiSafe = ui && typeof ui === 'object' ? ui : {};
const trend =
benchmarkNum > currentNum ? 'increasing' :
benchmarkNum < currentNum ? 'decreasing' :
'stable';
const changePercentNum = currentNum !== 0 ? ((benchmarkNum - currentNum) / currentNum) * 100 : 0;
const changePercentStr = `${changePercentNum > 0 ? '+' : ''}${changePercentNum.toFixed(1)}%`;
const overallTemplate =
uiSafe.aiOverallTemplate || 'Values show {trend} trend from {first} to {last} ({percent}).';
const insightsTemplate =
...
const insightsText = insightsTemplate
.replace('{peak}', fmtTick(benchmarkNum, yTitle))
.replace('{avg}', fmtTick((currentNum + benchmarkNum) / 2, yTitle));
let trendIcon = '✔️';
if (trend === 'increasing') trendIcon = '📈';
else if (trend === 'decreasing') trendIcon = '📉';
else trendIcon = '➡️';
  ...
  <figure class="chart-card" data-type="bar" data-widget="benchmark" data-spec="${esc(JSON.stringify({title, current: currentNum, benchmark: benchmarkNum, xTitle, yTitle}))}" data-debug-current="${currentNum}" data-debug-benchmark="${benchmarkNum}" data-debug-h1="${h1.toFixed(2)}" data-debug-h2="${h2.toFixed(2)}">
    ...
    <div class="canvas-wrap">
      ...
      <svg class="plot bar" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
        ...
        <rect x="${(p + (slot - bw) / 2).toFixed(1)}" y="${(h - p - h1).toFixed(1)}" width="${bw.toFixed(1)}" height="${h1.toFixed(1)}" fill="url(#grad-current-${uniqueId})" rx="4" ry="4" data-debug-bar="current" style="filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2))"/>
        <!-- Benchmark bar with gradient -->
        <rect x="${(p + slot + (slot - bw) / 2).toFixed(1)}" y="${(h - p - h2).toFixed(1)}" width="${bw.toFixed(1)}" height="${h2.toFixed(1)}" fill="url(#grad-benchmark-${uniqueId})" rx="4" ry="4" data-debug-bar="benchmark" style="filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2))"/>
        <!-- Value labels with background for better readability -->
        <rect x="${(p + slot * 0.5 - 25).toFixed(1)}" y="${(h - p - h1 - 28).toFixed(1)}" width="50" height="22" fill="rgba(0,0,0,0.7)" rx="4" ry="4"/>
        <text x="${(p + slot * 0.5).toFixed(1)}" y="${(h - p - h1 - 10).toFixed(1)}" fill="#FFFFFF" text-anchor="middle" font-size="18" font-weight="bold">${currentNum.toFixed(0)}</text>
        <rect x="${(p + slot * 1.5 - 25).toFixed(1)}" y="${(h - p - h2 - 28).toFixed(1)}" width="50" height="22" fill="rgba(0,0,0,0.7)" rx="4" ry="4"/>
        <text x="${(p + slot * 1.5).toFixed(1)}" y="${(h - p - h2 - 10).toFixed(1)}" fill="#FFFFFF" text-anchor="middle" font-size="18" font-weight="bold">${benchmarkNum.toFixed(0)}</text>
      </svg>
...
export function heatmapHTML({ title, rows = [], cols = [], data = [[]] }) {
  ...
  const aiSummary = `
    <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px; background:none; box-shadow:none;">
      <span style="font-weight:bold; color:#fff; /*background:#222;*/ padding:2px 8px; border-radius:4px; margin-right:8px;">Overall:</span>
      <span class="ai-overall" style="color:#fff;">Risk levels are distributed across the matrix, highlighting areas of concern.</span><br/>
      <span style="font-weight:bold; color:#fff; /*background:#222;*/ padding:2px 8px; border-radius:4px; margin-right:8px;">Key Insights:</span>
      <span class="ai-key-insights" style="color:#fff;">Highest risk: Top-right quadrant. Lowest risk: Bottom-left quadrant.</span>
  ...
  <section class="chart-card chart-heat" data-type="heat">
    ...
    <div class="chart-desc top-banner" style="font-size:14px; color:#66e6b0; margin:8px 0 6px 0;">Risk Heat Map Overview</div>
    <header class="chart-head"><h3>${t(esc(nice))}</h3></header>
  <div class="chart-desc" style="font-size:14px; color:#e5e7eb; margin:0 0 6px 0;">Description: Risk Heat Map</div>
  <div class="chart-desc" style="font-size:12px; color:#cbd5e1; margin:0 0 8px 0;">Risk levels: Low (green) / Medium (orange) / High (red). </div>
    <div class="canvas-wrap">
    ...
    <div class="hm-legend" style="display:flex; align-items:center; gap:12px; font-size:.85rem; margin-top:6px; color:#FFFFFF;">
      <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; border-radius:50%; background:#10B981; display:inline-block;"></span>Low</span>
      <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; border-radius:50%; background:#F59E0B; display:inline-block;"></span>Medium</span>
      <span style="display:flex; align-items:center; gap:6px;"><span style="width:12px; height:12px; border-radius:50%; background:#EF4444; display:inline-block;"></span>High</span>
    </div>
  </section>`;
}
...
/* ===========================================================
   ...
   ===========================================================*/
export function prioritizationMatrixHTML({ title = 'Initiative Prioritization', initiatives = [], nano5 = false } = {}) {
  ...
  */
  ...
  const aiSummary = `
    <div class="chart-summary" style="font-size:16px; color:#FFFFFF; margin-bottom:8px;">
      <span style="font-weight:bold; color:#fff; padding:2px 8px; border-radius:4px; margin-right:8px;">Overall:</span>
      <span class="ai-overall" style="color:#fff;">Initiatives are distributed across quadrants, highlighting both quick wins and major projects.</span><br/>
      <span style="font-weight:bold; color:#fff; padding:2px 8px; border-radius:4px; margin-right:8px;">Key Insights:</span>
      <span class="ai-key-insights" style="color:#fff;">Highest impact: Digital transformation. Most efficient: Quick wins.</span>
    </div>
  `;
  // Default initiatives if none provided
  const items = initiatives.length ? initiatives : [
  ...
  // Quadrant labels
  const labels = `
    <text x="${p + plotW/4}" y="${p + 30}" text-anchor="middle" fill="rgba(255,255,255,0.5)" font-size="16" font-weight="600">BIG BETS</text>
    <text x="${p + 3*plotW/4}" y="${p + 30}" text-anchor="middle" fill="#4CAF50" font-size="16" font-weight="700">QUICK WINS</text>
    <text x="${p + plotW/4}" y="${h - p - 10}" text-anchor="middle" fill="rgba(255,255,255,0.4)" font-size="16">FILL INS</text>
    <text x="${p + 3*plotW/4}" y="${h - p - 10}" text-anchor="middle" fill="rgba(255,255,255,0.5)" font-size="16" font-weight="600">MAJOR PROJECTS</text>
  `;
  ...
  const bubbles = items.map((item, i) => {
    ...
    return `
      ...
      <text x="${x}" y="${y + 4}" text-anchor="middle" fill="#FFFFFF" font-size="11" font-weight="700">${esc(item.name.substring(0, 15))}</text>
  ...
  const axes = `
    ...
    <text x="${w/2}" y="${h - 20}" text-anchor="middle" fill="#FFFFFF" font-size="18" font-weight="700">Effort / Complexity →</text>
    <text x="30" y="${h/2}" text-anchor="middle" fill="#FFFFFF" font-size="18" font-weight="700" transform="rotate(-90 30 ${h/2})">Business Impact →</text>
  `;
  ...
  <figure class="chart-card priority-matrix" data-widget="matrix" data-type="matrix">
    ...
    <header class="chart-head">
      ...
      <div class="chart-legend">
        <span style="color:#4CAF50">● Quick Wins</span>
        <span style="color:#FF9800; margin-left:1rem">● Major Projects</span>
        <span style="color:#F44336; margin-left:1rem">● Big Bets</span>
      </div>
    </header>
    <div class="canvas-wrap">
...
/* Ensure Priority Matrix shows by default (Steps 9) */
.chart-card.priority-matrix .plot{ display:block; }
/* Ensure priority matrix is visible by default in dashboards */
.chart-card.priority-matrix .plot { display: block; }
/* Matrix default visibility: show priority matrix plots by default */
.chart-card[data-type="matrix"] .plot{ display:block; }
...
export function renderChartFigure({ section, meta }) {
  ...
  switch ((type || 'line').toLowerCase()) {
    ...
    case 'matrix':
    case 'prioritization':
      html = prioritizationMatrixHTML({ id, title, meta });
      break;
    case 'benchmark':
